...
DEF VAR s-codalm AS CHAR INIT '10' NO-UNDO.        /* Almacén por defecto */
...
FIND FIRST Faccorre WHERE Faccorre.codcia = s-codcia
    ...
    NO-LOCK NO-ERROR.
IF NOT AVAILABLE Faccorre THEN DO:
    DISPLAY "NO está configurado el correlativo para doc PED division " s-coddiv
        WITH STREAM-IO NO-BOX NO-UNDERLINE NO-LABELS.
    RETURN ERROR.
END.
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH B-DPEDI OF B-CPEDI, FIRST Almmmatg OF B-DPEDI NO-LOCK BY B-DPEDI.NroItm: 
      ...
      IF NOT AVAILABLE Almtconv THEN DO:
          pMensaje = 'Se ha encontrado un problema con el producto ' + B-DPEDI.codmat + CHR(10) +
              'No se encuentra asignado al almacén ' + B-DPEDI.AlmDes + CHR(10).
          RETURN 'ADM-ERROR'.
      END.
      ...
      IF s-StkDis <= 0 THEN DO:
          pMensajeFinal = pMensajeFinal + 
              ...
              'en el almacén ' + B-DPEDI.AlmDes + CHR(10).
          RETURN 'ADM-ERROR'.
      END.
      /* **************************************************************************************** */
