...
DEFINE NEW SHARED VAR s-codcia AS INT  INIT 001   .
DEFINE NEW SHARED VAR s-codalm AS CHAR            .
DEFINE NEW SHARED VAR cl-codcia AS INT INIT 000   .
DEFINE NEW SHARED VAR pv-codcia AS INT INIT 000   .
DEFINE NEW SHARED VAR s-user-id AS CHAR INIT 'BATCH' .
...
FOR EACH OOMoviAlmacen NO-LOCK WHERE OOMoviAlmacen.codcia = s-codcia
    ...
    IF LAST-OF(OOMoviAlmacen.CodAlm) 
        ...
        PUT UNFORMATTED "   No. de ingreso " OOMoviAlmacen.NroSer " " OOMoviAlmacen.NroDoc SKIP.
        ...
        FOR EACH x-MoviAlmacen WHERE x-MoviAlmacen.codcia = ooMoviAlmacen.codcia AND
            ...
            IF TRUE <> (x-MoviAlmacen.almfinal > "") THEN DO:
                ...
                PUT UNFORMATTED "           No tiene almacen final" SKIP.
                x-items-completos = NO.
                LEAVE VERIFICAITEMS.
            END.
            IF NOT TRUE <> (x-MoviAlmacen.flgsit > "") THEN DO:
                /* ya genero OTR */
                PUT UNFORMATTED "           El flgsit indica que ya genero OTR" SKIP.
                x-items-completos = NO.
                LEAVE VERIFICAITEMS.
            END.
            IF x-MoviAlmacen.FlagMigracion <> "S" THEN DO:
                /* Aun no ingreso al kardex */
                PUT UNFORMATTED "           El flgest indica que aun no ingreso al kardex/stock" SKIP.
                x-items-completos = NO.
                LEAVE VERIFICAITEMS.
            END.            
            IF x-MoviAlmacen.UseInDropShipment <> "no" THEN DO:
                /* Es DropShipping */
                PUT UNFORMATTED "           El campo useInDropShipemnt tiene valor difere de 'no'" SKIP.
                x-items-completos = NO.
                LEAVE VERIFICAITEMS.
            END.
            ...
            IF NOT AVAILABLE almacen THEN DO:
                ...
                PUT UNFORMATTED "           El almacen del kardex/stock no existe" SKIP.
                x-items-completos = NO.
                LEAVE VERIFICAITEMS.
            END.
            ...
            IF AVAILABLE TabGener THEN DO:
                ...
                PUT UNFORMATTED "           La division del almacen de Kardex/Stock esta configurado como Sloting" SKIP.
                x-items-completos = NO.
                LEAVE VERIFICAITEMS.
            END.                
            ...
            IF NOT AVAILABLE almacen THEN DO:
                ...
                PUT UNFORMATTED "           El almacen de destino no existe en la tabla de almacenes" SKIP.
                x-items-completos = NO.
                LEAVE VERIFICAITEMS.
            END.
            ...
            BUFFER-COPY x-MoviAlmacen TO T-MoviAlmacen
                ASSIGN T-MoviAlmacen.T-Rowid = ROWID(x-MoviAlmacen).
        END.
        /*  */
        IF x-items-completos = YES THEN DO:
            RUN genera-otr(INPUT ROWID(ooMovialmacen), OUTPUT x-mensaje).
...
DO TRANSACTION ON ERROR UNDO, RETURN "ADM-ERROR" ON STOP UNDO, RETURN "ADM-ERROR":
    ...
    IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
        PUT UNFORMATTED '         ERROR no se pudo generar la OTR ' lMensaje SKIP.
