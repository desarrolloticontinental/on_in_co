...
DEF VAR pCodAlm  AS CHAR INIT '11,03,04,05,21,14' NO-UNDO.
...
DO k = 1 TO NUM-ENTRIES(pCodAlm):
    s-CodAlm = ENTRY(k, pCodAlm).

    PUT UNFORMATTED "Almacén " s-CodAlm SKIP.
...
FOR EACH integral.Almcrepo NO-LOCK WHERE integral.Almcrepo.codcia = s-codcia
        AND integral.Almcrepo.CodAlm = s-CodAlm
...
CREATE LogBacheros.
ASSIGN
    LogBacheros.Detalle = 'Actualiza el stock en la tabla Almacen_Stocks del almacén 506'
