...
FOR EACH FacTabla NO-LOCK WHERE FacTabla.CodCia = s-codcia 
    ...
    IF TRUE <> (s-codalm > '') THEN s-codalm = TRIM(almacen.codalm).
    s-codalm = s-codalm + ',' + TRIM(almacen.codalm).
END.
...
FOR EACH estavtas.Almacen_Stocks WHERE LOOKUP(estavtas.Almacen_Stocks.CodAlm, s-CodAlm) > 0
...
&IF DEFINED(EXCLUDE-Carga-Compras-506) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Compras-506 Procedure 
PROCEDURE Carga-Compras-506 :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
FOR EACH estavtas.Almacen_Stocks EXCLUSIVE-LOCK 
    WHERE LOOKUP(estavtas.Almacen_Stocks.CodAlm, s-CodAlm) > 0:
...
FOR EACH integral.Almacen NO-LOCK WHERE integral.Almacen.codcia = s-codcia
    AND LOOKUP(integral.Almacen.codalm, s-CodAlm) > 0
...
&IF DEFINED(EXCLUDE-Carga-Transferencias-506) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Transferencias-506 Procedure 
PROCEDURE Carga-Transferencias-506 :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
&IF DEFINED(EXCLUDE-Inconsistencias-506) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Inconsistencias-506 Procedure 
PROCEDURE Inconsistencias-506 :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
