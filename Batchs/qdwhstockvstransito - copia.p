...
FOR EACH estavtas.Almacen_Stocks WHERE estavtas.Almacen_Stocks.StkAct > 0:
    RUN gn/stock-comprometido-v2.p (estavtas.Almacen_Stocks.codmat, 
                            estavtas.Almacen_Stocks.codalm, 
                            NO,
                            OUTPUT x-StockComprometido).
/*     RUN stock-comprometido (estavtas.Almacen_Stocks.codmat,      */
/*                             estavtas.Almacen_Stocks.codalm,      */
/*                             NO,                                  */
/*                             OUTPUT x-StockComprometido).         */
    ASSIGN
...
/* REPOSICIONES */
FOR EACH integral.Almacen NO-LOCK WHERE integral.Almacen.codcia = s-codcia:
    FOR EACH integral.Almcrepo NO-LOCK WHERE integral.Almcrepo.codcia = integral.Almacen.codcia
...
FOR EACH integral.Almacen NO-LOCK WHERE integral.Almacen.codcia = s-codcia:
    FOR EACH integral.Almcmov NO-LOCK WHERE integral.Almcmov.codcia = integral.Almacen.codcia
...
FOR EACH integral.gn-divi NO-LOCK WHERE INTEGRAL.GN-DIVI.CodCia = s-codcia:
    FOR EACH INTEGRAL.FacCPedi NO-LOCK WHERE INTEGRAL.FacCPedi.CodCia = INTEGRAL.GN-DIVI.CodCia
...
FOR EACH estavtas.Almacen_Stocks:
