...
DEFINE TEMP-TABLE RESUMEN LIKE almdrepo
...
/* Barremos la tabla de intercambio */
FOR EACH OOReposiciones WHERE OOReposiciones.CodCia = s-codcia
    ...
    AND  OOReposiciones.CanPed > OOReposiciones.CanAte,
    FIRST Almmmatg OF OOReposiciones NO-LOCK,
    FIRST Almmmate OF OOReposiciones NO-LOCK:
    /* Stock Minimo */
    ASSIGN
        /* <<<< OJO: Tomamos este valor <<< */
        x-StockMinimo = OOReposiciones.CanPed -  OOReposiciones.CanAte  
    ...
    FOR EACH Almrepos NO-LOCK WHERE  almrepos.CodCia = s-codcia
        ...
        DISPLAY 'Procesado:' T-DREPO.codmat SKIP WITH STREAM-IO NO-BOX NO-LABELS.
        PAUSE 0.
    END.
END.
...
DO TRANSACTION ON ERROR UNDO, RETURN "ADM-ERROR" ON STOP UNDO, RETURN "ADM-ERROR":
    ...
    FOR EACH T-DREPO BREAK BY T-DREPO.AlmPed:
        ...
        FIND OOReposiciones WHERE ROWID(OOReposiciones) = T-DREPO.Campo-R
            EXCLUSIVE-LOCK NO-ERROR.
        IF AVAILABLE OOReposiciones THEN 
            ASSIGN
            /*OOReposiciones.FlagMigracion = 'S'*/
            OOReposiciones.CanAte = OOReposiciones.CanAte + almdrepo.CanApro.
        DELETE T-DREPO.
    END.
    RELEASE Faccorre.
