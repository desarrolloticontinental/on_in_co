...
DEF TEMP-TABLE Resumen NO-UNDO
...
DEF VAR x-Fecha-Proceso AS DATE NO-UNDO.
x-Fecha-Proceso = TODAY.
/*
x-Fecha-Proceso = DATE(02,15,2024).     /* Solo para pruebas */
*/

/* 17/10/23: Todo */
PUT 'INICIO: ' NOW SKIP.
/* 1) Buscamos los artículos que han tenido movimiento en el día */
FOR EACH almacen NO-LOCK WHERE almacen.codcia = s-codcia AND almacen.codalm = "11":
    FOR EACH almdmov NO-LOCK WHERE almdmov.codcia = almacen.codcia
        AND almdmov.codalm = almacen.codalm 
        AND almdmov.fchdoc = x-Fecha-Proceso,
        FIRST Almmmatg OF Almdmov NO-LOCK WHERE LOOKUP(TRIM(Almmmatg.catconta[1]), 'MC,MI') > 0: 
        FIND Resumen WHERE Resumen.codalm = almdmov.codalm 
            AND Resumen.codmat = almdmov.codmat
            NO-LOCK NO-ERROR.
        IF NOT AVAILABLE Resumen THEN DO:
            CREATE Resumen.
            ASSIGN
                Resumen.codalm = almdmov.codalm
                Resumen.codmat = almdmov.codmat.
        END.
    END.
END.
PUT 'Carga del temporal terminado: ' NOW SKIP.

FOR EACH Resumen EXCLUSIVE-LOCK,
    FIRST Almmmate NO-LOCK WHERE Almmmate.codcia = s-codcia AND
    Almmmate.codalm = Resumen.codalm AND
    Almmmate.codmat = Resumen.codmat,
    ...
    FIND LAST AlmStkge USE-INDEX Llave01 WHERE AlmStkge.CodCia = s-codcia AND
        ...
        AlmStkge.Fecha <= x-Fecha-Proceso
        NO-LOCK NO-ERROR.
    ...
    FIND LAST AlmStkal USE-INDEX Llave01 WHERE AlmStkal.CodCia = s-codcia AND
        ...
        AlmStkal.Fecha <= x-Fecha-Proceso
        NO-LOCK NO-ERROR.
