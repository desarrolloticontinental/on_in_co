...
FOR EACH Ventas_Cabecera EXCLUSIVE-LOCK WHERE Ventas_Cabecera.DateKey >= x-CodFchI AND 
...
&IF DEFINED(EXCLUDE-Carga-Almacen) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Almacen Procedure 
PROCEDURE Carga-Almacen :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    FOR EACH DimAlmacen EXCLUSIVE-LOCK:
        DELETE DimAlmacen.
    END.
    FOR EACH integral.Almacen NO-LOCK WHERE integral.Almacen.codcia = s-codcia:
        CREATE DimAlmacen.
        BUFFER-COPY integral.Almacen TO DimAlmacen.
    END.
...
&IF DEFINED(EXCLUDE-Carga-Cliente) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Cliente Procedure 
PROCEDURE Carga-Cliente :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    FOR EACH gn-clie NO-LOCK WHERE gn-clie.codcia = cl-codcia:
        FIND FIRST DimCliente OF gn-clie EXCLUSIVE-LOCK NO-ERROR.
...
------------------------------------------------------------------------------*/
    ...
    /* DIMENSION TIEMPO */
    RUN Carga-Tiempo.
    PUT 'Dimension Tiempo: ' NOW SKIP.
    PAUSE 0.
    /* DIMENSION CLIENTE */
    RUN Carga-Cliente.
    PUT 'Dimension Cliente: ' NOW SKIP.
    PAUSE 0.

    /* DIMENSION TARJETA CLIENTE EXCLUSIVO */
    RUN Carga-Tarjeta.
    PUT 'Dimension Tarjeta: ' NOW SKIP.
    PAUSE 0.

    /* DIMENSION PROVEEDOR */
    RUN Carga-Proveedor.
    PUT 'Dimension Proveedor: ' NOW SKIP.
    PAUSE 0.

    /* Dimension  División */
    RUN Carga-Division.
    PUT 'Dimension Division: ' NOW SKIP.
    PAUSE 0.

    /* DIMENSION ALMACENES */
    RUN Carga-Almacen.
    PUT 'Dimension Almacen: ' NOW SKIP.
    PAUSE 0.

    /* DIMENSION LINEAS DE PRODUCTOS */
    RUN Carga-Linea.
    PUT 'Dimension Lineas: ' NOW SKIP.
    PAUSE 0.

    /* DIMENSION PRODUCTO */
    RUN Carga-Producto.
    PUT 'Dimension Producto: ' NOW SKIP.
    PAUSE 0.

    /* DIMENSION LICENCIA */
    RUN Carga-Licencia.
    PUT 'Dimension Licencia: ' NOW SKIP.
    PAUSE 0.

    /* DIMENSION VENDEDOR */
    RUN Carga-Vendedor.
    PUT 'Dimension Vendedor: ' NOW SKIP.
    PAUSE 0.

    /* Dimension Ubicaciones */
    RUN Carga-Ubicacion.
    PUT 'Dimension Ubicacion: ' NOW SKIP.
    PAUSE 0.
...
&IF DEFINED(EXCLUDE-Carga-Division) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Division Procedure 
PROCEDURE Carga-Division :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    FOR EACH gn-divi NO-LOCK WHERE gn-divi.codcia = s-codcia:
        FIND FIRST DimDivision WHERE DimDivision.CodDiv = gn-divi.coddiv EXCLUSIVE-LOCK NO-ERROR.
...
&IF DEFINED(EXCLUDE-Carga-Licencia) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Licencia Procedure 
PROCEDURE Carga-Licencia :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    FIND FIRST DimLicencia WHERE DimLicencia.Licencia = "" EXCLUSIVE-LOCK NO-ERROR.
    ...
    FOR EACH Almtabla NO-LOCK WHERE Almtabla.tabla = "LC":
        FIND FIRST DimLicencia WHERE DimLicencia.Licencia = Almtabla.codigo EXCLUSIVE-LOCK NO-ERROR.
    ...
    FOR EACH Almtabla NO-LOCK WHERE Almtabla.tabla = "LN":
        FIND FIRST DimLicenciatario WHERE DimLicenciatario.Licenciatario = Almtabla.codigo EXCLUSIVE-LOCK NO-ERROR.
...
&IF DEFINED(EXCLUDE-Carga-Linea) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Linea Procedure 
PROCEDURE Carga-Linea :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    FOR EACH DimLinea EXCLUSIVE-LOCK:
        DELETE DimLinea.
    END.
    ...
    END.
    FOR EACH DimSubLinea EXCLUSIVE-LOCK:
        DELETE DimSubLinea.
    END.
...
&IF DEFINED(EXCLUDE-Carga-Producto) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Producto Procedure 
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH Almmmatg NO-LOCK WHERE Almmmatg.codcia = s-codcia:
        ...
        FIND FIRST DimProducto OF Almmmatg EXCLUSIVE-LOCK NO-ERROR.
        ...
        BUFFER-COPY Almmmatg 
            ...
            DimProducto.TpoArt      = Almmmatg.TpoArt            /* Datos adicionales para cálculo CLASIFICACION */
            DimProducto.CtgCtble    = Almmmatg.CatConta[1] 
            DimProducto.FchIng      = Almmmatg.FchIng
            NO-ERROR.
...
&IF DEFINED(EXCLUDE-Carga-Proveedor) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Proveedor Procedure 
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH gn-prov NO-LOCK WHERE gn-prov.codcia = pv-codcia:
        FIND FIRST DimProveedor OF gn-prov EXCLUSIVE-LOCK NO-ERROR.
...
&IF DEFINED(EXCLUDE-Carga-Tarjeta) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Tarjeta Procedure 
PROCEDURE Carga-Tarjeta :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    FOR EACH gn-card NO-LOCK:
        FIND FIRST DimTarjeta OF gn-card EXCLUSIVE-LOCK NO-ERROR.
...
&IF DEFINED(EXCLUDE-Carga-Tiempo) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Tiempo Procedure 
PROCEDURE Carga-Tiempo :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    END.
    DO Z = X TO Y:
        FIND FIRST DimFecha WHERE DimFecha.DateKey = Z EXCLUSIVE-LOCK NO-ERROR.
...
&IF DEFINED(EXCLUDE-Carga-Ubicacion) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Ubicacion Procedure 
PROCEDURE Carga-Ubicacion :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    FOR EACH DimUbicacion EXCLUSIVE-LOCK:
        DELETE DimUbicacion.
    END.
...
&IF DEFINED(EXCLUDE-Carga-Vendedor) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Vendedor Procedure 
PROCEDURE Carga-Vendedor :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    FOR EACH gn-ven NO-LOCK WHERE gn-ven.codcia = s-codcia:
        FIND FIRST DimVendedor OF gn-ven EXCLUSIVE-LOCK NO-ERROR.
...
FOR EACH DimDivision NO-LOCK,
    EACH CcbCdocu USE-INDEX Llave10 NO-LOCK WHERE CcbCdocu.CodCia = S-CODCIA 
        ...
        FIND Ventas_Cabecera WHERE ROWID(Ventas_Cabecera) = pRowid NO-LOCK.
        /* ******************************** */
        ASSIGN
           x-Coe = 1
           x-Can = 1.
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR':
    ...
    ASSIGN
        ...
        Ventas_Cabecera.CndCre = Ccbcdocu.cndcre
        /* 04/09/2024: Nueva información para Cálculo de Clasificaciones */
        Ventas_Cabecera.CodRef = Ccbcdocu.codref
        Ventas_Cabecera.NroRef = Ccbcdocu.nroref
        Ventas_Cabecera.TpoFac = Ccbcdocu.tpofac
        NO-ERROR.
    ...
    IF AVAILABLE Faccpedi THEN DO:
        ...
        CASE Faccpedi.CodDoc:
            ...
            END.
            OTHERWISE DO:
                ASSIGN Ventas_Cabecera.ListaBase = pCodDiv.
            END.
        END CASE.
    END.
/*     IF AVAILABLE Faccpedi THEN DO:                                                                           */
...
/*     END.                                                                                                                             */
    ...
    ASSIGN
        ...
        Ventas_Detalle.Cantidad = ( x-signo1 * x-CanDes )
    ...
    IF Ccbcdocu.CodMon = 1 THEN DO:
        ...
        ASSIGN
            Ventas_Detalle.ImpExtSIGV   = x-signo1 * x-ImpLinSIGV * x-coe / x-TpoCmbCmp
            Ventas_Detalle.ImpNacSIGV   = x-signo1 * x-ImpLinSIGV * x-coe.
...
/*             Ventas_Detalle.ImpNacSIGV   = Ventas_Detalle.ImpNacCIGV / ( 1 + ( x-PorIgv / 100) ). */
    ...
    IF Ccbcdocu.CodMon = 2 THEN DO:
        ...
        ASSIGN
            Ventas_Detalle.ImpExtSIGV   = x-signo1 * x-ImpLinSIGV * x-coe
            Ventas_Detalle.ImpNacSIGV   = x-signo1 * x-ImpLinSIGV * x-coe * x-TpoCmbVta.
/*             Ventas_Detalle.ImpExtSIGV   = Ventas_Detalle.ImpExtCIGV / ( 1 + ( x-PorIgv / 100) )  */
/*             Ventas_Detalle.ImpNacSIGV   = Ventas_Detalle.ImpNacCIGV / ( 1 + ( x-PorIgv / 100) ). */
    END.
    ...
    END.
    ASSIGN
        Ventas_Detalle.PromExtSIGV = x-signo1 * x-CanDes * x-CtoUni * x-coe / x-TpoCmbCmp
        Ventas_Detalle.PromExtCIGV = Ventas_Detalle.PromExtSIGV * ( 1 + ( x-PorIgv / 100) )
        Ventas_Detalle.PromNacSIGV = x-signo1 * x-CanDes * x-CtoUni * x-coe
    ...
    IF Almmmatg.TpoCmb > 0  THEN DO:
        IF Almmmatg.MonVta = 1 THEN DO:
            Ventas_Detalle.CostoNacCIGV = x-signo1 * x-CanDes * x-ImpCto * x-coe.
            Ventas_Detalle.CostoExtCIGV = x-signo1 * x-CanDes * x-ImpCto * x-coe / Almmmatg.TpoCmb.
            Ventas_Detalle.CostoNacSIGV = x-signo1 * x-CanDes * x-ImpCto / ( 1 + ( x-PorIgv / 100) ) * x-coe.
            Ventas_Detalle.CostoExtSIGV = x-signo1 * x-CanDes * x-ImpCto / ( 1 + ( x-PorIgv / 100) ) * x-coe / Almmmatg.TpoCmb.
        END.
        ELSE DO:
            Ventas_Detalle.CostoExtCIGV = x-signo1 * x-CanDes * x-ImpCto * x-coe.
            Ventas_Detalle.CostoNacCIGV = x-signo1 * x-CanDes * x-ImpCto * Almmmatg.TpoCmb * x-coe.
            Ventas_Detalle.CostoExtSIGV = x-signo1 * x-CanDes * x-ImpCto / ( 1 + ( x-PorIgv / 100) ) * x-coe.
            Ventas_Detalle.CostoNacSIGV = x-signo1 * x-CanDes * x-ImpCto / ( 1 + ( x-PorIgv / 100) ) * Almmmatg.TpoCmb * x-coe.
        END.
    END.
...
FOR EACH Ventas_Cabecera NO-LOCK WHERE Ventas_Cabecera.DateKey >= x-CodFchI AND
    ...
    /* 30/05/2024: Angie Quispe */
    FIND ccbcdocu WHERE ccbcdocu.codcia = s-codcia AND
        ccbcdocu.coddoc = Ventas_Cabecera.CodDoc AND
        ccbcdocu.nrodoc = Ventas_Cabecera.NroDoc
        NO-LOCK NO-ERROR.
    IF AVAILABLE ccbcdocu THEN t-Ventas_Cabecera.Hora = CcbCDocu.HorCie.
END.
...
/* 04/09/2024: Ya no es necesario, ya está tomado en cuenta en el proceso Carga-Ventas-Cabecera */
