...
FOR EACH faccpedi NO-LOCK WHERE faccpedi.codcia = s-codcia
    AND faccpedi.coddoc = 'COT' /*AND faccpedi.nroped = '015155274'*/
    AND faccpedi.fchped >= x-fchped
    AND faccpedi.fchped <= TODAY - 1
    AND faccpedi.codref <> "COT"
    AND faccpedi.tpoped <> "LF"     /* OJO: NO Lista Express */
...
FOR EACH t-cotizado:
    FOR EACH t-pedido WHERE t-pedido.codcia = s-codcia
        ...
        AND LOOKUP(t-pedido.flgest, 'C,S,E,O') = 0,
...
FOR EACH t-pedido WHERE LOOKUP(t-pedido.flgest, 'C,S,E,O') = 0:
