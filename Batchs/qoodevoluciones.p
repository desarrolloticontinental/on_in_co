...
FOR EACH OOMoviAlmacen EXCLUSIVE-LOCK WHERE OOMoviAlmacen.codcia = s-codcia
    ...
    IF ooMoviAlmacen.TipMov = "I" AND ooMoviAlmacen.CodMov = 90 THEN DO:
        /* ¿DropShipping? */
        IF OOMoviAlmacen.UseInDropShipment = "no" THEN DO:    /* NO DropShipping */
            FIND FIRST TabGener WHERE TabGener.CodCia = Almacen.CodCia AND 
                ...
                NO-LOCK NO-ERROR.
            IF AVAILABLE TabGener THEN NEXT.    /* NO Sloting */
        END.
        ELSE DO:
            /* Sí es DropShipping */
            RUN Registra-DropShipping (INPUT OOMoviAlmacen.codmat).
        END.
    END.
    /* NO Transferencias */
    IF ooMoviAlmacen.TipMov = "I" AND ooMoviAlmacen.CodMov = 03 THEN DO:
        NEXT.
    END.
    /* NO Ingresos por Devolución: 17/12/2021 Max Ramos */
    IF ooMoviAlmacen.TipMov = "I" AND ooMoviAlmacen.CodMov = 09 THEN DO:
        NEXT.
    END.
...
DO TRANSACTION ON ERROR UNDO, RETURN ON STOP UNDO, RETURN:
    ...
    /* 
        Ticket : 69731 
        23Ago2023, se envio correo con las conclusiones de coordinacion
        Los movimientos de salidas si se encuentran en OO_GRE_HEADER, significa que tiene GRE 
        la fecha del movimiento del kardex debe ser la de la emision de la GRE
     */
    IF OOMoviAlmacen.tipmov = 'S' THEN DO:
        FIND FIRST oo_gre_header WHERE oo_gre_header.ncorrelatio = OOMoviAlmacen.nrodoc NO-LOCK NO-ERROR.
        IF AVAILABLE oo_gre_header THEN DO:
            /* Salida de OO si debe tener GRE */
            FIND FIRST gre_header WHERE gre_header.ncorrelatio = OOMoviAlmacen.nrodoc NO-LOCK NO-ERROR.
            IF AVAILABLE gre_header THEN DO:
                /* La fecha del kardex es de la emision de la GRE*/
                ASSIGN Almcmov.FchDoc = gre_header.fechaEmisionGuia.
            END.
        END.
    END.
