...
/*
IF Almmmate.StkRep > 0 AND Almmmate.Libre_d01 > 0
*/
...
FOR EACH Almacen NO-LOCK WHERE codcia = s-codcia AND autmov = YES:
    IF LOOKUP(Almacen.codalm, '10,27,501,502,503,504,505,506,507,508,509') > 0 THEN NEXT.   /* NO UTILEX */
...
DO i = 1 TO NUM-ENTRIES(x-Almacenes):
    DISPLAY 'ALMACEN' ENTRY(I, x-Almacenes) TODAY STRING(TIME, 'hh:mm') SKIP WITH STREAM-IO NO-BOX NO-LABELS.
    PAUSE 0.
...
DEF VAR x-ControlHistorico AS CHAR NO-UNDO.
...
FOR EACH Almmmate NO-LOCK WHERE Almmmate.codcia = s-codcia 
    /*AND Almmmate.codmat = "044714"  /* Dato de Prueba */*/
    ...
    /*MESSAGE "Historico" pVentaDiaria SKIP pfactor.*/
    IF pVentaDiaria = 0 AND pFactor = 0 THEN DO:    /* NO HAY HISTORICO */
        /* Recurrimos a los minimos y maximos generales */
        ASSIGN
            x-StockMinimo = Almmmatg.Libre_d01
            x-StockMaximo = Almmmatg.Libre_d02
            pVentaDiaria = MINIMUM(Almmmatg.Libre_d01, Almmmatg.Libre_d02) / pDiasMinimo
            x-ControlHistorico = "CON MINIMOS Y MAXIMOS GENERALES".
        IF x-StockMinimo = 0 OR x-StockMaximo = 0 THEN x-ControlHistorico = "SIN HISTORICO".
    END.
    ELSE DO:
        ...
        ASSIGN
            ...
            x-ControlHistorico = "CON HISTORICO".
    END.
    /*MESSAGE "Minimos y maximos" x-StockMinimo x-StockMaximo pVentaDiaria.*/
    CREATE T-MATE.
    BUFFER-COPY Almmmate TO T-MATE
        ...
        T-MATE.Libre_c01 = x-ControlHistorico.
END.
...
END.
FOR EACH T-MATE:
    FIND Almmmate OF T-MATE EXCLUSIVE-LOCK NO-ERROR NO-WAIT.
...
/*         Almmmate.StkRep = T-MATE.StkRep */
        ...
        Almmmate.Libre_c01 = T-MATE.Libre_c01
        Almmmate.Libre_c02 = STRING(T-MATE.StkMin, '->>>,>>>,>>9.99')
        Almmmate.Libre_c03 = STRING(T-MATE.StkMax, '->>>,>>>,>>9.99')
        Almmmate.Libre_c04 = STRING(T-MATE.StkRep, '->>>,>>>,>>9.99').
END.
RELEASE Almmmate.
...
/*MESSAGE 'un año 90 dias atras' x-venta-ant.*/
...
/*MESSAGE '90 dias atras' x-venta-hoy.*/
...
/*MESSAGE 'año mes anterior' x-venta-mes.*/

IF x-Venta-Ant <= 0 THEN DO:    /* NO HAY HISTORICO */
    /*pVentaDiaria = x-Venta-Hoy.*/
    /* Recurrimos a los mínimos y máximos generales */
    ASSIGN
        pVentaDiaria = 0
        pFactor = 0.
    RETURN.
...
/*MESSAGE 'promedio' pfchini pfchfin pvtapromedio x-promedio x-items .*/
