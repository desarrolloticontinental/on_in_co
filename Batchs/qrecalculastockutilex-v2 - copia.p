...
/* *********************** */
PUT UNFORMATTED 'ACTUALIZAMOS KARDEX: ' NOW SKIP.
FOR EACH t-mate NO-LOCK:
    /*PUT UNFORMATTED t-mate.codalm ' ' t-mate.codmat ' ' NOW SKIP.*/
...
DO TRANSACTION ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE:
    ...
    FOR EACH AlmStkAl EXCLUSIVE-LOCK WHERE AlmStkAl.Codcia = S-CODCIA 
        ...
        DELETE AlmStkAl.
    END.                        
    FOR EACH Almmmate EXCLUSIVE-LOCK WHERE Almmmate.Codcia = S-CODCIA 
        AND Almmmate.CodAlm = pCodAlm
        AND Almmmate.CodMat = pCodMat ON ERROR UNDO, THROW:
        Almmmate.StkAct = 0.
    END.
    FOR EACH AlmDmov USE-INDEX Almd03 EXCLUSIVE-LOCK WHERE Almdmov.Codcia = S-CODCIA 
...
DO:
    pComprometido = 0.
    RUN ./gn/stock-comprometido-v2.p (pCodMat, pCodAlm, YES, OUTPUT pComprometido).
