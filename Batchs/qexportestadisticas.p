...
DEF TEMP-TABLE t-cliente
    ...
    FIELD telfnos       AS CHAR FORMAT 'x(40)'    
    ...
    FIELD clfcom        LIKE    gn-clie.clfcom
    .
...
DEFINE VAR x-Nom AS CHAR NO-UNDO.
...
DEF TEMP-TABLE Detalle NO-UNDO
    ...
    FIELD FchIng AS DATE FORMAT '99/99/9999'
    FIELD TpoArt AS CHAR
    FIELD SubTipo AS CHAR FORMAT 'x(50)'
    .
FOR EACH almmmatg NO-LOCK WHERE almmmatg.codcia = 001:
    ...
    ASSIGN
        ...
        detalle.SubTipo = Almmmatg.Libre_c01
        detalle.FchIng = Almmmatg.FchIng
        .            
        ...
        END.
    /* Limpiar texto */
    RUN lib/limpiar-texto (detalle.desmat, '', OUTPUT x-nom).
    ASSIGN
        detalle.desmat = x-Nom.
    RUN lib/limpiar-texto (detalle.desmar, '', OUTPUT x-nom).
    ASSIGN
        detalle.desmar = x-Nom.
    /* Licencia */
    FIND Almtabla WHERE Almtabla.Tabla = 'LC' AND Almtabla.Codigo = detalle.licencia NO-LOCK NO-ERROR.
    IF AVAILABLE Almtabla THEN DO:
        /*detalle.licencia = Almtabla.nombre.*/
        detalle.licenciatario = Almtabla.nomant.    /* Código del Licenciatario */
        /* Licenciatario */
...
/*         IF AVAILABLE Almtabla THEN detalle.licenciatario = almtabla.nombre. */
    ...
    FIND Almtabla WHERE Almtabla.tabla = "ST"
        ...
        NO-LOCK NO-ERROR.
    IF AVAILABLE Almtabla THEN Detalle.SubTipo = almtabla.Codigo + ' - ' + almtabla.Nombre.
END.
...
/* 04/03/2022 Se está exportando a las 6:30am */
...
END.
/* 13/05/2022: Otras amortizaciones */
...
DEF TEMP-TABLE t-ccbcdocu
    ...
    FIELD nroref LIKE ccbcdocu.nroref   /* En caso de BD: Nro de depósito */
    FIELD CodCta LIKE ccbcdocu.codcta   /* Cuenta Contable */
    FIELD FlgAte AS CHAR FORMAT 'x(8)'  /* Código del banco */
    FIELD FchAte LIKE ccbcdocu.fchate   /* Fecha de depósito */
    FIELD Glosa  LIKE ccbcdocu.glosa    /* Nro de cuenta bancaria */
    .
/* RHC 21/07/2020 TODOS los documentos */
...
FOR EACH FacDocum NO-LOCK WHERE FacDocum.CodCia = s-CodCia AND FacDocum.TpoDoc <> ?,
    ...
    IF LOOKUP(Ccbcdocu.coddoc, 'FAC,BOL') > 0 THEN DO:
        ...
        END.
    END.
    /* Caracteres extraños en la glosa */
    RUN lib/limpiar-texto (t-ccbcdocu.glosa, ' ', OUTPUT x-Glosa).
    ASSIGN
        t-ccbcdocu.glosa = x-Glosa.
END.
...
DEF VAR x-telefono AS CHAR NO-UNDO.

EMPTY TEMP-TABLE t-cliente.
FOR EACH gn-clie NO-LOCK WHERE gn-clie.codcia = 0
    AND gn-clie.codcli > '':        /* AND gn-clie.Fching >= (TODAY - 1): */
    x-telefono = gn-clie.Telfnos[1] + ',' + gn-clie.Telfnos[2] + ',' + gn-clie.Telfnos[3].
    CREATE t-cliente.
    BUFFER-COPY gn-clie EXCEPT gn-clie.telfnos TO t-cliente .
    FIND integral.VtaTabla WHERE integral.VtaTabla.CodCia = 001 AND 
        integral.VtaTabla.Tabla = 'CLNOPER' AND
        integral.VtaTabla.Llave_c1 = gn-clie.ruc
        NO-LOCK NO-ERROR.
    IF AVAILABLE integral.VtaTabla THEN t-cliente.exceptua = YES.
    ASSIGN
        t-cliente.e-maile = gn-clie.transporte[4].
    RUN lib/limpiar-texto (t-cliente.nomcli, '', OUTPUT x-nom).
    RUN lib/limpiar-texto (t-cliente.dircli, '', OUTPUT x-direc).
    t-cliente.nomcli = x-nom.
    t-cliente.dircli = x-direc.
    t-cliente.Telfnos = x-telefono.
END.
