...
DEFINE BUFFER B-PARAM FOR CmpAutomParam.
DEFINE TEMP-TABLE T-DREPO NO-UNDO LIKE CmpAutomDetail.
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      ...
      TABLE: B-PARAM B "?" ? INTEGRAL CmpAutomParam
      TABLE: T-DREPO T "?" NO-UNDO INTEGRAL CmpAutomDetail
...
/* ***************************  Main Block  *************************** */
/* OJO: SE VAN A CREAR PRE COMPRAS AUTOMATICAS */
DEF VAR s-TipMov AS CHAR INIT "CAN" NO-UNDO.
DEF VAR s-coddoc AS CHAR INIT 'CAN' NO-UNDO.    /* Reposición Automática Nocturna */
DEF VAR s-User-Id AS CHAR INIT "SYSTEM" NO-UNDO.
DEF VAR pOcurrencia LIKE CmpAutomParam.Ocurrencia NO-UNDO.
...
FOR EACH B-PARAM NO-LOCK WHERE B-PARAM.CodCia = s-CodCia 
    ...
    BY  B-PARAM.Orden:
    PUT UNFORMATTED B-PARAM.Orden ' ' B-PARAM.CodAlm ' ' B-PARAM.AlmPed ' ' NOW SKIP.
...
IF x-StkAct >= (x-StockMaximo * CmpAutomParam.PorRep / 100) THEN RETURN.    /* NO VA */
...
ELSE x-TipMat = "T".
    ...
    FIND FIRST Almacen WHERE Almacen.codcia = s-codcia 
        AND Almacen.codalm = CmpAutomParam.almped NO-LOCK NO-ERROR.
    IF NOT AVAILABLE Almacen THEN RETURN.

    /* CARGAMOS T-MATE-2 CON EL RESUMEN POR CD O SOLO EL DE LA TIENDA */
    RUN RESUMEN-POR-DESPACHO (CmpAutomParam.almped, T-MATE.codmat).     /* T-MATE-2 */
    FIND FIRST T-MATE-2 WHERE T-MATE-2.codcia = s-codcia
        AND T-MATE-2.codalm = CmpAutomParam.AlmPed
    ...
    ASSIGN
        ...
        T-DREPO.Item = x-Item
        T-DREPO.AlmPed = CmpAutomParam.almped
...
FOR EACH Almmmatg NO-LOCK WHERE Almmmatg.codcia = s-codcia
    ...
    AND (TRUE <> (CmpAutomParam.Lineas > "") OR LOOKUP(Almmmatg.codfam, CmpAutomParam.Lineas) > 0)
    AND (TRUE <> (CmpAutomParam.SubLineas > "") OR LOOKUP(Almmmatg.subfam, CmpAutomParam.SubLineas) > 0)
    AND (TRUE <> (CmpAutomParam.Proveedores > "") OR LOOKUP(Almmmatg.codpr1, CmpAutomParam.Proveedores) > 0)
    AND (TRUE <> (CmpAutomParam.Marcas > "") OR LOOKUP(Almmmatg.desmar, CmpAutomParam.Marcas) > 0):
    ...
    IF AVAILABLE FacTabla THEN DO:
        IF s-Reposicion = YES /* Campaña */ THEN DO:
            IF CmpAutomParam.Tipo = "General" AND LOOKUP(factabla.campo-c[1],x-Clasificaciones) = 0 THEN NEXT.
            IF CmpAutomParam.Tipo = "Utilex" AND LOOKUP(factabla.campo-c[2],x-Clasificaciones) = 0 THEN NEXT.
            IF CmpAutomParam.Tipo = "Mayorista" AND LOOKUP(factabla.campo-c[3],x-Clasificaciones) = 0 THEN NEXT.
        END.
        ELSE DO:
            /* No Campaña */
            IF CmpAutomParam.Tipo = "General" AND LOOKUP(factabla.campo-c[4],x-Clasificaciones) = 0 THEN NEXT.
            IF CmpAutomParam.Tipo = "Utilex" AND LOOKUP(factabla.campo-c[5],x-Clasificaciones) = 0 THEN NEXT.
            IF CmpAutomParam.Tipo = "Mayorista" AND LOOKUP(factabla.campo-c[6],x-Clasificaciones) = 0 THEN NEXT.
        END.
    END.
...
------------------------------------------------------------------------------*/
  ...
  DO TRANSACTION ON ERROR UNDO, RETURN "ADM-ERROR" ON STOP UNDO, RETURN "ADM-ERROR":
      ...
      FOR EACH T-DREPO WHERE T-DREPO.AlmPed = '998':
          DELETE T-DREPO.
      END.
      FOR EACH T-DREPO WHERE T-DREPO.AlmPed <> CmpAutomParam.AlmPed:
          DELETE T-DREPO.
      END.
      IF CmpAutomParam.Sector > '' THEN DO:
          FOR EACH T-DREPO NO-LOCK:
              IF LOOKUP(T-DREPO.CodZona, CmpAutomParam.Sector) = 0 THEN DELETE T-DREPO.
          END.
      END.
      ...
      /* Generación */
      /* Se va a crear en almcrepo, almdrepo y CmpAutomDetail */
      ...
      FOR EACH T-DREPO NO-LOCK,
          ...
          IF FIRST-OF(T-DREPO.AlmPed) THEN DO:
              ...
              ASSIGN
                  ...
                  almcrepo.Glosa = CmpAutomParam.Glosa
                  Almcrepo.FlgEst = "X".    /* En proceso */
              ASSIGN
                  almcrepo.VtaPuntual     = pVtaPuntual
                  almcrepo.MotReposicion  = CmpAutomParam.Motivo.
              ASSIGN
              ...
              /* Datos de Control */
              ASSIGN
                  CmpAutomParam.NroDoc = Almcrepo.nrodoc
                  CmpAutomParam.NroSer = Almcrepo.nroser
                  CmpAutomParam.TipMov = s-TipMov.
              ASSIGN
                  x-TotPeso  = 0
                  x-TotVolu  = 0
                  x-ImpTotal = 0
          ...
          /* ****************** */
          CREATE CmpAutomDetail.
          BUFFER-COPY T-DREPO TO CmpAutomDetail
              ASSIGN
              CmpAutomDetail.NroDoc = CmpAutomParam.NroDoc
              CmpAutomDetail.NroSer = CmpAutomParam.NroSer
              CmpAutomDetail.TipMov = CmpAutomParam.TipMov.
          ASSIGN
              CmpAutomDetail.FaltanteGrupo = fFaltanteGrupo().
      ...
      IF AVAILABLE CmpAutomDetail THEN RELEASE CmpAutomDetail.
  END.
...
FOR EACH CmpAutomParam EXCLUSIVE-LOCK WHERE CmpAutomParam.CodCia = s-CodCia 
    AND CmpAutomParam.Dia = WEEKDAY(LocalFechaProceso)
    AND CmpAutomParam.FechaInicio <> LocalFechaProceso
    ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ASSIGN
        CmpAutomParam.FechaFin = ?
        CmpAutomParam.FechaInicio = ?
        CmpAutomParam.HoraFin = ''
        CmpAutomParam.HoraInicio = ''
        CmpAutomParam.NroDoc = 0
        CmpAutomParam.NroSer = 0
        CmpAutomParam.Ocurrencia = ''
        CmpAutomParam.TipMov = ''.
END.
RETURN 'OK'.
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    {lib/lock-genericov3.i ~
        &Tabla="CmpAutomParam" ~
        &Alcance="FIRST" ~
        &Condicion="ROWID(CmpAutomParam) = ROWID(B-PARAM)" ~
        ...
        &TipoError="UNDO, RETURN 'ADM-ERROR'"}
    ASSIGN
        CmpAutomParam.FechaInicio = LocalFechaProceso
        CmpAutomParam.HoraInicio = STRING(TIME, 'HH:MM:SS').
    ASSIGN
        s-CodAlm = CmpAutomParam.CodAlm.
    FIND Almacen WHERE Almacen.codcia = s-codcia AND Almacen.codalm = B-PARAM.CodAlm NO-LOCK NO-ERROR.
    ASSIGN
    ...
    x-Clasificaciones = ''.
    j = 0.
    DO k = 1 TO NUM-ENTRIES(CmpAutomParam.Clasificacion):
        j = j + 1.
        x-Clasificaciones = x-Clasificaciones + (IF j = 1 THEN '' ELSE ',') +
            ENTRY(k, CmpAutomParam.Clasificacion).
    END.
    EMPTY TEMP-TABLE T-DREPO.
    ...
    END.
    ASSIGN
        CmpAutomParam.FechaFin = LocalFechaProceso
        CmpAutomParam.HoraFin = STRING(TIME, 'HH:MM:SS')
        CmpAutomParam.Ocurrencia = pOcurrencia.
END.
