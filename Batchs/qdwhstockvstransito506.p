...
PUT UNFORMATTED "Carga reposiciones " DATETIME(TODAY, MTIME) SKIP.
RUN Carga-Reposiciones.

PUT UNFORMATTED "Carga transferencias " DATETIME(TODAY, MTIME) SKIP.
RUN Carga-Transferencias.

PUT UNFORMATTED "Carga ordenes " DATETIME(TODAY, MTIME) SKIP.
RUN Carga-Ordenes.

PUT UNFORMATTED "Carga sloting " DATETIME(TODAY, MTIME) SKIP.
RUN Carga-sloting.
...
/* END.                                                                                                                                         */
    ...
    FOR EACH integral.OOComPend NO-LOCK WHERE integral.OOComPend.CodAlm = s-codalm AND 
        (integral.OOComPend.CanPed - integral.OOComPend.CanAte) > 0:
        FIND FIRST estavtas.Almacen_Stocks WHERE estavtas.Almacen_Stocks.codalm = integral.OOComPend.CodAlm AND 
            estavtas.Almacen_Stocks.codmat = integral.OOComPend.CodMat
...
&IF DEFINED(EXCLUDE-Carga-Ordenes) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Ordenes Procedure 
...
&IF DEFINED(EXCLUDE-Carga-Reposiciones) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Reposiciones Procedure 
PROCEDURE Carga-Reposiciones :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
/* REPOSICIONES */
FOR EACH integral.Almacen NO-LOCK WHERE integral.Almacen.codcia = s-codcia AND integral.Almacen.CodAlm = s-CodAlm,
...
&IF DEFINED(EXCLUDE-Carga-sloting) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-sloting Procedure 
PROCEDURE Carga-sloting :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEF VAR cAlmDes AS CHAR NO-UNDO.
FOR EACH integral.Almacen NO-LOCK WHERE integral.Almacen.codcia = s-codcia AND integral.Almacen.campo-c[9] <> "I" ,
    EACH integral.Almcmov NO-LOCK WHERE integral.Almcmov.codcia = integral.Almacen.codcia
        ...
        AND integral.Almcmov.fchdoc >= ADD-INTERVAL(TODAY, -60, 'days')
...
x-CtoLis = integral.Almmmatg.CtoTot.    /* CON IGV */
/* Calculamos SIN IGV */
IF Almmmatg.AftIgv = YES THEN DO:
    FIND FIRST Sunat_Fact_Electr_Taxs  WHERE Sunat_Fact_Electr_Taxs.TaxTypeCode = "IGV"
        ...
        NO-LOCK NO-ERROR.
    IF AVAILABLE Sunat_Fact_Electr_Taxs THEN x-CtoLis = x-CtoLis / ( 1 + Sunat_Fact_Electr_Taxs.Tax / 100).
END.
/* Costo Reposicion */
IF Almmmatg.monvta = 1 THEN ASSIGN estavtas.Almacen_Stocks.CostoMn = x-CtoLis.
ELSE ASSIGN estavtas.Almacen_Stocks.CostoMn = x-CtoLis * integral.Almmmatg.TpoCmb.
...
CREATE LogBacheros.
ASSIGN
    LogBacheros.Detalle = 'Actualiza el stock en la tabla Almacen_Stocks del almacén '+ s-codalm
...
END.
/* ****************************************** */
/* 01/03/2023 Artículos que no se van a pasar */
/* ****************************************** */
EMPTY TEMP-TABLE t-NoPasados.
FOR EACH integral.Almacen NO-LOCK WHERE integral.Almacen.codcia = s-codcia
        AND integral.Almacen.codalm = s-CodAlm
        AND integral.Almacen.Campo-c[9] <> "I",     /* SOLO Almacenes Activos */
    EACH integral.Almmmate NO-LOCK WHERE INTEGRAL.Almmmate.CodCia = integral.Almacen.codcia AND
        INTEGRAL.Almmmate.CodAlm = integral.Almacen.codalm:
    FIND FIRST estavtas.Almacen_Stocks WHERE estavtas.Almacen_Stocks.CodAlm = INTEGRAL.Almmmate.CodAlm AND
        estavtas.Almacen_Stocks.CodMat = INTEGRAL.Almmmate.codmat
        NO-LOCK NO-ERROR.
    IF AVAILABLE estavtas.Almacen_Stocks THEN NEXT.
    CREATE t-NoPasados.
    t-NoPasados.codalm = INTEGRAL.Almmmate.CodAlm.
    t-NoPasados.codmat = INTEGRAL.Almmmate.codmat.
END.
/* ****************************************** */
...
FOR EACH t-Almacen_Stocks NO-LOCK /*WHERE Almacen_Stocks.CodAlm = s-CodAlm*/:
    EXPORT DELIMITER "~029" t-Almacen_Stocks.
END.
OUTPUT CLOSE.

/* 01/03/2023 El texto con los NO migrados */
x-Archivo = "/home/v/IN/dbs/" + "almacen_stocks_no_migrados.txt".
OUTPUT TO VALUE(x-archivo) KEEP-MESSAGES.
FOR EACH t-NoPasados NO-LOCK:
    EXPORT DELIMITER "~029" t-NoPasados.
END.
OUTPUT CLOSE.
...
x-Archivo = "/home/v/IN/dbs/" + "almacen_stocks.txt".
OUTPUT TO VALUE(x-archivo) KEEP-MESSAGES.
FOR EACH Almacen_Stocks NO-LOCK /*WHERE Almacen_Stocks.CodAlm = s-CodAlm*/:
