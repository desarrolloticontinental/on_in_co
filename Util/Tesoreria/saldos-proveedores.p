...
DEF VAR x-ClfAux AS CHAR INIT '@PV'.
...
FOR EACH Cp-tpro NO-LOCK  WHERE 
         ...
         Cp-tpro.CORRELATIVO = lCorrelativo:
...
/* SALIDA A TEXTO */
OUTPUT TO c:\tmp\proveedores-con-saldo.txt.
FOR EACH DMOV NO-LOCK ,
    FIRST gn-prov NO-LOCK WHERE gn-prov.codcia = cb-codcia
    AND gn-prov.codpro = dmov.codaux
    BREAK BY dmov.codaux:
    IF FIRST-OF(dmov.codaux) THEN
    DISPLAY
        dmov.codaux
        gn-prov.NomPro.
END.
