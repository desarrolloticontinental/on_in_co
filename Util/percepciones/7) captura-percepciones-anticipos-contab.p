...
SYSTEM-DIALOG GET-FILE FILL-IN-Archivo
    ...
    TITLE "PERCEPCION POR ANTICIPOS"
    MUST-EXIST
    USE-FILENAME
    UPDATE OKpressed.
...
DEF TEMP-TABLE T-DMVTO LIKE ccbdmvto
    FIELD coddep LIKE ccbdmvto.coddoc.
...
REPEAT:
    ...
    ASSIGN
        ...
        T-DMVTO.nrodep = cValue.    /* REFERENCIA */
    /* CLIENTE */
    t-Column = t-Column + 2.
    cValue = chWorkSheet:Cells(t-Row, t-Column):VALUE.
    ASSIGN
        T-DMVTO.codcli = TRIM(cValue).
    /* REF */
    t-Column = t-Column + 2.
    cValue = chWorkSheet:Cells(t-Row, t-Column):VALUE.
    ASSIGN
        T-DMVTO.codref = TRIM(cValue).
    t-Column = t-Column + 1.
    cValue = chWorkSheet:Cells(t-Row, t-Column):VALUE.
    ASSIGN
        T-DMVTO.nroref = TRIM(cValue).
    /* PRECIO VENTA */
    t-Column = t-Column + 3.
    cValue = chWorkSheet:Cells(t-Row, t-Column):VALUE.
    ASSIGN
...
/*     MESSAGE t-dmvto.coddiv t-dmvto.codcli t-dmvto.nrodoc. */
END.
...
DEF VAR s-nrodoc AS INT INIT 000365 NO-UNDO.
...
FOR EACH T-DMVTO BREAK BY T-DMVTO.coddiv BY T-DMVTO.codcli BY T-DMVTO.nrodoc:
    IF FIRST-OF(T-DMVTO.coddiv) OR FIRST-OF(T-DMVTO.codcli) OR x-CuentaItems > 11
        ...
        ASSIGN
            ...
            CcbCMvto.CodMon = 1
            CcbCMvto.FchDoc = 12/31/2014    /*T-DMVTO.fchemi*/
            CcbCMvto.FlgEst = "P"
            CcbCMvto.Glosa = ""
            CcbCMvto.ImpTot = 0
            CcbCMvto.NroDoc = STRING(s-nroser, '999') + STRING(s-nrodoc, '999999')
            CcbCMvto.Libre_chr[1] = T-DMVTO.coddep
