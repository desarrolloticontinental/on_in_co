...
REPEAT:
    ...
    ASSIGN
        ...
        t-cdocu.coddoc = x-coddoc
        t-cdocu.nrodoc = x-nrodoc.

    t-Column = t-Column + 2.
    cValue = chWorkSheet:Cells(t-Row, t-Column):VALUE.
    ASSIGN
        t-cdocu.codmon = INTEGER(cValue).

    t-Column = t-Column + 1.
    cValue = chWorkSheet:Cells(t-Row, t-Column):VALUE.
    ASSIGN
        t-cdocu.tpocmb = DECIMAL(cValue).

    t-Column = t-Column + 4.
    cValue = chWorkSheet:Cells(t-Row, t-Column):VALUE.
    IF DECIMAL(cValue) > 0      /* 1ro NOTAS DE CREDITO */
        THEN  ASSIGN
                t-cdocu.imptot = t-cdocu.imptot + DECIMAL(cValue).
END.
...
FOR EACH t-cdocu:
    FIND ccbcdocu WHERE ccbcdocu.codcia = s-codcia
        ...
        NO-LOCK.
    RUN Nota-de-Credito-Debito ("N/C", "00003").
END.
EMPTY TEMP-TABLE t-cdocu.


/* SEGUNDO NOTAS DE DEBITO */
CREATE "Excel.Application" chExcelApplication.
chWorkbook = chExcelApplication:Workbooks:OPEN(FILL-IN-Archivo).
chWorkSheet = chExcelApplication:Sheets:ITEM(1).
ASSIGN
...
REPEAT:
    ...
    t-Column = t-Column + 4.
    cValue = chWorkSheet:Cells(t-Row, t-Column):VALUE.
    IF DECIMAL(cValue) < 0      /* 2do NOTAS DE DEBITO */
        THEN  ASSIGN
                t-cdocu.imptot = t-cdocu.imptot + DECIMAL(cValue).
END.
chExcelApplication:QUIT().
...
FOR EACH t-cdocu:
    t-cdocu.imptot = -1 * t-cdocu.imptot.
END.
FOR EACH t-cdocu WHERE t-cdocu.imptot <= 0:
    DELETE t-cdocu.
END.
FOR EACH t-cdocu:
    FIND ccbcdocu WHERE ccbcdocu.codcia = s-codcia
        ...
        AND ccbcdocu.nrodoc = t-cdocu.nrodoc
        NO-LOCK.
    RUN Nota-de-Credito-Debito ("N/D", "00002").
END.
