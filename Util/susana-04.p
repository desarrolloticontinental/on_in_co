...
for each almmmatg where codcia = 001 no-lock:
    ...
    then detalle.ultsal = almdmov.fchdoc.
    find last almdmov use-index almd03 where almdmov.codcia = 001
        and almdmov.codalm = '16'
        and almdmov.codmat = almmmatg.codmat
        and almdmov.tipmov = 'i'
        and (almdmov.codmov = 17 or almdmov.codmov = 03)
        no-lock no-error.
    if available almdmov
    then detalle.ulting = almdmov.fchdoc.
