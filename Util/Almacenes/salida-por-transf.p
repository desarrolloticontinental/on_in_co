...
DEF VAR s-codalm AS CHAR INIT '11'.        /* <<< OJO <<< */
...
/* FOR EACH ITEM:                       */
/*     DISPLAY item.codmat item.candes. */
/* END.                                 */
/* RETURN.                              */
...
REPEAT:
    ...
    IF x-linea <> '' THEN DO:
        ...
        ASSIGN
            ...
            ITEM.candes = DEC(SUBSTRING(x-linea,7))
            NO-ERROR.
...
FOR EACH ITEM:
    FIND Almmmate WHERE ALmmmate.codcia = s-codcia
        ...
        NO-LOCK.
    RUN vta2/stock-comprometido (ITEM.CodMat,
                                 s-CodAlm,
                                 OUTPUT pComprometido).
    ITEM.CanDes = MINIMUM(ITEM.candes, almmmate.stkact - pcomprometido).
    IF ITEM.candes <= 0 THEN DELETE ITEM.
END.
