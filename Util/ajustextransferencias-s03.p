...
def var s-user-id as char init 'ENE152006'.
def temp-table t-matg like almmmatg.

x-fchdoc = date(01,31,2006).    /* fecha de corte */
input from c:\tmp\sene06.PRN.
repeat:
    import unformatted x-linea.
    find almdmov where almdmov.codcia = s-codcia
        and almdmov.codalm = substring(x-linea,27,3)
        and almdmov.tipmov = substring(x-linea,47,1)
        and almdmov.codmov = integer(substring(x-linea,57,2))
        and almdmov.nroser = integer(substring(x-linea,67,3))
        and almdmov.nrodoc = integer(substring(x-linea,77,6))
        ...
        if not available b-cmov then do:
            ...
            buffer-copy almcmov to b-cmov
                assign b-cmov.fchdoc = x-fchdoc
                       /*b-cmov.nrorf1 = substring(x-linea,81,15)*/.
        end.
    end.        
end.
input close.
