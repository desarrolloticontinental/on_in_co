...
DEF VAR s-CodAlm AS CHAR INIT "34E" NO-UNDO.
...
FOR EACH detalle.
    ...
    IF x-alm11e > 0 THEN DO:
        ...
        ASSIGN
            ...
            t-det.codmat = detalle.codmat
            t-det.almdes = '11e'
            t-det.canped = x-alm11e.
    END.
    IF x-alm34e > 0 THEN DO:
        CREATE t-det.
        ASSIGN
...
/*             T-DET.CanPed = DECIMAL(SUBSTRING(x-Linea,7)).        */
/*     END.                                                         */
/* END.                                                             */
/* INPUT CLOSE.                                                     */
...
FOR EACH DCMP, FIRST Almmmatg WHERE almmmatg.codcia = s-codcia
    ...
    IF LAST-OF(Almmmatg.CodPr1) OR x-Cuenta-Items > 18
    THEN DO:
