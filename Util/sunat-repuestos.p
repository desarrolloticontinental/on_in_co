def var x-sdoini as dec format '->>>,>>>,>>9.99'.
def var x-sdofin as dec format '->>>,>>>,>>9.99'.
...
for each almdmov where codcia = 001
    ...
    first almmmatg of almdmov no-lock where codfam = '008' 
        and lookup(trim(subfam), '100,200,201') > 0:
    create t-matg.
...
output to c:\tmp\sunat-repuestos.txt.
for each t-matg:
    assign
        x-sdoini = 0
        x-sdofin = 0.
    find last almstkge where almstkge.codcia = 001
        ...
        and almstkge.fecha < 01/01/2003
        no-lock no-error.
    if available almstkge then x-sdoini = AlmStkge.StkAct.
    find last almstkge where almstkge.codcia = 001
        and almstkge.codmat = t-matg.codmat
        and almstkge.fecha <=
         12/31/2003
        no-lock no-error.
    if available almstkge then x-sdofin = AlmStkge.StkAct.
    display
        t-matg.codmat
        t-matg.desmat
        t-matg.desmar
