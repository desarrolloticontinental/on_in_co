...
&SCOPED-DEFINE l-lookup h_l-Proveedores
...
DEFINE VARIABLE h_l-proveedores AS HANDLE NO-UNDO.
...
ON WINDOW-CLOSE OF FRAME C-Dialog /* Proveedores */
DO:  
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'util/rosa/l-proveedores.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Código':U ,
             OUTPUT h_l-proveedores ).
       RUN set-position IN h_l-proveedores ( 1.19 , 2.00 ) NO-ERROR.
       ...
       RUN set-size IN h_p-option ( 1.92 , 14.29 ) NO-ERROR.

       /* Links to SmartLookup h_l-proveedores. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-proveedores ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-proveedores ,
             Btn_OK:HANDLE , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-option ,
             h_l-proveedores , 'AFTER':U ).
    END. /* Page 0 */
