...
&SCOPED-DEFINE l-lookup h_l-Provi
...
DEFINE VARIABLE h_l-provi AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 7.35 COL 49
     Btn_Cancel AT ROW 8.54 COL 49
     Btn_Help AT ROW 10.35 COL 49
     SPACE(1.13) SKIP(0.94)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'util/rosa/l-provi.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Código':U ,
             OUTPUT h_l-provi ).
       RUN set-position IN h_l-provi ( 1.19 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.92 , 45.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.19 , 48.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 13.00 ) NO-ERROR.

       /* Links to SmartLookup h_l-provi. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-provi ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-provi ,
