...
&SCOPED-DEFINE l-lookup h_l-Distr
...
DEFINE VARIABLE h_l-distr AS HANDLE NO-UNDO.
...
DEFINE FRAME C-Dialog
     Btn_OK AT ROW 7.54 COL 50
     Btn_Cancel AT ROW 8.73 COL 50
     Btn_Help AT ROW 10.54 COL 50
     SPACE(4.85) SKIP(0.75)
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'util/rosa/l-distr.w':U ,
             INPUT  FRAME C-Dialog:HANDLE ,
             INPUT  'Layout = ,
                     SortBy-Case = Código':U ,
             OUTPUT h_l-distr ).
       RUN set-position IN h_l-distr ( 1.38 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.92 , 46.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Style = Combo-Box,
                     ...
                     Font = 4':U ,
             OUTPUT h_p-option ).
       RUN set-position IN h_p-option ( 1.19 , 49.00 ) NO-ERROR.
       RUN set-size IN h_p-option ( 1.92 , 16.43 ) NO-ERROR.

       /* Links to SmartLookup h_l-distr. */
       RUN add-link IN adm-broker-hdl ( h_p-option , 'SortBy':U , h_l-distr ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_l-distr ,
