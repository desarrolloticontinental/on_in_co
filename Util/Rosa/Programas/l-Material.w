...
&Scoped-Define ENABLED-OBJECTS RECT-1 RECT-2 CMB-filtro br_table ~
FILL-IN-codigo FILL-IN-codigo-2 F-Fam FILL-IN-filtro F-SubFam RADIO-SET-1 
&Scoped-Define DISPLAYED-OBJECTS CMB-filtro FILL-IN-codigo FILL-IN-codigo-2 ~
F-Fam FILL-IN-filtro F-SubFam RADIO-SET-1 
...
DEFINE VARIABLE FILL-IN-codigo-2 AS CHARACTER FORMAT "X(6)":U 
     LABEL "Marca" 
     VIEW-AS FILL-IN 
     SIZE 6 BY .81
     BGCOLOR 15 FGCOLOR 0  NO-UNDO.
...
DEFINE FRAME F-Main
     CMB-filtro AT ROW 3.5 COL 2 NO-LABEL
     br_table AT ROW 4.65 COL 2
     FILL-IN-codigo AT ROW 1.38 COL 6 COLON-ALIGNED
     FILL-IN-codigo-2 AT ROW 2.35 COL 6 COLON-ALIGNED
     F-Fam AT ROW 1.38 COL 18 COLON-ALIGNED
     FILL-IN-filtro AT ROW 3.5 COL 24 NO-LABEL
     F-SubFam AT ROW 1.38 COL 31 COLON-ALIGNED
     RADIO-SET-1 AT ROW 1.58 COL 45 NO-LABEL
     RECT-1 AT ROW 1 COL 1
     RECT-2 AT ROW 1.19 COL 44
     "Tipo" VIEW-AS TEXT
          SIZE 4 BY .5 AT ROW 1.58 COL 40
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL F-Fam L-table-Win
ON LEAVE OF F-Fam IN FRAME F-Main /* Fam */
DO:
...
ON LEAVE OF F-SubFam IN FRAME F-Main /* SubFam */
DO:
    IF INPUT FILL-IN-codigo = 0 THEN RETURN.
    &IF "{&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}" &THEN
        FIND FIRST {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} WHERE
...
&Scoped-define SELF-NAME FILL-IN-codigo-2
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL FILL-IN-codigo-2 L-table-Win
ON LEAVE OF FILL-IN-codigo-2 IN FRAME F-Main /* Marca */
DO:
    IF INPUT FILL-IN-codigo = 0 THEN RETURN.
    &IF "{&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}" &THEN
        FIND FIRST {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} WHERE
