...
FOR EACH almmmatg WHERE codcia = s-codcia
    ...
    AND almmmatg.tpocmb = 3.25:
    x-factor = 3.25 / 3.
    ASSIGN
        almmmatg.tpocmb = 3.
    IF almmmatg.monvta = 1 THEN NEXT.   /* NO SOLES */
    DISPLAY almmmatg.codmat.
    PAUSE 0.
    ASSIGN
        almmmatg.prevta[1] = almmmatg.prevta[1] * x-factor
    ...
    Almmmatg.CtoTot = IF Almmmatg.AftIgv 
        THEN X-CTOTOT * (1 + FacCfgGn.PorIgv / 100)
        ELSE X-CTOTOT.
     ...
     IF Almmmatg.Prevta[2] > 0 THEN DO:
         ...
         IF Almmmatg.UndA <> "" THEN DO:
             ...
             IF NOT AVAILABLE Almtconv THEN DO:
                MESSAGE Almmmatg.codmat "Codigo de unidad no existe" VIEW-AS ALERT-BOX ERROR.
                NEXT.
             END.
