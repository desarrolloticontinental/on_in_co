...
DEFINE TEMP-TABLE tt-facdpedi LIKE facdpedi.
...
&Scoped-Define ENABLED-OBJECTS RECT-1 txtDesde txtFechaTope txtAlmOtros ~
BROWSE-2 Alm01 Tonelaje01 Alm01b Alm02 Tonelaje02 Alm02b Alm03 Tonelaje03 ~
Alm03b Alm04 txtDivisiones txtListaPrecio ChbxGrabaCot chbDetalleCot ~
TOGGLE-Bonificaciones btnProcesar rdBtnCual 
...
DEFINE FRAME fMain
     ...
     btnProcesar AT ROW 19.23 COL 57 WIDGET-ID 4
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-1 txtDesde txtFechaTope txtAlmOtros BROWSE-2 Alm01 Tonelaje01 
         Alm01b Alm02 Tonelaje02 Alm02b Alm03 Tonelaje03 Alm03b Alm04 
         txtDivisiones txtListaPrecio ChbxGrabaCot chbDetalleCot 
         TOGGLE-Bonificaciones btnProcesar rdBtnCual 
      WITH FRAME fMain IN WINDOW wWin.
...
FOR EACH facdpedi OF faccpedi NO-LOCK, FIRST almmmatg OF facdpedi NO-LOCK :
    ...
    ASSIGN tt-facdpedi.tipvta = x-Caso
...
FOR EACH tt-faccpedi :
    FOR EACH tt-facdpedi OF tt-faccpedi NO-LOCK,
            FIRST almmmatg OF tt-facdpedi NO-LOCK :
    
        lProcesadoDtl = YES.
    
        FIND FIRST tt-cot-hdr WHERE tt-cot-hdr.tt-divi = tt-facdpedi.coddiv AND
                                        tt-cot-hdr.tt-nrocot = tt-facdpedi.nroped EXCLUSIVE NO-ERROR.
    
        IF NOT AVAILABLE tt-cot-hdr THEN DO:
            CREATE tt-cot-hdr.
                ASSIGN tt-cot-hdr.tt-divi      = tt-faccpedi.coddiv
                        tt-cot-hdr.tt-nrocot   = tt-facdpedi.nroped
...
FOR EACH faccpedi WHERE faccpedi.codcia = s-codcia AND 
    ...
    END CASE.
    /* 
        Varificar la cotizacion si cuya lista de Precio, 
...
cRange = "U" + cColumn.
chWorkSheet:Range(cRange):Value = "Stk Disponible 29 (stk - reservado)".
cRange = "V" + cColumn.
...
FOR EACH tt-cot-final NO-LOCK :
    ...
    lStkReservado = 0.
    RUN vta2/stock-comprometido-v2.r(INPUT tt-cot-final.tt-codmat, INPUT '29',  OUTPUT lStkReservado).
    FIND FIRST almmmate WHERE almmmate.codcia = s-codcia AND
                        almmmate.codalm = '29' AND 
                        almmmate.codmat = tt-cot-final.tt-codmat NO-LOCK NO-ERROR.
    IF AVAILABLE almmmate THEN DO:
        cRange = "U" + cColumn.
        chWorkSheet:Range(cRange):Value = IF ((almmmate.stkact - lStkReservado) < 0) THEN 0 ELSE (almmmate.stkact - lStkReservado).
