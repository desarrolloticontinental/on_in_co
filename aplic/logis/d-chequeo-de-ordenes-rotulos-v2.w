...
DEFINE INPUT PARAMETER ppCodDoc  AS CHAR NO-UNDO.       /* Ojo pp */
DEFINE INPUT PARAMETER ppNroDoc  AS CHAR NO-UNDO.       /* Ojo pp */
...
DEFINE VAR pCodDoc AS CHAR INIT "".
...
&Scoped-Define ENABLED-OBJECTS BROWSE-14 BUTTON_Marcar BUTTON_Desmarcar ~
FILL-IN-oc FILL-IN-sap RADIO-SET-tipo-rotulo Btn_OK Btn_OK-2 
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-codplanilla FILL-IN-oc FILL-IN-sap ~
...
DEFINE BUTTON BUTTON_Desmarcar 
     LABEL "DESMARCAR TODOS" 
     SIZE 22 BY 1.12.

DEFINE BUTTON BUTTON_Marcar 
     LABEL "MARCAR TODOS" 
     SIZE 22 BY 1.12.
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 48 BY 17.12
...
DEFINE FRAME D-Dialog
     FILL-IN-codplanilla AT ROW 1.23 COL 18 COLON-ALIGNED WIDGET-ID 14
     BROWSE-14 AT ROW 2.46 COL 7 WIDGET-ID 200
     BUTTON_Marcar AT ROW 4.23 COL 57 WIDGET-ID 16
     BUTTON_Desmarcar AT ROW 5.58 COL 57 WIDGET-ID 18
     Btn_Cancel AT ROW 6.92 COL 57
     FILL-IN-oc AT ROW 20.12 COL 20.29 COLON-ALIGNED WIDGET-ID 4
     FILL-IN-sap AT ROW 21.19 COL 20.29 COLON-ALIGNED WIDGET-ID 6
     RADIO-SET-tipo-rotulo AT ROW 22.31 COL 10 NO-LABEL WIDGET-ID 8
     Btn_OK AT ROW 23.62 COL 8
     Btn_OK-2 AT ROW 23.62 COL 29 WIDGET-ID 12
     SPACE(37.28) SKIP(0.22)
...
/* SETTINGS FOR DIALOG-BOX D-Dialog
   FRAME-NAME                                                           */
/* BROWSE-TAB BROWSE-14 FILL-IN-codplanilla D-Dialog */
ASSIGN 
...
DO:
  ...
  IF radio-set-tipo-rotulo = 1 THEN DO:
      ...
      /*
      RUN carga-info-rotulo-v2.
      RUN imprime-rotulo-zebra-acumulativo.
      */
  END.
  ELSE DO:
      /* Supermercados */
...
ON CHOOSE OF Btn_OK-2 IN FRAME D-Dialog /* Nuevo Rotulo */
DO:
  ASSIGN radio-set-tipo-rotulo fill-in-oc fill-in-sap fill-in-oc.
...
&Scoped-define SELF-NAME BUTTON_Desmarcar
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BUTTON_Desmarcar D-Dialog
ON CHOOSE OF BUTTON_Desmarcar IN FRAME D-Dialog /* DESMARCAR TODOS */
DO:
  /*{&BROWSE-NAME}:DESELECT-ROWS().*/
  FOR EACH tt-w-report EXCLUSIVE-LOCK:
...
&Scoped-define SELF-NAME BUTTON_Marcar
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL BUTTON_Marcar D-Dialog
ON CHOOSE OF BUTTON_Marcar IN FRAME D-Dialog /* MARCAR TODOS */
DO:
  /*{&BROWSE-NAME}:SELECT-ALL().*/
  FOR EACH tt-w-report EXCLUSIVE-LOCK:
       tt-w-report.Campo-L[1] = YES.
  END.
  {&OPEN-QUERY-{&BROWSE-NAME}}
END.
...
IF ppCodDoc = 'HPK' THEN DO:
    ...
    /**/
    CREATE ttOrdenes.
        ASSIGN ttOrdenes.tCodDoc = hpk-vtacdocu.codref
                ttOrdenes.tNroDoc = hpk-vtacdocu.nroref
        .
END.
ELSE DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE carga-info-rotulo-v2 D-Dialog 
PROCEDURE carga-info-rotulo-v2 :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
IF ppCodDoc = 'HPK' THEN DO:
    FIND FIRST hpk-vtacdocu WHERE hpk-vtacdocu.codcia = s-codcia AND
        hpk-vtacdocu.coddiv = s-coddiv AND
        hpk-vtacdocu.codped = ppCodDOc AND 
        hpk-vtacdocu.nroped = ppNroDoc NO-LOCK NO-ERROR.
    IF NOT AVAILABLE hpk-vtacdocu THEN DO:
        MESSAGE "Imposible encontrar la HPK".
...
IF hpk-vtacdocu.codter <> 'ACUMULATIVO' THEN DO:
    /* Ubico la orden */
    FIND FIRST faccpedi WHERE faccpedi.codcia = s-codcia
        AND faccpedi.coddoc = pCodDoc
        AND faccpedi.nroped = pNroDoc
...
DEFINE VAR x-ord-cod AS CHAR.
DEFINE VAR x-ord-nro AS CHAR.
...
FOR EACH tt-w-report WHERE tt-w-report.campo-l[1] = YES :
    ...
    lFiler1 = tt-w-report.campo-c[1].

    IF hpk-vtacdocu.codter = 'ACUMULATIVO' THEN DO:
        x-articulo = "".

        BUSCAR_ARTICULO:
        FOR EACH tt-items-pickeados WHERE tt-items-pickeados.campo-c[5] = tt-w-report.campo-c[1] NO-LOCK :
            /* El Articulo */
            x-articulo = tt-items-pickeados.campo-c[1].
            LEAVE BUSCAR_ARTICULO.
        END.
        /* Busco la Orden */
        RUN jalar-orden-del-articulo(INPUT x-articulo, OUTPUT x-ord-cod, OUTPUT x-ord-nro).
        IF x-ord-cod = "ERROR" THEN NEXT.
        /* -------------------------------------------------------------------------------------------- */
        /* Ubico la orden */
        FIND FIRST faccpedi WHERE faccpedi.codcia = s-codcia
            AND faccpedi.coddoc = x-ord-cod
            AND faccpedi.nroped = x-ord-nro
            NO-LOCK NO-ERROR.
        ...
        IF faccpedi.coddoc = 'OTR' AND faccpedi.codref = 'PED' THEN DO:
            ...
            END.
            x-cliente-final = "".
            IF x-ord-cod = 'OTR' THEN x-cliente-final = faccpedi.codcli + " ".
            x-cliente-final = x-cliente-final + faccpedi.nomcli.
            x-direccion-final = faccpedi.dircli.
        END.
        /* Ic - 02Dic2016 - FIN */
    ...
    ASSIGN         
        ...
        rr-w-report.Campo-C[8] = cDir
        rr-w-report.Campo-C[9] = cSede
        rr-w-report.Campo-C[10] = lFiler1 /* tt-w-report.campo-c[1] */ /* ControlOD.NroEtq*/
        rr-w-report.Campo-D[2] = faccpedi.fchent
        rr-w-report.campo-c[11] = TRIM(lCodDoc + " " + lNroDoc)
        rr-w-report.campo-c[12] = x-cliente-intermedio
        rr-w-report.campo-c[13] = STRING(pNroBulto)+ "-" + pGraficoRotulo + "-" + STRING(i-nro,"999")
        rr-w-report.campo-c[14] = ""
        rr-w-report.campo-c[19] = pCodDoc    /* O/D */
        rr-w-report.campo-c[20] = pNroDoc    /* Nro */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE cargar-rotulos D-Dialog 
PROCEDURE cargar-rotulos :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    FIND FIRST vtacdocu WHERE vtacdocu.codcia = s-codcia AND
                                ...
                                vtacdocu.codped = pCodDoc AND       /* HPK */
                                vtacdocu.nroped = pNroDoc NO-LOCK NO-ERROR.
    ...
    FOR EACH tt-w-report WHERE tt-w-report.campo-l[1] = YES :
        CREATE rotulo-w-report.
            ASSIGN rotulo-w-report.Campo-C[1] = IF (AVAILABLE vtacdocu) THEN vtacdocu.codref ELSE "ERR"
                    ...
                    rotulo-w-report.Campo-C[3] = tt-w-report.campo-c[1]
                    rotulo-w-report.Campo-C[4] = ""
                    rotulo-w-report.Campo-C[30] = x-codper
                    rotulo-w-report.Campo-L[1] = tt-w-report.campo-l[1].
...
------------------------------------------------------------------------------*/
  DISPLAY FILL-IN-codplanilla FILL-IN-oc FILL-IN-sap RADIO-SET-tipo-rotulo 
      WITH FRAME D-Dialog.
  ENABLE BROWSE-14 BUTTON_Marcar BUTTON_Desmarcar FILL-IN-oc FILL-IN-sap 
         RADIO-SET-tipo-rotulo Btn_OK Btn_OK-2 
      WITH FRAME D-Dialog.
  VIEW FRAME D-Dialog.
...
x-file-zpl = "d:\tmp\" + REPLACE(ppCoddoc,"/","") + "-" + ppNroDOc + ".txt".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE imprime-rotulo-zebra-acumulativo D-Dialog 
PROCEDURE imprime-rotulo-zebra-acumulativo :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
x-file-zpl = "d:\tmp\" + REPLACE("HPK","/","") + "-" + "PRUEBA" + ".txt".
...
FOR EACH rr-w-report WHERE NO-LOCK BREAK BY rr-w-report.campo-c[19] BY rr-w-report.campo-c[20]:
    ...
    /* 
        Ic - 08Nov2019, para Brayan Acuña y a pedido de Max Ramos, debe imprimir la etiqueta HPK04000000905B004
...
FOR EACH rr-w-report WHERE NO-LOCK:
    ...
    */
    ...
    PUT STREAM REPORTE "^FD" + x-ean13 FORMAT 'x(14)' SKIP.
    PUT STREAM REPORTE "^FS" SKIP.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE jalar-orden-del-articulo D-Dialog 
PROCEDURE jalar-orden-del-articulo :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
DEFINE INPUT PARAMETER pArticulo AS CHAR.
DEFINE OUTPUT PARAMETER pCodOrd AS CHAR.
DEFINE OUTPUT PARAMETER pCodNro AS CHAR.

DEFINE VAR x-llave AS CHAR.

x-llave = ppCodDoc + "," + ppNroDoc.
...
FOR EACH x-almddocu WHERE x-almddocu.codcia = s-codcia AND
                            x-almddocu.codllave = x-llave AND
                            x-almddocu.codigo = pArticulo NO-LOCK:
    FIND FIRST ttOrdenes WHERE ttOrdenes.tcoddoc = x-almddocu.coddoc AND 
                                ttOrdenes.tnrodoc = x-almddocu.nrodoc NO-ERROR.
    IF NOT AVAILABLE ttOrdenes THEN DO:
        pCodOrd = x-almddocu.coddoc.
        pCodNro = x-almddocu.nrodoc.
        /*  */
        CREATE ttOrdenes.
        ASSIGN ttOrdenes.tcoddoc = x-almddocu.coddoc
                ttOrdenes.tnrodoc = x-almddocu.nrodoc.
        LEAVE BUSCA_ORDEN.
    END.
END.
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH tt-items-pickeados :
      CREATE tt-w-report.
        ASSIGN  /*tt-w-report.campo-c[1] = pCodDoc + "-" + pNroDOc + "-B" + STRING(x-sec,"999")*/
                tt-w-report.campo-c[1] = tt-items-pickeados.campo-c[5]
                tt-w-report.campo-l[1] = NO 
                /*tt-w-report.campo-i[1] = x-sec*/.
        x-codper = tt-items-pickeados.campo-c[30].
  END.
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH tt-items-pickeados WHERE tt-items-pickeados.campo-c[5] = pBulto NO-LOCK :
      x-retval = x-retval + (tt-items-pickeados.campo-f[3] * peso-de-articulo(tt-items-pickeados.campo-c[1])).
  END.
