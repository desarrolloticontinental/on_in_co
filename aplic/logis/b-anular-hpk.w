...
VtaCDocu.FchPed VtaCDocu.CodRef VtaCDocu.NroRef VtaCDocu.Glosa 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 93 BY 6.69
...
"VtaCDocu.FchPed" "Fecha" "99/99/9999" "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   = INTEGRAL.VtaCDocu.CodRef
     _FldNameList[5]   = INTEGRAL.VtaCDocu.NroRef
     _FldNameList[6]   = INTEGRAL.VtaCDocu.Glosa
...
{logis/i-hpk-con-obs.i {&txtMensaje}="pMensaje" ~
    &Control-RA="{&Otros-RA}" ~
    &Control-COT="{&Otros-COT}" ~
    &Control-PED="{&Otros-PED}"}
...
DO TRANSACTION ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE:
    ...
    IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
        IF TRUE <> (pMensaje > '') THEN pMensaje = "NO se pudo extornar la " + Vtacdocu.CodRef + " " + Vtacdocu.NroRef.
        UNDO, LEAVE.
    END.
END.
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    FOR EACH Vtaddocu OF Vtacdocu NO-LOCK:
        ...
        /* OJO: Como Vtaddocu.CanPed = 0 => Facdpedi.CanPed podría ser 0 */
        ASSIGN
