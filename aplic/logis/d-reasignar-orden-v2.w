...
DEFINE INPUT PARAMETER TABLE FOR tr-RutaD.
...
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-1 FILL-IN-phr 
...
DO:
    ...
    IF AVAILABLE di-rutaC THEN DO:
        ...
        END.
        ELSE DO:
            MESSAGE 'Seguro de reasignar las Ordenes hacia la nueva PHR ' + di-rutaC.nrodoc + '?'
...
DO TRANSACTION ON ERROR UNDO GRABAR_DATOS, RETURN 'ADM-ERROR' ON STOP UNDO GRABAR_DATOS, RETURN 'ADM-ERROR':
    ...
    FOR EACH tr-RutaD NO-LOCK:
        /* Barremos HPK si es que existen */
        FOR EACH x-vtacdocu EXCLUSIVE-LOCK WHERE x-vtacdocu.codcia = s-codcia AND
            x-vtacdocu.codref = tr-RutaD.codref AND       /* O/D */
            x-vtacdocu.nroref = tr-RutaD.nroref AND
        ...
        END.
        /* Mueve la orden a la PHR destino */
        FOR EACH di-rutaD EXCLUSIVE-LOCK WHERE di-rutaD.codcia = s-codcia AND
            ...
            di-rutaD.codref = tr-RutaD.codref AND     /* O/D */
            di-rutaD.nroref = tr-RutaD.nroref ON ERROR UNDO, THROW:

            x-codori = di-rutaC.coddoc .
            ...
            ASSIGN
                LogisLogControl.CodCia = s-CodCia
                LogisLogControl.CodDiv = s-CodDiv
                LogisLogControl.CodDoc = tr-RutaD.codref              /*O/D OTR */
                LogisLogControl.NroDoc = tr-RutaD.nroref
                LogisLogControl.Usuario = s-User-Id
                LogisLogControl.Evento = "PHR_REASIGN"
                LogisLogControl.Fecha = TODAY
                LogisLogControl.Hora = STRING(TIME, 'HH:MM:SS').
...
*/
RELEASE x-vtacdocu NO-ERROR.
RELEASE di-rutaD NO-ERROR.
