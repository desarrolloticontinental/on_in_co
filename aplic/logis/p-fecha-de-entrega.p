...
CASE TRUE:
    /* 03/04/2023 Flete Diferenciado RosaMendoza y HaroldSegura */
    WHEN LOOKUP(pCodDoc, "PED,O/D") > 0 AND CAN-FIND(FIRST VtaTabla WHERE VtaTabla.CodCia = s-codcia AND
                                      VtaTabla.Tabla = "CANALHR_DIV" AND
                                      VtaTabla.Llave_c1 = Almacen.CodDiv AND           /* División de Despacho */
                                      VtaTabla.Llave_c2 = pDivOri                      /* División Solicitante */
                                      NO-LOCK) THEN DO:
        /*MESSAGE 'flete diferenciado'.*/
        RUN logis/p-fecha-entrega-fletediferenciado.p (
