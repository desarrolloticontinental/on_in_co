...
/* Variables Salida a Excel o Texto */
...
INDEX IDX03 codaux codcta Voucher fecha-id coddoc nrodoc codope nroast fchdoc.
    ...
    DEF TEMP-TABLE t-Detalle
        FIELD CodAux    AS CHAR     FORMAT 'x(15)'      LABEL 'AUXILIAR'
        FIELD NomAux    AS CHAR     FORMAT 'x(60)'      LABEL 'NOMBRE'
        FIELD CodCta    AS CHAR     FORMAT 'x(10)'      LABEL 'CUENTA'
        FIELD NomCta    AS CHAR     FORMAT 'x(50)'      LABEL 'DESCRIPCION'
        FIELD FchDoc    AS DATE     FORMAT '99/99/9999' LABEL 'FECHA'
        FIELD CodDiv    AS CHAR     FORMAT 'x(5)'       LABEL 'DIVISION'
        FIELD Cco       AS CHAR                         LABEL 'CC'
        FIELD NroAst    AS CHAR                         LABEL 'ASIENTO'
        FIELD CodOpe    AS CHAR                         LABEL 'OPE'
        FIELD NroMes    AS INT      FORMAT '99'         LABEL 'MES'
        FIELD CodDoc    AS CHAR                         LABEL 'COD DOC'
        FIELD NroDoc    AS CHAR     FORMAT 'x(20)'      LABEL 'DOCUMENTO'
        FIELD FchVto    AS DATE     FORMAT '99/99/9999' LABEL 'VENCIMIENTO'
        FIELD NroRef    AS CHAR     FORMAT 'x(20)'      LABEL 'REFERENCIA'
        FIELD GloDoc    AS CHAR     FORMAT 'x(60)'      LABEL 'GLOSA'
        FIELD T-Importe AS DEC      FORMAT "->>>,>>>,>>9.99"    LABEL 'IMPORTE'
        FIELD T-Debe    AS DEC      FORMAT "->>>,>>>,>>9.99"    LABEL 'CARGO'
        FIELD T-Haber   AS DEC      FORMAT "->>>,>>>,>>9.99"    LABEL 'ABONO'
        FIELD Saldo     AS DEC      FORMAT "->>>,>>>,>>9.99"    LABEL 'SALDO ACTUAL'.
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH T-REPORT NO-LOCK USE-INDEX IDX03
        ...
        IF FIRST-OF (T-REPORT.codcta) THEN DO:
            ...
            ELSE x-nomcta = "".
        END.
        ACCUMULATE T-REPORT.T-debe  (SUB-TOTAL BY T-REPORT.nrodoc).
        ACCUMULATE T-REPORT.T-haber (SUB-TOTAL BY T-REPORT.nrodoc).
        X-CONREG = X-CONREG + 1. 
        IF LAST-OF(T-REPORT.NRODOC) AND X-CONREG > 0 THEN DO:
            Y-SALDO =
                (ACCUMULATE SUB-TOTAL BY T-REPORT.NroDoc T-REPORT.T-debe ) -
                (ACCUMULATE SUB-TOTAL BY T-REPORT.NroDoc T-REPORT.T-haber).
            CREATE T-Detalle.
            BUFFER-COPY T-REPORT TO T-DETALLE
                ASSIGN 
                T-DETALLE.NomAux = x-NomAux
                T-DETALLE.NomCta = x-NomCta
                T-DETALLE.Saldo  = Y-saldo.
            X-CONDOC = X-CONDOC + 1.
        END.
        ELSE DO:
            CREATE T-Detalle.
...
DEF VAR x-glodoc AS CHAR.
    ...
    FOR EACH T-REPORT NO-LOCK 
        ...
        ACCUMULATE T-REPORT.T-debe  (SUB-TOTAL BY T-REPORT.nrodoc).
        ACCUMULATE T-REPORT.T-haber (SUB-TOTAL BY T-REPORT.nrodoc).
        ACCUMULATE T-REPORT.T-debe  (SUB-TOTAL BY T-REPORT.codcta).
        ACCUMULATE T-REPORT.T-haber (SUB-TOTAL BY T-REPORT.codcta).
        ACCUMULATE T-REPORT.T-debe  (SUB-TOTAL BY T-REPORT.codaux).
        ACCUMULATE T-REPORT.T-haber (SUB-TOTAL BY T-REPORT.codaux).
        ACCUMULATE T-REPORT.T-debe  (TOTAL) .
        ACCUMULATE T-REPORT.T-haber (TOTAL) .
        IF LAST-OF(T-REPORT.NRODOC) THEN DO:
            Y-SALDO = (ACCUMULATE    SUB-TOTAL BY T-REPORT.NroDoc T-REPORT.T-debe ) -
                (ACCUMULATE    SUB-TOTAL BY T-REPORT.NroDoc T-REPORT.T-haber) .
            CREATE T-DETALLE.
            BUFFER-COPY T-REPORT TO T-DETALLE
                ASSIGN
                T-DETALLE.NomAux = x-NomAux
                T-DETALLE.NomCta = x-NomCta
                T-DETALLE.FchDoc = x-FchDoc
                T-DETALLE.CodDiv = x-CodDiv
                T-DETALLE.Cco    = x-Cco
                T-DETALLE.NroAst = x-NroAst
                T-DETALLE.CodOpe = x-CodOpe
                T-DETALLE.NroMes = x-NroMes
                T-DETALLE.FchVto = x-FchVto
                T-DETALLE.GloDoc = x-GloDoc
                T-DETALLE.T-Importe= x-Importe
                T-DETALLE.T-Debe = (ACCUMULATE SUB-TOTAL BY T-REPORT.NroDoc T-REPORT.T-debe)
                T-DETALLE.T-Haber= (ACCUMULATE SUB-TOTAL BY T-REPORT.NroDoc T-REPORT.T-haber)
                T-DETALLE.Saldo  = y-Saldo.
        END.
    END.
