...
t-Almmmatg.StkMax t-Almmmatg.Prevta[1] 
&Scoped-define ENABLED-TABLES-IN-QUERY-br_table ITEM t-Almmmatg
&Scoped-define FIRST-ENABLED-TABLE-IN-QUERY-br_table ITEM
&Scoped-define SECOND-ENABLED-TABLE-IN-QUERY-br_table t-Almmmatg
...
"t-Almmmatg.StkMax" "Stock!Disponible" "Z,ZZZ,ZZ9.99" "decimal" 14 0 ? ? ? ? yes ? no no "8.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"t-Almmmatg.Prevta[1]" "Precio A" ">>,>>9.999999" "decimal" 14 0 ? ? ? ? yes ? no no "10.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
t-Almmmatg.StkMax:READ-ONLY IN BROWSE {&browse-name} = YES.
t-Almmmatg.Prevta[1]:READ-ONLY IN BROWSE {&browse-name} = YES.
...
FOR EACH t-Almmmatg EXCLUSIVE-LOCK:
    ...
    IF AVAILABLE Almmmate THEN DO:
        ...
        /*MESSAGE pCodAlm t-Almmmatg.CodMat Almmmate.StkAct pComprometido.*/
    END.
...
------------------------------------------------------------------------------*/
  ...
  /* *********************************************************************************** */
  /* ***************************************************************************************************** */
  /* 22/05/2023 Calculamos el stock */
  /* ***************************************************************************************************** */
/*   DEF VAR pComprometido AS DECI NO-UNDO.                             */
/*   DEF VAR x-StkAct AS DECI NO-UNDO.                                  */
/*                                                                      */
/*   x-StkAct = 0.                                                      */
/*   FIND FIRST Almmmate WHERE Almmmate.codcia = t-Almmmatg.codcia AND  */
/*       Almmmate.codalm = s-AlmDes AND                                 */
/*       Almmmate.codmat = t-Almmmatg.codmat                            */
/*       NO-LOCK NO-ERROR.                                              */
/*   IF AVAILABLE Almmmate THEN DO:                                     */
/*       ASSIGN x-StkAct = Almmmate.StkAct.                             */
/*       RUN gn/stock-comprometido-v2 (t-Almmmatg.CodMat,               */
/*                                     s-AlmDes,                        */
/*                                     FALSE,                           */
/*                                     OUTPUT pComprometido).           */
/*       ASSIGN x-StkAct = x-StkAct - pComprometido.                    */
...
/*       NO-LOCK NO-ERROR.                                              */
/*   /* En unidades A */                                                */
/*   IF AVAILABLE Almtconv THEN x-StkAct = x-StkAct / Almtconv.Equival. */
/*   DISPLAY x-StkAct @ t-Almmmatg.StkMax WITH BROWSE {&browse-name}.   */
  /* ***************************************************************************************************** */
  /* ***************************************************************************************************** */
...
/*   DELETE PROCEDURE hProc.                                                                                 */
  ...
  /* ***************************************************************************************************** */
  /* 22/05/2023 Calculamos el precio unitario */
  /* Disparamos solo el primero */
  /* ***************************************************************************************************** */
/*   DEF VAR s-MonVta AS INTE NO-UNDO.                                                                          */
/*   DEF VAR f-CtoUni AS DECI NO-UNDO.                                                                          */
...
/*           NO-LOCK NO-ERROR.                                                                                  */
/*       IF NOT AVAILABLE Almtconv THEN DO:                                                                     */
/*           MESSAGE "ERROR en la tabla de conversiones" SKIP "Avisar al Gestor de Maestro de Artículos"        */
/*               VIEW-AS ALERT-BOX ERROR.                                                                       */
/*           ITEM.CanPed:SCREEN-VALUE IN BROWSE {&BROWSE-NAME} = "0.00".                                        */
/*           APPLY 'ENTRY':U TO ITEM.CanPed.                                                                    */
/*           RETURN "ADM-ERROR".                                                                                */
/*       END.                                                                                                   */
/*       F-FACTOR = Almtconv.Equival.                                                                           */
/*       x-PreUni = (f-PreVta + f-FleteUnitario) * f-Factor.                                                    */
/*       /*t-Almmmatg.CtoUnd = f-CtoUni * f-Factor.*/                                                           */
/*       DISPLAY x-PreUni @ t-Almmmatg.Prevta[1] WITH BROWSE {&browse-name}.                                    */
/*   END.                                                                                                       */
  /* ***************************************************************************************************** */
  /* ***************************************************************************************************** */
