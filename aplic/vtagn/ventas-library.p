...
DEFINE TEMP-TABLE PEDI2-1 NO-UNDO LIKE VtaDDocu.
...
DEFINE VAR pNewCodDoc AS CHAR.
DEFINE VAR pNewNroDoc AS CHAR.
DEFINE VAR cNewCodPHR AS CHAR.
DEFINE VAR cNewNroPHR AS CHAR.

DEF VAR s-PorIgv LIKE Ccbcdocu.PorIgv.

DEFINE VAR imprimirLogMsg AS LOG INIT FALSE.
...
/* Settings for THIS-PROCEDURE
   ...
   Temp-Tables and Buffers:
      ...
      TABLE: PEDI2-1 T "?" NO-UNDO INTEGRAL VtaDDocu
...
FOR EACH PriCDctoxImport NO-LOCK WHERE PriCDctoxImport.CodCia = s-codcia AND
    ...
    FOR EACH B-DPEDI OF B-CPEDI NO-LOCK WHERE B-DPEDI.Libre_c05 <> "OF",
        ...
        IF NOT CAN-FIND(FIRST PriDDctoxImport WHERE PriDDctoxImport.CodCia = PriCDctoxImport.codcia AND
            ...
            FOR EACH PriDDctoxImport NO-LOCK WHERE PriDDctoxImport.CodCia = PriCDctoxImport.codcia AND
                PriDDctoxImport.Id = PriCDctoxImport.Id AND
                PriDDctoxImport.Tabla = "REGLA" AND
                PriDDctoxImport.Tipo <> "SKU":  /* 04/06/2024: NO tomar en cuenta la regla por SKU */
    ...
    DO x-Item = 1 TO 5:
        IF x-ImpTot >= PriCDctoxImport.Importe[x-Item] AND PriCDctoxImport.Importe[x-Item] > 0 THEN DO:
            Y-DSCTOS = PriCDctoxImport.Descuento[x-Item].
        END.
    END.
    IF Y-DSCTOS > 0 THEN DO:       
        RUN DCTO_IMP_ACUM_Graba (INPUT Y-DSCTOS, 
                                 INPUT s-PorIgv, 
                                 INPUT s-CodCli,
                                 INPUT s-Cmpbnte,
                                 OUTPUT pMensaje).
...
DEF VAR Y-DSCTOS2 AS DECI NO-UNDO.
...
DEF VAR x-Forma2 AS CHAR INIT 'EXCLUYENTE'   NO-UNDO.   /* OJO: Valor por defecto */

/* Buscamos configuración */
FIND FacTabla WHERE FacTabla.codcia = s-codcia AND
    FacTabla.tabla = 'KEY_TPO_DCTO_ACUM' AND
...
FIND FacTabla WHERE FacTabla.codcia = s-codcia AND
    ...
    FacTabla.codigo = x-TipDto NO-LOCK NO-ERROR.
IF AVAILABLE FacTabla AND LOOKUP(FacTabla.Campo-c[2], 'EXCLUYENTE,ACUMULADO') > 0 THEN x-Forma2 = FacTabla.Campo-c[2].
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    FOR EACH ResumenxLinea NO-LOCK,
        ...
        /* ********************************************************************************************** */
        {vtagn/ventas-library-dcto-acum.i}
        /* ********************************************************************************************** */
        /* ********************************************************************************************** */
...
DEF VAR Y-DSCTOS2 AS DEC NO-UNDO.                    /* Descuento por Volumen y/o Promocional */
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    FOR EACH ResumenxLinea NO-LOCK,
        ...
        /* 04/03/2024: C.Camus y V.Gonzales */
        /* ********************************************************************************************** */
        Y-DSCTOS = ResumenxLinea.Por_Dsctos[3].
        {vtagn/ventas-library-dcto-acum.i}
        /* ********************************************************************************************** */
        /* ********************************************************************************************** */
        /* ***************************************************************** */
...
DEF VAR Y-DSCTOS AS DECI DECIMALS 4 INIT 0 NO-UNDO.
DEF VAR Y-DSCTOS2 AS DECI DECIMALS 4 INIT 0 NO-UNDO.
...
FOR EACH VtaTabla NO-LOCK WHERE VtaTabla.CodCia = s-CodCia AND
    ...
    IF TODAY <= VtaTabla.Rango_fecha[1] THEN DO:
        Y-DSCTOS = VtaTabla.Rango_Valor[1].
        LEAVE.
    END.
END.

IF Y-DSCTOS <= 0 THEN RETURN 'OK'.
...
FOR EACH T-VtamDctoVolTime NO-LOCK, EACH T-VtapDctoVolTime OF T-VtamDctoVolTime NO-LOCK:
    ...
    RUN DXVACUMTIME_Graba-Descuentos (INPUT X-TIPDTO, x-Forma, x-Forma2, OUTPUT pMensaje).
...
DEF VAR x-Canti AS DEC NO-UNDO.
DEF VAR x-Rango AS DEC NO-UNDO.
DEF VAR Y-DSCTOS AS DECIMAL DECIMALS 4 NO-UNDO.
DEF VAR Y-DSCTOS2 AS DECIMAL DECIMALS 4 NO-UNDO.
...
&IF DEFINED(EXCLUDE-DXVACUMTIME_Graba-Descuentos) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE DXVACUMTIME_Graba-Descuentos Procedure 
...
------------------------------------------------------------------------------*/
    ...
    DEF INPUT PARAMETER x-Forma2 AS CHAR.
    DEF OUTPUT PARAMETER pMensaje AS CHAR NO-UNDO.
    ...
    DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
        ...
        FOR EACH T-DPEDI NO-LOCK WHERE T-DPEDI.Libre_c01 = "*", 
            ...
            CASE x-Forma:
                WHEN "UNICO" THEN DO:
                    ASSIGN
                        Facdpedi.PreUni = Facdpedi.PreBas.  /* Se cambia al precio de oficina */
                END.
            END CASE.
            /* ********************************************************************************************** */
            /* ***************************************************************** */
...
&IF DEFINED(EXCLUDE-PED_Add_Record_Riqra) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE PED_Add_Record_Riqra Procedure 
PROCEDURE PED_Add_Record_Riqra :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEF INPUT PARAMETER s-PorIgv AS DEC.
DEF INPUT PARAMETER ppCodDoc AS CHAR.
DEF INPUT PARAMETER ppNroDoc AS CHAR. 
DEF OUTPUT PARAMETER ppFchEnt AS DATE.
DEF OUTPUT PARAMETER pxNroPCO AS CHAR.
DEF OUTPUT PARAMETER ps-NroCot AS CHAR.
DEF OUTPUT PARAMETER ps-CodAlm AS CHAR.
DEF OUTPUT PARAMETER ps-Tipo-Abastecimiento AS CHAR.
DEF OUTPUT PARAMETER TABLE FOR PEDI. 
DEF OUTPUT PARAMETER pRetval AS CHAR.

FIND FIRST B-CPEDI WHERE B-CPEDI.codcia = s-codcia AND
                        B-CPEDI.coddoc = ppCodDoc AND B-CPEDI.nroped = ppNroDoc NO-LOCK NO-ERROR.

IF NOT AVAILABLE B-CPEDI THEN DO:
    pRetVal = "Pedido comercial " + ppCodDoc + " " + ppNroDoc + " no existe".
    RETURN "ADM-ERROR".
END.
IF B-CPEDI.flgest = "A" THEN DO:
    pRetVal = "Pedido comercial " + ppCodDoc + " " + ppNroDoc + " esta anulado".
    RETURN "ADM-ERROR".
END.
IF B-CPEDI.flgest <> "P" THEN DO:
    pRetVal = "Pedido comercial " + ppCodDoc + " " + ppNroDoc + " su flgest es " + B-CPEDI.flgest + " imposible generar pedido logistico".
    RETURN "ADM-ERROR".
END.

lProcesoBatch = YES.
cMsgProcesoBatch = "".
pCodCot = ppCodDoc.   /* COT */
pNroCot = ppNroDoc.
s-PorIgv = B-CPEDI.PorIgv.

RUN PED_Add-Record  (INPUT s-PorIgv,
                       OUTPUT ppFchEnt,
                       OUTPUT pxNroPCO,
                       OUTPUT ps-NroCot,
                       OUTPUT ps-CodAlm,
                       OUTPUT ps-Tipo-Abastecimiento,
                       OUTPUT TABLE PEDI).

pRetval = cMsgProcesoBatch.
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH Facdpedi OF COTIZACION NO-LOCK,
      ...
      ASSIGN
          ...
          s-StkComprometido = 0.
      IF x-StkAct > 0 THEN RUN gn/Stock-Comprometido-v2 (Facdpedi.CodMat, x-CodAlm, YES, OUTPUT s-StkComprometido).
      /* ******************************************************************** */
...
IF lProcesoBatch = NO THEN DO:
    ...
    IF NOT AVAILABLE Ccbadocu THEN DO:    
        ...
        END.
        ELSE DO:
            pMensaje = "Aún NO ha ingresado los datos del transportista".
            RETURN "ADM-ERROR".
        END.
    END.
END.
    ...
    */
    ...
    IF LOOKUP(FacCPedi.Lista_de_Precios, '00000,00017,00018') > 0 THEN DO:
        ...
        IF Faccpedi.FlgEst = "W" THEN DO:
            pMensaje = "Margen Utiliad!" + FacCPedi.Libre_c05.
            LEAVE RLOOP.
        END.
    END.
...
&IF DEFINED(EXCLUDE-PED_despachar_pedido_riqra) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE PED_despachar_pedido_riqra Procedure 
PROCEDURE PED_despachar_pedido_riqra :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEFINE OUTPUT PARAMETER pNewCodOD AS CHAR NO-UNDO.
...
IF NOT AVAILABLE Faccpedi THEN DO:
    pMensaje = "Documento " + pCodDoc + " " + pNroDoc + " no existe".
    RETURN "ADM-ERROR".
END.

IF Faccpedi.flgest <> "G" THEN DO:
    pMensaje = "Documento " + pCodDoc + " " + pNroDoc + " ya NO esta para aprobar (flgest = 'G') " + CHR(10) +
        "actualmente tiene estado (" + Faccpedi.flgest + ")".
    RETURN "ADM-ERROR".
END.

pMensaje = "".
RUN PED_Despachar-Pedido (INPUT ROWID(Faccpedi),
                                   OUTPUT pMensaje).
...
pNewCodOD =  pNewCodDoc.
pNewNroOD =  pNewNroDoc.
...
&IF DEFINED(EXCLUDE-PPX_Importar-Excel) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE PPX_Importar-Excel Procedure 
PROCEDURE PPX_Importar-Excel :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEF INPUT PARAMETER pCodCli AS CHAR NO-UNDO.      /* Se debe chequear contra el excel */
DEF INPUT PARAMETER FILL-IN-Archivo AS CHAR NO-UNDO.

DEF INPUT-OUTPUT PARAMETER TABLE FOR PEDI2.
DEF OUTPUT PARAMETER pMensaje AS CHAR NO-UNDO.
...
/* Chequear el cliente */
DEF VAR cCodCli AS CHAR FORMAT 'x(15)' NO-UNDO.
DEF VAR iCodCli AS INT64 NO-UNDO.
cColumn = "3".
cRange = "I" + cColumn.
cCodCli = chWorkSheet:Range(cRange):Value.
cCodCli = TRIM(ENTRY(1,cCodCli,'.')).
cCodCli = FILL('0',11) + cCodCli.
cCodCli = SUBSTRING(cCodCli, LENGTH(cCodCli) - 10).

IF pCodCli <> cCodCli THEN DO:
    pMensaje = "El Excel NO pertenece al cliente".
    {lib\excel-close-file.i}
    RETURN 'ADM-ERROR'.
END.

/* Capturamos Promotor */
DEF VAR cPromotor AS CHAR NO-UNDO.
cColumn = "4".
cRange = "I" + cColumn.
cPromotor = TRIM(chWorkSheet:Range(cRange):Value).

/* NO borramos si hay algo ya digitado */
EMPTY TEMP-TABLE PEDI2-1.
...
REPEAT iColumn = 9 TO 65000:
    ...
    CREATE PEDI2-1.
    ASSIGN
        PEDI2-1.nroitm = I-NroItm
        PEDI2-1.codcia = s-codcia
        PEDI2-1.codmat = x-codmat
        PEDI2-1.canped = x-canped
        PEDI2-1.undvta = x-codund
        PEDI2-1.factor = 1.
...
DEF VAR pSugerido AS DEC.
DEF VAR pEmpaque  AS DEC.      /* Empaque sugerido */

FOR EACH PEDI2-1:
    FIND FIRST PEDI2 WHERE PEDI2.codmat = PEDI2-1.codmat NO-LOCK NO-ERROR.
    IF AVAILABLE PEDI2 THEN DO:
        NEXT.
    END.
        
    CREATE PEDI2.
    BUFFER-COPY PEDI2-1 TO PEDI2.
    ASSIGN
        PEDI2.CanAte = 0
        PEDI2.Libre_d01 = PEDI2.CanPed.
    IF TRUE <> (PEDI2.Libre_c01 > '') THEN PEDI2.Libre_c01 = '0001'.
    IF TRUE <> (PEDI2.Libre_c02 > '') THEN PEDI2.Libre_c02 = '0001'.
    /* 14/12/23: Datos del Promotor */
    FIND Almmmatg WHERE Almmmatg.codcia = s-codcia AND Almmmatg.codmat = PEDI2.CodMat NO-LOCK.
    FIND VtaTabla WHERE Vtatabla.codcia = s-codcia AND
        Vtatabla.tabla = 'EXPOPROMOTOR' AND
        Vtatabla.llave_c1 = s-coddiv AND
        Vtatabla.llave_c2 = '51135890' AND    /* Proveedor */
        Vtatabla.llave_c3 = cPromotor         /* Promotor */
        NO-LOCK NO-ERROR.
    IF AVAILABLE VtaTabla THEN PEDI2.Libre_c03 = '51135890|' + cPromotor + '|' + VtaTabla.Libre_c02 + '|' + Almmmatg.CodFam.
    /* *********************************************************************************************** */
    /* Cantidad Sugerida */
    /* *********************************************************************************************** */
...
IF imprimirLogMsg THEN RUN lib/p-write-log-txt.r("Generar O/D", 'VTA_Genera-OD => Ped.Log : ' + B-CPEDI.coddoc + " " + B-CPEDI.nroped).
IF imprimirLogMsg THEN RUN lib/p-write-log-txt.r("Generar O/D", 'VTA_Genera-OD => Ped.Log : ' + B-CPEDI.coddoc + " " + B-CPEDI.nroped + " Trasaccion ").
...
DO TRANSACTION ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE :
    ...
    */
    ...
    xNewCodDoc = FacCPedi.CodDoc.
    xNewNroDoc = FacCPedi.NroPed.
    ...
    IF imprimirLogMsg THEN RUN lib/p-write-log-txt.r("Generar O/D", 'VTA_Genera-OD => Ped.Log : ' + B-CPEDI.coddoc + " " + B-CPEDI.nroped + " logis/p-fecha-de-entrega ").
    ...
    /* TRACKING */
    /* ******************************************************************** */
    IF imprimirLogMsg THEN RUN lib/p-write-log-txt.r("Generar O/D", 'VTA_Genera-OD => Ped.Log : ' + B-CPEDI.coddoc + " " + B-CPEDI.nroped + " vtagn/pTracking-04 ").
    ...
    /* Detalle de la OD */
    /* ******************************************************************** */
    IF imprimirLogMsg THEN RUN lib/p-write-log-txt.r("Generar O/D", 'VTA_Genera-OD => Ped.Log : ' + B-CPEDI.coddoc + " " + B-CPEDI.nroped + " Detalle Items ").
...
/*     RUN Actualiza-Pedido (+1). */
    IF imprimirLogMsg THEN RUN lib/p-write-log-txt.r("Generar O/D", 'VTA_Genera-OD => Ped.Log : ' + B-CPEDI.coddoc + " " + B-CPEDI.nroped + " Actualiza-Saldo-Referencia ").
    ...
    /* *************************************************************************** */
    IF imprimirLogMsg THEN RUN lib/p-write-log-txt.r("Generar O/D", 'VTA_Genera-OD => Ped.Log : ' + B-CPEDI.coddoc + " " + B-CPEDI.nroped + " ML_Genera-OD-Control ").
    ...
    IF Faccpedi.TipVta = "Si" THEN DO:
        /* Ic - 16Ago2024, todos los Tramites documentarios no deben generar PHR ni HPK */        
    END.
    ELSE DO:
        /* 03/02/2022 Generación de PHR */
        IF lProcesoBatch = NO THEN DO:      /* Para caso de Riqra la generacion de PHR/HPK se realizara en otro proceso */
            IF imprimirLogMsg THEN RUN lib/p-write-log-txt.r("Generar O/D", 'VTA_Genera-OD => Ped.Log : ' + B-CPEDI.coddoc + " " + B-CPEDI.nroped + " VTA_Genera-PHR_y_HPK ").
            ...
            IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
                ...
                UNDO LoopGrabarData, LEAVE.        
            END.
            IF imprimirLogMsg THEN RUN lib/p-write-log-txt.r("Generar O/D", 'VTA_Genera-OD => Ped.Log : ' + B-CPEDI.coddoc + " " + B-CPEDI.nroped + " VTA_Genera-PHR_y_HPK (" + pMensaje + ")").
        END.
    END.
    IF imprimirLogMsg THEN RUN lib/p-write-log-txt.r("Generar O/D", 'VTA_Genera-OD => Ped.Log : ' + B-CPEDI.coddoc + " " + B-CPEDI.nroped + " Termino ").
END.
DELETE PROCEDURE hMaster.

IF imprimirLogMsg THEN RUN lib/p-write-log-txt.r("Generar O/D", 'VTA_Genera-OD => Ped.Log : ' + B-CPEDI.coddoc + " " + B-CPEDI.nroped + " Salioooooo ").

SESSION:SET-WAIT-STATE('').

IF TRUE <> (pMensaje > "") THEN DO:
    IF lProcesoBatch = YES THEN DO:
        pNewCodDoc = xNewCoddoc.
        pNewNroDoc = xNewNroDoc.
    END.
...
IF imprimirLogMsg THEN RUN lib/p-write-log-txt.r("VTA_Genera-PHR_y_HPK", 'inicia ').
...
xCodDoc = ordenes.coddoc.
xNroDoc = ordenes.nroped.

IF imprimirLogMsg THEN RUN lib/p-write-log-txt.r("VTA_Genera-PHR_y_HPK", 'Orden : ' + ordenes.coddoc + " " + ordenes.nroped).
...
IF imprimirLogMsg THEN RUN lib/p-write-log-txt.r("VTA_Genera-PHR_y_HPK", 'Orden : ' + ordenes.coddoc + " " + ordenes.nroped + " logis/p-datos-sede-auxiliar.r ").
...
IF imprimirLogMsg THEN RUN lib/p-write-log-txt.r("VTA_Genera-PHR_y_HPK", 'Orden : ' + ordenes.coddoc + " " + ordenes.nroped + " Transaccion ").

DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    /* ****************************************** */
    /* 05/06/2024: Actualizar el estado de la O/D */
    /* ****************************************** */
    FIND ORDENES WHERE ROWID(ORDENES) = pRowid EXCLUSIVE-LOCK NO-ERROR NO-WAIT.
    IF ERROR-STATUS:ERROR = YES THEN DO:
        {lib/mensaje-de-error.i &CuentaError="pCuenta" &MensajeError="pMensaje"}
        UNDO, RETURN 'ADM-ERROR'.
    END.
    ...
    ELSE DO:
        ...
        END.
    END.
        xnewCod = di-rutaC.coddoc.
        xnewNro = di-rutaC.nrodoc.
    CREATE DI-RutaD.
    ASSIGN
    ...
    IF imprimirLogMsg THEN RUN lib/p-write-log-txt.r("VTA_Genera-PHR_y_HPK", 'Orden : ' + ordenes.coddoc + " " + ordenes.nroped + " HPK_Genera-HPK-Master ").
    ...
    IF imprimirLogMsg THEN RUN lib/p-write-log-txt.r("VTA_Genera-PHR_y_HPK", 'Orden : ' + ordenes.coddoc + " " + ordenes.nroped + " HPK_Genera-HPK-Master (" + pMensaje + ")").
...
IF imprimirLogMsg THEN RUN lib/p-write-log-txt.r("VTA_Genera-PHR_y_HPK", 'Orden : ' + ordenes.coddoc + " " + ordenes.nroped + " antes de cerrar ").
...
IF imprimirLogMsg THEN RUN lib/p-write-log-txt.r("VTA_Genera-PHR_y_HPK", 'Orden : ' + xcoddoc + " " + xnrodoc + " Nuevo PHR :(" + cNewCodPHR + " " + cNewNroPHR + ") FIN ").
...
&IF DEFINED(EXCLUDE-VTA_Genera-PHR_y_HPK_riqra) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE VTA_Genera-PHR_y_HPK_riqra Procedure 
PROCEDURE VTA_Genera-PHR_y_HPK_riqra :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEF INPUT PARAMETER pCodDoc AS CHAR NO-UNDO.
DEF INPUT PARAMETER pNroDoc AS CHAR NO-UNDO.
DEFINE OUTPUT PARAMETER pNewCodPHR AS CHAR NO-UNDO.
...
IF NOT (Faccpedi.flgest = "P" AND faccpedi.flgsit = 'K') THEN DO:
    pMensaje = "Documento " + pCodDoc + " " + pNroDoc + " ya NO esta pendiente para generar PHR-HPK (flgest = 'P' - flgsit = 'K') " + CHR(10) +
        "actualmente tiene estado (" + Faccpedi.flgest + ")".
    RETURN "ADM-ERROR".
END.
...
RUN PED_Despachar-Pedido (INPUT ROWID(Faccpedi),
                                   OUTPUT pMensaje).
*/
RUN VTA_Genera-PHR_y_HPK(INPUT rowid(Faccpedi), OUTPUT pMensaje) NO-ERROR.

IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
    RETURN "ADM-ERROR".
END.

pNewCodPHR =  cNewCodPHR.
pNewNroPHR =  cNewNroPHR.
