...
&IF DEFINED(EXCLUDE-fget-glosa-ctasbancarias) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD fget-glosa-ctasbancarias Procedure 
FUNCTION fget-glosa-ctasbancarias RETURNS CHARACTER
...
&IF DEFINED(EXCLUDE-fget-glosa-detraccion) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD fget-glosa-detraccion Procedure 
FUNCTION fget-glosa-detraccion RETURNS CHARACTER
  ( INPUT pTipoDocumento AS CHAR )  FORWARD.
...
IF NOT AVAILABLE factabla THEN DO:
    pReturn = "667|El Servidor del WebService no esta configurado(" + x-config-servidor + ")".
    RETURN "ADM-ERROR".
END.
...
IF x-enviar-servidor-prueba THEN DO:
    MESSAGE 'Seguro de enviar el comprobante al ' SKIP
        'servidor ' x-servidor-ip skip
        'puerto ' x-servidor-puerto 
...
IF RETURN-VALUE = "ADM-ERROR" THEN DO:
    IF LOOKUP(pTipoDocmto,"FAC,BOL") > 0 THEN DO:
        pReturn = "999|ERROR el comprobante requiere GUIA DE REMISION".
    END.
    ELSE DO:
...
IF x-retval = "OK" THEN DO:
    ...
    pReturn = pReturn + "|" + STRING(b-ccbcdocu.totalValorVentaNetoOpGravadas). /*x-vvta-gravada*/
    pReturn = pReturn + "|" + STRING(b-ccbcdocu.totalValorVentaNetoOpNoGravada).   /*x-vvta-inafectas*/
    pReturn = pReturn + "|" + STRING(b-ccbcdocu.totalValorVentaNetoOpExoneradas).   /*x-vvta-exoneradas*/
    pReturn = pReturn + "|" + STRING(b-ccbcdocu.totalTributosOpeGratuitas).     /*x-vvta-gratuitas*/
    pReturn = pReturn + "|" + STRING(b-ccbcdocu.totalImpuestos).          /* x-total-igv Total impuestos */
    pReturn = pReturn + "|" + STRING(b-ccbcdocu.totalIGV).         /* x-total-igv2 Total IGV */
    pReturn = pReturn + "|" + STRING(0.00).                /*x-imp-isc*/
    pReturn = pReturn + "|" + STRING(b-ccbcdocu.totalVenta).        /*x-imp-total*/
...
/* Ic - 04Jul2024, jorge cantera cliente recoge no genera GRE */
FIND FIRST x-faccpedi WHERE x-faccpedi.codcia = 1 AND x-faccpedi.coddoc = b-ccbcdocu.libre_c01 AND
                                    x-faccpedi.nroped = b-ccbcdocu.libre_c02 NO-LOCK NO-ERROR.

IF AVAILABLE x-faccpedi THEN DO:
    IF x-faccpedi.cliente_Recoge = YES THEN RETURN 'OK'.
END.
...
DEFINE VAR xmlGRE AS LONGCHAR.
RUN guia-remision(OUTPUT xmlGRE).

IF LENGTH(xmlGRE) = 0 THEN DO:
    RETURN "ADM-ERROR".
END.

p-xml-boleta = p-xml-boleta + xmlGRE.
...
*/
COPY-LOB x-XML-documento TO FILE x-nombre-del-xml + ".xml" NO-ERROR.
...
DEFINE VAR x-rspta1 AS LONGCHAR.
...
/*COPY-LOB x-rspta1 TO FILE "d:\rsptal-biz.txt" NO-ERROR.*/
...
DEFINE VAR x-enviar-guia-remision AS LOG.
DEFINE VAR cOrdenCompra AS CHAR.
p-xml-factura = "".

/* Orden de Compra */
IF b-ccbcdocu.nroord <> ? AND  b-ccbcdocu.nroord <> "" THEN DO:
    cOrdenCompra = TRIM(b-ccbcdocu.nroord).
    IF LENGTH(cOrdenCompra) > 20 THEN cOrdenCompra = SUBSTRING(cOrdenCompra,1,20).
    /*p-xml-factura = p-xml-factura + "<ordenCompra>" + b-ccbcdocu.nroord + "</ordenCompra>".*/
    p-xml-factura = p-xml-factura + "<ordenCompra>" + cOrdenCompra + "</ordenCompra>".
END.
...
p-xml-factura = p-xml-factura + xmlGRE.
...
FOR EACH bx-ccbcdocu WHERE bx-ccbcdocu.codcia = s-codcia  AND
                            bx-ccbcdocu.codref = b-ccbcdocu.coddoc AND 
                            bx-ccbcdocu.nroref = b-ccbcdocu.nrodoc AND
                            bx-ccbcdocu.coddoc = 'G/R' NO-LOCK :

    lGuiaRemision = IF(bx-ccbcdocu.nrodoc = ?) THEN "" ELSE TRIM(bx-ccbcdocu.nrodoc). 
    IF bx-ccbcdocu.flgest = "A" THEN NEXT.

    IF lGuiaRemision <> "" THEN DO:
        x-sec = x-sec + 1.
        IF x-sec <= 5 THEN DO:
            /* Bizlinks, solo tiene 5 referencias como maximo */
            /* lGuiaRemision = "0" + SUBSTRING(bx-ccbcdocu.nrodoc,1,3) + "-" +  Ic - 24Abr2024, GRE empieza con T */
            lGuiaRemision = "T" + SUBSTRING(bx-ccbcdocu.nrodoc,1,3) + "-" + 
                            SUBSTRING(bx-ccbcdocu.nrodoc,4) NO-ERROR.
...
FOR EACH sunat_anticipos WHERE sunat_anticipos.codcia = 1 AND
                                sunat_anticipos.coddoc = pCodDoc AND
                                sunat_anticipos.nrodoc = pNroDoc NO-LOCK:

    IF x-XML-anticipos = "" THEN DO :
        x-XML-anticipos = "<anticipo>".
    END.
    ELSE DO:
        x-XML-anticipos = x-XML-anticipos + "<anticipo>".
    END.
    x-XML-anticipos = x-XML-anticipos + fgenera-tag("indicador",TRIM(sunat_anticipos.indicador)).
    x-XML-anticipos = x-XML-anticipos + fgenera-tag("numeroOrdenAnticipo",TRIM(sunat_anticipos.numeroOrdenAnticipo)).
    x-XML-anticipos = x-XML-anticipos + fgenera-tag("tipoDocumentoEmisorAnticipo",TRIM(sunat_anticipos.tipoDocumentoEmisorAnticipo)).
    x-XML-anticipos = x-XML-anticipos + fgenera-tag("numeroDocumentoEmisorAnticipo",TRIM(sunat_anticipos.numeroDocumentoEmisorAnticipo)).
    x-XML-anticipos = x-XML-anticipos + fgenera-tag("tipoDocumentoAnticipo",TRIM(sunat_anticipos.tipoDocumentoAnticipo)).
    x-XML-anticipos = x-XML-anticipos + fgenera-tag("serieNumeroDocumentoAnticipo",TRIM(sunat_anticipos.serieNumeroDocumentoAnticipo)).
    x-XML-anticipos = x-XML-anticipos + fgenera-tag("totalPrepagadoAnticipo",TRIM(STRING(sunat_anticipos.totalPrepagadoAnticipo,">>>>>>>>9.99"))).
    x-XML-anticipos = x-XML-anticipos + fgenera-tag("fechaPago",TRIM(sunat_anticipos.fechaPago)).
...
------------------------------------------------------------------------------*/
    ...
    x-total-a-pagar = b-ccbcdocu.totalVenta. /* imptot.*/ /*x-totalPrecioVenta.*/

    x-xml-factoring = "<formaPagoNegociable>1</formaPagoNegociable>".
    x-xml-factoring = x-xml-factoring + "<montoNetoPendiente>" + TRIM(STRING(x-total-a-pagar,">>>>>>>9.99")) + "</montoNetoPendiente>".
    REPEAT x-sec = 1 TO x-cuotas:
        IF x-cuotas = 1 THEN DO:
            x-impte-cuota = x-total-a-pagar.
        END.
        ELSE DO:
            IF x-sec = x-cuotas THEN DO:
                x-impte-cuota = x-total-a-pagar - x-suma-cuotas.
            END.
            ELSE DO:
                x-impte-cuota = TRUNCATE(b-ccbcdocu.totalVenta * (x-cuotas-factor[x-sec] / 100),2).
    ...
    ELSE DO: 
        IF x-Ruc-Cli = "" THEN DO:
            ...
            IF AVAILABLE x-gn-clie AND x-gn-clie.libre_c01 = 'E' THEN DO:
                /* extranjero */                
                x-Ruc-Cli = x-gn-clie.dni.
                IF TRUE <> (x-Ruc-Cli > "")  THEN DO:
                    x-Ruc-Cli = "11111111111".
                END.
                x-Tipo-Ide = '4'.       /* Carnet extranjeria */
            END.
            ELSE DO:
                /* BUscamos error */
...
IF x-enviar-servidor-prueba = YES THEN DO:
    x-aaaa-mm-dd = SUBSTRING(STRING(TODAY,"99-99-9999"),7,4) + "-".
    x-aaaa-mm-dd = x-aaaa-mm-dd + SUBSTRING(STRING(TODAY,"99-99-9999"),4,2) + "-" .
    x-aaaa-mm-dd = x-aaaa-mm-dd + SUBSTRING(STRING(TODAY,"99-99-9999"),1,2).
END.
ELSE DO:
...
END.
ELSE DO:
    p-xml-header = p-xml-header + "<serieNumero>" +  x-ser + "-" + x-nro + "</serieNumero>".
END.

IF x-enviar-servidor-prueba = YES THEN x-hora-emision = "00:01:01".
...
/* Anticipos A/C */
x-XML-anticipos = "".
RUN generar-xml-anticipos(INPUT b-ccbcdocu.coddoc,INPUT b-ccbcdocu.nrodoc).

IF b-ccbcdocu.totalValorVentaNetoOpGravadas > 0 OR b-ccbcdocu.totalDocumentoAnticipo > 0 THEN p-xml-header = p-xml-header + "<totalValorVentaNetoOpGravadas>" + TRIM(STRING(b-ccbcdocu.totalValorVentaNetoOpGravadas,">>>>>>>9.99")) + "</totalValorVentaNetoOpGravadas>".
/*
IF b-ccbcdocu.impdto2 > 0 THEN DO:
    p-xml-header = p-xml-header + "<descuentosGlobales>" + TRIM(STRING(x-descuentosGlobales,">>>>>>>9.99")) + "</descuentosGlobales>".
    p-xml-header = p-xml-header + "<porcentajeDsctoGlobal>" + TRIM(STRING(x-porcentajeDescuentoGlobal,">>>>>>>9.9999")) + "</porcentajeDsctoGlobal>".
    p-xml-header = p-xml-header + "<montoBaseDescuentoGlobal>" + TRIM(STRING(x-montoBaseDsctoGlobal,">>>>>>>9.99")) + "</montoBaseDescuentoGlobal>".
END.
*/

IF b-ccbcdocu.totalValorVentaNetoOpNoGravada > 0 THEN p-xml-header = p-xml-header + "<totalValorVentaNetoOpNoGravada>" + TRIM(STRING(b-ccbcdocu.totalValorVentaNetoOpNoGravada,">>>>>>>9.99")) + "</totalValorVentaNetoOpNoGravada>".
IF b-ccbcdocu.totalValorVentaNetoOpGratuitas > 0 THEN DO:   
    p-xml-header = p-xml-header + "<totalValorVentaNetoOpGratuitas>" + TRIM(STRING(b-ccbcdocu.totalValorVentaNetoOpGratuitas,">>>>>>>9.99")) + "</totalValorVentaNetoOpGratuitas>".
    p-xml-header = p-xml-header + "<totalTributosOpeGratuitas>" + TRIM(STRING(b-ccbcdocu.totalTributosOpeGratuitas,">>>>>>>9.99")) + "</totalTributosOpeGratuitas>".
END.       
IF b-ccbcdocu.totalDocumentoAnticipo > 0 THEN DO:
    IF b-ccbcdocu.totalValorVentaNetoOpExoneradas > 0 THEN DO:
        p-xml-header = p-xml-header + "<totalValorVentaNetoOpExoneradas>" + TRIM(STRING(b-ccbcdocu.totalValorVentaNetoOpExoneradas,">>>>>>>9.99")) + "</totalValorVentaNetoOpExoneradas>".
    END.
END.
ELSE DO:
    p-xml-header = p-xml-header + "<totalValorVentaNetoOpExoneradas>" + TRIM(STRING(b-ccbcdocu.totalValorVentaNetoOpExoneradas,">>>>>>>9.99")) + "</totalValorVentaNetoOpExoneradas>".
END.
p-xml-header = p-xml-header + "<totalIgv>" + TRIM(STRING(b-ccbcdocu.totalIgv,">>>>>>>9.99")) + "</totalIgv>".
p-xml-header = p-xml-header + "<totalImpuestos>" + TRIM(STRING(b-ccbcdocu.totalImpuestos,">>>>>>>9.99")) + "</totalImpuestos>".

IF b-ccbcdocu.totalDocumentoAnticipo > 0 THEN DO:
    p-xml-header = p-xml-header + fgenera-tag("totalDocumentoAnticipo",TRIM(STRING(b-ccbcdocu.totalDocumentoAnticipo,">>>>>>>9.99"))).
    p-xml-header = p-xml-header + fgenera-tag("montoBaseDsctoGlobalAnticipo",TRIM(STRING(b-ccbcdocu.montoBaseDsctoGlobalAnticipo,">>>>>>>9.99"))).
    p-xml-header = p-xml-header + fgenera-tag("porcentajeDsctoGlobalAnticipo",TRIM(STRING(b-ccbcdocu.porcentajeDsctoGlobalAnticipo,">>9.99999"))).
    p-xml-header = p-xml-header + fgenera-tag("totalDsctoGlobalesAnticipo",TRIM(STRING(b-ccbcdocu.totalDsctoGlobalesAnticipo,">>>>>>>9.99"))).
END.
/* ***************************************************************************************************************** */
...
*/
/* ***************************************************************************************************************** */
p-xml-header = p-xml-header + "<totalValorVenta>" + TRIM(STRING(b-ccbcdocu.totalValorVenta,">>>>>>>9.99")) + "</totalValorVenta>".
p-xml-header = p-xml-header + "<totalPrecioVenta>" + TRIM(STRING(b-ccbcdocu.totalPrecioVenta,">>>>>>>9.99")) + "</totalPrecioVenta>".
p-xml-header = p-xml-header + "<totalVenta>" + TRIM(STRING(b-ccbcdocu.totalVenta,">>>>>>>9.99")) + "</totalVenta>".
...
END.
IF TRUE <> (x-dir-Cli > "") THEN x-dir-Cli= "-".
...
/* Importe en Letras */
x-importe-letras = "".
RUN lib\_numero.R(INPUT b-ccbcdocu.totalVenta, 2, 1, OUTPUT x-importe-letras).
...
IF b-ccbcdocu.montoBaseICBPER > 0 THEN DO:
    IF b-ccbcdocu.coddoc = 'FAC' OR b-ccbcdocu.coddoc = 'BOL' OR b-ccbcdocu.coddoc = 'N/C' THEN DO:
        IF b-ccbcdocu.acubon[10] > 0 THEN DO:
            p-xml-header = p-xml-header + "<montoBaseICBPER>" + TRIM(STRING(b-ccbcdocu.montoBaseICBPER,">>>>>>>9.99"))+ "</montoBaseICBPER>".             
            p-xml-header = p-xml-header + "<totalMontoICBPER>" + TRIM(STRING(b-ccbcdocu.totalMontoICBPER,">>>>>>>9.99"))+ "</totalMontoICBPER>".
        END.
    END.
END.
/* Personalizados */
...
/* Glosa Cuentas Bancarias */
p-xml-header = p-xml-header + fget-glosa-ctasbancarias(b-Ccbcdocu.CodDoc).
/* Glosa detraccion */
p-xml-header = p-xml-header + fget-glosa-detraccion(b-Ccbcdocu.CodDoc).
...
IF AVAILABLE VtaDTabla AND VtaDTabla.Llave = "BL" THEN DO:
    ...
    IF LOOKUP(b-Ccbcdocu.CodDoc, 'BOL,FAC') > 0 AND b-Ccbcdocu.TpoFac = "S" THEN DO:
        p-xml-header = p-xml-header + "<codigoAuxiliar250_7>9675</codigoAuxiliar250_7>".
        p-xml-header = p-xml-header + "<textoAuxiliar250_7>Factura de servicio afecta a retencion</textoAuxiliar250_7>".
        p-xml-header = p-xml-header + "<codigoAuxiliar250_8>9676</codigoAuxiliar250_8>".
        p-xml-header = p-xml-header + "<textoAuxiliar250_8>Prueba % por ciento</textoAuxiliar250_8>".
...
&IF DEFINED(EXCLUDE-guia-remision) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE guia-remision Procedure 
PROCEDURE guia-remision :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEFINE OUTPUT PARAMETER xmlGRE AS LONGCHAR.
...
FOR EACH bx-ccbcdocu WHERE bx-ccbcdocu.codcia = s-codcia  AND
    ...
    IF lGuiaRemision <> "" THEN DO:
        ...
        IF x-sec <= 5 THEN DO:
            ...
            lGuiaRemision = "T" + SUBSTRING(bx-ccbcdocu.nrodoc,1,3) + "-" + 
                            SUBSTRING(bx-ccbcdocu.nrodoc,4) NO-ERROR.
            xmlGRE = xmlGRE + "<tipoReferencia_" + STRING(x-sec) + ">09</tipoReferencia_" + STRING(x-sec) + ">".
            xmlGRE = xmlGRE + "<numeroDocumentoReferencia_" + STRING(x-sec) + ">" + lGuiaRemision + "</numeroDocumentoReferencia_" + STRING(x-sec)  + ">".
        END.
    END.
END.
...
&IF DEFINED(EXCLUDE-impuesto-icbper-borrar) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE impuesto-icbper-borrar Procedure 
PROCEDURE impuesto-icbper-borrar :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEFINE OUTPUT PARAMETER pMontoTributoBolsaPlastico AS DEC NO-UNDO.
...
pMontoTributoBolsaPlastico = x-implin.

/*pXMLicbper = pXMLicbper + "<impuestoBolsaPlastico>" + TRIM(STRING(x-implin,">>>>>>>9.99")) + "</impuestoBolsaPlastico>".*/
/* Segun correo de Bizlinks del dia 29Oct2021 */
pXMLicbper = pXMLicbper + "<impuestoBolsaPlastico>" + TRIM(STRING(x-precio-ICBPER,">>>>>>>9.99")) + "</impuestoBolsaPlastico>".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION fget-glosa-ctasbancarias Procedure 
FUNCTION fget-glosa-ctasbancarias RETURNS CHARACTER
  ( INPUT pTipoDocumento AS CHAR ) :

DEFINE VAR lRetVal AS CHAR INIT "".

DEFINE VAR cTabla AS CHAR INIT "CONFIG-CMPTE-ELECTRONICO".
DEFINE VAR cLlave_C1 AS CHAR INIT "GLOSA".
DEFINE VAR cLlave_C2 AS CHAR INIT "CTAS-BANCARIAS".

IF LOOKUP(pTipoDocumento, 'BOL,FAC,N/D') > 0 THEN DO:
    FOR EACH x-vtatabla WHERE x-vtatabla.codcia = 1 AND x-vtatabla.tabla = cTabla AND x-vtatabla.llave_c1 = cLlave_C1 
                            AND x-vtatabla.llave_c2 = cLlave_C2 NO-LOCK BY x-vtatabla.llave_c3 :
        IF NOT (TRUE <> (x-vtatabla.llave_c4 > "")) AND NOT (TRUE <> (x-vtatabla.llave_c5 > "")) THEN DO:
            lRetVal = lRetVal + "<codigoAuxiliar250_" + TRIM(x-vtatabla.llave_c3) + ">" + trim(x-vtatabla.llave_c4) +  "</codigoAuxiliar250_" + TRIM(x-vtatabla.llave_c3) + ">".
            lRetVal = lRetVal + "<textoAuxiliar250_" + TRIM(x-vtatabla.llave_c3) + ">" + trim(x-vtatabla.llave_c5) +  "</textoAuxiliar250_" + TRIM(x-vtatabla.llave_c3) + ">".            
        END.        
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION fget-glosa-detraccion Procedure 
FUNCTION fget-glosa-detraccion RETURNS CHARACTER
  ( INPUT pTipoDocumento AS CHAR ) :
...
DEFINE VAR cTabla AS CHAR INIT "CONFIG-CMPTE-ELECTRONICO".
DEFINE VAR cLlave_C1 AS CHAR INIT "GLOSA".
DEFINE VAR cLlave_C2 AS CHAR INIT "DETRACCION".
DEFINE VAR cLlave_C3 AS CHAR INIT "VTA-MINIMA".

DEFINE VAR x-ImpTot AS DEC INIT 0.
DEFINE VAR xImpTot AS DEC INIT 0.
DEFINE VAR x-VtaMinima AS DEC INIT 700.
  

IF LOOKUP(pTipoDocumento,'BOL,FAC') > 0 THEN DO:
    cLlave_C1 = "DETRACCION".
    cLlave_C2 = "MONTO".
    /**/
    FIND FIRST x-vtatabla WHERE x-vtatabla.codcia = 1 AND x-vtatabla.tabla = cTabla AND x-vtatabla.llave_c1 = cLlave_C1 
                                AND x-vtatabla.llave_c2 = cLlave_C2 AND 
                                x-vtatabla.llave_c3 = cLlave_C3 NO-LOCK NO-ERROR.
    IF AVAILABLE x-vtatabla THEN DO:
        IF x-vtatabla.valor[1] <> ? AND x-vtatabla.valor[1] > 0 THEN x-VtaMinima = x-vtatabla.valor[1].
    END.
    ...
    FOR EACH b-Ccbddocu OF b-Ccbcdocu NO-LOCK, FIRST almmserv NO-LOCK WHERE almmserv.CodCia = b-Ccbddocu.codcia AND
        almmserv.codmat = b-Ccbddocu.codmat AND
        almmserv.AftDetraccion = YES:        
        /*xImpTot = b-Ccbcdocu.ImpTot.*/
        xImpTot = b-Ccbddocu.ImpLin.
        IF b-Ccbcdocu.CodMon = 2  THEN xImpTot = xImpTot * b-Ccbcdocu.TpoCmb.
        x-ImpTot = x-ImpTot + xImpTot.
        /* Solo para importes mayores a S/700.00 */        
        IF x-ImpTot > x-VtaMinima THEN DO:
            LEAVE BUSCATOPE.
        END.        
    END.
    IF x-ImpTot > x-VtaMinima THEN DO:
        cLlave_C1 = "GLOSA".
        cLlave_C2 = "DETRACCION".
        FOR EACH x-vtatabla WHERE x-vtatabla.codcia = 1 AND x-vtatabla.tabla = cTabla AND x-vtatabla.llave_c1 = cLlave_C1 
                                AND x-vtatabla.llave_c2 = cLlave_C2 NO-LOCK BY x-vtatabla.llave_c3 :
            IF NOT (TRUE <> (x-vtatabla.llave_c4 > "")) AND NOT (TRUE <> (x-vtatabla.llave_c5 > "")) THEN DO:
                lRetVal = lRetVal + "<codigoAuxiliar250_" + trim(x-vtatabla.llave_c3) + ">" + trim(x-vtatabla.llave_c4) +  "</codigoAuxiliar250_" + trim(x-vtatabla.llave_c3) + ">".
