...
DEFINE OUTPUT PARAMETER pExisteMsg AS LOG.
...
DEFINE VAR iSec AS INT.
DEFINE VAR cTipoCmptes AS CHAR.
...
/*IF LOOKUP(pTipoDocumento,'BOL,FAC') > 0 THEN DO:*/
    ...
    IF AVAILABLE x-vtatabla THEN DO:
        ...
        cTipoCmptes = TRIM(x-vtatabla.llave_c6).
    END.

    IF LOOKUP(pTipoDocumento,cTipoCmptes) =  0 THEN RETURN.
...
FOR EACH ccbddocu OF ccbcdocu NO-LOCK BREAK BY Ccbddocu.NroItm:
    ...
    RUN PRINT_carga-detraccion(ccbcdocu.coddoc,OUTPUT x-msg1).
    IF x-msg1 = NO THEN DO:
        RUN PRINT_carga-retencion(ccbcdocu.coddoc).
    END.
...
&IF DEFINED(EXCLUDE-PRINT_carga-retencion) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE PRINT_carga-retencion Procedure 
PROCEDURE PRINT_carga-retencion :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEFINE VAR cTabla AS CHAR INIT "CONFIG-CMPTE-ELECTRONICO".
DEFINE VAR cLlave_C1 AS CHAR INIT "GLOSA".
DEFINE VAR cLlave_C2 AS CHAR INIT "RETENCION".
...
FIND FIRST x-vtatabla WHERE x-vtatabla.codcia = 1 AND x-vtatabla.tabla = cTabla AND x-vtatabla.llave_c1 = cLlave_C1 
                            AND x-vtatabla.llave_c2 = cLlave_C2 NO-LOCK NO-ERROR.

IF NOT AVAILABLE x-vtatabla THEN RETURN.

cTipoCmptes = TRIM(x-vtatabla.llave_c6).

IF TRUE <> (cTipoCmptes > "") THEN RETURN.

IF LOOKUP(pTipoDocumento, cTipoCmptes) > 0 THEN DO:
