...
DEFINE VAR cRespuestaSunat AS CHAR.
...
&IF DEFINED(EXCLUDE-PRINT_fget-bultos) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD PRINT_fget-bultos Procedure 
FUNCTION PRINT_fget-bultos RETURNS INTEGER
...
&IF DEFINED(EXCLUDE-PRINT_fget-count-invoice-printed) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD PRINT_fget-count-invoice-printed Procedure 
FUNCTION PRINT_fget-count-invoice-printed RETURNS INTEGER
  ( INPUT pCodDoc AS CHAR, INPUT pNroDoc AS CHAR )  FORWARD.
...
&IF DEFINED(EXCLUDE-PRINT_carga-ctasbancarias) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE PRINT_carga-ctasbancarias Procedure 
PROCEDURE PRINT_carga-ctasbancarias :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
IF LOOKUP(pTipoDocumento, 'BOL,FAC,N/D') > 0 THEN DO:
    FOR EACH x-vtatabla WHERE x-vtatabla.codcia = 1 AND x-vtatabla.tabla = cTabla AND x-vtatabla.llave_c1 = cLlave_C1 
        ...
        IF NOT (TRUE <> (x-vtatabla.llave_c4 > "")) AND NOT (TRUE <> (x-vtatabla.llave_c5 > "")) THEN DO:
            ...
            ASSIGN w-report.Campo-c[iSec] = trim(x-vtatabla.llave_c5).
            iSec = iSec + 1.
...
&IF DEFINED(EXCLUDE-PRINT_carga-detraccion) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE PRINT_carga-detraccion Procedure 
PROCEDURE PRINT_carga-detraccion :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEFINE INPUT PARAMETER pTipoDocumento AS CHAR.
...
IF LOOKUP(pTipoDocumento,'BOL,FAC') > 0 THEN DO:
    ...
    BUSCATOPE:
    FOR EACH b-Ccbddocu OF Ccbcdocu NO-LOCK, FIRST almmserv NO-LOCK WHERE almmserv.CodCia = b-Ccbddocu.codcia AND
        almmserv.codmat = b-Ccbddocu.codmat AND
        almmserv.AftDetraccion = YES:        
        /*xImpTot = b-Ccbcdocu.ImpTot.*/
        xImpTot = b-Ccbddocu.ImpLin.
        
        IF Ccbcdocu.CodMon = 2  THEN xImpTot = xImpTot * Ccbcdocu.TpoCmb.
...
RUN lib/_numero(lTotalPagar, 2, 1, OUTPUT X-EnLetras).
...
FOR EACH ccbddocu OF ccbcdocu NO-LOCK BREAK BY Ccbddocu.NroItm:
    ...
    ASSIGN  
        ...
        /*w-report.campo-f[1] = CcbDDocu.MontoBaseIGV + CcbDDocu.ImporteIGV*/       /* SIN ARIMETICA */
        w-report.campo-f[1] = ccbddocu.implin - ccbddocu.impdto2    /*Ccbddocu.cImporteTotalConImpuesto*/
        ...
        w-report.campo-f[3] = Ccbddocu.preuni           /* SIN ARIMETICA */
    ...
    ASSIGN  
        /* Op. Gravadas */
        w-report.campo-f[6] = ccbcdocu.impvta   /*Ccbcdocu.TotalValorVentaNetoOpGravadas*/
        /* Op. Gratuitas */
        w-report.campo-f[7] = lOpeGratuitas     /*Ccbcdocu.TotalValorVentaNetoOpGratuitas*/
        /* Op. Exoneradas */
        w-report.campo-f[8] = ccbcdocu.impexo   /*Ccbcdocu.TotalValorVentaNetoOpExoneradas*/
        /* Op. Inafectas */
        w-report.campo-f[9] = IF (ccbcdocu.porigv > 0) THEN 0 ELSE ccbcdocu.impvta  /*Ccbcdocu.TotalValorVentaNetoOpNoGravada*/
        /* Total Dsctos */
        ...
        /* IGV */
        w-report.campo-f[13] = ccbcdocu.impigv  /*CcbCDocu.TotalIGV*/
        /* Total a Pagar */
        w-report.campo-f[14] = lTotalPagar.  /*Ccbcdocu.TotalVenta.*/
    ASSIGN
...
/**/
RUN PRINT_carga-ctasbancarias(ccbcdocu.coddoc) .
RUN PRINT_carga-detraccion(ccbcdocu.coddoc).
/*
...
IF x-enviar-guia-remision = YES THEN DO:
    DEFINE BUFFER bx-ccbcdocu FOR ccbcdocu.
     FOR EACH bx-ccbcdocu WHERE bx-ccbcdocu.codcia = s-codcia AND                                   
         ...
         x-guia-remision = x-guia-remision + "T" + SUBSTRING(bx-ccbcdocu.nrodoc,1,3) + "-" + SUBSTRING(bx-ccbcdocu.nrodoc,4). 
     END.                                                                                           
    RELEASE bx-ccbcdocu.
END.
...
FIND FacCorre WHERE FacCorre.CodCia = Ccbcdocu.codcia AND 
    ...
    NO-LOCK NO-ERROR.
IF pNombreImpresora > "" THEN lImpresora = pNombreImpresora.
ELSE IF AVAILABLE FacCorre AND FacCorre.PRINTER > '' THEN lImpresora = FacCorre.Printer.
/*
MESSAGE "pNombreImpresora " pNombreImpresora SKIP
        "lImpresora " lImpresora SKIP
        "pImprimeDirecto " pImprimeDirecto.
*/
/* ********************************************************************************************** */
...
CASE TRUE:
    ...
    OTHERWISE DO:
        ...
        IF TRUE <> (lImpresora > '') THEN DO:     
            ...
            x-filer = PRINT_flog-epos-txt("IMPRIMIR DOCMTO (IMPRESION DIRECTA - IMPRESORA PREDETERMINADA) : " + ccbcdocu.coddiv + " " + ccbcdocu.coddoc + " " + ccbcdocu.nrodoc).
            ...
            IF Success = NO THEN DO:
                x-filer = PRINT_flog-epos-txt("IMPRIMIR DOCMTO (IMPRESION DIRECTA - IMPRESORA PREDETERMINADA - NO HAY) : " + ccbcdocu.coddiv + " " + ccbcdocu.coddoc + " " + ccbcdocu.nrodoc).
        ...
        ELSE DO:
            /* Impresora definida */            
            s-printer-name = lImpresora.
            x-filer = PRINT_flog-epos-txt("IMPRIMIR DOCMTO (IMPRESION DIRECTA - IMPRESORA ENVIADA) : " + lImpresora + " - " + ccbcdocu.coddiv + " " + ccbcdocu.coddoc + " " + ccbcdocu.nrodoc).
            RUN lib/_port-name-v2.p (s-printer-name, OUTPUT s-port-name).
...
/* # DE BULTOS */
/* ****************************************************************************** */
DEF VAR pNroBultos AS CHAR INIT "" NO-UNDO.
DEF VAR iNroBultos AS INTE INIT 0 NO-UNDO.

iNroBultos = PRINT_fget-bultos(Ccbcdocu.coddoc, Ccbcdocu.nrodoc).
IF iNroBultos > 0 THEN DO:
    pNroBultos = "BULTOS : " + STRING(iNroBultos).
END.
...
RB-OTHER-PARAMETERS = "s-nomcia = CONTINENTAL SAC" +
                        ...
                        "~npNroBultos = " + pNroBultos +
                        "~npImporteLetras = " + X-EnLetras
                        .
...
DEFINE VAR xDireccionPtoLlegada AS CHAR.
DEFINE VAR xUbigeoPtoLlegada AS CHAR.
DEFINE VAR xObservaciones AS CHAR.

DEFINE VAR xDireccionesSUNAT AS CHAR INIT "DIRECCIONSUNAT".
DEFINE VAR xDivisionOrigen AS CHAR.
...
IF x-gre_header.m_tpomov = "GRVTA" THEN DO:
    ...
    IF AVAILABLE ccbcdocu THEN DO:
        ...
        cSede = ccbcdocu.sede.
        xDireccionPtoLlegada = ccbcdocu.dircli.
        FIND FIRST gn-clieD WHERE gn-clieD.codcia = 0 AND gn-clieD.codcli = ccbcdocu.codcli AND
                                    gn-clieD.sede = cSede /*b-ccbcdocu.sede*/ NO-LOCK NO-ERROR.
        IF AVAILABLE gn-clieD THEN DO:
            xUbigeoPtoLlegada = TRIM(gn-clieD.coddept) + TRIM(gn-clieD.codprov) + TRIM(gn-clieD.coddis).
        END.                               
        xDivisionOrigen = ccbcdocu.divori.
    END.
    ...
    */
    ...
    IF AVAILABLE x-ccbadocu THEN DO:
        ...
        IF NOT (TRUE <> (x-ccbadocu.libre_c[20] > "")) THEN DO:
            ...
            cSede = x-ccbadocu.libre_c[20].                 /* Ubigeo */
            xDireccionPtoLlegada = x-ccbadocu.libre_c[12].   /* Direccion agencia de transporte */
            /* Buscar el ubigeo de la sede del proveedor */
            FIND FIRST gn-provD WHERE gn-provD.codcia = 0 AND gn-provD.codpro = x-ccbadocu.libre_c[9]
                                        AND gn-provD.sede = cSede NO-LOCK NO-ERROR.
            IF AVAILABLE gn-provD THEN DO:
                xUbigeoPtoLlegada = TRIM(gn-provD.coddept) + TRIM(gn-provD.codprov) + TRIM(gn-provD.coddis).
            END.     
        END.
    ...
    /* 08Mar2024, Rosa Mendoza/Jorge Cantera, las direcciones de entrega y llegada son las mismas de la sunat */
    FIND FIRST x-factabla WHERE x-factabla.codcia = s-codcia AND x-factabla.tabla = xDireccionesSUNAT AND
                                x-factabla.codigo = xDivisionOrigen NO-LOCK NO-ERROR.
    IF AVAILABLE x-factabla THEN DO:
        ASSIGN cDireccionEntrega = x-gre_header.direccionPtoLlegada
        xDireccionPtoLlegada = x-gre_header.direccionPtoLlegada
        xUbigeoPtoLlegada = x-gre_header.ubigeoPtoLlegada.
    END.
...
cObservaciones = TRIM(cObservaciones) + " - No. PGRE : " + STRING(x-gre_header.ncorrelatio).
xObservaciones = "".

RUN lib/limpiar-texto(cObservaciones," ",OUTPUT xObservaciones).

cObservaciones = xObservaciones.
    ...
    CREATE b-w-report.
        ASSIGN b-w-report.task-no = s-task-no
                ...
                b-w-report.campo-c[30] = lPathGraficoQR.
                 IF x-gre_header.m_tpomov = "GRVTA" THEN DO:
                     ASSIGN b-w-report.campo-c[9] = xDireccionPtoLlegada
                     b-w-report.campo-c[10] = xUbigeoPtoLlegada.
                 END.
                 ELSE DO:
                     ASSIGN b-w-report.campo-c[9] = x-gre_header.direccionPtoLlegada
...
/**/
cRespuestaSunat = TRIM(x-gre_header.m_rspta_sunat).
...
cNombreDelReporte = "gre_A4".
IF cRespuestaSunat = "ENVIADO A SUNAT" THEN DO:
    cNombreDelReporte = "gre_A4_PGRE".
END.
ASSIGN
    RB-REPORT-LIBRARY = s-report-library + "gre/gre_rpt.prl"
    RB-REPORT-NAME = cNombreDelReporte  /*"gre_A4"*/
    RB-INCLUDE-RECORDS = "O"
    RB-FILTER = " w-report.task-no = " + STRING(s-task-no) /*+  
...
IF RB-PRINT-DESTINATION = "D" THEN iCopias = 1.
...
/* NOTA: el parámetro pFormatoTck va a ser reconfigurado dentro de esta librería*/
DEFINE INPUT PARAMETER pImprimeDirecto AS LOG.          /* YES o NO */
/* NOTA: el parámetro pImprimeDirecto va a ser reconfigurado dentro de esta librería */
DEFINE INPUT PARAMETER pNombreImpresora AS CHAR.        /* Si viene en blanco toma la del correlativo o la predeterminada */
...
/* ****************************************************************************** */
/* ****************************************************************************** */
/* 26/07/2023: Configuración del FORMATO DE IMPRESION */
/* Ccbcdocu.TpoFac = "CO" VENTA MOSTRADOR, en caso contrario "CR" VENTA CREDITO */
/* ****************************************************************************** */
/* ****************************************************************************** */
DEF VAR x-TipoVenta AS CHAR INIT "CO" NO-UNDO.  /* Valor por defecto */

IF Ccbcdocu.TpoFac > '' AND LOOKUP(TRIM(Ccbcdocu.TpoFac),"CO,CR") > 0
    THEN x-TipoVenta = (IF Ccbcdocu.TpoFac = "CO" THEN "CO" ELSE "CR").
ELSE DO:
    IF Ccbcdocu.Tipo > '' THEN x-TipoVenta = (IF Ccbcdocu.Tipo = 'MOSTRADOR' THEN 'CO' ELSE 'CR').
END.

FIND VtaTabla WHERE VtaTabla.CodCia = s-codcia AND
    VtaTabla.Tabla = "CFG_FMT_IMPRESION" AND
    VtaTabla.Llave_c1 = pCodDiv AND
    VtaTabla.Llave_c2 = x-TipoVenta AND
    VtaTabla.LLave_c3 = pCodDoc
    NO-LOCK NO-ERROR.
IF AVAILABLE VtaTabla THEN DO:
    /* Formato de Impresión */
    CASE VtaTabla.Llave_c4:
        WHEN "TCK" THEN ASSIGN RB-REPORT-NAME = "ticket comprobante electronico" pFormatoTCK = YES.
        WHEN "A4b" THEN ASSIGN RB-REPORT-NAME = "Docto Venta Electronico PrePrint" pFormatoTCK = NO.
        WHEN "A4m" THEN ASSIGN RB-REPORT-NAME = "docto venta electronico" pFormatoTCK = NO.
    END CASE.
END.

ASSIGN RB-REPORT-NAME = "Docto Venta Electronico PrePrint" .
ASSIGN pFormatoTCK = NO.

IF AVAILABLE VtaTabla AND LOOKUP(TRIM(VtaTabla.Libre_c01), 'SI,NO') > 0
    /* Impresión Directa o no */
    THEN pImprimeDirecto = (IF TRIM(VtaTabla.Libre_c01) = "SI" THEN YES ELSE NO).

    pImprimeDirecto = YES.  /* Luego borrarrrrrr */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION PRINT_fget-bultos Procedure 
FUNCTION PRINT_fget-bultos RETURNS INTEGER
  ( INPUT pCodDoc AS CHAR, INPUT pNroDoc AS CHAR) :
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  Se va a buscar en base al comprobante, sino por la O/D
------------------------------------------------------------------------------*/

  DEF VAR iNroBultos AS INTE INIT 0 NO-UNDO.
  DEF BUFFER LocalComprobante FOR Ccbcdocu.

  FIND LocalComprobante WHERE LocalComprobante.codcia = s-codcia AND
      LocalComprobante.coddoc = pCodDoc AND
      LocalComprobante.nrodoc = pNroDoc
      NO-LOCK NO-ERROR.
  IF NOT AVAILABLE LocalComprobante THEN RETURN 0.

  FOR EACH Ccbcbult NO-LOCK WHERE CcbCBult.CodCia = LocalComprobante.codcia AND
      CcbCBult.CodDoc = LocalComprobante.coddoc AND
      CcbCBult.NroDoc = LocalComprobante.nrodoc:
      iNroBultos = iNroBultos + CcbCBult.Bultos.
  END.
  IF iNroBultos = 0 THEN DO:
      /* Buscamos por la O/D */
      FOR EACH Ccbcbult NO-LOCK WHERE CcbCBult.CodCia = LocalComprobante.codcia AND
          CcbCBult.CodDoc = LocalComprobante.libre_c01 AND
          CcbCBult.NroDoc = LocalComprobante.libre_c02:
          iNroBultos = iNroBultos + CcbCBult.Bultos.
      END.
  END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION PRINT_fget-count-invoice-printed Procedure 
FUNCTION PRINT_fget-count-invoice-printed RETURNS INTEGER
  ( INPUT pCodDoc AS CHAR, INPUT pNroDoc AS CHAR ) :

  DEFINE VAR iQtyImpresiones AS INT INIT 0.

  FOR EACH Invoices_Printed NO-LOCK WHERE Invoices_Printed.CodCia = s-codcia AND
      Invoices_Printed.CodDoc = pCodDoc AND
      Invoices_Printed.NroDoc = pNroDoc AND
      Invoices_Printed.Version_Printed = "R":
