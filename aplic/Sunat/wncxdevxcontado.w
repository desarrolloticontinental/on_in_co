...
Sintaxis:
    ...
    run ccb/wnotacreditodebito.r(NC|N|REBATE|CONTADO)
...
DEF NEW SHARED VAR s-Tipo   AS CHAR INIT "MOSTRADOR".
/* Nuevo parametro */
IF NUM-ENTRIES(pParametros,'|') >= 4 THEN ASSIGN s-Tipo = ENTRY(4,pParametros,'|').
...
FOR EACH FacCorre NO-LOCK WHERE FacCorre.CodCia = s-CodCia
    AND FacCorre.CodDiv = s-CodDiv
    AND FacCorre.CodDoc = s-CodDoc,
    FIRST Ccbdterm NO-LOCK WHERE CcbDTerm.CodCia = s-CodCia
    AND CcbDTerm.CodDiv = s-CodDiv
    AND CcbDTerm.CodDoc = s-CodDoc
    AND CcbDTerm.CodTer = s-CodTer
    AND CcbDTerm.NroSer = FacCorre.NroSer:
    IF FacCorre.ID_Pos <> "" AND s-Tipo <> "" AND FacCorre.ID_Pos <> s-Tipo THEN NEXT.
    IF cListItems = "" THEN cListItems = STRING(FacCorre.NroSer,"999").
    ELSE cListItems = cListItems + "," + STRING(FacCorre.NroSer,"999").
END.
IF TRUE <> (cListItems > '') THEN DO:
    MESSAGE 'NO definido el documento' s-coddoc 'para la caja' s-codter SKIP
...
DEFINE VARIABLE h_vncxdevolucion AS HANDLE NO-UNDO.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'CCB/vncxdevolucion.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_vncxdevolucion ).
       RUN set-position IN h_vncxdevolucion ( 2.62 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.38 , 103.43 ) */

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('1':U) NO-ERROR.

       /* Links to SmartViewer h_vncxdevolucion. */
       RUN add-link IN adm-broker-hdl ( h_p-updv13 , 'TableIO':U , h_vncxdevolucion ).
       RUN add-link IN adm-broker-hdl ( h_qnotacreditodebito , 'Record':U , h_vncxdevolucion ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv13 ,
             h_p-navico , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_vncxdevolucion ,
    ...
    WHEN 1 THEN DO:
       ...
       RUN set-position IN h_qnotacreditodebito ( 1.15 , 97.00 ) NO-ERROR.
