...
*/
/*
MESSAGE "PROGRAMA DESCONTINUADO..." SKIP
    "POR FAVOR COMUNICARSE CON EL AREA DE TI" SKIP
    "-----------------------------------------"
    "MUCHAS GRACIAS!!" VIEW-AS ALERT-BOX INFORMATION.

pReturn = "ESTA INTENTANTO USAR UN PROGRAMA DESCONTINUADO!!!".
...
DEFINE VAR x-sumatoria-ImporteTotalSinImpuesto AS DEC.

DEFINE VAR x-totalValorVentaNetoOpGratuitas AS DEC.
DEFINE VAR x-ImporteValorVentaNetoOpGravadas AS DEC.
DEFINE VAR x-totalTributosOpeGratuitas AS DEC.
...
x-log-envio = flog-envio-xml("      14/12/2021 9:15").
x-log-envio = flog-envio-xml("/* ---------------------------------------------------------------------------------- */").
...
&IF DEFINED(EXCLUDE-excepcion) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE excepcion Procedure 
PROCEDURE excepcion :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
DEFINE INPUT-OUTPUT PARAMETER x-porcentaje-dscto-global-anticipo AS DEC NO-UNDO.

/*
    Autorizado por Daniel Llican y Mayra Padilla el dia 14Dic2021
*/
IF ccbcdocu.nrodoc = '119000024' OR  ccbcdocu.nrodoc = '119000013' THEN DO:
...
DEFINE VAR x-importe-uni-sin-impsto AS DEC.
DEFINE VAR x-imp-unitario-dscto-sin-igv AS DEC.
DEFINE VAR x-precio-venta-unitario-sin-igv AS DEC.
DEFINE VAR x-importe-dscto-sin-igv AS DEC. 
DEFINE VAR x-descuento-unitario AS DEC.
DEFINE VAR x-unitario-sin-impuesto2 AS DEC. 

DEFINE VAR x-existe-icbper AS LOG.
DEFINE VAR x-codfam-icbper AS CHAR.
DEFINE VAR x-impte-unit-sin-igv AS DEC.
DEFINE VAR x-MontoTributoBolsaPlastico AS DEC.
...
x-totalTributosOpeGratuitas = 0.
x-ImporteValorVentaNetoOpGravadas = 0.
x-totalValorVentaNetoOpGratuitas = 0.
x-totalTributosOpeGratuitas = 0.
...
FOR EACH b-ccbddocu OF b-ccbcdocu NO-LOCK BY b-ccbddocu.nroitm :
    ...
    IF b-ccbddocu.coddoc = 'BOL' OR b-ccbddocu.coddoc = 'FAC' THEN DO:
        IF b-Ccbddocu.Por_Dsctos[1] > 0 OR b-Ccbddocu.Por_Dsctos[2] > 0 OR
                                 b-Ccbddocu.Por_Dsctos[3] > 0 OR             
                                b-Ccbddocu.dcto_otros_factor > 0 THEN DO:
            ...
            x-factor-descuento = ROUND(x-factor-descuento / 100 , 4).
        END.
        /* B2C - 07Ene2022, segun Ruben para ecomerce */
        IF x-factor-descuento <= 0 THEN DO:
            IF b-ccbddocu.impdto2 > 0 THEN DO:
               x-factor-descuento = ROUND(b-ccbddocu.impdto2 / b-ccbddocu.implin,4).
            END.
        END.
    END.
    IF x-es-transferencia-gratuita = YES THEN DO:
        x-codigo-importe-unitario-con-igv = "01".                               
        x-codigo-razon-exoneracion = '15'.                  /*15 o 21*/
        x-codigo-importe-referencial = "02".                
        x-unitario-sin-impuesto = 0.
        x-unitario-con-impuesto = 0.
        x-importe-referencial = ROUND(b-ccbddocu.preuni / ( 1 + (b-ccbcdocu.porigv / 100 )),2) .
        x-importe-sin-impuesto = ROUND(x-importe-referencial * b-ccbddocu.candes,2).
        x-monto-base-igv = x-importe-sin-impuesto.

        IF b-ccbddocu.aftigv = YES THEN DO:        
            x-tasa-igv = b-ccbcdocu.porigv.            
            x-totalValorVentaNetoOpGratuitas = x-totalValorVentaNetoOpGratuitas + x-monto-base-igv.
            x-importe-igv = (x-monto-base-igv * (b-ccbcdocu.porigv / 100 )).
            x-totalTributosOpeGratuitas = x-totalTributosOpeGratuitas + x-importe-igv.
        END.
        ELSE DO:
            /* Exonerado */
    ...
    END.
    ELSE DO:
        IF (x-factor-descuento >= 1 OR b-ccbddocu.implin <= 0.0050) OR (b-ccbddocu.preuni <= 0.0050) THEN DO:        
            /* Bonificaciones (Precio Unitario es menor igual <= 0.0050) */
            IF b-ccbddocu.preuni <= 0.06 THEN DO:
                IF b-ccbddocu.coddoc = 'BOL' OR b-ccbddocu.coddoc = 'FAC' THEN DO:
                    /**/
                    x-codigo-importe-unitario-con-igv = "01".                               
                    x-codigo-razon-exoneracion = '15'.                  /*15*/
                    x-codigo-importe-referencial = "02".                
                    x-unitario-sin-impuesto = 0.
                    x-unitario-con-impuesto = 0.
                    x-tasa-igv = b-ccbcdocu.porigv.
    
                    x-importe-referencial = ROUND(0.06 / ( 1 + (b-ccbcdocu.porigv / 100)),2).
                    x-monto-base-igv = ROUND(x-importe-referencial * b-ccbddocu.candes,2).
                    x-importe-sin-impuesto = x-monto-base-igv.
                    x-totalValorVentaNetoOpGratuitas = x-totalValorVentaNetoOpGratuitas + x-monto-base-igv.
                    x-importe-igv = ROUND((x-monto-base-igv * (b-ccbcdocu.porigv / 100 )),2).
                    x-totalTributosOpeGratuitas = x-totalTributosOpeGratuitas + x-importe-igv.
                END.
                ELSE DO:
            ...
            ELSE DO:
                ...
                x-unitario-con-impuesto = 0.
                x-tasa-igv = b-ccbcdocu.porigv.   
                x-importe-referencial = b-ccbddocu.preuni / ( 1 + (b-ccbcdocu.porigv / 100 )) . 
                x-importe-sin-impuesto = x-importe-referencial * b-ccbddocu.candes.
                x-monto-base-igv = x-importe-referencial * b-ccbddocu.candes.
                x-totalValorVentaNetoOpGratuitas = x-totalValorVentaNetoOpGratuitas + x-monto-base-igv.
                x-importe-igv = (x-monto-base-igv * (b-ccbcdocu.porigv / 100 )).
                x-totalTributosOpeGratuitas = x-totalTributosOpeGratuitas + x-importe-igv.
            END.
        ...
        ELSE DO:    
            IF b-ccbddocu.aftigv = NO THEN DO:        
                ...
                ELSE DO:
                    ...
                    x-precio-venta-unitario-sin-igv = b-ccbddocu.preuni /*/ (1 + ( b-ccbcdocu.porigv / 100 ))*/.
                    x-unitario-sin-impuesto = x-precio-venta-unitario-sin-igv.
                    IF x-factor-descuento > 0 THEN DO:
                        x-importe-base-descuento = x-precio-venta-unitario-sin-igv * b-ccbddocu.candes.
                    END.                    
                    x-importe-descuento = x-importe-base-descuento * x-factor-descuento.
                    x-importe-sin-impuesto = x-precio-venta-unitario-sin-igv * b-ccbddocu.candes - x-importe-descuento.
                    x-monto-base-igv = x-importe-sin-impuesto.
                    x-importe-igv = x-monto-base-igv * ( b-ccbcdocu.porigv / 100 ).
                    x-unitario-con-impuesto = x-unitario-sin-impuesto - (x-importe-descuento / b-ccbddocu.candes).
                    x-vvta-exoneradas = x-vvta-exoneradas + x-importe-sin-impuesto.
                    x-tasa-igv = 0.
                    x-importe-igv = 0.                                            
                END.
            END.
            ELSE DO:
                IF b-ccbddocu.impigv <= 0 AND b-ccbddocu.impdto > 0  THEN DO:
                    /* Exonerada */
                    x-importe-sin-impuesto = b-ccbddocu.implin - b-ccbddocu.impigv.
                    x-unitario-sin-impuesto = x-importe-sin-impuesto / b-ccbddocu.candes.
                    x-importe-con-impuesto = b-ccbddocu.implin.
                ...
                ELSE DO:
                    /* Venta GRAVADA */              
                    IF b-ccbddocu.impdto <= 0 AND b-ccbddocu.impdto2 <= 0 THEN DO:  /* Sin descuentos */
                        x-precio-venta-unitario-sin-igv = ROUND(b-ccbddocu.preuni / (1 + ( b-ccbcdocu.porigv / 100 )),4).
                        x-unitario-sin-impuesto = x-precio-venta-unitario-sin-igv.          
                        x-importe-sin-impuesto = ROUND(x-precio-venta-unitario-sin-igv * b-ccbddocu.candes,2).
                        x-monto-base-igv = x-importe-sin-impuesto.
                        x-importe-igv = ROUND(x-monto-base-igv * ( b-ccbcdocu.porigv / 100 ),2).
                        ...
                        x-codigo-importe-unitario-con-igv = "01".
                    END.
                    ELSE DO:    /* Con descuentos */
                        x-precio-venta-unitario-sin-igv = ROUND(b-ccbddocu.preuni / (1 + ( b-ccbcdocu.porigv / 100 )),4).
                        x-unitario-sin-impuesto = x-precio-venta-unitario-sin-igv.
                        x-importe-base-descuento = ROUND(x-precio-venta-unitario-sin-igv * b-ccbddocu.candes,2).
                        x-importe-descuento = ROUND(x-importe-base-descuento * x-factor-descuento,2).
                        x-importe-sin-impuesto = x-precio-venta-unitario-sin-igv * b-ccbddocu.candes - x-importe-descuento.
                        x-monto-base-igv = x-importe-sin-impuesto.
                        x-importe-igv = x-monto-base-igv * ( b-ccbcdocu.porigv / 100 ).
                        x-unitario-con-impuesto = ROUND((x-importe-sin-impuesto + x-importe-igv) / b-ccbddocu.candes,4).
    ...
    IF x-codigo-razon-exoneracion = '15' OR x-codigo-razon-exoneracion = '13' THEN DO:
        /**/ 
    END.
    ELSE DO:
        IF x-codigo-razon-exoneracion = '20' THEN DO:
        END.
        ELSE DO:
            x-ImporteValorVentaNetoOpGravadas = x-ImporteValorVentaNetoOpGravadas + x-importe-sin-impuesto.            
            ...
            x-sumatoria-ImporteTotalSinImpuesto = x-sumatoria-ImporteTotalSinImpuesto + x-importe-sin-impuesto.
        END.        
        x-total-igv2 = x-total-igv2 + x-importe-igv.              /* IGV */
        x-total-igv = x-total-igv + x-importe-igv.               /* Total Impuestos */
    END.        
    ...
    */   
    ...
    IF x-importe-referencial > 0 THEN DO:
        pXML-DETAIL = pXML-DETAIL + "<importeReferencial>" + TRIM(STRING(x-importe-referencial,">>>>>>>9.9999")) + "</importeReferencial>".
    ...
    /* Ic - 09Mar2020, Impuesto Bolsa Plastica */   
     IF AVAILABLE almmmatg AND almmmatg.codfam = x-linea-bolsas-plastica THEN DO:
        x-XMLicbper = "".
        x-MontoTributoBolsaPlastico = 0.
        RUN impuesto-icbper(OUTPUT x-XMLicbper, INPUT b-Ccbddocu.candes, OUTPUT x-MontoTributoBolsaPlastico).

        x-importe-igv = x-importe-igv + x-MontoTributoBolsaPlastico.
...
------------------------------------------------------------------------------*/
    ...
    x-total-a-pagar = x-imp-total.   /*b-ccbcdocu.imptot.*/ /*x-totalPrecioVenta.*/
    ...
    REPEAT x-sec = 1 TO x-cuotas:
        IF x-enviar-servidor-prueba = YES THEN DO:
            x-aaaa-mm-dd = SUBSTRING(STRING(TODAY + x-sec,"99-99-9999"),7,4) + "-".
            x-aaaa-mm-dd = x-aaaa-mm-dd + SUBSTRING(STRING(TODAY + x-sec,"99-99-9999"),4,2) + "-" .
            x-aaaa-mm-dd = x-aaaa-mm-dd + SUBSTRING(STRING(TODAY + x-sec,"99-99-9999"),1,2).
        END.
        ELSE DO:
...
/* Parche x que em imptot no concide con el el calculado  */
DEFINE VAR x-tipo-factura AS CHAR.
x-imp-total =  b-ccbcdocu.imptot.
x-tipo-factura = "".
IF x-es-transferencia-gratuita = YES  THEN DO:
    /* Transferencia gratuita */
    x-vvta-inafectas = 0.    /*b-ccbcdocu.impvta.*/
    x-vvta-gravada = 0.
    x-total-igv = 0.
    x-total-igv2 = 0.
END.
...
END.
x-vvta-gratuitas = x-totalValorVentaNetoOpGratuitas.
...
IF b-ccbcdocu.coddoc = 'BOL' OR b-ccbcdocu.coddoc = 'FAC' THEN DO:
    ...
    RUN generar-xml-anticipos(INPUT b-ccbcdocu.coddoc,
                                  ...
                                  OUTPUT z-texto-xml).
END.
DEFINE VAR x-montoBaseDsctoGlobal AS DEC INIT 0.
DEFINE VAR x-descuentosGlobales AS DEC INIT 0.
DEFINE VAR x-porcentajeDescuentoGlobal AS DEC INIT 0.

x-vvta-gravada = x-sumatoria-ImporteTotalSinImpuesto.
/* b-ccbcdocu.acubon[10] = total monto ICBPER */
x-totalValorVenta = x-totalValorVenta. /*- (b-ccbcdocu.impdto2 / (1 + (b-ccbcdocu.porigv / 100))).*/
x-totalPrecioVenta = x-totalValorVenta + b-ccbcdocu.acubon[10] + ROUND((x-totalValorVenta * (b-ccbcdocu.porigv / 100)) , 2).
IF x-vvta-exoneradas > 0 THEN DO:
    x-totalPrecioVenta = x-totalPrecioVenta + x-vvta-exoneradas.
    x-totalValorVenta = x-totalValorVenta + x-vvta-exoneradas.
END.
x-imp-total = x-totalPrecioVenta.

IF b-ccbcdocu.coddoc = 'BOL' OR b-ccbcdocu.coddoc = 'FAC' THEN DO:
    /* Descuento GLOBAL */
    /*
    IF b-ccbcdocu.impdto2 > 0 THEN DO:
        x-montoBaseDsctoGlobal = x-sumatoria-ImporteTotalSinImpuesto.
        x-descuentosGlobales = (b-ccbcdocu.impdto2 / (1 + (b-ccbcdocu.porigv / 100))).
        x-porcentajeDescuentoGlobal = x-descuentosGlobales / x-sumatoria-ImporteTotalSinImpuesto.
        x-vvta-gravada = x-montoBaseDsctoGlobal - x-descuentosGlobales.
        x-total-igv2 = x-vvta-gravada * ((b-ccbcdocu.porigv / 100)).
        x-imp-total = x-vvta-gravada + x-total-igv2.
    END.    
    */
END.
DEFINE VAR x-filer10 AS DEC.
IF z-tot-anticipos = 0 THEN DO:
    x-filer10 = b-ccbcdocu.imptot. /*b-ccbcdocu.impvta + b-ccbcdocu.impigv.*/
    x-filer10 = x-filer10 - x-imp-total.
    IF x-filer10 < 0 THEN x-filer10 = x-filer10 * -1.
    IF x-filer10 < 1 THEN DO:   /* Si la diferencia es x decimales */
        x-imp-total = b-ccbcdocu.imptot. /* b-ccbcdocu.impvta + b-ccbcdocu.impigv.*/
    END.
END.
...
IF z-tot-anticipos > 0 THEN DO:
    ...
    IF z-tot-anticipos >= x-imp-total THEN DO:
        ...
        IF z-tot-anticipos > x-imp-total THEN DO:
            x-total-anticipos = x-imp-total.
            x-monto-base-dscto-global-anticipo = x-vvta-gravada.
            x-total-dscto-globales-anticipo = x-vvta-gravada.
        END.                    
        x-porcentaje-dscto-global-anticipo = 1.        
        x-total-igv2 = 0. 
        x-total-igv = 0. 
    ...
    ELSE DO:        
        ...
        x-total-igv = x-total-igv2.
        x-porcentaje-dscto-global-anticipo = ROUND(x-total-dscto-globales-anticipo / x-monto-base-dscto-global-anticipo,6).        
        /* 14Dic2021 - El importe vvta del anticipo con vvta de la factura se diferencia x 0.01, el redondeo estaba en 5 lo baje a 4 */
        /*IF x-porcentaje-dscto-global-anticipo >= 1  THEN DO:*/
        IF x-porcentaje-dscto-global-anticipo >= 0.99995  THEN DO:
            x-porcentaje-dscto-global-anticipo = 1.
            x-total-igv2 = 0.
            x-total-igv = 0.
            x-imp-total = 0.
...
END.
/* Impuestos ICBPER */
x-total-igv = x-total-igv + b-ccbcdocu.acubon[10]. /* Total Impuestos */
...
IF x-vvta-gratuitas > 0 THEN DO:   
    p-xml-header = p-xml-header + "<totalValorVentaNetoOpGratuitas>" + TRIM(STRING(x-vvta-gratuitas,">>>>>>>9.99")) + "</totalValorVentaNetoOpGratuitas>".
    p-xml-header = p-xml-header + "<totalTributosOpeGratuitas>" + TRIM(STRING(X-totalTributosOpeGratuitas,">>>>>>>9.99")) + "</totalTributosOpeGratuitas>".
END.       
IF z-tot-anticipos > 0 THEN DO:
    IF x-vvta-exoneradas > 0 THEN DO:
...
IF x-enviar-icbper = YES THEN DO:
    IF b-ccbcdocu.coddoc = 'FAC' OR b-ccbcdocu.coddoc = 'BOL' THEN DO:
        IF b-ccbcdocu.acubon[10] > 0 THEN DO:
            p-xml-header = p-xml-header + "<montoBaseICBPER>" + TRIM(STRING(x-precio-ICBPER,">>>>>>>9.99"))+ "</montoBaseICBPER>".             
