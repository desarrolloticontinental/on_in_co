...
&Scoped-define QUERY-STRING-br_table FOR EACH CcbCDocu WHERE ~{&KEY-PHRASE} ~
      AND {&CONDICION} USE-INDEX LLAVE05 NO-LOCK
...
&Scoped-Define ENABLED-OBJECTS f-desde f-hasta C-CMPBTE FILL-IN-codigo ~
CMB-filtro FILL-IN-filtro br_table RECT-1 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      CcbCDocu.CodDoc COLUMN-LABEL "Tipo!Comp" FORMAT "x(3)":U
      CcbCDocu.NroDoc FORMAT "XXX-XXXXXXXX":U WIDTH 9.86 COLUMN-BGCOLOR 11
            LABEL-BGCOLOR 8
      CcbCDocu.NomCli COLUMN-LABEL "Nombre Cliente" FORMAT "x(50)":U
            COLUMN-BGCOLOR 15
      CcbCDocu.CodVen COLUMN-LABEL "Vend." FORMAT "x(5)":U
...
/* Query rebuild information for BROWSE br_table
     ...
     _Where[1]         = "{&CONDICION} USE-INDEX LLAVE05"
     _FldNameList[1]   > integral.CcbCDocu.CodDoc
"CcbCDocu.CodDoc" "Tipo!Comp" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > integral.CcbCDocu.NroDoc
"CcbCDocu.NroDoc" ? "XXX-XXXXXXXX" "character" 11 ? ? 8 ? ? no ? no no "9.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > integral.CcbCDocu.NomCli
"CcbCDocu.NomCli" "Nombre Cliente" ? "character" 15 ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > integral.CcbCDocu.CodVen
"CcbCDocu.CodVen" "Vend." "x(5)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
  IF AVAILABLE CcbCDocu THEN  DO:
      ...
      RUN ccb/r-imprime-orddesp.p (INPUT ROWID(Ccbcdocu), 
                                   INPUT ENTRY(1, Ccbcdocu.codalm), 
                                   INPUT 'DUPLICADO',
                                   INPUT YES,
                                   OUTPUT pMensaje).
      IF pMensaje <> "OK" AND pMensaje > "" THEN MESSAGE pMensaje VIEW-AS ALERT-BOX WARNING.
  END.
...
------------------------------------------------------------------------------*/
  ...
  /* Solicitamos formato del papel 1: Ticketera  2: A4 */
  DEF VAR pFormato AS INT INIT 2 NO-UNDO.

/*   RUN sunat\d-formato-impresion ( OUTPUT pFormato ). */
/*   IF pFormato = 0 THEN RETURN.                       */
  ...
  x-formato-tck = (IF pFormato = 1 THEN YES ELSE NO).      /* YES : Formato Ticket,  NO : Formato A4 */
