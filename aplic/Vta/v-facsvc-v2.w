...
"US$", 2
     ...
     CcbCDocu.Glosa AT ROW 7.35 COL 11 COLON-ALIGNED
          ...
          SIZE 55 BY .81
     "v2" VIEW-AS TEXT
          SIZE 4 BY .5 AT ROW 7.46 COL 86 WIDGET-ID 4
...
FIND Faccpedi WHERE Faccpedi.codcia = s-codcia
   ...
   FIND Gn-ven OF faccpedi NO-LOCK.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE local-add-record-old V-table-Win 
PROCEDURE local-add-record-old :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
------------------------------------------------------------------------------*/
 ...
 /* v2
  ...
  FIND Faccpedi WHERE Faccpedi.codcia = s-codcia
          AND Faccpedi.coddoc = s-codref
          AND Faccpedi.nroped = cPedido
          EXCLUSIVE-LOCK NO-ERROR.

  IF NOT AVAILABLE faccpedi OR faccpedi.FlgEst <> 'P' THEN DO:
    MESSAGE 'Pedido no válido ó ya no esta pendiente de facturar' VIEW-AS ALERT-BOX ERROR.
    UNDO, RETURN 'ADM-ERROR'.
  END.
...
/*     Ccbcdocu.nrodoc = STRING(Faccorre.nroser, '999') + STRING(Faccorre.correlativo, '999999') */
    ...
    */
      ...
      Ccbcdocu.lugent = Faccpedi.lugent.
