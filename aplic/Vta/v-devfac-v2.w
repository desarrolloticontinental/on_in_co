...
&Scoped-Define ENABLED-FIELDS Almcmov.LPN Almcmov.CodRef Almcmov.NroRf1 ~
...
Almcmov.NroDoc Almcmov.LPN Almcmov.CodRef Almcmov.NroRf1 Almcmov.CodCli ~
Almcmov.CodMon Almcmov.Observ 
&Scoped-define DISPLAYED-TABLES Almcmov
&Scoped-define FIRST-DISPLAYED-TABLE Almcmov
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-Estado FILL-IN_NomCli ~
FILL-IN_RucCli FILL-IN_DirCli FILL-IN-MotDev 
...
DEFINE FRAME F-Main
     Almcmov.NroRf2 AT ROW 1 COL 13 COLON-ALIGNED
     ...
     Almcmov.FchDoc AT ROW 1 COL 72 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 10 BY .81
     Almcmov.NroDoc AT ROW 1.77 COL 13 COLON-ALIGNED
          LABEL "No. Ingreso" FORMAT "999999999"
          VIEW-AS FILL-IN 
          SIZE 12 BY .81
          FONT 0
     Almcmov.LPN AT ROW 2.42 COL 46.57 COLON-ALIGNED WIDGET-ID 4
          LABEL "RTV"
          VIEW-AS FILL-IN 
          SIZE 15.72 BY .81
     Almcmov.CodRef AT ROW 2.54 COL 6.71
          LABEL "Referencia"
          VIEW-AS COMBO-BOX INNER-LINES 3
          LIST-ITEMS "FAC","BOL","FAI" 
          DROP-DOWN-LIST
          SIZE 7.86 BY 1
     Almcmov.NroRf1 AT ROW 2.54 COL 21.14 COLON-ALIGNED NO-LABEL FORMAT "x(15)"
          VIEW-AS FILL-IN 
          SIZE 17.86 BY .81
          FONT 0
     Almcmov.CodCli AT ROW 3.31 COL 13 COLON-ALIGNED FORMAT "x(11)"
          VIEW-AS FILL-IN 
          SIZE 13 BY .81
     FILL-IN_NomCli AT ROW 3.31 COL 26 COLON-ALIGNED NO-LABEL
     FILL-IN_RucCli AT ROW 3.31 COL 72 COLON-ALIGNED
     FILL-IN_DirCli AT ROW 4.08 COL 13 COLON-ALIGNED
     Almcmov.CodMon AT ROW 4.08 COL 74 NO-LABEL
          VIEW-AS RADIO-SET HORIZONTAL
          RADIO-BUTTONS 
                    "S/.", 1,
"US$", 2
          SIZE 11.43 BY .81
     Almcmov.Observ AT ROW 4.85 COL 13 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 50 BY .81
     FILL-IN-MotDev AT ROW 5.62 COL 13 COLON-ALIGNED WIDGET-ID 2
     "v1" VIEW-AS TEXT
          SIZE 8 BY .5 AT ROW 5.23 COL 72 WIDGET-ID 6
     "Moneda" VIEW-AS TEXT
          SIZE 6.14 BY .58 AT ROW 4.12 COL 67
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
    ...
    FOR EACH CcbDDocu NO-LOCK WHERE CcbDDocu.CodCia = CcbCDocu.CodCia 
        ...
        x-Cantidad = CcbDDocu.CanDes - CcbDDocu.CanDev.

        /* Parche solo por hoy - Caso CENCOSUD 03Nov2021 */
        /*IF s-CodAlm = '11L' THEN x-Cantidad = CcbDDocu.CanDes.*/
        ...
        IF x-Cantidad > 0 THEN DO:
            ...
            ASSIGN 
                ...
                DMOV.CanDes = x-Cantidad
                DMOV.CanDev = x-Cantidad
            ...
            &IF {&ARITMETICA-SUNAT} &THEN
                IF DMOV.CanDes = Ccbddocu.CanDes THEN DO:   /* DEVOLUCION TOTAL */
                    ASSIGN
                        DMOV.ImpLin = CcbDDocu.MontoBaseIGV + CcbDDocu.ImporteIGV /*Ccbddocu.cImporteTotalConImpuesto*/
                        DMOV.PreUni = DMOV.ImpLin / DMOV.CanDes.                   /*Ccbddocu.ImporteUnitarioConImpuesto.*/
                END.
                ELSE DO:    /* DEVOLUCION PARCIAL */
                    ASSIGN
                        DMOV.PreUni = (CcbDDocu.MontoBaseIGV + CcbDDocu.ImporteIGV) / DMOV.CanDes       /*Ccbddocu.ImporteUnitarioConImpuesto*/
            ...
            &ELSE
                IF DMOV.CanDes = Ccbddocu.CanDes THEN DO:   /* DEVOLUCION TOTAL */
                    ASSIGN
                        ...
                        DMOV.PreUni = DMOV.ImpLin / DMOV.CanDes.
                END.
                ELSE DO:    /* DEVOLUCION PARCIAL */
                    ASSIGN
    ...
    /* Devolucion total */
    RUN set-devolucion-total IN lh_Handle(INPUT YES). 
...
/*   IF cReturnValue = 'ADM-ERROR' THEN UNDO, RETURN 'ADM-ERROR'. */
  ...
  END.
  ASSIGN 
    Almcmov.NroRf2 = STRING(I-NroSer,"999") + STRING(FacCorre.Correlativo,"999999").
  ASSIGN 
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute PRIOR to standard behavior. */
   ...
   IF s-acceso-total = NO THEN DO:
       /*
            Ic - 26Ene2023, correo se Susuana Leon, solo el supervisor puede ANULAR
            IF almcmov.fchdoc <> dFchCie THEN DO:
       */
       /*IF almcmov.fchdoc = dFchCie THEN DO:*/
           MESSAGE 'NO esta autorizado a realizar ANULACIONES'
   ...
   IF dFchCie <> ? AND almcmov.fchdoc <= dFchCie THEN DO:
...
DO WITH FRAME {&FRAME-NAME} :
   ...
   FOR EACH DMOV NO-LOCK:
       X = X + 1.
   END.
   /* 18/02/2023: E. Urbano no permitir grabar sin items registrados */
   IF X <= 0 THEN DO:
       MESSAGE 'NO ha registrado items a devolver' VIEW-AS ALERT-BOX WARNING.
       RETURN 'ADM-ERROR'.
   END.
...
/*    END.                                                                                */
   ...
   IF TRUE <> (Almcmov.CodCli:SCREEN-VALUE > "") THEN DO:
      MESSAGE "Código de cliente no debe ser blanco" VIEW-AS ALERT-BOX WARNING.
      APPLY "ENTRY" TO Almcmov.CodCli.
      RETURN "ADM-ERROR".   
   END.
   /* SOLO OPENORANGE */
   ...
   IF pClienteOpenOrange = YES THEN DO:
       MESSAGE "Cliente NO se puede antender por Continental" SKIP
           "Sólo se le puede atender por OpenOrange"
           VIEW-AS ALERT-BOX WARNING.
       APPLY "ENTRY" TO Almcmov.CodCli.
       RETURN "ADM-ERROR".   
   END.
   ...
   IF TRUE <> (Almcmov.NroRf1:SCREEN-VALUE > "") THEN DO:
      MESSAGE "Numero de documento no debe ser blanco" VIEW-AS ALERT-BOX WARNING.
   ...
   FIND CcbCDocu WHERE CcbCDocu.CodCia = S-CODCIA 
       ...
       NO-LOCK NO-ERROR.
   IF NOT AVAILABLE CcbCDocu THEN DO:
      MESSAGE "COMPROBANTE NO EXISTE:" Almcmov.CodRef:SCREEN-VALUE Almcmov.NroRf1:SCREEN-VALUE VIEW-AS ALERT-BOX WARNING.
      APPLY "ENTRY" TO Almcmov.NroRf1.
      RETURN "ADM-ERROR".   
   END.
   IF CcbCDocu.CodCli <> Almcmov.CodCli:SCREEN-VALUE THEN DO:
      MESSAGE "DOCUMENTO NO PERTENECE AL CLIENTE" Almcmov.CodCli:SCREEN-VALUE VIEW-AS ALERT-BOX WARNING.
      APPLY "ENTRY" TO Almcmov.NroRf1.
      RETURN "ADM-ERROR".   
   END.
   IF LOOKUP(CcbCDocu.FlgEst,"C,P") = 0 THEN DO:
      MESSAGE "COMPROBANTE NO VALIDO" VIEW-AS ALERT-BOX WARNING.
      APPLY "ENTRY" TO Almcmov.NroRf1.
      RETURN "ADM-ERROR".   
   END.
   IF CcbCDocu.FlgSit = "D" THEN DO:
      MESSAGE "El comprobante ya fue devuelto" VIEW-AS ALERT-BOX WARNING.
      APPLY "ENTRY" TO Almcmov.NroRf1.
      RETURN "ADM-ERROR".   
   END.
   IF C-NROFAC = "" THEN DO:
      MESSAGE "No asignó ninguna Factura" VIEW-AS ALERT-BOX WARNING.
      APPLY "ENTRY" TO Almcmov.CodCli.
      RETURN "ADM-ERROR".   
   END.
   ...
   IF CcbCDocu.CodCli = "20100070970" THEN DO:
       if TRUE <> (Almcmov.lpn:SCREEN-VALUE > "") THEN DO:
           MESSAGE "Para SUPERMERCADOS PERUANOS debe ingresar el RTV" VIEW-AS ALERT-BOX WARNING.
           APPLY "ENTRY" TO Almcmov.lpn.
           RETURN "ADM-ERROR".   
       END.       
   END.

   /* Ic - 27Ago2019, si la devolucion es total no debe tener N/C que nosean x devolucion */
   DEFINE VAR x-devol-total AS LOG.
   DEFINE VAR x-cuantas-nc AS INT.

   RUN get-devolucion-total IN lh_handle(OUTPUT x-devol-total).
   ...
   FIND FIRST x-vtatabla WHERE x-vtatabla.codcia = s-codcia AND
                               ...
                               x-vtatabla.llave_c2 = ccbcdocu.coddoc AND
                               x-vtatabla.llave_c3 = ccbcdocu.nrodoc NO-LOCK NO-ERROR.
   IF AVAILABLE x-vtatabla THEN x-devol-total = NO.

   IF x-devol-total = YES THEN DO:
       x-cuantas-nc = 0.
       /* Cuantas N/C que no sean x devolucion de mercaderia tiene */
       ...
       IF x-cuantas-nc > 0 THEN DO:
           MESSAGE "El documento " + ccbcdocu.coddoc + "-" + ccbcdocu.nrodoc SKIP
                    "Tiene " + STRING(x-cuantas-nc,">>9") + " Nota(s) de Credito" SKIP
   ...
   IF x-cuantas-nc > 0 THEN DO:
       MESSAGE "El documento " + ccbcdocu.coddoc + "-" + ccbcdocu.nrodoc SKIP
                ...
                "Imposible realizar la devolución total"
           VIEW-AS ALERT-BOX WARNING.
       RETURN "ADM-ERROR".
   END.
