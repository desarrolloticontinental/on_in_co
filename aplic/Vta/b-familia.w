...
DEFINE INPUT PARAMETER input-var-1 AS CHARACTER.
DEFINE INPUT PARAMETER input-var-2 AS CHARACTER.
DEFINE INPUT PARAMETER input-var-3 AS CHARACTER.
DEFINE OUTPUT PARAMETER output-var-1 AS CHARACTER.

/*
DEFINE VAR input-var-1 AS CHARACTER.
DEFINE VAR input-var-2 AS CHARACTER.
DEFINE VAR input-var-3 AS CHARACTER.
DEFINE VAR output-var-1 AS CHARACTER.
*/
...
&Scoped-define FIELDS-IN-QUERY-br_table INTEGRAL.Almtfami.desfam ~
INTEGRAL.Almtfami.codfam 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH INTEGRAL.Almtfami WHERE ~{&KEY-PHRASE} ~
      AND INTEGRAL.Almtfami.CodCia = 1 NO-LOCK ~
    BY INTEGRAL.Almtfami.desfam
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH INTEGRAL.Almtfami WHERE ~{&KEY-PHRASE} ~
      AND INTEGRAL.Almtfami.CodCia = 1 NO-LOCK ~
    BY INTEGRAL.Almtfami.desfam.
&Scoped-define TABLES-IN-QUERY-br_table INTEGRAL.Almtfami
&Scoped-define FIRST-TABLE-IN-QUERY-br_table INTEGRAL.Almtfami
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      INTEGRAL.Almtfami.desfam FORMAT "X(30)":U WIDTH 52.43
      INTEGRAL.Almtfami.codfam FORMAT "X(3)":U WIDTH 7.43
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME F-Main
     ...
     f-Mensaje AT ROW 8.27 COL 5 COLON-ALIGNED NO-LABEL WIDGET-ID 2
...
"desfam" ? ? "character" ? ? ? ? ? ? no ? no no "52.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > INTEGRAL.Almtfami.codfam
"codfam" ? ? "character" ? ? ? ? ? ? no ? no no "7.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
  ...
  {src/adm/template/snd-list.i "INTEGRAL.Almtfami"}
