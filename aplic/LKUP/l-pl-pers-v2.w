...
DEFINE VAR x-PatPer AS CHAR NO-UNDO.
DEFINE VAR x-MatPer AS CHAR NO-UNDO.

/* Preprocesadores para condiciones */
&SCOPED-DEFINE CONDICION ( pl-pers.codcia = s-codcia  ~
AND (TRUE <> (x-PatPer > '') OR pl-pers.patper BEGINS x-PatPer) ~
AND (TRUE <> (x-MatPer > '') OR pl-pers.matper BEGINS x-MatPer))

&SCOPED-DEFINE CODIGO pl-pers.codper
...
&Scoped-define FIELDS-IN-QUERY-br_table PL-PERS.patper PL-PERS.matper ~
PL-PERS.nomper 
&Scoped-define ENABLED-FIELDS-IN-QUERY-br_table 
&Scoped-define QUERY-STRING-br_table FOR EACH PL-PERS WHERE ~{&KEY-PHRASE} ~
      AND {&Condicion} NO-LOCK ~
    ~{&SORTBY-PHRASE}
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH PL-PERS WHERE ~{&KEY-PHRASE} ~
      AND {&Condicion} NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table PL-PERS
&Scoped-define FIRST-TABLE-IN-QUERY-br_table PL-PERS


/* Definitions for FRAME F-Main                                         */

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS FILL-IN-codigo FILL-IN-filtro br_table ~
RECT-1 
&Scoped-Define DISPLAYED-OBJECTS FILL-IN-codigo FILL-IN-filtro 
...
<SORTBY-OPTIONS>
Código|y||INTEGRAL.PL-PERS.codper|yes
Descripción|||INTEGRAL.PL-PERS.patper|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
* Set attributes related to SORTBY-OPTIONS */
RUN set-attribute-list (
    'SortBy-Options = "':U + 'Código,Descripción' + '",
     SortBy-Case = ':U + 'Código').
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 91.14 BY 8.62
...
DEFINE FRAME F-Main
     ...
     FILL-IN-filtro AT ROW 2.12 COL 9.72
...
ON LEAVE OF FILL-IN-filtro IN FRAME F-Main /* Buscar por nombre */
DO:
  ASSIGN {&SELF-NAME}.
  x-PatPer = ''.
  x-MatPer = ''.
  x-PatPer = {&SELF-NAME}.
  IF NUM-ENTRIES({&SELF-NAME}, ' ') > 0 THEN DO:
      x-PatPer = ENTRY(1,{&SELF-NAME}, ' ') NO-ERROR.
      x-MatPer = ENTRY(2,{&SELF-NAME}, ' ') NO-ERROR.
  END.
...
------------------------------------------------------------------------------*/
  ...
  CASE RETURN-VALUE:
    WHEN 'Código':U THEN DO:
      &Scope SORTBY-PHRASE BY PL-PERS.codper
      {&OPEN-QUERY-{&BROWSE-NAME}}
    END.
    WHEN 'Descripción':U THEN DO:
      &Scope SORTBY-PHRASE BY PL-PERS.patper
      {&OPEN-QUERY-{&BROWSE-NAME}}
    END.
    OTHERWISE DO:
...
IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}} THEN
    ASSIGN
        ...
        output-var-2 = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.codper
        output-var-3 = {&FIRST-TABLE-IN-QUERY-{&BROWSE-NAME}}.patper.
