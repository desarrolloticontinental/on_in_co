...
DEFINE VARIABLE C-VTA AS CHAR NO-UNDO.

/* Preprocesadores para condiciones */
&SCOPED-DEFINE CONDICION ( CcbCDocu.CodCia = S-CODCIA  AND CcbCDocu.CodCli BEGINS input-var-1 ~
AND CcbCDocu.flgest BEGINS input-var-2 AND CcbCDocu.flgsit BEGINS input-var-3)
...
CcbCDocu.NomCli CcbCDocu.FchDoc CcbCDocu.NroRef C-MON @ C-MON ~
CcbCDocu.SdoAct CcbCDocu.ImpTot CcbCDocu.FlgUbi CcbCDocu.CodCta ~
CcbCDocu.CodAge 
...
<FOREIGN-KEYS>
Nombres que inicien|y||integral.CcbCDocu.NomCli
...
RUN set-attribute-list (
    'Keys-Accepted = "Nombres que inicien,Nombres que contengan",
     Keys-Supplied = ':U).
...
<SORTBY-OPTIONS>
Documento|y||integral.CcbCDocu.CodCia|yes,integral.CcbCDocu.CodDoc|yes,integral.CcbCDocu.NroDoc|yes
Cliente|||integral.CcbCDocu.CodCia|yes,integral.CcbCDocu.CodCli|yes,integral.CcbCDocu.CodDoc|yes,integral.CcbCDocu.NroDoc|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
* Set attributes related to SORTBY-OPTIONS */
RUN set-attribute-list (
    'SortBy-Options = "Documento,Cliente",
     Sort-Case = Documento':U).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      CcbCDocu.NroDoc FORMAT "XXX-XXXXXXXX":U WIDTH 10.72
      CcbCDocu.NomCli FORMAT "x(40)":U
      CcbCDocu.FchDoc COLUMN-LABEL "Fecha Dcmto" FORMAT "99/99/9999":U
      CcbCDocu.NroRef COLUMN-LABEL "Numero!De Deposito" FORMAT "X(15)":U
      ...
      CcbCDocu.FlgUbi FORMAT "X":U
      CcbCDocu.CodCta FORMAT "X(10)":U
      CcbCDocu.CodAge FORMAT "X(10)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 85.72 BY 8.5
...
"CcbCDocu.NroDoc" ? "XXX-XXXXXXXX" "character" ? ? ? ? ? ? no ? no no "10.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"CcbCDocu.FchDoc" "Fecha Dcmto" ? "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[5]   > integral.CcbCDocu.NroRef
"CcbCDocu.NroRef" "Numero!De Deposito" "X(15)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"CcbCDocu.SdoAct" "Saldo Actual" ? "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     ...
     _FldNameList[9]   = integral.CcbCDocu.FlgUbi
     _FldNameList[10]   = integral.CcbCDocu.CodCta
     _FldNameList[11]   = integral.CcbCDocu.CodAge
...
ON LEAVE OF FILL-IN-codigo IN FRAME F-Main /* Deposito */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE RETURN-VALUE:
    WHEN 'Nombres que inicien':U THEN DO:
       &Scope KEY-PHRASE ( {&Filtro1} )
       RUN get-attribute ('SortBy-Case':U).
       CASE RETURN-VALUE:
         WHEN 'Documento':U THEN DO:
           &Scope SORTBY-PHRASE BY CcbCDocu.CodCia BY CcbCDocu.CodDoc BY CcbCDocu.NroDoc
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         WHEN 'Cliente':U THEN DO:
           &Scope SORTBY-PHRASE BY CcbCDocu.CodCia BY CcbCDocu.CodCli BY CcbCDocu.CodDoc BY CcbCDocu.NroDoc
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         OTHERWISE DO:
