...
(COMBO-BOX-Marcas = 'Todas' OR T-DREPO.DesMar = COMBO-BOX-Marcas ) AND ~
...
(FILL-IN-PorcReposicion = 0 OR T-DREPO.PorcReposicion <= FILL-IN-PorcReposicion) AND ~
(RADIO-SET_Empaque = 'Ambos' OR T-DREPO.Origen = RADIO-SET_Empaque) ~
)
...
(COMBO-BOX-Marcas = 'Todas' OR BT-DREPO.DesMar = COMBO-BOX-Marcas) AND ~
...
(FILL-IN-PorcReposicion = 0 OR BT-DREPO.PorcReposicion <= FILL-IN-PorcReposicion) AND ~
(RADIO-SET_Empaque = 'Ambos' OR BT-DREPO.Origen = RADIO-SET_Empaque) ~
)
...
&Scoped-Define ENABLED-OBJECTS RECT-29 RECT-30 COMBO-BOX-Marcas ~
COMBO-BOX-Almacen FILL-IN-PorcReposicion RADIO-SET_Empaque COMBO-BOX-Sector ~
FILL-IN-DesMat BUTTON-Limpiar-Orden COMBO-BOX-Lineas COMBO-BOX-SubLineas ~
br_table BUTTON-998 
&Scoped-Define DISPLAYED-OBJECTS COMBO-BOX-Marcas COMBO-BOX-Almacen ~
FILL-IN-PorcReposicion RADIO-SET_Empaque FILL-IN-Inner COMBO-BOX-Sector ~
FILL-IN-DesMat FILL-IN-Master COMBO-BOX-Lineas COMBO-BOX-SubLineas ~
FILL-IN-Reposicion FILL-IN-Total FILL-IN-Peso FILL-IN-Volumen FILL-IN-Stock ~
...
DEFINE VARIABLE RADIO-SET_Empaque AS CHARACTER INITIAL "Ambos" 
     VIEW-AS RADIO-SET VERTICAL
     RADIO-BUTTONS 
          "Ambos", "Ambos",
"Solo empaque Master", "Master",
"Solo Saldos", "Saldo"
     SIZE 20 BY 2.42 NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      T-DREPO.DesMat FORMAT "X(60)":U WIDTH 37.86
      T-DREPO.DesMar COLUMN-LABEL "Marca" FORMAT "x(20)":U WIDTH 13.43
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 138 BY 16.96
...
DEFINE FRAME F-Main
     COMBO-BOX-Marcas AT ROW 1.27 COL 6 COLON-ALIGNED WIDGET-ID 72
     COMBO-BOX-Almacen AT ROW 1.27 COL 41 COLON-ALIGNED WIDGET-ID 80
     FILL-IN-PorcReposicion AT ROW 1.27 COL 72 COLON-ALIGNED WIDGET-ID 136
     RADIO-SET_Empaque AT ROW 1.27 COL 89 NO-LABEL WIDGET-ID 142
     FILL-IN-Inner AT ROW 1.27 COL 120 COLON-ALIGNED WIDGET-ID 70
     COMBO-BOX-Sector AT ROW 2.08 COL 6 COLON-ALIGNED WIDGET-ID 88
     FILL-IN-DesMat AT ROW 2.08 COL 41 COLON-ALIGNED WIDGET-ID 90
     FILL-IN-Master AT ROW 2.08 COL 120 COLON-ALIGNED WIDGET-ID 62
     BUTTON-Limpiar-Orden AT ROW 2.88 COL 8 WIDGET-ID 132
     COMBO-BOX-Lineas AT ROW 2.88 COL 41 COLON-ALIGNED WIDGET-ID 74
     COMBO-BOX-SubLineas AT ROW 2.88 COL 59 COLON-ALIGNED WIDGET-ID 76
     FILL-IN-Reposicion AT ROW 2.88 COL 120 COLON-ALIGNED WIDGET-ID 64
     ...
     "F10: Kardex Almacén Despacho  F11: Faltante/Sobrante" VIEW-AS TEXT
          ...
          FONT 0
     RECT-29 AT ROW 1 COL 1 WIDGET-ID 138
     RECT-30 AT ROW 1 COL 110 WIDGET-ID 140
...
/* BROWSE-TAB br_table FILL-IN-Reposicion F-Main */
ASSIGN 
...
"T-DREPO.DesMat" ? "X(60)" "character" ? ? ? ? ? ? no ? no no "37.86" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.T-DREPO.DesMar
"T-DREPO.DesMar" "Marca" ? "character" ? ? ? ? ? ? no ? no no "13.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
    ...
    IF T-DREPO.CONTROLDespacho = YES THEN DO:
      ...
      ASSIGN
          ...
          .
  END.
  IF T-DREPO.Sector = "*" THEN DO:
      ASSIGN
          T-DREPO.CodMat:BGCOLOR IN BROWSE {&BROWSE-NAME} = 14
          T-DREPO.DesMar:BGCOLOR IN BROWSE {&BROWSE-NAME} = 14
          T-DREPO.DesMat:BGCOLOR IN BROWSE {&BROWSE-NAME} = 14.
      ASSIGN
          T-DREPO.CodMat:FGCOLOR IN BROWSE {&BROWSE-NAME} = 0
          T-DREPO.DesMar:FGCOLOR IN BROWSE {&BROWSE-NAME} = 0
          T-DREPO.DesMat:FGCOLOR IN BROWSE {&BROWSE-NAME} = 0.
  END.
END.
...
&Scoped-define SELF-NAME RADIO-SET_Empaque
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL RADIO-SET_Empaque B-table-Win
ON VALUE-CHANGED OF RADIO-SET_Empaque IN FRAME F-Main
DO:
    ASSIGN {&SELF-NAME}.
    RUN dispatch IN THIS-PROCEDURE ('open-query':U).
END.
...
/*             IF BT-DREPO.Origen = "MAN" THEN s-TipMov = "M". */
            ...
            /* ************************************** */
        END.
        /* 1-9-2023 Códigos Repetidos */
        FIND FIRST Almdrepo WHERE almdrepo.CodCia = almcrepo.codcia AND
            ...
            almdrepo.CodMat = BT-DREPO.codmat
            EXCLUSIVE-LOCK NO-ERROR.
        ...
        ELSE DO:
            ASSIGN
                almdrepo.CanGen = almdrepo.CanGen + BT-DREPO.cangen.
            ASSIGN
                almdrepo.CanReq = almdrepo.cangen
                almdrepo.CanApro = almdrepo.cangen.
        END.
...
IF NOT AVAILABLE B-MATE THEN DO:
    MESSAGE 'Código NO asignado en el almacén' s-CodAlm SKIP
        'Contactar con el Gestor de SKU para completar la configuración de los almacenes'
        VIEW-AS ALERT-BOX ERROR.
...
FIND B-MATE WHERE B-MATE.codcia = s-codcia
    ...
    NO-LOCK NO-ERROR.
IF NOT AVAILABLE B-MATE THEN DO:
    MESSAGE 'Código NO asignado en el almacén' T-DREPO.AlmPed:SCREEN-VALUE IN BROWSE {&browse-name} SKIP
        'Contactar con el Gestor de SKU para completar la configuración de los almacenes'
        VIEW-AS ALERT-BOX ERROR.
    APPLY 'entry':U TO T-DREPO.CodMat IN BROWSE {&browse-name}.
    RETURN 'ADM-ERROR'.
