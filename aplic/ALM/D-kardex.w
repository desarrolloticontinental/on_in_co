...
HastaC DesdeF HastaF nCodMon R-Tipo TOGGLE-CatConta T-Resumen Btn_OK ~
Btn_Cancel BUTTON-1 
...
DEFINE VARIABLE TOGGLE-CatConta AS LOGICAL INITIAL no 
     LABEL "Todas las categorías contables" 
     VIEW-AS TOGGLE-BOX
...
DEFINE FRAME F-Main
     ...
     TOGGLE-CatConta AT ROW 8.27 COL 48 WIDGET-ID 10
     T-Resumen AT ROW 9.12 COL 48
...
------------------------------------------------------------------------------*/
  DISPLAY F-CodFam F-DesFam F-SubFam F-DesSub x-Licencia x-NomLic DesdeC HastaC 
          DesdeF HastaF nCodMon R-Tipo TOGGLE-CatConta x-mensaje 
      WITH FRAME F-Main IN WINDOW W-Win.
  ENABLE RECT-57 F-CodFam F-SubFam x-Licencia DesdeC HastaC DesdeF HastaF 
         nCodMon R-Tipo TOGGLE-CatConta T-Resumen Btn_OK Btn_Cancel BUTTON-1 
      WITH FRAME F-Main IN WINDOW W-Win.
...
/*     chWorkSheet:Range(cRange):Value = "P.Unitario". */
    ...
    FOR EACH Almmmatg NO-LOCK WHERE Almmmatg.CodCia = S-CODCIA 
        ...
        AND (TOGGLE-CatConta = YES OR LOOKUP(TRIM(Almmmatg.CatConta[1]), 'AF,SV,XX') = 0)
...
/*         END.                                                                             */
        ...
        chWorkSheet:Range(cRange):Value = f-valcto.
/*         cRange = "P" + cColumn.                           */
/*         chWorkSheet:Range(cRange):Value = almdmov.preuni. */
...
------------------------------------------------------------------------------*/
  ...
  DEFINE FRAME F-REPORTE
      ...
      Almdmov.NroSer COLUMN-LABEL "Serie!Interno"
      Almdmov.NroDoc COLUMN-LABEL "Numero!Interno"
  ...
  DEFINE FRAME F-HEADER
      ...
      "Alm Mov Interno       Documento Ref  G/R         FAC          Ori/Des Proveedor   Documento    INGRESOS    SALIDAS   CTO.INGRE. CTO.PROMEDIO       SALDO         TOTAL " SKIP
      "-----------------------------------------------------------------------------------------------------------------------------------------------------------------------" SKIP
/*     123 123 1234 123456789 123456789012 123456789012 123456789012 1234567 12345678901 99/99/9999 >>>>>>9.99 >>>>>>9.99 >>>>>>9.9999 (>>>>9.9999) (>>>>>>9.99) (>>>>>>>9.99)
*/
  ...
  FOR EACH Almmmatg NO-LOCK WHERE Almmmatg.CodCia = S-CODCIA 
          ...
          AND (TOGGLE-CatConta = YES OR LOOKUP(TRIM(Almmmatg.CatConta[1]), 'AF,SV,XX') = 0)
          AND (TRUE <> (x-licencia > '') OR Almmmatg.Licencia[1] = x-Licencia),
      EACH Almdmov NO-LOCK USE-INDEX ALMD02 WHERE Almdmov.CodCia = Almmmatg.CodCia 
          AND Almdmov.codmat = Almmmatg.CodMat 
          AND Almdmov.FchDoc >= DesdeF
          AND Almdmov.FchDoc <= HastaF,
      ...
      FIND Almcmov WHERE Almcmov.CodCia = Almdmov.codcia 
                    ...
                    AND  Almcmov.TipMov = Almdmov.tipmov 
                    AND  Almcmov.CodMov = Almdmov.codmov 
                    AND  Almcmov.NroSer = Almdmov.NroSer
                    AND  Almcmov.NroDoc = Almdmov.nrodoc 
                   NO-LOCK NO-ERROR.
...
------------------------------------------------------------------------------*/
DO WITH FRAME {&FRAME-NAME}:
    ENABLE ALL EXCEPT F-DesFam F-DesSub x-NomLic x-mensaje TOGGLE-CatConta.
END.
...
------------------------------------------------------------------------------*/
  ...
  IF TRUE <> (DesdeC > '') THEN DO:
      ...
      DesdeC = Almmmatg.CodMat.
  END.
  IF TRUE <> (HastaC > '') THEN DO:
      FIND LAST Almmmatg WHERE Almmmatg.codcia = s-codcia NO-LOCK NO-ERROR.
      HastaC = Almmmatg.CodMat.
  END.
/*   FOR EACH Almmmatg NO-LOCK WHERE Almmmatg.CodCia = S-CODCIA                                */
/*           AND (TRUE <> (DesdeC > '') OR Almmmatg.CodMat >= DesdeC)                          */
/*           AND (TRUE <> (HastaC > '') OR Almmmatg.CodMat <= HastaC)                          */
...
/*           AND (TRUE <> (x-licencia > '') OR Almmmatg.Licencia[1] = x-Licencia),             */
  FOR EACH Almmmatg NO-LOCK WHERE Almmmatg.CodCia = S-CODCIA 
      ...
      EACH Almdmov NO-LOCK USE-INDEX ALMD02 WHERE Almdmov.CodCia = Almmmatg.CodCia 
          ...
          AND Almdmov.FchDoc >= DesdeF
          AND Almdmov.FchDoc <= HastaF,
      FIRST Almcmov NO-LOCK WHERE Almcmov.CodCia = Almdmov.codcia
          AND Almcmov.CodAlm = Almdmov.codalm
          AND Almcmov.TipMov = Almdmov.tipmov
          AND Almcmov.CodMov = Almdmov.codmov
          AND Almcmov.NroSer = Almdmov.nroser
      ...
      F-TotIng  = 0.
      IF nCodmon = x-Codmon THEN DO:
         IF Almdmov.Tipmov = 'I' OR Almdmov.TipMov = 'S' THEN DO:
            F-PreIng  = Almdmov.PreUni / Almdmov.Factor.
            F-TotIng  = Almdmov.ImpCto.
         END.
         ELSE DO:
      ...
      F-Saldo   = Almdmov.StkAct.
      F-VALCTO = Almdmov.StkAct * Almdmov.VctoMn1.
      F-VALCTO = (IF F-VALCTO = ? THEN 0 ELSE F-VALCTO).
      F-PRECIO = Almdmov.VctoMn1.
      ACCUMULATE F-Ingreso (TOTAL BY Almmmatg.CodMat).
      ACCUMULATE F-Salida  (TOTAL BY Almmmatg.CodMat).
      PUT STREAM REPORT UNFORMATTED
          ...
          Almdmov.CodAlm  "|"
          S-CODMOV "|"
          STRING(Almdmov.NroDoc,"999999999") "|".
      ...
      ELSE  IF Almdmov.TipMov = "S" AND ALmtmovm.MOvCmp = YES THEN
          PUT STREAM REPORT UNFORMATTED F-PreIng "|".
      ELSE PUT STREAM REPORT UNFORMATTED "|".
      PUT STREAM REPORT UNFORMATTED
          F-PRECIO "|"
          ...
          F-VALCTO "|".
      CASE TRUE:
          WHEN Almcmov.TipMov = "S" AND Almcmov.CodMov = 02 THEN DO:
              FIND Ccbcdocu WHERE Ccbcdocu.codcia = s-codcia AND
                  Ccbcdocu.coddoc = Almcmov.codref AND
                  Ccbcdocu.nrodoc = Almcmov.nroref NO-LOCK NO-ERROR.
              IF AVAILABLE Ccbcdocu 
                  THEN PUT STREAM REPORT Ccbcdocu.divori '|'.
              ELSE PUT STREAM REPORT '|'.
          END.
          WHEN Almcmov.TipMov = "I" AND Almcmov.CodMov = 09 THEN DO:
              FIND Ccbcdocu WHERE Ccbcdocu.codcia = s-codcia AND
                  Ccbcdocu.coddoc = Almcmov.codref AND
                  Ccbcdocu.nrodoc = Almcmov.nroref NO-LOCK NO-ERROR.
              IF AVAILABLE Ccbcdocu 
