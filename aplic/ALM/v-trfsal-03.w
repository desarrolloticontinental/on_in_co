...
DO:
  ...
  DEFINE VAR cDivOri AS CHAR.
  DEFINE VAR cDivDes AS CHAR.

  FIND Almacen WHERE Almacen.CodCia = s-CodCia AND
       Almacen.CodAlm = s-codalm  NO-LOCK NO-ERROR.
  IF NOT AVAILABLE Almacen THEN DO:
     MESSAGE "Almacen Origen no existe" VIEW-AS ALERT-BOX INFORMATION.
     RETURN NO-APPLY.
  END.
  cDivOri = almacen.coddiv.
  ...
  IF cDivDes <> cDivOri THEN DO:
      MESSAGE "Los almacenes deben pertenecer a la misma division" VIEW-AS ALERT-BOX INFORMATION.
      RETURN NO-APPLY.
  END.
...
/*
  IF Almcmov.NroSer = 0 THEN RUN Formato.
  ELSE IF AlmCmov.FlgEst <> "A" THEN DO:
      /*RUN ALM\R-ImpGui (ROWID(Almcmov)).*/
...
DO WITH FRAME {&FRAME-NAME} :
   ...
   IF Almcmov.AlmDes:SCREEN-VALUE = "" THEN DO:
         MESSAGE "No Ingreso el Almacen Destino" VIEW-AS ALERT-BOX INFORMATION.
   ...
   IF Almacen.Campo-C[1] = "XD" THEN DO:
      MESSAGE "Almacén Destino NO puede ser de Cross Docking" VIEW-AS ALERT-BOX INFORMATION.
      APPLY "ENTRY" TO Almcmov.AlmDes.
      RETURN "ADM-ERROR".   
   END.
   IF Almcmov.AlmDes:SCREEN-VALUE = Almtdocm.CodAlm THEN DO:
         MESSAGE "Almacen no puede transferirse a si mismo" VIEW-AS ALERT-BOX INFORMATION.
         APPLY "ENTRY" TO Almcmov.AlmDes.
         RETURN "ADM-ERROR".   
   END.
...
/*                x-es-diferente-division x-es-trans-interna.   */
   ...
   IF I-NRO = 0 THEN DO:
      MESSAGE "No existen articulos a transferir" VIEW-AS ALERT-BOX INFORMATION.
