...
DEFINE VARIABLE TOGGLE-nuevoformato AS LOGICAL INITIAL yes 
...
"T-REPORT.Campo-C[2]" "EAN 13" "X(15)" "character" ? ? ? ? ? ? no ? no no "13" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > Temp-Tables.T-REPORT.Campo-C[3]
"T-REPORT.Campo-C[3]" "Descripción" "X(80)" "character" ? ? ? ? ? ? no ? no no "67.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
------------------------------------------------------------------------------*/
    ...
    IF gn-divi.CanalVenta = "MIN" THEN DO:
        ...
        IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
            MESSAGE pMensaje VIEW-AS ALERT-BOX INFO BUTTONS OK.
            RETURN.
        END.
        dPreUni = f-PreVta.
        ...
        IF toggle-nuevoformato = YES THEN DO:
            ...
            FOR EACH VtaDctoPromMin NO-LOCK WHERE VtaDctoPromMin.CodCia = s-CodCia AND 
                ...
                dDstoVo1 = VtaDctoPromMin.Descuento. 
                dDstoVo1 = MAXIMUM(dDstoVo1, x-Old-Descuento). 
                x-Old-Descuento = dDstoVo1.
            END. 
            IF dDstoVo1 > 0 THEN DO:
                /* Impresión Precio Base SIN Descuentos */
                x-dsctopromo = "Antes: S/ " + STRING(f-PreVta,'>>>9.99').
                dPreUni  = ROUND(dPreUni * ( 1 - ( DECI(dDstoVo1) / 100 ) ),4). /* Aplicado el Descuento */
            END.
        END.
        /* Ic - 06Jun2019 */
        ...
        IF AVAILABLE Vtalistamingn AND vtalistaminGn.DtoVolR[1] > 0 THEN DO:
            cNotita = '*Prom.solo para pago con efectivo.'.
            dDstoVo1 = ROUND(f-PreVta * ( 1 - ( vtalistaminGn.DtoVolD[1]) / 100  ),4).
            dPrecio = dDstoVo1.
            x-DtoVol  = 'x ' + STRING(vtalistaminGn.DtoVolR[1],'>>9') + ' ' + Almmmatg.undbas + '= S/' + STRING(dPrecio,">>9.99") + ' c/u'. 
        END.
        IF AVAILABLE Vtalistamingn AND vtalistaminGn.DtoVolR[2] > 0 THEN DO:
            dDstoVo2 = ROUND(f-PreVta * ( 1 - ( DECI(vtalistaminGn.DtoVolD[2]) / 100 ) ),4).
            dPrecio = dDstoVo2.
...
/*
    ...
    IF gn-divi.CanalVenta = "MIN" THEN DO:
        ...
        IF toggle-nuevoformato = YES THEN DO:
            ...
            FOR EACH VtaDctoPromMin NO-LOCK WHERE VtaDctoPromMin.CodCia = s-CodCia AND 
                VtaDctoPromMin.CodDiv = s-CodDiv AND 
                VtaDctoPromMin.CodMat = VtaListaMinGn.CodMat AND 
                (TODAY >= VtaDctoPromMin.FchIni AND TODAY <= VtaDctoPromMin.FchFin) AND 
                VtaDctoPromMin.flgest = 'A': 
                /* Solo en caso de EVENTOS existe el VIP, MR */ 
                dDstoVo1 = VtaDctoPromMin.Descuento. 
...
------------------------------------------------------------------------------*/
    ...
    ELSE DO:
        ...
        x-file-zpl = SESSION:TEMP-DIRECTORY + "PrecioUtilex.txt".
...
x-codmat2 = T-REPORT.Campo-C[1]:SCREEN-VALUE IN BROWSE {&browse-name}.

IF TRUE <> (x-codmat2 > "") THEN DO:
    RETURN 'ADM-ERROR'.
END.
...
IF NOT AVAILABLE Almmmatg THEN DO:        
    BELL.
    ASSIGN T-REPORT.Campo-C[1]:SCREEN-VALUE IN BROWSE {&browse-name} = ''.
