...
&Scoped-define QUERY-STRING-br_table FOR EACH T-MoviAlmacen WHERE ~{&KEY-PHRASE} NO-LOCK, ~
  ...
  AND OOMoviAlmacen.CodCia = T-MoviAlmacen.CodCia ~
...
/* Query rebuild information for BROWSE br_table
  ...
  AND INTEGRAL.OOMoviAlmacen.CodCia = Temp-Tables.T-MoviAlmacen.CodCia
  AND INTEGRAL.OOMoviAlmacen.CodAlm = Temp-Tables.T-MoviAlmacen.CodAlm"
...
CICLO:
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    /* Verificamos si ya ha sido grabado */
    FIND FIRST Almcmov  WHERE Almcmov.codcia = t-MoviAlmacen.CodCia
        AND Almcmov.codalm =  t-MoviAlmacen.CodAlm
        AND Almcmov.tipmov =  t-MoviAlmacen.TipMov
        AND Almcmov.codmov =  t-MoviAlmacen.CodMov
        AND Almcmov.nroser =  t-MoviAlmacen.NroSer
        AND Almcmov.nrodoc =  t-MoviAlmacen.NroDoc
        NO-LOCK NO-ERROR NO-WAIT.
    IF AVAILABLE Almcmov THEN DO:
        pMensaje = "ERROR: Cabecera ya registrada (almacén " + t-MoviAlmacen.CodAlm + " mov " +
            t-MoviAlmacen.TipMov + " " + STRING(t-MoviAlmacen.CodMov,'99') + " número " +
            STRING(t-MoviAlmacen.NroSer,'999') + STRING(t-MoviAlmacen.NroDoc) + ")" + CHR(10) + CHR(10) +
            'Proceso abortado'.
        UNDO, RETURN 'ADM-ERROR'.
    END.
    CREATE Almcmov.
    BUFFER-COPY t-MoviAlmacen 
        TO Almcmov
        ASSIGN 
    ...
    DETALLE:
    REPEAT:
        FIND FIRST b-ooMoviAlmacen WHERE b-ooMoviAlmacen.CodCia = t-MoviAlmacen.CodCia
            AND b-ooMoviAlmacen.CodAlm = t-MoviAlmacen.CodAlm
            AND b-ooMoviAlmacen.TipMov = t-MoviAlmacen.TipMov
            AND b-ooMoviAlmacen.CodMov = t-MoviAlmacen.CodMov
            AND b-ooMoviAlmacen.NroSer = t-MoviAlmacen.NroSer
            AND b-ooMoviAlmacen.NroDoc = t-MoviAlmacen.NroDoc 
        ...
        IF ERROR-STATUS:ERROR = YES THEN DO:
            ...
            UNDO CICLO, RETURN 'ADM-ERROR'.
        END.
        FIND FIRST Almdmov OF Almcmov WHERE Almdmov.CodMat = b-ooMoviAlmacen.CodMat NO-LOCK NO-ERROR NO-WAIT.
        IF NOT AVAILABLE Almdmov THEN DO:
            ...
            BUFFER-COPY Almcmov TO Almdmov
                ...
                NO-ERROR.
            IF ERROR-STATUS:ERROR = YES THEN DO:
                pMensaje = "ERROR: Detalle ya registrado (almacén " + t-MoviAlmacen.CodAlm + " mov " +
                    t-MoviAlmacen.TipMov + STRING(t-MoviAlmacen.CodMov) + " número " + STRING(t-MoviAlmacen.NroSer, '999') + 
                    STRING(t-MoviAlmacen.NroDoc, '999999999') + " producto " + b-ooMoviAlmacen.CodMat + ")" + CHR(10) +
                    'Proceso Abortado'.
                UNDO CICLO, RETURN 'ADM-ERROR'.
            END.
        END.
        ...
        x-Rowid = ROWID(Almdmov).
        RUN alm/ALMACSTK (x-Rowid).     /* Esta rutina asigna automáticamente el artículo en el almacén */
        IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
            pMensaje = "ERROR: Stock (almacén " + t-MoviAlmacen.CodAlm + " mov " + 
                t-MoviAlmacen.TipMov + STRING(t-MoviAlmacen.CodMov) + " número " + STRING(t-MoviAlmacen.NroSer, '999') +
                STRING(t-MoviAlmacen.NroDoc, '999999999') + " producto " + b-ooMoviAlmacen.CodMat + ")" + CHR(10) +
                'Proceso Abortado'.
            UNDO CICLO, RETURN 'ADM-ERROR'.
        ...
        RUN alm/almacpr1 (x-Rowid, 'U').
        IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
            pMensaje = "ERROR:Kardex (almacen " + t-MoviAlmacen.CodAlm + " mov " +
...
RLOOP:
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    FIND CURRENT ooMoviAlmacen NO-LOCK NO-ERROR NO-WAIT.
    IF NOT({&Condicion}) THEN DO:
        pMensaje = 'El registro ha sido modificado por otro usuario' + CHR(10) +
            'Proceso abortado'.
        UNDO RLOOP, RETURN 'ADM-ERROR'.
