...
DEFINE VAR debugON AS LOG.
DEFINE VAR cArticuloDebug AS CHAR.

debugON = NO.
cArticuloDebug = "400763".
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Cantidad-Reponer-A W-Win 
PROCEDURE Carga-Cantidad-Reponer-A :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
IF debugON THEN DO:
    FIND FIRST T-MATE WHERE t-matg.codmat = cArticuloDebug NO-LOCK NO-ERROR.
    IF AVAILABLE t-matg THEN DO:
        MESSAGE "Si esta (Carga-Cantidad-Reponer-A)" cArticuloDebug.
    END.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Cantidad-Reponer-B W-Win 
PROCEDURE Carga-Cantidad-Reponer-B :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
FOR EACH T-DREPO BY T-DREPO.CodMat:
    ...
    x-Item = x-Item + 1.
END.
IF AVAILABLE(T-DREPO) THEN RELEASE T-DREPO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Carga-Productos W-Win 
...
IF debugON THEN DO:
    FIND FIRST T-MATG WHERE t-matg.codmat = cArticuloDebug NO-LOCK NO-ERROR.
    IF AVAILABLE t-matg THEN DO:
        MESSAGE "Si esta (Carga-Productos)" cArticuloDebug.
    END.
END.
...
/*
30Oct2024 : con el nuevo calculo de clasificaciones, quedo sin efecto
...
/* Tabla: T-MATE ********************************************************* */
/* *********************************************************************** */
RUN Carga-Cantidad-Reponer-A.

/* ******************************************************************************** */
/* CARGAMOS LA CANTIDAD A REPONER ************************************************* */
/* ******************************************************************************** */
RUN Carga-Cantidad-Reponer-B.
...
FOR EACH T-DREPO-2 NO-LOCK, FIRST Almmmatg OF T-DREPO-2 NO-LOCK:
...
IF AVAILABLE(T-DREPO) THEN RELEASE T-DREPO.
IF AVAILABLE(T-DREPO-2) THEN RELEASE T-DREPO-2.
