...
DEF TEMP-TABLE Detalle
    ...
    FIELD CanxMes   AS DEC  LABEL "Cantidad"    FORMAT '->>>,>>>,>>>,>>9.99' INIT 0
    FIELD VtaxMesMe AS DEC  LABEL "Ventas US$"  FORMAT '->>>,>>>,>>>,>>9.99' INIT 0
    FIELD VtaxMesMn AS DEC  LABEL "Ventas S/."  FORMAT '->>>,>>>,>>>,>>9.99' INIT 0
    FIELD CtoxMesMe AS DEC  LABEL "Costo US$"   FORMAT '->>>,>>>,>>>,>>9.99' INIT 0
    FIELD CtoxMesMn AS DEC  LABEL "Costo S/."   FORMAT '->>>,>>>,>>>,>>9.99' INIT 0
    FIELD ProxMesMe AS DEC  LABEL "Promedio US$"    FORMAT '->>>,>>>,>>>,>>9.99' INIT 0
    FIELD proxMesMn AS DEC  LABEL "Promedio S/."    FORMAT '->>>,>>>,>>>,>>9.99' INIT 0
    FIELD VtaxMesMn_ant AS DEC  LABEL "Ventas Ant S/."  FORMAT '->>>,>>>,>>>,>>9.99' INIT 0
    FIELD CtoxMesMn_ant AS DEC  LABEL "Costo Ant S/."   FORMAT '->>>,>>>,>>>,>>9.99' INIT 0
    FIELD VtaxMesMn_act AS DEC  LABEL "Ventas S/."  FORMAT '->>>,>>>,>>>,>>9.99' INIT 0
    FIELD CtoxMesMn_act AS DEC  LABEL "Costo S/."   FORMAT '->>>,>>>,>>>,>>9.99' INIT 0
    FIELD margen_ant AS DEC  LABEL "Ganancia Ant S/."   FORMAT '->>>,>>>,>>>,>>9.99' INIT 0
    FIELD margen_act AS DEC  LABEL "Ganancia S/."   FORMAT '->>>,>>>,>>>,>>9.99' INIT 0
    FIELD fcrecimiento AS DEC  LABEL "Crecimiento S/."   FORMAT '->>>,>>9.9999' INIT 0
    FIELD fventas AS DEC  LABEL "Factor Vtas"   FORMAT '->>>,>>9.999999' INIT 0
    FIELD fmargen AS DEC  LABEL "Factor Margen"   FORMAT '->>>,>>9.999999' INIT 0
    FIELD fcreci AS DEC  LABEL "Factor Crecimiento"   FORMAT '->>>,>>9.999999' INIT 0
    FIELD fventas1 AS DEC  LABEL "Factor Vtas"   FORMAT '->>>,>>9.999999' INIT 0
    FIELD fmargen1 AS DEC  LABEL "Factor Margen"   FORMAT '->>>,>>9.999999' INIT 0
    FIELD fcreci1 AS DEC  LABEL "Factor Crecimiento"   FORMAT '->>>,>>9.999999' INIT 0
    FIELD sumafactor AS DEC  LABEL "Suma de factores de Crecimiento"   FORMAT '->>>,>>9.999999' INIT 0
    FIELD clasificacion AS CHAR FORMAT 'X(2)' 
    FIELD iranking AS INT  LABEL "Ranking"   FORMAT '->>>,>>9.999999' INIT 0
...
DEFINE VAR lCuantosAnnos AS INT .
...
DEFINE TEMP-TABLE tventas_detalle LIKE ventas_detalle.
...
ChkBx-todos-menos-inst optGrpCampana chk_saveranking Chkbx-Institucionales 
...
DEFINE VARIABLE ChkBx-General AS LOGICAL INITIAL no 
     LABEL "General (1)" 
     VIEW-AS TOGGLE-BOX
     SIZE 13.86 BY .77 NO-UNDO.

DEFINE VARIABLE Chkbx-Institucionales AS LOGICAL INITIAL no 
     LABEL "Solo Institucionales (4)" 
     VIEW-AS TOGGLE-BOX
     SIZE 27 BY .77 NO-UNDO.

DEFINE VARIABLE ChkBx-todos-menos-inst AS LOGICAL INITIAL no 
     LABEL "General - Menos Utilex e Institucionales (3)" 
     VIEW-AS TOGGLE-BOX
     SIZE 39.86 BY .77 NO-UNDO.

DEFINE VARIABLE Chkbx-Utilex-Inst AS LOGICAL INITIAL no 
     LABEL "Solo Utilex  + Institucionales(2)" 
     VIEW-AS TOGGLE-BOX
     SIZE 27 BY .77 NO-UNDO.
...
DEFINE FRAME fMain
     ...
     Chkbx-Utilex-Inst AT ROW 17.69 COL 7.14 WIDGET-ID 56
     ChkBx-todos-menos-inst AT ROW 18.5 COL 7.14 WIDGET-ID 58
     ...
     Chkbx-Institucionales AT ROW 19.27 COL 7.14 WIDGET-ID 72
     ...
     "v3.0" VIEW-AS TEXT
          SIZE 5.43 BY .62 AT ROW 5.81 COL 83 WIDGET-ID 70
          FGCOLOR 12 
...
/* SETTINGS FOR BUTTON btnRecalcularQtyArt IN FRAME fMain
   NO-ENABLE                                                            */
/* SETTINGS FOR TOGGLE-BOX Chkbx-Institucionales IN FRAME fMain
   NO-ENABLE                                                            */
...
END.
    ...
    IF ((lYear MODULO 4) = 0 AND (lYear MODULO 100) <> 0) OR (lyear MODULO 400)=0 THEN DO:
        /* Año bisiesto */
        dDesde_ant = dDesde - ( 366 * lCuantosAnnos ).
    END.
    ELSE DO:
        dDesde_ant = dDesde - ( 365 * lCuantosAnnos ).
    END.

    lYear = YEAR(dHasta).
    IF ((lYear MODULO 4) = 0 AND (lYear MODULO 100) <> 0) OR (lyear MODULO 400)=0 THEN DO:
        /* Año bisiesto */
        dHasta_ant = dHasta - ( 366 * lCuantosAnnos ).
    END.
    ELSE DO:
        dHasta_ant = dHasta - ( 365 * lCuantosAnnos ).
    END.
    ...
    IF ChkBx-General = YES THEN DO:
        ...
        RUN ue-procesar.
    END.
    /* Clasificacion Utilex */
    IF ChkBx-Utilex-Inst = YES THEN DO:
        ASSIGN lGrupo = 2.
        lFileTxt = lFileTxt + "-utilex".
        RUN ue-procesar.
    END.
    /* Clasificacion General - Utilex e institucionales */
    IF ChkBx-Todos-menos-inst = YES THEN DO:
        ASSIGN lGrupo = 3.
        lFileTxt = lFileTxt + "-mayorista".
        RUN ue-procesar.
    END.
    /* Clasificacion institucionales */
    IF ChkBx-Institucionales = YES THEN DO:
        ASSIGN lGrupo = 4.
        lFileTxt = lFileTxt + "-institucionales".
        RUN ue-procesar.
    END.
...
------------------------------------------------------------------------------*/
  DISPLAY txt-msg pVenta pMargen pCreci pTotal pFactor qValor_A qValor_B 
          ...
          ChkBx-todos-menos-inst optGrpCampana chk_saveranking 
          Chkbx-Institucionales 
...
x-ruta = session:temp-directory.
X-archivo = x-ruta + "data-paretro-resto.txt".
...
IF p-especial = NO THEN DO:
    /*X-archivo = "d:\xpciman\"+ lFileTxt + "-standard.txt".*/
    X-archivo = x-ruta + lFileTxt + "-standard.txt".
END.
ELSE DO:
    /*X-archivo = "d:\xpciman\" + lFileTxt + "-especial.txt".*/
    X-archivo = x-ruta + lFileTxt + "-especial.txt".
END.
...
IF p-especial = YES THEN DO:
    ...
    /* ESPECIAL : Calcula los valores A,B,C,D,E,F */
...
IF p-especial = NO THEN DO:    
    FOR EACH detalle :
        ...
        IF (lConteo MODULO 1000) = 0 THEN DO:
...
/*
IF p-espcial = NO THEN DO:
    chWorkSheet:SaveAs("d:\xpciman\data-paretro-standard.xls").
END.
ELSE DO:
    chWorkSheet:SaveAs("d:\xpciman\data-paretro-especial.xls").
END.
*/
...
EMPTY TEMP-TABLE tclf_calculo_art.
EMPTY TEMP-TABLE tventas_detalle.

lConteo = 0.
txt-msg:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "Transacciones, Van...(" + STRING(lConteo,">>>,>>>,>>9") + 
            " " + IF(p-especial = YES) THEN "Especial" ELSE "Standard" + ") ".

FOR EACH ventas_detalle WHERE ventas_detalle.datekey >= p-Desde AND ventas_detalle.datekey <= p-Hasta NO-LOCK:
    IF ((lGrupo = 2 OR lGrupo = 4) AND 
                            LOOKUP(ventas_detalle.coddiv,lDivisiones) > 0 ) OR
                            ( (lGrupo = 1 OR lGrupo = 3) AND 
                            ( LOOKUP(ventas_detalle.coddiv,lDivisiones) = 0 )) THEN DO:
        /**/
    ...
    IF NOT AVAILABLE tclf_calculo_art THEN DO:
        ...
        IF NOT AVAILABLE almmmatg THEN DO:
            ASSIGN tclf_calculo_art.sipasa = 'N'.
            NEXT.
        END.
        IF almmmatg.tpoart <> 'A' OR LOOKUP(almmmatg.codfam, lFamiExo ) > 0 THEN DO:
            ASSIGN tclf_calculo_art.sipasa = 'N'.
            NEXT.
        END.            
        IF p-especial = YES THEN DO:
            FIND FIRST art-esp WHERE art-esp.codmat = cCodMat NO-ERROR.
            IF NOT AVAILABLE art-esp THEN DO:
                ASSIGN tclf_calculo_art.sipasa = 'N'.
                NEXT.
            END.
            IF NOT (art-esp.fching > dHasta) THEN DO:
                ASSIGN tclf_calculo_art.sipasa = 'N'.
                NEXT.
            END.
        END.
    ...
    END.
    CREATE tventas_detalle.
    BUFFER-COPY ventas_Detalle TO tventas_detalle.
END.
...
FOR EACH tventas_detalle NO-LOCK:
    cCodMat = tventas_detalle.codmat.
    lConteo = lConteo + 1.
    ...
    IF p-especial = NO THEN DO:
        /* Standard */        
        FIND FIRST Detalle WHERE detalle.codmat = cCodMat USE-INDEX IdxKey01 NO-ERROR.    
        IF NOT AVAILABLE Detalle THEN DO:
            FIND FIRST dimproducto WHERE dimproducto.codmat = tventas_detalle.codmat NO-LOCK NO-ERROR.
            FIND FIRST dimlinea WHERE dimlinea.codfam = tventas_detalle.codfam NO-LOCK NO-ERROR.
            FIND FIRST dimsublinea WHERE dimsublinea.codfam = tventas_detalle.codfam AND dimsublinea.subfam = tventas_detalle.subfam NO-LOCK NO-ERROR.

            ASSIGN                 
                cProducto = IF (AVAILABLE DimProducto) THEN DimProducto.desmat ELSE "ERR"
                cLinea = tventas_detalle.codfam + ' ' + IF (AVAILABLE DimLinea) THEN DimLinea.NomFam ELSE "ERR"
                cSublinea = tventas_detalle.subfam + ' ' + IF (AVAILABLE DimSubLinea) THEN DimSubLinea.NomSubFam ELSE "ERR"
                cMarca = IF (AVAILABLE DimProducto) THEN DimProducto.desmar ELSE "ERR"
                cUnidad = IF (AVAILABLE DimProducto) THEN DimProducto.undstk ELSE "ERR"
                cLicencia = IF (AVAILABLE DimProducto) THEN DimProducto.licencia ELSE "ERR".
            ...
            */
            ...
            ASSIGN detalle.codmat = cCodmat
                ...
                detalle.tpoart = "A"    /*IF (AVAILABLE almmmatg) THEN almmmatg.tpoart ELSE "X"*/.
        ...
        IF p-Llave = 'ACTUAL' THEN DO:
            Detalle.VtaxMesMn_act = detalle.VtaxMesMn_act + tventas_detalle.ImpNacCIGV.
            Detalle.CtoxMesMn_act = detalle.CtoxMesMn_act + tventas_detalle.CostoNacCIGV.
        END.
        ELSE DO:
            Detalle.VtaxMesMn_ant = Detalle.VtaxMesMn_ant + tventas_detalle.ImpNacCIGV.
            Detalle.CtoxMesMn_ant = Detalle.CtoxMesMn_ant + tventas_detalle.CostoNacCIGV.
        END.
        /* Lo elimino */
    ...
    ELSE DO:
        ...
        IF NOT AVAILABLE detalle THEN DO:        
            ...
            FIND FIRST art-esp WHERE art-esp.codmat = cCodMat AND art-esp.fching > dHasta NO-ERROR.
            IF AVAILABLE art-esp THEN DO:
                FIND FIRST Detalle-esp WHERE detalle-esp.codmat = cCodMat USE-INDEX IdxKey01 NO-ERROR.
                ...
                IF p-Llave = 'ACTUAL' THEN DO:
                    Detalle-esp.VtaxMesMn_act = Detalle-esp.VtaxMesMn_act + tventas_detalle.ImpNacCIGV.
                    Detalle-esp.CtoxMesMn_act = Detalle-esp.CtoxMesMn_act + tventas_detalle.CostoNacCIGV.
                END.
                ELSE DO:
                    Detalle-esp.VtaxMesMn_ant = Detalle-esp.VtaxMesMn_ant + tventas_detalle.ImpNacCIGV.
                    Detalle-esp.CtoxMesMn_ant = Detalle-esp.CtoxMesMn_ant + tventas_detalle.CostoNacCIGV.
                END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE proc_movimientos_data wWin 
PROCEDURE proc_movimientos_data :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE proc_movimientos_old wWin 
PROCEDURE proc_movimientos_old :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
FOR EACH dimDivision WHERE ((lGrupo = 2 OR lGrupo = 4) AND 
                            LOOKUP(dimDivision.coddiv,lDivisiones) > 0 ) OR
                            ( (lGrupo = 1 OR lGrupo = 3) AND 
                            ( LOOKUP(dimDivision.coddiv,lDivisiones) = 0 )) NO-LOCK:

    FOR EACH VentasxProducto USE-INDEX index00 WHERE (VentasxProducto.Datekey >= p-Desde AND 
                                            ventasxproducto.datekey <= p-Hasta) AND
                                            ventasxproducto.coddiv = dimDivision.coddiv NO-LOCK,
    ...
    FOR EACH VentasxProducto NO-LOCK WHERE VentasxProducto.Datekey >= p-Desde AND 
        ventasxproducto.datekey <= p-Hasta AND 
        (((lGrupo = 2 OR lGrupo = 4) AND lookup(ventasxproducto.coddiv,lDivisiones) > 0) OR      /* Utilex o Institucionales  */
         (lGrupo = 1 OR lGrupo = 3) and (lookup(ventasxproducto.coddiv,lDivisiones) = 0)) ,                                    /* Todos o Mayoristas */
        FIRST dimProducto OF VentasxProducto NO-LOCK , 
        FIRST DimLinea OF DimProducto NO-LOCK WHERE LOOKUP (DimProducto.codfam, lFamiExo ) = 0,
        FIRST DimSubLinea OF DimProducto NO-LOCK:
   */ 
        ...
        lConteo = lConteo + 1.
        IF (lConteo MODULO 1500) = 0 THEN DO:
            txt-msg:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "Van...( " + STRING(lConteo,">>>,>>>,>>9") + 
               " " + IF(p-especial = YES) THEN "Especial" ELSE "Standard" + ") " NO-ERROR.
        END.
...
IF ((lYear MODULO 4) = 0 AND (lYear MODULO 100) <> 0) OR (lyear MODULO 400)=0 THEN DO:
    /* Año bisiesto */
    lxDesde_ant = lxDesde - ( 366 * lCuantosAnnos).
END.
ELSE DO:
    lxDesde_ant = lxDesde - ( 365 * lCuantosAnnos).
END.
...
IF ((lYear MODULO 4) = 0 AND (lYear MODULO 100) <> 0) OR (lyear MODULO 400)=0 THEN DO:
    /* Año bisiesto */
    lxHasta_ant = lxHasta - ( 366 * lCuantosAnnos).
END.
ELSE DO:
    lxHasta_ant = lxHasta - ( 365 * lCuantosAnnos) .
END.
...
FOR EACH art-esp :
    ...
    ELSE DO:
        IF art-esp.fching <= dDesde THEN DO:
            ...
            ASSIGN art-esp.clasificacion = 'XF'.
            */
            ASSIGN art-esp.clasificacion = 'F'.
        END.
        ELSE DO:
            /*------------------*/
            FIND Detalle WHERE detalle.codmat = cCodMat USE-INDEX IdxKey01 NO-ERROR.   
            IF NOT AVAILABLE detalle THEN DO:        
                ...
                IF NOT AVAILABLE Detalle-esp THEN DO:
                    ...
                    FIND FIRST dimproducto WHERE dimproducto.codmat = almmmatg.codmat NO-LOCK NO-ERROR.
                    FIND FIRST dimlinea WHERE dimlinea.codfam = almmmatg.codfam NO-LOCK NO-ERROR.
                    FIND FIRST dimsublinea WHERE dimsublinea.codfam = almmmatg.codfam AND dimsublinea.subfam = almmmatg.subfam NO-LOCK NO-ERROR.
    
                    ASSIGN                 
                        cProducto = IF (AVAILABLE DimProducto) THEN DimProducto.desmat ELSE "ERR"
                        cLinea = almmmatg.codfam + ' ' + IF (AVAILABLE DimLinea) THEN DimLinea.NomFam ELSE "ERR"
                        cSublinea = almmmatg.subfam + ' ' + IF (AVAILABLE DimSubLinea) THEN DimSubLinea.NomSubFam ELSE "ERR"
                        cMarca = IF (AVAILABLE DimProducto) THEN DimProducto.desmar ELSE "ERR"
                        cUnidad = IF (AVAILABLE DimProducto) THEN DimProducto.undstk ELSE "ERR"
                        cLicencia = IF (AVAILABLE DimProducto) THEN DimProducto.licencia ELSE "ERR".
...
IF p-especial = NO THEN DO:
    ...
    FOR EACH almmmatg WHERE almmmatg.tpoart = 'A' AND LOOKUP (almmmatg.codfam, lFamiExo ) = 0 NO-LOCK:
        ...
        */
        ASSIGN factabla.campo-c[lGrupo + lCampana] = IF (almmmatg.fching <= dHasta) THEN "F" ELSE ""
...
/* Todos */
IF lGrupo = 1 THEN lDivisiones = "*AAA*,*BBB*".


/* Utilex */
/* Considerar SOLO UTILEX */
/*IF lGrupo = 2 THEN lDivisiones = "00023,00027,00501,00502,00503,00504,00506,00507,00508,00511".*/

/* Considerar UTILEX + INSTITUCIONALES */
IF lGrupo = 2 THEN lDivisiones = "00023,00027,00501,00502,00503,00504,00506,00507,00508,00510,00511,00512,00513,00514,00516,00517,00518,00520,00521,00024".
/* Mayorista - (Utilex + institucionales) */
IF lGrupo = 3 THEN lDivisiones = "00023,00027,00501,00502,00503,00504,00506,00507,00508,00510,00511,00512,00513,00514,00516,00517,00518,00520,00521,00024".
/* Institucionales*/
IF lGrupo = 4 THEN lDivisiones = "00024".

EMPTY TEMP-TABLE detalle.
EMPTY TEMP-TABLE Detalle-esp.
EMPTY TEMP-TABLE art-esp.

txt-msg:SCREEN-VALUE IN FRAME {&FRAME-NAME} = 'Los articulos...' . 

/* Tabla de Articulos */
FOR EACH almmmatg WHERE almmmatg.codcia = s-codcia AND almmmatg.tpoart = 'A' NO-LOCK:
    /*IF NOT (almmmatg.tpoart = 'A') THEN NEXT.*/
    IF LOOKUP (almmmatg.codfam, lFamiExo ) > 0 THEN NEXT.
    CREATE art-esp.
        ASSIGN art-esp.codmat = almmmatg.codmat
            art-esp.producto = almmmatg.desmat
            art-esp.tpoart = almmmatg.tpoart
