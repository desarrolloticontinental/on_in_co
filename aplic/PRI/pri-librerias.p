...
/* ****************************************************************************************************** */
/* 6to. 21/12/2022 Productos Exonerados */
/* ****************************************************************************************************** */
FIND FIRST VtaTabla WHERE VtaTabla.codcia = s-codcia
    AND VtaTabla.tabla = 'MMLX'
    AND VTaTabla.llave_c1 = pCodMat
    NO-LOCK NO-ERROR.
IF AVAILABLE VtaTabla THEN RETURN 'OK'.     /* OJO */

/* ****************************************************************************************************** */
/* 1ro. Verificamos que la división controle margen de utilidad */
/* ****************************************************************************************************** */
IF pCodDiv > '' THEN DO:    /* Siempre y cuando se envíe un valor */
    /* 26/12/2022: Solo va a depender de la configuración de los jefes de línea */
...
END.
/* ****************************************************************************************************** */
/* 21/12/2022: Rutina de un solo golpe */
/* ****************************************************************************************************** */
DEF VAR x-Costo AS DECI NO-UNDO.
...
RUN web_api-pricing-margen IN hLocalProc (pCodDiv,
                                          pCodMat,
                                          pPeldano,
                                          "C",                /* Clasificación C por defecto */
                                          "000",              /* Condición de venta Contado por defecto */
                                          ...
                                          OUTPUT pPreUni,
                                          OUTPUT pError).
/* 17/01/2023: Eliam Quispe */
/* No tomar en cuenta si no tiene ni precio ni costo */
IF pPreUni<= 0 OR x-Costo <= 0 THEN DO:
    pError = ''.
    RETURN 'OK'.
END.
/*IF RETURN-VALUE = 'ADM-ERROR' THEN RETURN 'ADM-ERROR'.*/
...
DEF VAR f-Factor AS DECI INIT 1.00 NO-UNDO.
/* ASSIGN                                                         */
/*     f-Factor = Almtconv.Equival                                */
/*     pPreUni = pPreUni / f-Factor.   /* En unidades de stock */ */
...
/* ******************************************** */
/* Chequeo 0: no debe estar excluido del margen */
/* ******************************************** */
FIND FIRST VtaTabla WHERE VtaTabla.codcia = s-codcia
    AND VtaTabla.tabla = 'MMLX'
    AND VTaTabla.llave_c1 = pCodMat
    NO-LOCK NO-ERROR.
IF AVAILABLE VtaTabla THEN RETURN "OK".
/* ******************************************** */
...
/* RHC: 14/10/2022 El COSTO DE REPOSICION debe salir del PRICING */
/* ****************************************************************************************************** */
DEFINE VAR x-Costo AS DECI NO-UNDO.
DEFINE VAR priMonVta AS INTE NO-UNDO.
DEFINE VAR priTpoCmb AS DECI NO-UNDO.
...
&IF DEFINED(EXCLUDE-PRI_Valida-Margen-Utilidad-Flash) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE PRI_Valida-Margen-Utilidad-Flash Procedure 
PROCEDURE PRI_Valida-Margen-Utilidad-Flash :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       Similar al PRI_Valida-Margen-Utilidad pero recibe como parámetro el CtoUni
------------------------------------------------------------------------------*/
  ...
  DEF INPUT PARAMETER pUndVta AS CHAR.
  DEF INPUT PARAMETER pPreUni AS DECI.
  DEF INPUT PARAMETER pCtoUni AS DECI.
  DEF OUTPUT PARAMETER pMargen AS DECI.
  DEF OUTPUT PARAMETER pLimite AS DECI.
  DEF OUTPUT PARAMETER pError AS CHAR NO-UNDO.
  ...
  FIND FIRST VtaTabla WHERE VtaTabla.codcia = s-codcia
      ...
      NO-LOCK NO-ERROR.
  IF AVAILABLE VtaTabla THEN RETURN "OK".
  IF pCtoUni = 0 OR pCtoUni = ? THEN DO:
      pError = 'Error en el costo unitario' + CHR(10) +
                  'Revisar lo siguiente:' + CHR(10) + 
                  'Artículo = ' + pCodMat + CHR(10) +
                  'Costo Unitario = ' + STRING(pCtoUni).
      RETURN 'ADM-ERROR'.
  END.
  /* ******************************************** */
...
/*   END.                                                                     */
  ...
  /* Margen calculado en base al COSTO */
  ASSIGN
      X-MARGEN = ROUND( ((( pPreUni / pCtoUni ) - 1 ) * 100 ), 2 )
      pMargen = x-Margen
      pLimite = x-Margen.
  /* ****************************************************************************************************** */
