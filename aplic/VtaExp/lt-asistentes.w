...
AND ExpAsist.Fecha >= TODAY - 1

&SCOPED-DEFINE CODIGO CodCli

/* Preprocesadores para cada campo filtro */
&SCOPED-DEFINE FILTRO1 ( NomCli BEGINS FILL-IN-filtro )
&SCOPED-DEFINE FILTRO2 ( INDEX ( NomCli , FILL-IN-filtro ) <> 0 )
...
&Scoped-define OPEN-QUERY-br_table OPEN QUERY br_table FOR EACH ExpAsist WHERE ~{&KEY-PHRASE} ~
      AND {&Condicion} NO-LOCK ~
    ~{&SORTBY-PHRASE}.
&Scoped-define TABLES-IN-QUERY-br_table ExpAsist
...
<SORTBY-OPTIONS>
Código|y||INTEGRAL.ExpAsist.CodCli|yes
Descripción|||INTEGRAL.ExpAsist.NomCli|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ExpAsist.CodCli COLUMN-LABEL "Código" FORMAT "X(11)":U WIDTH 10.57
      ExpAsist.NomCli FORMAT "x(60)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 66.14 BY 8.62
...
"CodCli" "Código" ? "character" ? ? ? ? ? ? no ? no no "10.57" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
