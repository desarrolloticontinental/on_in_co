...
IF LOOKUP(pParam,'PA,E') = 0 THEN RETURN ERROR.
/* PA digitado por el vendedor, por aprobar por administrador
   E Aprobado por el Administrador, por aprobar por supervisor
   */
...
DEFINE VARIABLE h_b-aprobar-cotizacion AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b-aprobar-cotizacion-det AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     BUTTON-4 AT ROW 1.27 COL 3 WIDGET-ID 2
     BUTTON-5 AT ROW 1.27 COL 21 WIDGET-ID 4
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 164.29 BY 24.88
...
ON CHOOSE OF BUTTON-4 IN FRAME F-Main /* APROBAR */
DO:
  RUN Aprobar IN h_b-aprobar-cotizacion.
END.
...
ON CHOOSE OF BUTTON-5 IN FRAME F-Main /* RECHAZAR */
DO:
  RUN Rechazar IN h_b-aprobar-cotizacion.
END.
...
{src/adm/template/windowmn.i}
    ...
    AND FacDocum.CodDoc = s-CodDoc NO-LOCK NO-ERROR.
    
    CASE TRUE:
        WHEN AVAILABLE FacDocum AND s-FlgEst = "PA" THEN {&WINDOW-NAME}:TITLE = "APROBACION DEL ADMINISTRADOR: " + FacDocum.NomDoc.
        WHEN AVAILABLE FacDocum AND s-FlgEst = "E" THEN {&WINDOW-NAME}:TITLE = "APROBACION DEL SUPERVISOR: " + FacDocum.NomDoc.
    END CASE.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'web/b-aprobar-cotizacion.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-aprobar-cotizacion ).
       RUN set-position IN h_b-aprobar-cotizacion ( 2.62 , 3.00 ) NO-ERROR.
       RUN set-size IN h_b-aprobar-cotizacion ( 6.69 , 161.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'LABEL-FONT = 4,
                     ...
                     LABELS = Detalle|Cabecera':U ,
             OUTPUT h_tab95 ).
       RUN set-position IN h_tab95 ( 9.62 , 3.00 ) NO-ERROR.
       RUN set-size IN h_tab95 ( 15.88 , 161.00 ) NO-ERROR.

       /* Links to SmartTab95 h_tab95. */
       RUN add-link IN adm-broker-hdl ( h_tab95 , 'Page':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-aprobar-cotizacion ,
             BUTTON-5:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_tab95 ,
             h_b-aprobar-cotizacion , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/web/b-aprobar-cotizacion-det.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-aprobar-cotizacion-det ).
       RUN set-position IN h_b-aprobar-cotizacion-det ( 10.69 , 5.00 ) NO-ERROR.
       RUN set-size IN h_b-aprobar-cotizacion-det ( 14.58 , 157.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-aprobar-cotizacion-det. */
       RUN add-link IN adm-broker-hdl ( h_b-aprobar-cotizacion , 'Record':U , h_b-aprobar-cotizacion-det ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-aprobar-cotizacion-det ,
             h_tab95 , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtagn/v-cotizacion-unificada.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v-cotizacion-unificada ).
       RUN set-position IN h_v-cotizacion-unificada ( 10.69 , 5.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.77 , 135.57 ) */

       /* Links to SmartViewer h_v-cotizacion-unificada. */
       RUN add-link IN adm-broker-hdl ( h_b-aprobar-cotizacion , 'Record':U , h_v-cotizacion-unificada ).
