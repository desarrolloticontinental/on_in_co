...
DO:
  ...
  IF TRUE <> (FacCPedi.DirCli:SCREEN-VALUE IN FRAME {&FRAME-NAME} > '') THEN DO:
      FIND FIRST gn-clied OF gn-clie WHERE Gn-ClieD.Sede = "@@@" NO-LOCK NO-ERROR.
      IF AVAILABLE gn-clied THEN DISPLAY Gn-ClieD.DirCli @ Faccpedi.DirCli WITH FRAME {&FRAME-NAME}.
  END.
...
FOR EACH Facdpedi OF Faccpedi NO-LOCK WHERE Facdpedi.Libre_c05 <> 'OF':
    ...
    IF Facdpedi.Libre_c04 > '' /*AND LOOKUP(ITEM.Libre_c04, 'PROM,VOL') = 0*/ THEN DO:
        LogRecalcular = YES.
        ASSIGN
            ITEM.Por_Dsctos[1] = 0
            ITEM.Por_Dsctos[2] = 0
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH FacdPedi OF FaccPedi NO-LOCK WHERE Facdpedi.Libre_c05 <> 'OF':
        ...
        ASSIGN
            ...
            ITEM.Libre_d03 = FacdPedi.Libre_d03.
        IF ITEM.Libre_d03 > 0 THEN DO:
            ASSIGN
                ITEM.CanPed = ITEM.CanPed - ITEM.Libre_d03
                ITEM.Libre_d03 = 0
...
------------------------------------------------------------------------------*/
  ...
  {web/i-descuentos-finales.i &Condicion="IF FacTabla.Campo-L[2] = NO THEN NEXT."}
...
/*   IF RETURN-VALUE = "ADM-ERROR" THEN UNDO, RETURN "ADM-ERROR".                     */
  ...
  */                
  ...
  FOR EACH ITEM NO-LOCK WHERE ITEM.Libre_c05 = "OF", FIRST Almmmatg OF ITEM NO-LOCK, FIRST Almtfami OF Almmmatg NO-LOCK:
      /* **************************************************************************************** */
      ...
      IF s-StkDis <= 0 THEN DO:
          ...
          MESSAGE 'Regalo/Promoción ' + ITEM.codmat + ' NO tiene Stock Disponible suficiente en el almacén ' + ITEM.AlmDes SKIP
