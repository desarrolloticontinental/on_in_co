...
/* 16Ago2022 - Ic */
IF ENTRY(1, pParametro) = "PPLE" THEN DO:
    s-CodDoc = "PLE".
END.
...
DEFINE SHARED VAR gTerminalServer AS CHAR.
...
DEF NEW SHARED VAR s-VerificaStock AS LOG INIT YES.
...
ASSIGN
    ...
    .
/* Buscamos Peldaño de Precios */
/* Define si la cotización verifica o no stock del almacén */
/* FIND FIRST FacTabla WHERE FacTabla.CodCia = s-codcia AND          */
/*     FacTabla.Tabla = 'GRUPO_DIVGG' AND                            */
/*     FacTabla.Codigo =  GN-DIVI.Grupo_Divi_GG                      */
/*     NO-LOCK NO-ERROR.                                             */
/* IF AVAILABLE FacTabla THEN s-VerificaStock = FacTabla.Campo-L[1]. */

/* 26/12/2023: NO verifica stock en la Expo */
s-VerificaStock = YES.
IF s-TpoPed = "E" THEN s-VerificaStock = NO.
...
DEF NEW SHARED VAR s-Solicita-Aprobacion AS LOGICAL INIT NO.

FIND FIRST FacTabla WHERE FacTabla.CodCia = s-codcia AND
    FacTabla.Tabla = 'GN-DIVI' AND 
    FacTabla.Codigo = pCodDiv
    NO-LOCK NO-ERROR.
IF AVAILABLE FacTabla THEN s-Solicita-Aprobacion = FacTabla.Campo-L[6].


DEFINE NEW SHARED VAR x-order-type AS CHAR.     /* PREMIUM/STANDARD */


DEFINE NEW SHARED TEMP-TABLE tt-listaexpress-sugeridos LIKE w-report.
...
DEFINE VARIABLE h_f-aritmetica-de-sunat AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_f-cot-gral-listaexpress-txt AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_f-gen-cot-vtahor-riqra AS HANDLE NO-UNDO.
DEFINE VARIABLE h_f-get-cant-vs-saldo AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     COMBO-NroSer AT ROW 1.27 COL 76.71 WIDGET-ID 4
     "Buscar:" VIEW-AS TEXT
          SIZE 6 BY .5 AT ROW 1.54 COL 88 WIDGET-ID 6
...
CASE TRUE:
    WHEN AVAILABLE FacDocum THEN W-Win:TITLE = ">>> " + FacDocum.NomDoc + " - MAYORISTA - PRICING".
    WHEN s-CodDoc = "COT" THEN W-Win:TITLE = ">>> PEDIDO COMERCIAL - MAYORISTA - PRICING".
END CASE.
CASE s-TpoPed:
    WHEN "LF" THEN W-Win:TITLE = W-Win:TITLE + "- LISTA EXPRESS".
    WHEN "PPLE" THEN W-Win:TITLE = W-Win:TITLE + "- LISTA EXPRESS".
...
/* 07/09/2022: Cambio de título */
W-Win:TITLE = ">>> PEDIDO COMERCIAL".
...
END.
W-Win:TITLE = W-Win:TITLE + " <<< - Terminal : " + gTerminalServer.
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_q-pedido ).
       RUN set-position IN h_q-pedido ( 1.00 , 94.00 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       ...
       RUN set-position IN h_f-cot-gral-exporta-a-excel ( 1.00 , 172.00 ) NO-ERROR.
       ...
       RUN set-position IN h_f-cot-gral-exporta-a-pdf ( 1.00 , 179.00 ) NO-ERROR.
       ...
       RUN set-position IN h_f-cot-gral-expo-pag1 ( 1.27 , 147.00 ) NO-ERROR.
       ...
       RUN set-position IN h_f-imp-excel-plantilla ( 3.42 , 147.00 ) NO-ERROR.
       ...
       RUN set-position IN h_f-cot-gral-super ( 4.50 , 147.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.19 , 25.00 ) */
       ...
       RUN set-position IN h_f-cot-gral-imp-market ( 6.65 , 147.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.08 , 25.00 ) */
       ...
       RUN set-position IN h_f-gen-cot-even ( 7.73 , 147.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.12 , 30.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/web/f-gen-cot-vtahor-riqra.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_f-gen-cot-vtahor-riqra ).
       RUN set-position IN h_f-gen-cot-vtahor-riqra ( 8.81 , 147.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.12 , 37.14 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/web/f-cot-gral-envio-admin.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_f-cot-gral-envio-admin ).
       RUN set-position IN h_f-cot-gral-envio-admin ( 9.88 , 147.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.08 , 28.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/web/f-get-cant-vs-saldo.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_f-get-cant-vs-saldo ).
       RUN set-position IN h_f-get-cant-vs-saldo ( 10.96 , 147.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.08 , 30.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/web/f-aritmetica-de-sunat.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_f-aritmetica-de-sunat ).
       RUN set-position IN h_f-aritmetica-de-sunat ( 12.04 , 147.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.08 , 23.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'web/b-cotizacion-sunat-flash-v2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-cotizacion-general ).
       RUN set-position IN h_b-cotizacion-general ( 13.15 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-cotizacion-general ( 13.46 , 182.00 ) NO-ERROR.
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_f-gen-cot-vtahor-riqra ,
             h_f-gen-cot-even , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_f-cot-gral-envio-admin ,
             h_f-gen-cot-vtahor-riqra , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_f-get-cant-vs-saldo ,
             h_f-cot-gral-envio-admin , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_f-aritmetica-de-sunat ,
             h_f-get-cant-vs-saldo , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-cotizacion-general ,
             h_f-aritmetica-de-sunat , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/web/f-cot-gral-pag2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_f-cot-gral-pag2 ).
       RUN set-position IN h_f-cot-gral-pag2 ( 1.27 , 147.00 ) NO-ERROR.
       ...
       RUN set-position IN h_f-cot-gral-imp-ped-excel ( 3.42 , 147.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.12 , 25.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/vtagn/f-cot-gral-listaexpress-txt.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_f-cot-gral-listaexpress-txt ).
       RUN set-position IN h_f-cot-gral-listaexpress-txt ( 4.50 , 147.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.12 , 25.00 ) */
       ...
       RUN set-position IN h_f-cot-gral-super-pag2 ( 5.58 , 147.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.08 , 25.00 ) */
       ...
       RUN set-position IN h_f-cot-gral-cotrecu ( 6.65 , 147.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.12 , 25.00 ) */
       ...
       RUN set-position IN h_f-cot-gral-sede-cliente-2 ( 7.73 , 147.00 ) NO-ERROR.
       ...
       RUN set-position IN h_f-cot-gral-matriz-de-ventas ( 11.77 , 147.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.08 , 19.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'aplic/web/t-cotizacion-pricing-v2.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_t-cotizacion-unificada ).
       RUN set-position IN h_t-cotizacion-unificada ( 13.12 , 1.72 ) NO-ERROR.
       RUN set-size IN h_t-cotizacion-unificada ( 13.46 , 183.00 ) NO-ERROR.
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_f-cot-gral-imp-ped-excel ,
             h_v-cotizacion-unificada , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_f-cot-gral-listaexpress-txt ,
             h_f-cot-gral-imp-ped-excel , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_f-cot-gral-super-pag2 ,
             h_f-cot-gral-listaexpress-txt , 'AFTER':U ).
    ...
    WHEN 4 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     AddFunction = Multiple-Records':U ,
             OUTPUT h_p-updv95 ).
       RUN set-position IN h_p-updv95 ( 24.96 , 2.00 ) NO-ERROR.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Genera-Cot-VtaHor-Riqra W-Win 
PROCEDURE Genera-Cot-VtaHor-Riqra :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
RUN web/d-gen-cot-vtahor-riqra.w (INPUT pCodDiv, 
                                  OUTPUT pCodVen, 
                                  OUTPUT pCodRef, 
                                  OUTPUT pNroRef).
IF TRUE <> (pCodRef > '') OR TRUE <> (pNroRef > '') THEN RETURN.

DEFINE VAR hProc AS HANDLE NO-UNDO.

RUN web/gen-cot-eventos-library.r PERSISTENT SET hProc.

/* Actualizamos el tipo de pedido */
RUN Change-TpoPed IN hproc ( INPUT s-TpoPed).

RUN COT_Genera-COT-por-PET IN hProc (INPUT pCodDiv,
                                     INPUT pCodVen,
                                     INPUT pCodRef,
                                     INPUT pNroRef,
                                     INPUT s-CodAlm,    /* 15/7/24 Nuevo parámetro */
                                     OUTPUT pNroPed,
                                     OUTPUT pMensaje).
...
RUN vtagn/d-gen-cot-eventos.w (INPUT pCodDiv, OUTPUT pCodVen, OUTPUT pCodRef, OUTPUT pNroRef).
IF TRUE <> (pCodRef > '') OR TRUE <> (pNroRef > '') THEN RETURN.


DEFINE VAR hProc AS HANDLE NO-UNDO.

RUN web/gen-cot-eventos-library PERSISTENT SET hProc.
RUN COT_Genera-COT-por-PET IN hProc (INPUT pCodDiv,
                                     INPUT pCodVen,
                                     INPUT pCodRef,
                                     INPUT pNroRef,
...
DEFINE VAR cDivisionVta AS CHAR.

CASE L-Handle:
    WHEN "Cantidad-vs-Saldo" THEN DO:
        RUN Cantidad-vs-Saldo IN h_v-cotizacion-unificada.
    END.
    ...
    WHEN "Transferencia-de-Saldos" THEN DO:
        ...
        IF pRowidS <> ? THEN DO:
            RUN Posiciona-Registro IN h_q-pedido ( INPUT pRowidS /* ROWID */).
            RUN Procesa-Handle ("Recalculo").
        END.
    END.
    ...
    END.
    WHEN "Importar-ListaExpress-TXT" THEN DO:
        RUN Importar-ListaExpress-TXT IN h_v-cotizacion-unificada.
    END.
    WHEN "Aritmetica-de-Sunat" THEN DO:
        MESSAGE 'Procedemos a aplicar el cálculo de ARITMETICA DE SUNAT?'
            VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO UPDATE rpta AS LOG.
        IF rpta = YES THEN RUN Aritmetica-de-Sunat IN h_v-cotizacion-unificada.
    END.
    WHEN "Aritmetica-Sunat-Off" THEN DO:
        RUN dispatch IN h_f-aritmetica-de-sunat ('hide':U).
    END.
    WHEN "Aritmetica-Sunat-On" THEN DO:
        RUN dispatch IN h_f-aritmetica-de-sunat ('view':U).
    END.
    WHEN "Boton-Aritmetica-Sunat" THEN DO:
        /* Ic - 17Abr2024 */
        RUN dispatch IN h_f-aritmetica-de-sunat ('hide':U).
        RUN datos-activacion-boton-arimetica-sunat IN h_v-cotizacion-unificada(OUTPUT cOrigen, OUTPUT cDivisionVta).                
        IF cOrigen = "RIQRA" THEN DO:
            RUN dispatch IN h_f-aritmetica-de-sunat ('view':U).
        END.
        ELSE DO:
            /* Verificamos configuración */
            IF CAN-FIND(FIRST FacTabla WHERE FacTabla.codcia = s-codcia AND
                        FacTabla.Tabla = "ACTIVAR_ARIMETICA" AND
                        FacTabla.Codigo = cDivisionVta NO-LOCK)
                THEN DO:
                RUN dispatch IN h_f-aritmetica-de-sunat ('view':U).
            END.
        END.            
    END.
    WHEN "Genera-Cot-VtaHor-Riqra" THEN DO:
        RUN Genera-Cot-VtaHor-Riqra.
    END.
    OTHERWISE DO:
...
CASE L-Handle:
    WHEN "Pagina1"  THEN DO WITH FRAME {&FRAME-NAME}:
        ...
        RUN dispatch IN h_f-gen-cot-vtahor-riqra ('hide':U).
        ...
        CASE s-TpoPed:
            ...
            END.
            OTHERWISE DO:
                /* Verificamos configuración Vta Hor. Riqra */
                IF CAN-FIND(FIRST FacTabla WHERE FacTabla.codcia = s-codcia AND
                            FacTabla.Tabla = "CFG_VTAHOR_RIQRA" AND
                            FacTabla.Codigo = s-CodDiv NO-LOCK)
                    THEN RUN dispatch IN h_f-gen-cot-vtahor-riqra ('view':U).
            END.
        END CASE.
        /* Repintamos pantalla */
        RUN dispatch IN h_v-cotizacion-unificada ('display-fields':U).
        /* *************************************************** */
        /* Solo para INSTITUCIONALES: 19/07/2022 Carla Tenazoa */
        /* *************************************************** */
        FIND gn-divi WHERE gn-divi.codcia = s-codcia
            AND gn-divi.coddiv = pCodDiv
            NO-LOCK NO-ERROR.
        IF AVAILABLE gn-divi AND GN-DIVI.CanalVenta = "INS" 
            THEN RUN dispatch IN h_f-cot-gral-exporta-a-excel ('view':U).
        /* *************************************************** */
    END.
    WHEN "Pagina2"  THEN DO WITH FRAME {&FRAME-NAME}:
        ...
        RUN dispatch IN h_f-cot-gral-envio-admin ('hide':U).
        RUN dispatch IN h_f-cot-gral-listaexpress-txt ('hide':U).
        CASE s-TpoPed:
            WHEN "S" THEN DO:       /* Supermercados */
            ...
            END.
            WHEN "PPLE" THEN DO:
                RUN dispatch IN h_f-cot-gral-listaexpress-txt ('view':U).
            END.
    ...
    WHEN "Disable-Head" THEN DO:
        ...
        RUN dispatch IN h_f-cot-gral-listaexpress-txt ('disable':U).
    END.
    WHEN "Enable-Head" THEN DO:
        ...
        RUN dispatch IN h_f-cot-gral-listaexpress-txt ('enable':U).
    END.
    WHEN "browse" THEN DO:
          IF h_b-cotizacion-general <> ? THEN RUN dispatch IN h_b-cotizacion-general ('open-query':U).
