...
&Scoped-Define ENABLED-OBJECTS RECT-27 RECT-25 RECT-26 RECT-28 
...
FacCPedi.Libre_d01 FacCPedi.Sede FacCPedi.CodOrigen FacCPedi.CodPos ~
FacCPedi.CodRef FacCPedi.NroRef FacCPedi.NroOrigen FacCPedi.Cliente_Recoge ~
FacCPedi.CodAlm 
...
"No", ""
     ...
     FacCPedi.DirCli AT ROW 4.5 COL 15 COLON-ALIGNED WIDGET-ID 44
          ...
          SIZE 77 BY .81
          BGCOLOR 11 FGCOLOR 0 
     FacCPedi.ordcmp AT ROW 4.5 COL 109 COLON-ALIGNED WIDGET-ID 118 FORMAT "X(20)"
          VIEW-AS FILL-IN 
          SIZE 20 BY .81
          BGCOLOR 11 FGCOLOR 0 
...
"5", 5
     ...
     FacCPedi.CodOrigen AT ROW 9.35 COL 129 COLON-ALIGNED NO-LABEL WIDGET-ID 186
          VIEW-AS FILL-IN 
          SIZE 7.14 BY .81
     ...
     FacCPedi.NroRef AT ROW 9.88 COL 111 COLON-ALIGNED NO-LABEL WIDGET-ID 122
          VIEW-AS FILL-IN 
          SIZE 12 BY .81
     FacCPedi.NroOrigen AT ROW 10.15 COL 129 COLON-ALIGNED NO-LABEL WIDGET-ID 188
          VIEW-AS FILL-IN 
          SIZE 12.14 BY .81
...
DEFINE FRAME F-Main
     ...
     RECT-28 AT ROW 8.81 COL 129 WIDGET-ID 190
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1 SCROLLABLE 
         FONT 4 WIDGET-ID 100.
...
/* SETTINGS FOR FILL-IN FacCPedi.CodCli IN FRAME F-Main
   EXP-LABEL                                                            */
/* SETTINGS FOR FILL-IN FacCPedi.CodOrigen IN FRAME F-Main
   NO-ENABLE                                                            */
...
DO:
  ...
  DEF VAR cClienteValidacion AS CHAR.
  DEF VAR cRucClientePrimerosDig AS CHAR.
  DEF VAR cRucPrimerosDig AS CHAR.
  ...
  ************************************************************ */
  cClienteValidacion = s-CodCli + "|" + "1".
  RUN pri/p-verifica-cliente.r (INPUT cClienteValidacion,   /*s-CodCli,*/
                                INPUT s-CodDoc,
                                INPUT s-CodDiv).
  IF RETURN-VALUE = 'ADM-ERROR' THEN RETURN NO-APPLY.
  /* ****************************************** */
  ...
  /* DATOS DEL CLIENTE */
  /* ****************************************** */
  cRucPrimerosDig = "20,10,15,17".  /* Ruc Persona Juridica, Natural, Extranjera/Fuerzas armadas */
  cRucClientePrimerosDig = SUBSTRING(gn-clie.Ruc,1,2).
  DISPLAY 
      gn-clie.NomCli @ Faccpedi.NomCli
      gn-clie.Ruc    @ Faccpedi.RucCli
      gn-clie.DirCli @ Faccpedi.DirCli
    ...
    /* Ic - 17Set2023, correo carla tenazoa */
   IF TRUE <> (FacCPedi.RucCli:SCREEN-VALUE > '') THEN DO:
       ASSIGN Faccpedi.atencion:SCREEN-VALUE = gn-clie.dni.
   END.
   ELSE DO:
        IF LOOKUP(cRucClientePrimerosDig,cRucPrimerosDig) = 0 THEN DO:
            ASSIGN Faccpedi.ruccli:SCREEN-VALUE = ""
                    Faccpedi.atencion:SCREEN-VALUE = gn-clie.dni.
        END.
   END.
...
DO:
  ...
  /* 08/05/2024: Kenshiro Vta Horizontal Riqra */
  DEFINE VAR pOutput AS CHAR.
  DEFINE VAR hProc AS HANDLE NO-UNDO.

  RUN web/web-library.p PERSISTENT SET hProc.

  RUN web_api-satelite-vendedor IN hProc (SELF:SCREEN-VALUE,
                                          "15",                 /* Venta Horizontal */
                                          OUTPUT pOutput).

  DELETE PROCEDURE hProc.
  CASE pOutput:
      WHEN "0" THEN TRUE.   /* Vendedor NO en satélite */
      WHEN "1" THEN DO:     /* Vendedor SI en satélite */
          MESSAGE "Vendedor autorizado solo para ventas web" VIEW-AS ALERT-BOX WARNING.
          SELF:SCREEN-VALUE = "".
          F-NomVen:SCREEN-VALUE = "".
          RETURN NO-APPLY.
      END.
      OTHERWISE DO:
          MESSAGE pOutput VIEW-AS ALERT-BOX ERROR.
          SELF:SCREEN-VALUE = "".
          F-NomVen:SCREEN-VALUE = "".
          RETURN NO-APPLY.
      END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Cantidad-vs-Saldo V-table-Win 
PROCEDURE Cantidad-vs-Saldo :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
RUN web/d-cotizacion-cant-vs-saldo.w (ROWID(Faccpedi),
                                      s-CodAlm).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE datos-activacion-boton-arimetica-sunat V-table-Win 
PROCEDURE datos-activacion-boton-arimetica-sunat :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
DEFINE OUTPUT PARAMETER cOrigen AS CHAR NO-UNDO.
DEFINE OUTPUT PARAMETER cDivisionVta AS CHAR NO-UNDO.

cOrigen = "".
cDivisionVta = "".

IF AVAILABLE faccpedi THEN DO:
    cOrigen = Faccpedi.CodOrigen.
    cDivisionVta = Faccpedi.CodDiv.
END.
...
------------------------------------------------------------------------------*/
  ...
  {web/i-descuentos-finales.i &Condicion="~
      CASE s-TpoPed: ~
            WHEN 'E' THEN DO:     /* Eventos */ ~
                IF FacTabla.Campo-L[3] = NO THEN NEXT. ~
            END. ~
            OTHERWISE DO: ~
...
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       Ajustamos a la ARITMETICA DE SUNAT
------------------------------------------------------------------------------*/
...
/* ***************************************************************** */
CASE l-IncIgv:
    WHEN YES THEN f-ImpTot = FacCPedi.TotalPrecioVenta.
    WHEN NO  THEN f-ImpTot = FacCPedi.TotalValorVenta.
END CASE.
/* ***************************************************************** */
/* ************************ cargamos variables ********************* */
/* ***************************************************************** */
FIND gn-ven WHERE gn-ven.CodCia = FacCPedi.CodCia AND gn-ven.CodVen = FacCPedi.CodVen NO-LOCK NO-ERROR.
C-NomVen = FacCPedi.CodVen.
IF AVAILABLE gn-ven THEN C-NomVen = C-NomVen + " - " + gn-ven.NomVen.
...
P = t-Column.
FOR EACH facdpedi OF faccpedi NO-LOCK, FIRST almmmatg OF facdpedi NO-LOCK BREAK BY FacDPedi.NroPed BY FacDPedi.NroItm DESC:
    /* 07/05/2024: Kenshiro */
    CASE l-IncIgv:
        WHEN NO THEN ASSIGN 
            F-PreUni = ROUND(FacDPedi.ImporteTotalSinImpuesto / FacDPedi.CanPed, 4)
            F-ImpLin = FacDPedi.ImporteTotalSinImpuesto.
        WHEN YES THEN ASSIGN 
            F-PreUni = ROUND(FacDPedi.cImporteTotalConImpuesto / FacDPedi.CanPed, 4)
            F-ImpLin = FacDPedi.cImporteTotalConImpuesto.
    END CASE.
    /* *************************************************************************** */
    /* *************************************************************************** */
    /*Agrega Row*/
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Genera-Excel2-Old V-table-Win 
PROCEDURE Genera-Excel2-Old :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
/* 31/05/2024: Control del cliente */
IF TRUE <> (FacCPedi.CodCli:SCREEN-VALUE IN FRAME {&FRAME-NAME} > "") THEN DO:
    MESSAGE "Debe ingresar primero el código del cliente" VIEW-AS ALERT-BOX WARNING.
    APPLY "ENTRY":U TO FacCPedi.CodCli.
    RETURN "ADM-ERROR".
END.

/* 11/11/2024: Debe estar configurado */
FIND FIRST cmedidefclientes WHERE cmedidefclientes.CodCia = s-codcia AND
    cmedidefclientes.codcli = FacCPedi.CodCli:SCREEN-VALUE IN FRAME {&FRAME-NAME}
    NO-LOCK NO-ERROR.
IF NOT AVAILABLE cmedidefclientes THEN DO:
    MESSAGE 'NO está configurado el cliente en ningún proveedor de archivos EDI' SKIP
...
SYSTEM-DIALOG GET-FILE x-Archivo
    ...
    RETURN-TO-START-DIR
    TITLE 'Selecciona al archivo texto (EDI)'
    MUST-EXIST
    USE-FILENAME
    UPDATE x-Ok.
...
DEFINE VAR x-len-oc  AS INT INIT 12 NO-UNDO.         /* Caracteres de la O/C */
DEFINE VAR x-init-oc AS INT INIT 1  NO-UNDO.
DEFINE VAR x-long-oc AS INT INIT 35 NO-UNDO.
DEFINE VAR x-order-oc AS INT INIT 5 NO-UNDO.

/* Buscamos ubicación de acuerdo a la configuración */
/* Valor por defecto */
FIND FIRST cmediclientespec WHERE cmediclientespec.CodCia = s-codcia AND
    cmediclientespec.codcli = cmedidefclientes.codcli AND
    cmediclientespec.CodPro = cmedidefclientes.CodPro AND
    cmediclientespec.Identificador = "ENC" 
    NO-LOCK NO-ERROR.
IF AVAILABLE cmediclientespec THEN DO:
    FIND cmedidefdet WHERE cmedidefdet.CodCia = s-codcia AND
        cmedidefdet.CodPro = cmedidefclientes.CodPro AND
        cmedidefdet.Identificador = "ENC" AND 
        cmedidefdet.Order = cmediclientespec.Order
        NO-LOCK NO-ERROR.
    IF AVAILABLE cmedidefdet THEN x-long-oc = cmediclientespec.Longitud.
    x-order-oc = cmediclientespec.Order.
    x-init-oc  = cmediclientespec.Inicio.
    x-len-oc   = cmediclientespec.Longitud.
END.
...
/*         END.                                           */
        IF NUM-ENTRIES(x-Linea) >= x-order-oc + 1 THEN DO:
            x-Item = ENTRY(x-order-oc + 1,x-Linea).
            IF LENGTH(x-Item) > (x-len-oc + x-init-oc - 1) THEN DO:
                x-Item = SUBSTRING(x-Item,x-init-oc,x-len-oc).
            END.
        END.
    ...
    END.
    /* Cliente: Código IBC registrado en el maestro de clientes */
    IF x-Linea BEGINS 'IVAD' AND NUM-ENTRIES(x-Linea) > 1 THEN DO:
        ...
        FIND FIRST gn-clie WHERE gn-clie.codcia = cl-codcia
            ...
            AND gn-clie.flgsit = 'A'            /* Debe estar ACTIVO */
            NO-LOCK NO-ERROR.
        IF AVAILABLE gn-clie THEN DO:
            /* 31/05/2024: Control del cliente vs lo digitado */
            IF Faccpedi.codcli:SCREEN-VALUE IN FRAME {&FRAME-NAME} <> gn-clie.CodCli THEN DO:
                MESSAGE "El código IBC del archivo texto NO coincide con el del cliente que usted ha registrado" + CHR(10) +
                    "Información del archivo texto: " + x-Linea
                    VIEW-AS ALERT-BOX WARNING.
                APPLY "ENTRY":U TO FacCPedi.CodCli.
                RETURN.
            END.
...
------------------------------------------------------------------------------*/
  ...
  IF AVAILABLE FacCPedi THEN DO WITH FRAME {&FRAME-NAME}:
      ...
      */
      /* IC - 17Abr2024 */
      RUN Procesa-Botones IN lh_handle ("Boton-Aritmetica-Sunat").      
  END.
...
DEFINE VAR cClienteValidacion AS CHAR.
...
DO WITH FRAME {&FRAME-NAME} :
      ...
      /* **********************************************************
       ...
       Ic 26Abr2024, validar el cliente al grabar (Carla Huari) 
      ************************************************************ */
      cClienteValidacion = Faccpedi.CodCli:SCREEN-VALUE + "|" + "2".
      RUN pri/p-verifica-cliente.r (INPUT cClienteValidacion,    /*s-CodCli,*/
                                    INPUT s-CodDoc,
                                    INPUT s-CodDiv).
      IF RETURN-VALUE = 'ADM-ERROR' THEN RETURN NO-APPLY.
    ...
    /* VALIDACION DEL VENDEDOR */
     ...
     RUN web/web-library.p PERSISTENT SET hProc.
     RUN web_api-satelite-vendedor IN hProc (FacCPedi.CodVen:SCREEN-VALUE,
                                             "15",                 /* Venta Horizontal */
                                             OUTPUT pOutput).
     DELETE PROCEDURE hProc.
     CASE pOutput:
    ...
    DEFINE VAR xOrdenCmp AS CHAR NO-UNDO.

    xOrdenCmp = faccpedi.ordcmp:SCREEN-VALUE.
    IF TRUE <> (xOrdenCmp > "") THEN xOrdenCmp = "".

    IF  xOrdenCmp > "" AND LENGTH(xOrdenCmp) > 20 THEN DO:
        MESSAGE "La orden de compra debe tener como maximo 20 Caracteres" VIEW-AS ALERT-BOX INFORMATION.
        RETURN "ADM-ERROR".
    END.
