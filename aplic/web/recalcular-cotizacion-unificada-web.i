...
DEFINE VAR hProcCantidad AS HANDLE NO-UNDO.
DEFINE VAR pCantidad AS DECI INIT 0 NO-UNDO.

RUN pri/pri-librerias.p PERSISTENT SET hProcCantidad.

ACTUALIZACION:
FOR EACH ITEM WHERE ITEM.Libre_c05 <> "OF", FIRST Almmmatg OF ITEM NO-LOCK:
    ASSIGN
    ...
    */
    ...
    /* 26/05/2023: YA NO VA */
    /*
    RUN PRI_Cantidad-Fac-Neta IN hProcCantidad (pCodDiv,
                                        s-CodCli,
                                        Almmmatg.CodMat,
                                        OUTPUT pCantidad).
    ...
    */
    ...
    IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
        IF {&pMuestraError} THEN MESSAGE pMensaje VIEW-AS ALERT-BOX ERROR.
        DELETE ITEM.
        NEXT ACTUALIZACION.
    END.
...
DELETE PROCEDURE hProcCantidad.
