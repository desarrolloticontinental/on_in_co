...
FacCPedi.CodMon FacCPedi.TpoLic FacCPedi.CodAlm FacCPedi.NroRef ~
...
FacCPedi.ordcmp FacCPedi.Sede FacCPedi.CodOrigen FacCPedi.NroOrigen ~
FacCPedi.CodPos FacCPedi.Cliente_Recoge FacCPedi.Glosa FacCPedi.TipVta ~
FacCPedi.EmpaqEspec FacCPedi.Libre_c04 
...
DEFINE FRAME F-Main
     ...
     FacCPedi.CodOrigen AT ROW 8.27 COL 102 COLON-ALIGNED NO-LABEL WIDGET-ID 152
          VIEW-AS FILL-IN 
          SIZE 7.43 BY .81
          BGCOLOR 14 FGCOLOR 0 
...
"Si", "Si":U
     ...
     FacCPedi.EmpaqEspec AT ROW 10.69 COL 107 WIDGET-ID 176
          ...
          SIZE 17 BY .77
          BGCOLOR 14 FGCOLOR 0 
     FILL-IN-embalado-especial AT ROW 9.08 COL 68 COLON-ALIGNED NO-LABEL WIDGET-ID 184
     FacCPedi.Libre_c04 AT ROW 11.96 COL 14 COLON-ALIGNED WIDGET-ID 186
          LABEL "Observaciones" FORMAT "x(80)"
           VIEW-AS TEXT 
          SIZE 71 BY .88
          FGCOLOR 4 FONT 6
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH Facdpedi OF Faccpedi NO-LOCK WHERE Facdpedi.Libre_c05 <> 'OF':    /* SIN PROMOCIONES */
      ...
      /* 25/07/2022 CCamus Cambio en las promociones */
...
------------------------------------------------------------------------------*/
  ...
  /* 24/11/23 C.Tenazoa no bloquear COT vencidas */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Captura-Saldo-Cot V-table-Win 
...
DEF OUTPUT PARAMETER pTotalCot AS DECI.
DEF OUTPUT PARAMETER pSaldoCot AS DECI.
...
FIND COTIZACION WHERE COTIZACION.codcia = s-codcia
    ...
    AND COTIZACION.nroped = FacCPedi.NroRef:SCREEN-VALUE IN FRAME {&FRAME-NAME}
    NO-LOCK NO-ERROR.
IF NOT AVAILABLE COTIZACION THEN RETURN.

ASSIGN
    pTotalCot = COTIZACION.ImpTot.
FOR EACH Facdpedi OF COTIZACION NO-LOCK WHERE Facdpedi.CanAte < Facdpedi.CanPed:
    pSaldoCot = pSaldoCot + (Facdpedi.CanPed - Facdpedi.CanAte) * (Facdpedi.ImpLin / Facdpedi.CanPed).
END.

RUN GET-ATTRIBUTE('ADM-NEW-RECORD').
IF RETURN-VALUE = 'NO' THEN DO:
    ASSIGN
        pSaldoCot = pSaldoCot + Faccpedi.ImpTot.
END.
...
END.
/* Reposicionamos el registro y pintamos */
FIND Faccpedi WHERE ROWID(Faccpedi) = x-Rowid NO-LOCK NO-ERROR.

RUN dispatch IN THIS-PROCEDURE ('display-fields':U).
IF pMensaje > "" THEN MESSAGE pMensaje VIEW-AS ALERT-BOX ERROR.
ELSE MESSAGE "Generación de Orden de Despacho Completada" VIEW-AS ALERT-BOX INFORMATION.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Excel-Normal V-table-Win 
PROCEDURE Excel-Normal :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Excel-Sunat V-table-Win 
PROCEDURE Excel-Sunat :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    FOR EACH b-facdpedi OF faccpedi NO-LOCK,
        FIRST almmmatg OF b-facdpedi NO-LOCK
        BREAK BY b-facdpedi.NroPed BY b-facdpedi.NroItm:
        dPreUni = b-FacDPedi.ImporteUnitarioConImpuesto.
        dImpLin = b-FacDPedi.cImporteTotalConImpuesto.
        iRow = iRow + 1.
        cColumn = STRING(iRow).
        cRange = "A" + cColumn.
        chWorkSheet:Range(cRange):VALUE = STRING(b-facdpedi.nroitm, '>>>9').
        ...
        chWorkSheet:Range(cRange):VALUE = b-facdpedi.canate.
        cRange = "I" + cColumn.
        chWorkSheet:Range(cRange):VALUE = b-FacDPedi.FactorDescuento * 100.
        cRange = "J" + cColumn.
        chWorkSheet:Range(cRange):VALUE = dPreUni.
        cRange = "K" + cColumn.
        chWorkSheet:Range(cRange):VALUE = dImpLin.
...
------------------------------------------------------------------------------*/
  ...
  &IF {&ARITMETICA-SUNAT} &THEN 
  RUN vtagn/impresion-ped-pdf-sunat.r (ROWID(FacCPedi)).
  &ELSE
  RUN vtagn/impresion-ped-pdf.r (ROWID(FacCPedi)).
  &ENDIF
...
------------------------------------------------------------------------------*/
  ...
  DEFIN OUTPUT PARAMETER pMensaje AS CHAR NO-UNDO.
  ...
  DETALLES:
  FOR EACH PEDI EXCLUSIVE-LOCK, FIRST Almmmatg OF PEDI NO-LOCK, FIRST Almtfami OF Almmmatg NO-LOCK BY PEDI.NroItm:
      ...
      IF LocalVerificaStock = YES THEN DO:
          ...
          IF x-StkAct > 0 THEN DO:
              ...
              IF AVAILABLE FacTabla THEN DO:
                  ...
                  IF AVAILABLE Facdpedi AND s-StkComprometido > 0 THEN
          ...
          IF s-StkDis <= 0 THEN DO:
              pMensajeFinal = pMensajeFinal + CHR(10) +
                  'El STOCK DISPONIBLE está en CERO para el producto ' + PEDI.codmat + 
...
------------------------------------------------------------------------------*/
  ...
  /* ******************************************************************** */
  /* 4/12/23: S.Leon Tipo de Cambio de Caja Cobranzas */
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ...
      FIND VtaTabla WHERE VtaTabla.CodCia = s-codcia AND
          ...
          NO-LOCK NO-ERROR.
      IF AVAILABLE VtaTabla AND VtaTabla.Valor[1] > 0 THEN DISPLAY TODAY + VtaTabla.Valor[1] @ Faccpedi.FchVen.
      /* *************************************************************************************** */
      /* *************************************************************************************** */
      ASSIGN
...
------------------------------------------------------------------------------*/
  ...
  */
  ...
  /* *********************************************** */
  /* 22/10/2024: Control de A/C con ARITMETICA SUNAT */
  /* *********************************************** */
  DEF VAR pTpoLic AS LOG NO-UNDO.

  RUN VALIDA-AC (OUTPUT pTpoLic, OUTPUT pMensaje).
  IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
      UNDO, RETURN 'ADM-ERROR'.
  END.
  ...
  ASSIGN combo-box-doc:SENSITIVE = NO.
...
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ...
      ASSIGN
          FacCPedi.Libre_c03:SENSITIVE = NO
          combo-box-doc:SENSITIVE = NO.
      /* 26/01/2024: NO debe ser editable */
      ASSIGN
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE PedidosLogisticosxAlmacen V-table-Win 
PROCEDURE PedidosLogisticosxAlmacen :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DO WITH FRAME {&FRAME-NAME}:
    ...
    RUN GET-ATTRIBUTE('ADM-NEW-RECORD').
    IF RETURN-VALUE = 'NO' THEN DO:
        pFchEnt = INPUT FacCPedi.FchEnt.      /* OJO */
        IF FacCPedi.Ubigeo[2] <> "@PV" THEN DO:
            pFchEnt = INPUT FacCPedi.Libre_f02.   /* Pactada con el cliente */
            RUN logis/p-fecha-de-entrega (
                FacCPedi.CodDoc,              /* Documento actual */
                FacCPedi.NroPed,
                INPUT-OUTPUT pFchEnt,
        ...
        END.
    END.
    ELSE pFchEnt = INPUT FacCPedi.Libre_f02.   /* Pactada con el cliente */
    IF pMensaje > '' THEN DO:
        MESSAGE pMensaje VIEW-AS ALERT-BOX ERROR.
        UNDO, RETURN 'ADM-ERROR'.
    END.
    /* *************************************************************************************** */
    DISPLAY pFchEnt @ FacCPedi.FchEnt WITH FRAME {&FRAME-NAME}.
    /* *************************************************************************************** */
    /* RHC 25/08/17 Acuerdo de reunión, correo del 24/08/17 */
    /* *************************************************************************************** */
    IF s-FmaPgo = '002' THEN DO:
        ...
        IF x-boleta-deposito > "" THEN DO:
            ...
            FOR EACH PEDI NO-LOCK:
                F-TOT = F-TOT + PEDI.ImpLin.
            END.
            IF TRUE <> (x-cod-boleta-deposito > "") THEN DO:
                MESSAGE 'Ingrese el Cod. Doc. del depósito' VIEW-AS ALERT-BOX ERROR
            ...
            RUN valida-deposito (INPUT x-cod-boleta-deposito, 
                                 ...
                                 OUTPUT pMensaje).
            IF RETURN-VALUE = "ADM-ERROR" THEN DO:
                IF TRUE <> (pMensaje > '') THEN pMensaje = 'Debe seleccionar el Doc. depósito'.
                pMensaje = pMensaje + CHR(10) + CHR(10) /*+ 'Comunicarse con el Gestor C&C'*/.
...
/*     DEF VAR pTpoLic AS LOG NO-UNDO.                    */
/*                                                        */
/*     RUN Verifica-AC (OUTPUT pTpoLic, OUTPUT pMensaje). */
/*     IF RETURN-VALUE = 'ADM-ERROR' THEN DO:             */
/*         MESSAGE pMensaje VIEW-AS ALERT-BOX ERROR.      */
/*         RETURN 'ADM-ERROR'.                            */
/*     END.                                               */
/*     s-TpoLic = pTpoLic.     /* OJO */                  */
    ...
    IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
        ...
        END.
        RUN Procesa-Handle IN lh_handle ("Display-Detail").
        RETURN 'ADM-ERROR'.
    END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE VALIDA-AC V-table-Win 
PROCEDURE VALIDA-AC :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       SE SUPONE QUE YA SE ACTUALIZÓ EL DETALLE CON LA ARITMETICA DE SUNAT
------------------------------------------------------------------------------*/
  ...
  DO WITH FRAME {&FRAME-NAME}:
      ...
      END.
      FOR EACH PEDI NO-LOCK:
          x-ImpTot = x-ImpTot + PEDI.cImporteTotalConImpuesto.  /* OJO */
      END.
...
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       NO PASA si no cubre el pedido logístico
------------------------------------------------------------------------------*/
...
DO WITH FRAME {&FRAME-NAME}:
    FIND FIRST Ccbcdocu WHERE Ccbcdocu.codcia = s-codcia
        ...
        AND Ccbcdocu.nrodoc = pNumeroDeposito
        AND Ccbcdocu.codcli = s-CodCli
        NO-LOCK NO-ERROR.
    ...
    IF Ccbcdocu.FmaPgo <> s-FmaPgo THEN DO:
        pMsgRet = "La condición de venta registrada en la Boleta de Depósito NO es CONTADO ANTICIPADO".
        RETURN 'ADM-ERROR'.
    END.
    ...
    */
    ...
    IF AVAILABLE vtatabla THEN DO:
        /* ¿? */
    END.
    IF s-CodMon = Ccbcdocu.CodMon THEN DO:
        IF Ccbcdocu.sdoact < pTot THEN DO:
            pMsgRet = "El saldo del " + pCodDeposito + " NO cubre el monto total".
        END.
    END.
    ELSE DO:
        IF Ccbcdocu.codmon = 1 THEN DO:
            IF Ccbcdocu.sdoact < (pTot * s-Tpocmb) THEN DO:
                pMsgRet = "El saldo del " + pCodDeposito + " NO cubre el monto total".
            END.
        END.
        ELSE DO:
            IF Ccbcdocu.sdoact < (pTot / FaccPedi.Tpocmb) THEN DO:
                pMsgRet = "El saldo del " + pCodDeposito + " NO cubre el monto total".
            END.
        END.
    END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE valida-deposito-old V-table-Win 
PROCEDURE valida-deposito-old :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
------------------------------------------------------------------------------*/
      ...
      IF LocalVerificaStock = YES THEN DO:
          ...
          ELSE DO:    /* EMPAQUE OTROS */
              IF s-FlgEmpaque = YES THEN DO:
                  ...
                  RUN vtagn/p-sugerido-ped-logistico.p (INPUT PEDI.CodMat,
  ...
  */                
  RUN {&Promocion} (COTIZACION.Libre_c01, 
                    s-CodCli, 
                    Faccpedi.CodAlm:SCREEN-VALUE IN FRAME {&FRAME-NAME}, 
  ...
  FOR EACH PEDI NO-LOCK WHERE PEDI.Libre_c05 = "OF", FIRST Almmmatg OF PEDI NO-LOCK, FIRST Almtfami OF Almmmatg NO-LOCK:
      ...
      IF s-StkDis <= 0 THEN DO:
          ...
          MESSAGE 'Regalo/Promoción ' + PEDI.codmat + ' NO tiene Stock Disponible suficiente en el almacén ' + PEDI.AlmDes SKIP
              'Se continuará con la grabación' VIEW-AS ALERT-BOX WARNING.
          DELETE PEDI.
          NEXT.
      END.
      f-Factor = PEDI.Factor.
