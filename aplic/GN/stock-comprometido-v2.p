...
FOR EACH x-gre_detail WHERE x-gre_detail.reserva_stock = "SI" and x-gre_detail.codmat = pCodMat NO-LOCK,
...
CASE pCodDoc:
    ...
    OTHERWISE DO:   /* OTR y O/D */
        FOR EACH B-DPEDI NO-LOCK WHERE B-DPEDI.codcia = s-CodCia
            ...
            IF B-DPEDI.CanAte >= B-DPEDI.canped THEN NEXT.
            pComprometido = pComprometido + (B-DPEDI.Factor * (B-DPEDI.CanPed - B-DPEDI.canate)).
            /*MESSAGE b-dpedi.coddoc b-dpedi.nroped b-dpedi.flgest x-flg-reserva-stock.*/
        END.
    END.
END CASE.
