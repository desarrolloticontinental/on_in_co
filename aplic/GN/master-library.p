...
DEFINE BUFFER x-ooMoviAlmacen FOR ooMoviAlmacen.
DEFINE BUFFER r-ooMoviAlmacen FOR ooMoviAlmacen.


DEFINE VAR x-crossdocking-compras AS LOG INIT NO.
...
&IF DEFINED(EXCLUDE-ALM_Genera-PHR_y_HPK) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE ALM_Genera-PHR_y_HPK Procedure 
PROCEDURE ALM_Genera-PHR_y_HPK :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       Solo se puede generar una PHR y una HPK siempre que se cumpla lo siguiente:
                El almacén despacho y Destino deben ser de la misma CD
                La división debe ser solo para PR (Picking por Rutas)
------------------------------------------------------------------------------*/
DEF INPUT PARAMETER pRowid AS ROWID.        /* OTR */
DEF OUTPUT PARAMETER pMensaje AS CHAR NO-UNDO.

/* ******************************************************************************** */
/* 30/03/2022 EN caso de INCIDENCIAS, solo las que vienen por MAL ESTADO */
/* ******************************************************************************** */

IF x-crossdocking-compras = NO THEN DO: 
    IF B-CREPO.TipMov = "INC" AND
        B-CREPO.CodRef = "INC" AND
        B-CREPO.Incidencia <> "M"
        THEN RETURN "OK".
END.
...
DEF BUFFER ORIGEN FOR gn-divi.
DEF BUFFER DESTINO FOR gn-divi.

FIND ORDENES WHERE ROWID(ORDENES) = pRowid NO-LOCK NO-ERROR.
IF NOT AVAILABLE ORDENES THEN DO:
    pMensaje = "NO se pudo ubicar el registro de la OTR".
    RETURN 'ADM-ERROR'.
END.
/* ******************************************************************************** */
/* Deben pertener a la misma sede (Transferencias Internas) */
/* ******************************************************************************** */
DEF VAR x-AlmOrigen  AS CHAR NO-UNDO.
DEF VAR x-AlmDestino AS CHAR NO-UNDO.
DEF VAR x-DivOrigen  AS CHAR NO-UNDO.
DEF VAR x-DivDestino AS CHAR NO-UNDO.

ASSIGN
    x-AlmOrigen  = ORDENES.CodAlm
    x-AlmDestino = ORDENES.CodCli.
FIND Almacen WHERE Almacen.codcia = s-codcia AND Almacen.codalm = x-AlmOrigen NO-LOCK NO-ERROR.
IF NOT AVAILABLE Almacen THEN RETURN 'ADM-ERROR'.
x-DivOrigen = Almacen.coddiv.
FIND Almacen WHERE Almacen.codcia = s-codcia AND Almacen.codalm = x-AlmDestino NO-LOCK NO-ERROR.
IF NOT AVAILABLE Almacen THEN RETURN 'ADM-ERROR'.
x-DivDestino = Almacen.coddiv.

/* OJO >>> 28/03/2022 Va a haber para transf internas y externas 
IF x-DivOrigen <> x-DivDestino THEN RETURN 'OK'.
*/
...
/* 28/03/2022 De acuerdo al destino */
DEF VAR pObserv AS CHAR NO-UNDO.
IF x-DivOrigen <> x-DivDestino THEN pObserv = "TRF EXT".
ELSE pObserv = "TRF INT".
pObserv = pObserv + " - " + x-DivDestino.
FIND gn-divi WHERE gn-divi.codcia = s-codcia
    AND gn-divi.coddiv = x-DivDestino NO-LOCK NO-ERROR.
IF AVAILABLE gn-div THEN pObserv = pObserv + " " + GN-DIVI.DesDiv.
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    FIND FIRST Di-RutaC WHERE DI-RutaC.CodCia = s-codcia AND
        ...
        /*DI-RutaC.Observ BEGINS "TRANSFERENCIA DIRECTA"*/
        NO-LOCK NO-ERROR.
    ...
    IF s-adm-new-record = YES THEN DO:
        ...
        RELEASE FacCorre.
/*         ASSIGN                                         */
/*             DI-RutaC.Observ = "TRANSFERENCIA DIRECTA". */
    END.
    ELSE DO:
...
CASE pCodDoc:
    ...
    END.
    WHEN "CDC" THEN DO:
        /* Cross Docking Compras */
        RUN Genera-OTR-cdc(INPUT pRowid, OUTPUT pFechaEntrega, OUTPUT pMensaje).
...
&IF DEFINED(EXCLUDE-Genera-OTR-cdc) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Genera-OTR-cdc Procedure 
PROCEDURE Genera-OTR-cdc :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEFINE VAR x-coddoc AS CHAR INIT 'OTR'.
DEFINE VAR x-div-despacho AS CHAR NO-UNDO.
DEFINE VAR I-NPEDI AS INT.

DEFINE VAR x-mensaje2 AS CHAR.

x-crossdocking-compras = YES.

/* Validaciones */
FIND FIRST x-ooMoviAlmacen WHERE ROWID(x-ooMoviAlmacen) = pRowId NO-LOCK NO-ERROR.
IF NOT AVAILABLE x-ooMoviAlmacen THEN DO:
    pMensaje = "No se pudo ubicar el registro de ingreso x compra".
    RETURN "ADM-ERROR".
END.

IF TRUE <> (x-ooMoviAlmacen.almfinal > "")  THEN DO:
    pMensaje = "El registro Almcen:" + x-ooMoviAlmacen.codalm + " Tipo:" + x-ooMoviAlmacen.tipmov + 
                " Cod.Mov:" + STRING(x-ooMoviAlmacen.codmov) + " Serie:" + STRING(x-ooMoviAlmacen.nroser) + 
                " Nro.Doc:" + STRING(x-ooMoviAlmacen.nrodoc) + " No tiene configurado el Almacen final".
    RETURN "ADM-ERROR".
END.

FIND FIRST Almacen WHERE Almacen.codcia = s-codcia
    AND Almacen.codalm = x-ooMoviAlmacen.codalm NO-LOCK NO-ERROR.
IF NOT AVAILABLE almacen THEN DO:
    pMensaje = 'Almacén de despacho ' + x-ooMoviAlmacen.codalm + ' NO existe'.
    RETURN "ADM-ERROR".
END.
x-div-despacho = Almacen.CodDiv.

FIND FIRST Almacen WHERE Almacen.codcia = s-codcia
    AND Almacen.codalm = x-ooMoviAlmacen.almfinal NO-LOCK NO-ERROR.
IF NOT AVAILABLE almacen THEN DO:
    pMensaje = 'Almacén final ' + x-ooMoviAlmacen.almfinal + ' NO existe'.
    RETURN "ADM-ERROR".
END.
FIND gn-divi WHERE gn-divi.codcia = s-codcia
    AND gn-divi.coddiv = Almacen.CodDiv
    NO-LOCK NO-ERROR.
IF NOT AVAILABLE gn-divi THEN DO:
    pMensaje = 'División ' + Almacen.CodDiv + ' del almacén destino(" + x-ooMoviAlmacen.almfinal + ") NO configurada'.
    RETURN "ADM-ERROR".
END.
...
/* ************************************************************************************** */
FIND FIRST FacCorre WHERE FacCorre.CodCia = S-CODCIA AND
    FacCorre.CodDiv = x-div-despacho AND
    FacCorre.CodDoc = x-CODDOC AND
    FacCorre.FlgEst = YES
    NO-LOCK NO-ERROR.
IF NOT AVAILABLE FacCorre THEN DO:
    pMensaje = "Codigo de Documento " + x-coddoc + " No configurado para la division " + x-div-despacho.
    RETURN "ADM-ERROR".
END.
/*
/* Datos de la División de Despacho */
FIND gn-divi WHERE gn-divi.codcia = s-codcia
    AND gn-divi.coddiv = x-div-despacho.
    NO-LOCK NO-ERROR.
IF NOT AVAILABLE gn-divi THEN DO:
    pMensaje = 'División ' + x-div-despacho + ' NO configurada'.
    RETURN "ADM-ERROR".
END.
...
RUN Genera-OTR-cdc-grabar(INPUT pRowid, OUTPUT pFechaEntrega, OUTPUT pMensaje, OUTPUT x-mensaje2).
...
&IF DEFINED(EXCLUDE-Genera-OTR-cdc-grabar) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Genera-OTR-cdc-grabar Procedure 
PROCEDURE Genera-OTR-cdc-grabar :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
DEF INPUT PARAMETER pRowid AS ROWID NO-UNDO.                    /* ROWID del documento */
DEF OUTPUT PARAMETER pFechaEntrega AS DATE NO-UNDO.             /* Devuelve la Fecha de Entrega Programado */
DEF OUTPUT PARAMETER pMensaje AS CHAR NO-UNDO.        
DEF OUTPUT PARAMETER pMensaje2 AS CHAR NO-UNDO.        
...
DEFINE VAR x-divdes AS CHAR NO-UNDO.
...
x-codalm    = x-ooMoviAlmacen.codalm.
x-almfinal  = x-ooMoviAlmacen.almfinal.
x-tipmov    = x-ooMoviAlmacen.tipmov.
x-codmov    = x-ooMoviAlmacen.codmov.
x-nroser    = x-ooMoviAlmacen.nroser.
x-nrodoc    = x-ooMoviAlmacen.nrodoc.
...
x-divdes = Almacen.CodDiv.
lDivDespacho = Almacen.CodDiv.
lAlmDespacho = x-ooMoviAlmacen.codalm.
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    */
    ...
    ASSIGN
        iNroSKU = 0
        iPeso = 0.
    FOR EACH r-ooMoviAlmacen WHERE r-ooMoviAlmacen.codalm = x-codalm AND 
                                    r-ooMoviAlmacen.tipmov = x-tipmov AND 
                                    r-ooMoviAlmacen.codmov = x-codmov AND
                                    r-ooMoviAlmacen.nroser = x-nroser AND
                                    r-ooMoviAlmacen.nrodoc = x-nrodoc,
                                    FIRST Almmmatg OF r-ooMoviAlmacen NO-LOCK:
        iNroSKU = iNroSKU + 1.
        iPeso = iPeso + (r-ooMoviAlmacen.CanDes * r-ooMoviAlmacen.Factor) * Almmmatg.PesMat.                                    
    END.
    */
    /* **************************************************************************** */
    ...
    FIND Almacen WHERE Almacen.codcia = s-codcia AND 
        Almacen.codalm = x-almfinal NO-LOCK NO-ERROR.
    /* **************************************************************************** */
    CREATE Faccpedi.
    ASSIGN 
        ...
        FacCPedi.Glosa  = x-ooMoviAlmacen.observ
        FacCPedi.codmon = x-ooMoviAlmacen.codmon
        FacCPedi.tpocmb = x-ooMoviAlmacen.tpocmb
        NO-ERROR.
    IF ERROR-STATUS:ERROR = YES THEN DO:
    ...
    */
    ...
    ASSIGN FacCPedi.VtaPuntual = NO /*B-CREPO.VtaPuntual*/.
    ...
    */
    ASSIGN FacCPedi.DivDes = x-divdes.
    ...
    FOR EACH r-ooMoviAlmacen WHERE r-ooMoviAlmacen.codalm = x-codalm AND 
        ...
        CREATE facdpedi.
            ...
            ASSIGN facdpedi.codcia = 1
                    facdpedi.factor = r-ooMoviAlmacen.factor
                    facdpedi.undvta = Almmmatg.UndBas
                    facdpedi.tipvta = ""
                    facdpedi.codmat = r-ooMoviAlmacen.codmat
                    facdpedi.canped = r-ooMoviAlmacen.candes
                    facdpedi.preuni = 0
                    facdpedi.implin = 0
                    facdpedi.aftigv = YES
                    facdpedi.aftisc = NO
                    ...
                    /*facdpedi.almdes = x-almfinal*/
                    facdpedi.almdes = lAlmDespacho
                    facdpedi.canpick = 0
                    facdpedi.preuni  = r-ooMoviAlmacen.linpreuni.
    END.
    /*
    IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
    ...
    */
    ...
    END.
    /* IMPORTES en S/  ----------- Costos de reposicionnnn revisarrrr */
    ASSIGN
    ...
    */
    ...
    /* *************************************************************************** */
    RUN ALM_Genera-PHR_y_HPK (INPUT ROWID(Faccpedi),    /* OTR */
                              OUTPUT pMensaje).   
    IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
