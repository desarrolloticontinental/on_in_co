...
DEFINE TEMP-TABLE T-Detalle
    ...
    FIELD Atendido  AS DEC FORMAT '->>>,>>>,>>>,>>9.99' LABEL "Import. Atendido Ped.Logístico"
    FIELD PCO       AS DEC FORMAT '->>>,>>>,>>>,>>9.99' LABEL "Import. Atendido Pre-Ped.Logístico"
    ...
    FIELD CanAtePCO LIKE Facdpedi.canate LABEL "Cantidad Atendida Pre-Ped.Logístico"
    FIELD CanAte LIKE Facdpedi.canate LABEL "Cantidad Atendida Ped.Logístico"
    ...
    FIELD PesoAte   AS DEC LABEL "Peso Atendido Ped.Logístico"
    FIELD PesoAtePCO AS DEC LABEL "Peso Atendido Pre-Ped.Logístico"
    FIELD ImpAte    AS DEC FORMAT '->>>,>>>,>>>,>>9.99' LABEL "Import. Atendido Ped.Logístico"
    FIELD ImpAtePCO AS DEC FORMAT '->>>,>>>,>>>,>>9.99' LABEL "Import. Atendido Pre-Ped.Logístico"
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD fImpxPCO B-table-Win 
FUNCTION fImpxPCO RETURNS DECIMAL
  ( /* parameter-definitions */ )  FORWARD.
...
/* Query rebuild information for BROWSE br_table
     ...
     _TblOptList       = ", FIRST"
     _OrdList          = "Temp-Tables.COTIZACION.NroPed|yes"
...
DO:
  ...
  IF TRUE <> (x-CodDiv > '') THEN DO:
      MESSAGE 'NO ha seleccionado ninguna División Orihen' SKIP
          'Confirma la Aplicación del Filtro?'
...
FOR EACH gn-divi NO-LOCK WHERE x-CodDiv = '' OR LOOKUP(gn-divi.CodDiv, x-CodDiv) > 0,
    ...
    AND {&Condicion}:
    IF NOT (TOGGLE-1 = YES OR LOOKUP(FacCPedi.FlgEst, "A,V") = 0) THEN NEXT.
    ...
    IF NOT (x-FlgEst = 'Todos' OR LOOKUP(Faccpedi.FlgEst, x-FlgEst) > 0) THEN NEXT.
    ...
    /* 22/10/2024: Separamos PCO */
    ASSIGN
        COTIZACION.AcuBon[6] = fImpxPCO()
        COTIZACION.AcuBon[4] = COTIZACION.AcuBon[4] - COTIZACION.AcuBon[6]
        .
...
------------------------------------------------------------------------------*/
    ...
    REPEAT WHILE AVAILABLE COTIZACION:
        ...
        BUFFER-COPY Faccpedi
            ...
            T-Detalle.PCO = COTIZACION.AcuBon[6]
    ...
    lOptions = lOptions + "coddiv,libre_c01,nroped,fchped,fchven,fchent,nomcli," +
        "lincre,sdoant,codven,imptot,estado,avance,items,peso,cuota,atendido,pendiente,codalm,pco".
...
------------------------------------------------------------------------------*/
    ...
    REPEAT WHILE AVAILABLE COTIZACION:
        ...
        FOR EACH DETALLE OF Faccpedi NO-LOCK,FIRST Almmmatg OF DETALLE NO-LOCK :
            ...
            /* 24/10/2024: Carla Tenazoa */
            /* Calculamos PCO */
            FOR EACH PCO NO-LOCK WHERE PCO.codcia = Faccpedi.codcia AND
                PCO.codref = Faccpedi.coddoc AND      /* COT */
                PCO.nroref = Faccpedi.nroped AND
                PCO.coddoc = 'PCO' AND
                PCO.flgest = "P",
                EACH B-DPEDI OF PCO NO-LOCK WHERE B-DPEDI.codmat = DETALLE.codmat:
                /* Cantidades */
                ASSIGN
                    T-Detalle.CanAtePCO = T-Detalle.CanAtePCO + B-DPEDI.CanPed
                    T-Detalle.CanAte = T-Detalle.CanAte - B-DPEDI.CanPed
                    .
                /* Peso */
                ASSIGN
                    T-Detalle.PesoAtePCO = T-Detalle.PesoAtePCO + (B-DPEDI.CanPed * Almmmatg.PesMat)
                    T-Detalle.PesoAte = T-Detalle.PesoAte - (B-DPEDI.CanPed * Almmmatg.PesMat)
                    .
                /* Importes */
                ASSIGN
                    T-Detalle.ImpAtePCO = T-Detalle.ImpAtePCO + (B-DPEDI.CanPed * (DETALLE.ImpLin / DETALLE.CanPed))
                    T-Detalle.ImpAte = T-Detalle.ImpAte - (B-DPEDI.CanPed * (DETALLE.ImpLin / DETALLE.CanPed))
                    .

            END.
            ASSIGN
                T-Detalle.CantxAte = (T-Detalle.CanPed - T-Detalle.CanAte)
                T-Detalle.PesoxAte = (T-Detalle.CanPed - T-Detalle.CanAte) * Almmmatg.PesMat
                T-Detalle.ImpxAte = (T-Detalle.CanPed - T-Detalle.CanAte) * (DETALLE.ImpLin / DETALLE.CanPed)
                .
...
/*         "lincre,sdoant,lugent,distent,codven,estado,peso,pendiente,codalm," +                      */
/*         "nroitm,codmat,desmat,desmar,undvta,pesounit,cubiunit,canped,pesolin,preuni,dctomanual," + */
/*         "dctoevento,dctovolprom,implin,pesoate,impate,cantxate,pesoxate,impxate," +                */
/*         "canatepco,canate,pesoatepco,ImpAtePCO".   /*atendido,PCO*/                                */
    lOptions = lOptions + "coddiv,libre_c01,nroped,fchped,fchven,fchent,codcli,nomcli," +
        "lincre,sdoant,lugent,distent,codven,estado,peso,codalm," +
        "nroitm,codmat,desmat,desmar,undvta,pesounit,cubiunit,canped,pesolin,preuni,dctomanual," +
        "dctoevento,dctovolprom,implin,pesoate,impate,cantxate,pesoxate,impxate," +
        "canatepco,canate,pesoatepco,ImpAtePCO".   /*atendido,PCO*/
    pOptions = pOptions + CHR(1) + lOptions.
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH Facdpedi OF Faccpedi NO-LOCK:
      ASSIGN
          xImpAte = xImpAte + (FacDPedi.cImporteTotalConImpuesto / facdpedi.CanPed) * Facdpedi.CanAte.
  END.
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH Facdpedi OF Faccpedi NO-LOCK:
      ASSIGN
          xImpTot = xImpTot + 
          (FacDPedi.cImporteTotalConImpuesto / Facdpedi.CanPed) * (Facdpedi.CanPed - Facdpedi.CanAte).
  END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION fImpxPCO B-table-Win 
FUNCTION fImpxPCO RETURNS DECIMAL
  ( /* parameter-definitions */ ) :
/*------------------------------------------------------------------------------
  Purpose:  
...
------------------------------------------------------------------------------*/
  ...
  FOR EACH PCO NO-LOCK WHERE PCO.codcia = Faccpedi.codcia AND
      ...
      PCO.coddoc = 'PCO' AND
      PCO.flgest = "P",
      EACH DETALLE OF PCO NO-LOCK,
      FIRST Facdpedi OF Faccpedi NO-LOCK WHERE Facdpedi.codmat = DETALLE.codmat:
      ASSIGN
          xImpTot = xImpTot + (FacDPedi.cImporteTotalConImpuesto / Facdpedi.CanPed) * DETALLE.CanPed.
  END.
