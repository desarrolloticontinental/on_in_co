...
/*IF USERID("DICTDB") = "MASTER" THEN s-User-Id = "VTA-63".*/
...
&Scoped-Define ENABLED-OBJECTS F-Numero RECT-23 
...
DEFINE FRAME FRAME-A
     ...
     RECT-23 AT ROW 1 COL 1 WIDGET-ID 2
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Devuelve-Registro q-tables 
PROCEDURE Devuelve-Registro :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
------------------------------------------------------------------------------*/
  ...
  CASE s-coddoc:
      WHEN 'COT' THEN s-Titulo = 'Cotizaciones'.
      WHEN 'PED' THEN s-Titulo = 'Pedidos'.
      WHEN 'O/D' THEN s-Titulo = 'Ordenes de Despacho'.
  END CASE.

  DO ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE ON ENDKEY UNDO, LEAVE:
    RUN vta2/c-cotizacion (s-Titulo).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Posiciona-Registro q-tables 
...
DEF INPUT PARAMETER pRowid AS ROWID.
   ...
   FIND {&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}} WHERE ROWID({&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}}) = pRowid 
       NO-LOCK NO-ERROR.
   IF AVAIL {&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}} THEN DO:
      REPOSITION {&QUERY-NAME}  TO ROWID pRowid NO-ERROR.
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
FIND FIRST FacCPedi WHERE {&CONDICION} AND
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE Ultimo-Registro q-tables 
PROCEDURE Ultimo-Registro :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
FIND LAST {&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}} WHERE {&CONDICION} NO-LOCK NO-ERROR.
IF AVAILABLE {&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}} THEN DO:
    REPOSITION {&QUERY-NAME}  TO ROWID ROWID({&FIRST-TABLE-IN-QUERY-{&QUERY-NAME}}) NO-ERROR.
    IF NOT ERROR-STATUS:ERROR THEN RUN dispatch IN THIS-PROCEDURE ('get-next':U).
END.
