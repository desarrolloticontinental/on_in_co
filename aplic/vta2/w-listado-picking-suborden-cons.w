...
DEFINE NEW SHARED VAR s-CodDoc AS CHAR INIT 'ODC'.
...
&Scoped-Define ENABLED-OBJECTS RECT-1 RECT-2 RADIO-SET-CodDoc ChkChequeo ~
chbxImpresos txtDesde txtHasta BtnExcel RADIO-SET-1 BUTTON-13 rsSectores ~
txtSectoresImprimir BtnDone BUTTON-15 
&Scoped-Define DISPLAYED-OBJECTS RADIO-SET-CodDoc ChkChequeo chbxImpresos ~
txtDesde txtHasta RADIO-SET-1 rsSectores txtSectores txtLeyenda ~
...
DEFINE VARIABLE h_b-dimprime-odc AS HANDLE NO-UNDO.
...
DEFINE VARIABLE RADIO-SET-CodDoc AS CHARACTER INITIAL "ODC" 
     ...
     RADIO-BUTTONS 
          "Orden de Despacho Consolidada (ODC)", "ODC",
"Orden de TRansferencia Consolidada (OTC)", "OTC"
     SIZE 87 BY 1
...
DO:
    ASSIGN txtDesde txtHasta chkchequeo chbxImpresos rsSectores.
    ltxtDesde = txtDesde.
    ltxtHasta = txtHasta.
    ...
    RUN dispatch IN h_b-dimprime-odc ('open-query':U).
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/b-listado-picking-suborden-cons.w':U ,
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'vta2/b-dimprime-odc.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b-dimprime-odc ).
       RUN set-position IN h_b-dimprime-odc ( 12.38 , 2.00 ) NO-ERROR.
       RUN set-size IN h_b-dimprime-odc ( 11.92 , 111.00 ) NO-ERROR.

       /* Links to SmartBrowser h_b-dimprime-odc. */
       RUN add-link IN adm-broker-hdl ( h_b-picking-ordenes-todos , 'Record':U , h_b-dimprime-odc ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-picking-ordenes-todos ,
             rsSectores:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b-dimprime-odc ,
...
OCXFile = SEARCH( "w-listado-picking-suborden-cons.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "w-listado-picking-suborden-cons.wrx":U SKIP(1)
...
------------------------------------------------------------------------------*/
  DISPLAY RADIO-SET-CodDoc ChkChequeo chbxImpresos txtDesde txtHasta RADIO-SET-1 
          rsSectores txtSectores txtLeyenda txtSectoresImprimir txtArtSinPeso 
      WITH FRAME F-Main IN WINDOW sW-Win.
  ENABLE RECT-1 RECT-2 RADIO-SET-CodDoc ChkChequeo chbxImpresos txtDesde 
         txtHasta BtnExcel RADIO-SET-1 BUTTON-13 rsSectores txtSectoresImprimir 
         BtnDone BUTTON-15 
      WITH FRAME F-Main IN WINDOW sW-Win.
