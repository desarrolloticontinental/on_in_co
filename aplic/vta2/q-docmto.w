...
&SCOPED-DEFINE CONDICION ( CcbCDocu.CodCia = S-CODCIA AND ~
                           ...
                           CcbCDocu.NroDoc BEGINS STRING(S-NROSER, ENTRY(1,x-Formato,'-')) )
...
&Scoped-Define ENABLED-OBJECTS F-Numero RECT-2 
...
/* Query rebuild information for QUERY Query-Main
     ...
     _Where[1]         = "{&CONDICION}"
     _Design-Parent    is WINDOW q-tables @ ( 1.19 , 13.43 )
*/  /* QUERY Query-Main */
&ANALYZE-RESUME
...
------------------------------------------------------------------------------*/
  ...
  DO ON ERROR UNDO, LEAVE ON STOP UNDO, LEAVE ON ENDKEY UNDO, LEAVE:
    RUN lkup/c-doctpo-01 ('Documentos').
...
IF output-var-1 = ? THEN DO:
    FIND CcbCDocu WHERE CcbCDocu.CodCia = S-CODCIA 
        AND CcbCDocu.CodDoc = s-coddoc 
        AND CcbCDocu.NroDoc = STRING(s-NroSer,'999') +
                                STRING(f-Numero,'999999')
        NO-LOCK NO-ERROR.
    IF AVAILABLE CcbCDocu THEN output-var-1 = ROWID(CcbCDocu).
END.
IF OUTPUT-VAR-1 <> ? THEN DO:
