...
&Scoped-define QUERY-STRING-Query-Main FOR EACH FacCPedi WHERE ~{&KEY-PHRASE} ~
 ...
 AND (integral.FacCPedi.FlgEst = "E" ~
 OR FacCPedi.FlgEst = "P") ~
 AND FacCPedi.NroPed BEGINS STRING(s-NroSer,'999') ~
 AND FacCPedi.fchven >= TODAY ~
 AND LOOKUP(integral.FacCPedi.TpoPed, "M") = 0 NO-LOCK ~
...
&Scoped-Define ENABLED-OBJECTS RECT-2 F-Numero 
...
/* Query rebuild information for QUERY Query-Main
 ...
 AND (integral.FacCPedi.FlgEst = ""E""
 OR integral.FacCPedi.FlgEst = ""P"")
 AND integral.FacCPedi.NroPed BEGINS STRING(s-NroSer,'999')
 AND integral.FacCPedi.fchven >= TODAY
 AND LOOKUP(integral.FacCPedi.TpoPed, ""M"") = 0"
     _Design-Parent    is WINDOW q-tables @ ( 1 , 9.29 )
*/  /* QUERY Query-Main */
&ANALYZE-RESUME
