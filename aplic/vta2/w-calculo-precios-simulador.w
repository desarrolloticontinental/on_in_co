...
DEFINE TEMP-TABLE ttListaPreciosEventosPromocional NO-UNDO
...
DEFINE TEMP-TABLE ttListaPreciosEventosVolumen NO-UNDO
    FIELDS  tItem       AS  CHAR    FORMAT 'x(10)'  LABEL "Item"
    FIELDS  tCodmat     AS  CHAR    FORMAT 'x(6)'   LABEL "Codigo"
    FIELDS  tDesMat     AS  CHAR    FORMAT 'x(80)'  LABEL "Descripcion"
    FIELDS  tUndVta     AS  CHAR    FORMAT 'x(5)'   LABEL "Und.Vta"
...
DEFINE TEMP-TABLE ttListaPreciosPW NO-UNDO
...
DEFINE IMAGE IMAGE-1 FILENAME "IMG\auditor.ico" SIZE 12 BY 1.5.
...
DEFINE VAR x-prevtafin AS DEC INIT 0.
...
FOR EACH factabla WHERE factabla.codcia = s-codcia AND
    ...
    LOOP_DTL_EDVXSALDOD:
    FOR EACH x-factabla NO-LOCK WHERE x-factabla.codcia = s-codcia AND 
        x-factabla.tabla = 'EDVXSALDOD' AND 
        x-factabla.codigo BEGINS factabla.codigo:
        x-codmat = x-factabla.campo-c[1].
        FIND FIRST almmmatg WHERE almmmatg.codcia = s-codcia AND
            almmmatg.codmat = x-codmat AND 
            almmmatg.tpoart = "A" NO-LOCK NO-ERROR.
        ...
        Fi-Mensaje = "PROCESANDO: " + Almmmatg.codmat.
        ...
        REPEAT x-sec = 1 TO 10:
            IF factabla.Valor[x-sec] > 0 THEN DO:
                ...
                IF x-hdr = YES THEN DO:
                    ...
                    ASSIGN 
                        ttListaPreciosEventosVolumen.tItem = "" /*x-item*/
                    ...
                    IF x-flete-unitario > 0 THEN DO:
                        ...
                        ASSIGN
                            ttListaPreciosEventosVolumen.tPrecFinal = x-PreVta + (x-flete-unitario / (1 - y-dsctos / 100)).
                    END.
                ...
                IF x-flete-unitario > 0 THEN DO:
                    /* El flete afecta al precio unitario resultante */
                    ASSIGN
                        x-PreVtaFin = x-PreVta + (x-flete-unitario / (1 - y-dsctos / 100)).     /* Nuevo valor afectado con el flete */
                END.
                /* Margen de utilidad */
                ...
                /**/
                RUN _graba_detalle(INPUT x-sec, INPUT x-cantidad, INPUT y-dsctos, INPUT x-prevta, INPUT x-PreVtaFin, INPUT x-margen).
...
PROCEDURE _graba_detalle:
    ...
    DEFINE INPUT PARAMETER xPreVtaFin AS DEC.
    ...
    IF xsec = 1 THEN tPFinal01 = IF(xdsctos <> 0) THEN xPreVtaFin * ( 1 - xdsctos / 100) ELSE xPreVtaFin.
    ...
    IF xsec = 2 THEN tPFinal02 = IF(xdsctos <> 0) THEN xPreVtaFin * ( 1 - xdsctos / 100) ELSE xPreVtaFin.
    ...
    IF xsec = 3 THEN tPFinal03 = if(xdsctos <> 0) THEN xPreVtaFin * ( 1 - xdsctos / 100) ELSE xPreVtaFin.
    ...
    IF xsec = 4 THEN tPFinal04 = if(xdsctos <> 0) THEN xPreVtaFin * ( 1 - xdsctos / 100) ELSE xPreVtaFin.
    ...
    IF xsec = 5 THEN tPFinal05 = if(xdsctos <> 0) THEN xPreVtaFin * ( 1 - xdsctos / 100) ELSE xPreVtaFin.
    ...
    IF xsec = 6 THEN tPFinal06 = if(xdsctos <> 0) THEN xPreVtaFin * ( 1 - xdsctos / 100) ELSE xPreVtaFin.
    ...
    IF xsec = 7 THEN tPFinal07 = if(xdsctos <> 0) THEN xPreVtaFin * ( 1 - xdsctos / 100) ELSE xPreVtaFin.
    ...
    IF xsec = 8 THEN tPFinal08 = if(xdsctos <> 0) THEN xPreVtaFin * ( 1 - xdsctos / 100) ELSE xPreVtaFin.
    ...
    IF xsec = 9 THEN tPFinal09 = if(xdsctos <> 0) THEN xPreVtaFin * ( 1 - xdsctos / 100) ELSE xPreVtaFin.
    ...
    IF xsec = 10 THEN tPFinal10 = if(xdsctos <> 0) THEN xPreVtaFin * ( 1 - xdsctos / 100) ELSE xPreVtaFin.
        ...
        REPEAT x-sec = 1 TO 10:
            IF factabla.Valor[x-sec] > 0 THEN DO:
                ...
                ASSIGN
                    ttListaPreciosEventosVolumen.tminimo = x-minimo.
                /**/
                RUN _graba_detalle_2(INPUT x-sec, INPUT x-cantidad, INPUT y-dsctos, INPUT x-prevta, INPUT x-PreVtaFin, INPUT x-Margen).
            END.
        END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE dscto-x-vol-y-prom W-Win 
PROCEDURE dscto-x-vol-y-prom :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
FOR EACH almmmatg NO-LOCK WHERE almmmatg.codcia = s-codcia AND 
    ...
    almmmatg.tpoart = "A" AND       /* Solo activos */
    (x-subfamilia = 'Todos' OR almmmatg.subfam = x-subfamilia):
    /* Un proveedor especifico */
    IF RADIO-SET-proveedor = 2 THEN DO:
        IF almmmatg.codpr1 <> fill-in-proveedor THEN NEXT.
    END.
    FIND FIRST gn-prov WHERE gn-prov.codcia = pv-codcia AND gn-prov.codpro = almmmatg.codpr1 NO-LOCK NO-ERROR.
    ...
    IF Almmmatg.CHR__02 = "T" AND pTipoVenta = "EVENTOS" THEN DO:
        /* Busca en el "pedido" de evento: Solo TERCEROS */
        FIND FIRST Almcatvtad WHERE Almcatvtad.codcia = s-codcia AND
            Almcatvtad.coddiv = x-lista-de-precio AND
            Almcatvtad.codmat = Almmmatg.codmat AND
            CAN-FIND(FIRST ALmcatvtac OF Almcatvtad NO-LOCK)
    ...
    CASE TRUE:
        WHEN radio-set-calculo > 2 AND AVAILABLE VtaListaMay THEN DO:   /* Listas por Divisón */
            /* VOLUMEN */
            x-hdr = YES.
            REPEAT x-sec = 1 TO 10:
                IF vtalistamay.DtoVolR[x-sec] > 0 THEN DO:
                    ...
                    IF X-TIPDTO = "VOL" THEN DO:
                        ...
                        IF x-flete-unitario > 0 THEN DO:
                            /* El flete afecta al precio unitario resultante */
                            ASSIGN
                                x-PreVtaFin = x-PreVta + (x-flete-unitario / (1 - y-dsctos / 100)).     /* Nuevo valor afectado conel flete */
                        END.
                        ...
                        x-PreVtaFin = if(y-dsctos <> 0) THEN x-PreVtaFin * ( 1 - y-dsctos / 100) ELSE x-PreVtaFin.
                        ...
                        ASSIGN 
                            ttListaPreciosPW.tItem = String(x-itemPW)
                ...
                END.
            END.
        END.    /* WHEN radio-set-calculo > 2 AND AVAILABLE VtaListaMay */
        OTHERWISE DO:
            x-cantidad = 1.
            x-nro-dec = 4.
            x-prebas = 0.
            ...
            IF radio-set-calculo = 1 OR X-TIPDTO BEGINS "PRO" THEN DO:
                ...
                ASSIGN 
                    ttListaPreciosEventosPromocional.tItem = "" /*x-item*/
                ...
                IF x-flete-unitario > 0 THEN DO:
                    ...
                    ASSIGN
                        ttListaPreciosEventosVolumen.tPrecFinal = x-PreVta + (x-flete-unitario / (1 - y-dsctos / 100))
                        x-PreVtaFin = ttListaPreciosEventosVolumen.tPrecFinal.     /* Nuevo valor afectado conel flete */
                END.
                ...
                x-filer1 = x-PreVtaFin * ( 1 - (x-despacho-anticipado / 100) ).
                x-filer2 = x-filer1 * ( 1 - (x-descuento-financiero / 100) ).
                x-filer3 = x-filer2 * ( 1 - (x-descuento-rebate / 100) ).
                ...
                ASSIGN 
                    ...
                    ttListaPreciosPW.tUndVta = x-undvta
                    ttListaPreciosPW.tMarca = almmmatg.desmar
                    ttListaPreciosPW.tPrecLista = STRING(x-PreVtaFin,"->,>>>,>>9.9999")
                    ttListaPreciosPW.tPrecLin13 = IF (almmmatg.codfam = '013') THEN STRING(x-PreVtaFin,"->,>>>,>>9.9999") ELSE ' '
...
DEF INPUT PARAMETER x-PreVtaFin AS DECI.
DEF INPUT PARAMETER x-Margen AS DECI.
DEF INPUT PARAMETER x-Minimo AS DECI.

/*MESSAGE ttListaPreciosEventosVolumen.tcodmat x-cantidad x-sec y-dsctos x-prevta x-prevtafin.*/
...
IF x-sec = 1 THEN ttListaPreciosEventosVolumen.tPFinal01 = if(y-dsctos <> 0) THEN x-PreVtaFin * ( 1 - y-dsctos / 100) ELSE x-PreVtaFin.
...
IF x-sec = 2 THEN ttListaPreciosEventosVolumen.tPFinal02 = if(y-dsctos <> 0) THEN x-PreVtaFin * ( 1 - y-dsctos / 100) ELSE x-PreVtaFin.
...
IF x-sec = 3 THEN ttListaPreciosEventosVolumen.tPFinal03 = if(y-dsctos <> 0) THEN x-PreVtaFin * ( 1 - y-dsctos / 100) ELSE x-PreVtaFin.
...
IF x-sec = 4 THEN ttListaPreciosEventosVolumen.tPFinal04 = if(y-dsctos <> 0) THEN x-PreVtaFin * ( 1 - y-dsctos / 100) ELSE x-PreVtaFin.
...
IF x-sec = 5 THEN ttListaPreciosEventosVolumen.tPFinal05 = if(y-dsctos <> 0) THEN x-PreVtaFin * ( 1 - y-dsctos / 100) ELSE x-PreVtaFin.
...
IF x-sec = 6 THEN ttListaPreciosEventosVolumen.tPFinal06 = if(y-dsctos <> 0) THEN x-PreVtaFin * ( 1 - y-dsctos / 100) ELSE x-PreVtaFin.
...
IF x-sec = 7 THEN ttListaPreciosEventosVolumen.tPFinal07 = if(y-dsctos <> 0) THEN x-PreVtaFin * ( 1 - y-dsctos / 100) ELSE x-PreVtaFin.
...
IF x-sec = 8 THEN ttListaPreciosEventosVolumen.tPFinal08 = if(y-dsctos <> 0) THEN x-PreVtaFin * ( 1 - y-dsctos / 100) ELSE x-PreVtaFin.
...
IF x-sec = 9 THEN ttListaPreciosEventosVolumen.tPFinal09 = if(y-dsctos <> 0) THEN x-PreVtaFin * ( 1 - y-dsctos / 100) ELSE x-PreVtaFin.
...
IF x-sec = 10 THEN ttListaPreciosEventosVolumen.tPFinal10 = if(y-dsctos <> 0) THEN x-PreVtaFin * ( 1 - y-dsctos / 100) ELSE x-PreVtaFin.
