...
AND faccpedi.coddiv = input-var-2 ~
AND faccpedi.coddoc = input-var-1 ~
AND faccpedi.nroped BEGINS STRING(s-nroser, '999') ~
AND ( input-var-3 = '' OR faccpedi.usuario = input-var-3 ) ~
AND (pCodDiv = '' OR Faccpedi.Libre_c01 = pCodDiv) ~
...
FacCPedi.fchven FacCPedi.NomCli 
...
<SORTBY-OPTIONS>
Código|y||INTEGRAL.FacCPedi.CodCia|yes,INTEGRAL.FacCPedi.CodDoc|yes,INTEGRAL.FacCPedi.NroPed|yes
Descripción|||INTEGRAL.FacCPedi.CodCia|yes,INTEGRAL.FacCPedi.CodDoc|yes,INTEGRAL.FacCPedi.NomCli|yes
</SORTBY-OPTIONS> 
<SORTBY-RUN-CODE>
************************
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table L-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      FacCPedi.fchven FORMAT "99/99/9999":U
      FacCPedi.NomCli COLUMN-LABEL "Cliente" FORMAT "x(50)":U
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 69.14 BY 8.62
...
"FacCPedi.NroPed" "<<Cotizacion>>" ? "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   = INTEGRAL.FacCPedi.FchPed
     _FldNameList[3]   > INTEGRAL.FacCPedi.fchven
"FacCPedi.fchven" ? "99/99/9999" "date" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
    ...
    WHEN 'Nombres que inicien con':U THEN DO:
       ...
       CASE RETURN-VALUE:
         WHEN 'Código':U THEN DO:
           &Scope SORTBY-PHRASE BY FacCPedi.CodCia BY FacCPedi.CodDoc BY FacCPedi.NroPed
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         WHEN 'Descripción':U THEN DO:
           &Scope SORTBY-PHRASE BY FacCPedi.CodCia BY FacCPedi.CodDoc BY FacCPedi.NomCli
           {&OPEN-QUERY-{&BROWSE-NAME}}
         END.
         OTHERWISE DO:
