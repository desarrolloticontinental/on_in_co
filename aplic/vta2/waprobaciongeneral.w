...
/* Sintaxis:
    pParametro: lista de parámetros separados por '|'
                <CodDoc<|<FlgEst>[|CE]
...
END.
IF s-FlgEst = "X" THEN DO:  /* Solo para aprobación de Créditos y Cobranzas */
    s-CndVta = "NCE".   /* NO Contraentrega */
    IF NUM-ENTRIES(pParametro,'|') >= 3 AND ENTRY(3,pParametro,'|') <> ''
        THEN s-CndVta = ENTRY(3,pParametro,'|').
    /* Nuevo parámetro: s-CndVta = '900' */
END.

/* Consistencia */
IF LOOKUP(s-CodDoc, 'PED') = 0 THEN DO:
    MESSAGE 'Documento no válido:' s-coddoc SKIP 'Proceso abortado' VIEW-AS ALERT-BOX ERROR.
    RETURN ERROR.
END.
IF LOOKUP(s-FlgEst, 'T,X,W,WL,WC') = 0 THEN DO:
    MESSAGE 'Estado no válido:' s-flgest SKIP 'Proceso abortado' VIEW-AS ALERT-BOX ERROR.
    RETURN ERROR.
END.
