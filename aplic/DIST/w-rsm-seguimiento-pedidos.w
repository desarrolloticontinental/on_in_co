...
DEFINE FRAME fMain
     ...
     ChkbxGenTxt AT ROW 7.88 COL 19 WIDGET-ID 22
     txtProcesar AT ROW 8.12 COL 57 WIDGET-ID 8
     ChkbxResumen AT ROW 8.65 COL 19 WIDGET-ID 26
     "v2" VIEW-AS TEXT
          SIZE 8 BY .5 AT ROW 4.77 COL 74 WIDGET-ID 36
...
ON CHOOSE OF BUTTON-2 IN FRAME fMain /* Limpiar nombre de file TXT */
DO:
...
ON LEAVE OF txtCoddivi IN FRAME fMain /* Alguna division de venta ? */
DO:
...
DO:
  ASSIGN txtCoddivi txtdesde txthasta txtDesDivi ChkBxGenTxt ChkbxResumen txtfile.
...
IF p-Fase = 'ORDEN' THEN DO:
    ...
    FOR EACH ccbcbult WHERE ccbcbult.codcia = s-codcia AND
                            ...
                            ccbcbult.nrodoc = od_faccpedi.nroped NO-LOCK:
    ...
    FOR EACH vtacdocu WHERE vtacdocu.codcia = s-codcia AND 
                                ...
                                vtacdocu.flgest <> 'A'  NO-LOCK BY vtacdocu.fecsac BY vtacdocu.horsac :
    ...
    FOR EACH controlOD WHERE controlOD.codcia = s-codcia AND 
                                ...
                                controlOD.nrodoc = od_faccpedi.nroped NO-LOCK by controlOD.fchchq desc by controlOD.horchq DESC :
        ...
        */
        ...
        IF od_faccpedi.EmpaqEsp = YES THEN DO:
            ...
            FOR EACH vtacdocu WHERE vtacdocu.codcia = s-codcia AND vtacdocu.codref = od_faccpedi.coddoc AND
                                        vtacdocu.nroref = od_faccpedi.nroped AND 
                                        vtacdocu.codped = 'HPK' AND vtacdocu.flgest <> 'A' :
            ...
            FOR EACH vtacdocu WHERE vtacdocu.codcia = s-codcia AND vtacdocu.codref = od_faccpedi.coddoc AND
                ...
                FIND FIRST ChkTareas WHERE ChkTareas.codcia = s-codcia AND ChkTareas.coddiv = od_faccpedi.divdes AND
                                                ChkTareas.coddoc = vtacdocu.codped AND chkTareas.nroped = vtacdocu.nroped NO-LOCK NO-ERROR.
...
IF p-Fase = 'DOCUMENTO' THEN DO:
    ...
    /* Ic - 09Dic2021  */
    FIND FIRST almcmov WHERE almcmov.codcia = s-codcia AND almcmov.codref = ccbcdocu.coddoc AND almcmov.nroref = ccbcdocu.nrodoc AND 
        almcmov.flgest <> 'A' AND almcmov.tipmov = 'I' AND almcmov.codmov = 09 NO-LOCK NO-ERROR.
    IF AVAILABLE almcmov THEN DO:
        ASSIGN tt-seguimiento.tt-fini_pi = almcmov.fchdoc
...
FOR EACH x-gn-divi WHERE x-gn-divi.codcia = s-codcia AND x-gn-divi.campo-log[5] = YES NO-LOCK:  /* Solo CDs */
                        
    FOR EACH faccpedi WHERE faccpedi.codcia = s-codcia AND faccpedi.divdes = x-gn-divi.coddiv AND
                        faccpedi.coddoc = 'PED' AND faccpedi.flgest <> 'A' AND
                        (faccpedi.fchped >= txtDesde AND faccpedi.fchped <= txtHasta) NO-LOCK:
        IF NOT (txtCodDivi = "" OR faccpedi.coddiv = txtCodDivi) THEN NEXT.
        IF NOT TRUE <> (txtfile > "") THEN DO:
        ...
        FIND FIRST tabprovi WHERE gn-clie.coddept = tabprovi.coddepto AND gn-clie.codprov = tabprovi.codprovi NO-LOCK NO-ERROR.
        IF AVAILABLE tabprovi THEN lProv = tabprovi.nomprovi.
...
FOR EACH faccpedi NO-LOCK WHERE faccpedi.codcia = s-codcia AND 
    ...
    FIND FIRST gn-divi WHERE gn-divi.codcia = s-codcia AND gn-divi.coddiv = faccpedi.divdes NO-LOCK NO-ERROR.  
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE ue-ventas-old wWin 
