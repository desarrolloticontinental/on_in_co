...
fPendiente() @ x-PorCobrar T-RUTAD.Libre_c02 FacCPedi.Glosa ~
t-rutad.lugent @ x-lugar-ent FacCPedi.CodRef FacCPedi.NroRef T-RUTAD.FlgEst ~
T-RUTAD.Docs @ x-Docs T-RUTAD.Libre_c03 
...
&Scoped-define QUERY-STRING-br_table FOR EACH T-RUTAD WHERE ~{&KEY-PHRASE} NO-LOCK, ~
  ...
  AND FacCPedi.NroPed = T-RUTAD.NroRef ~
      AND (COMBO-BOX_FmaPgo = 'Todos' OR FacCPedi.FmaPgo = COMBO-BOX_FmaPgo) NO-LOCK, ~
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br_table B-table-Win _STRUCTURED
  QUERY br_table NO-LOCK DISPLAY
      ...
      T-RUTAD.Estado @ x-Estado COLUMN-LABEL "Situación" FORMAT "x(20)":U
            WIDTH 16.29
      ...
      T-RUTAD.ImpCob COLUMN-LABEL "Importe" FORMAT ">>>,>>9.99":U
            WIDTH 6.86
      T-RUTAD.Libre_d01 COLUMN-LABEL "Peso!kg" FORMAT ">>>,>>9.99":U
      T-RUTAD.Libre_d02 COLUMN-LABEL "Volumen!m3" FORMAT ">>>,>>9.99":U
      ...
      T-RUTAD.Libre_c03 COLUMN-LABEL "Transportista" FORMAT "x(80)":U
  ENABLE
      T-RUTAD.CodRef
      T-RUTAD.NroRef
/* _UIB-CODE-BLOCK-END */
...
/* Query rebuild information for BROWSE br_table
  ...
  AND INTEGRAL.FacCPedi.NroPed = Temp-Tables.T-RUTAD.NroRef"
     _Where[2]         = "(COMBO-BOX_FmaPgo = 'Todos' OR FacCPedi.FmaPgo = COMBO-BOX_FmaPgo)"
...
"T-RUTAD.Estado @ x-Estado" "Situación" "x(20)" ? ? ? ? ? ? ? no ? no no "16.29" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"T-RUTAD.Libre_d01" "Peso!kg" ">>>,>>9.99" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[19]   > Temp-Tables.T-RUTAD.Libre_d02
"T-RUTAD.Libre_d02" "Volumen!m3" ">>>,>>9.99" "decimal" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"fPendiente() @ x-PorCobrar" "Pendiente!de cobro" ? ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[24]   > Temp-Tables.T-RUTAD.Libre_c02
"T-RUTAD.Libre_c02" "Tipo!BCP" "x(20)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[25]   > INTEGRAL.FacCPedi.Glosa
...
"t-rutad.lugent @ x-lugar-ent" "Lugar de entrega" "x(60)" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[27]   = INTEGRAL.FacCPedi.CodRef
     _FldNameList[28]   = INTEGRAL.FacCPedi.NroRef
     _FldNameList[29]   > Temp-Tables.T-RUTAD.FlgEst
...
"T-RUTAD.Docs @ x-Docs" "# Docs." ">>9" ? ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[31]   > Temp-Tables.T-RUTAD.Libre_c03
"T-RUTAD.Libre_c03" "Transportista" "x(80)" "character" ? ? ? ? ? ? no ? no no ? yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
CASE TRUE:
    ...
    OTHERWISE DO:
        x-sql = "FOR EACH T-RUTAD WHERE T-RUTAD.codcia = " + STRING(s-codcia) + " NO-LOCK, " +
            ...
            "AND FacCPedi.CodDoc = T-RUTAD.CodRef " +
            "AND FacCPedi.NroPed = T-RUTAD.NroRef " +
            "AND FacCPedi.FmaPgo = '" + COMBO-BOX_FmaPgo + "' NO-LOCK, " +
            "FIRST GN-DIVI OF FacCPedi NO-LOCK, " +
            "FIRST DI-RutaD WHERE DI-RutaD.CodCia = T-RUTAD.CodCia " +
            "AND DI-RutaD.CodDiv = T-RUTAD.CodDiv " +
            "AND DI-RutaD.CodDoc = T-RUTAD.CodDoc " +
...
END CASE.
/*
x-sql = "PRESELECT EACH T-RUTAD WHERE T-RUTAD.codcia = " + STRING(s-codcia) + " NO-LOCK ".
MESSAGE x-SQL.
*/
...
/*     TTT-RUTAD.Estado = fEstado(). */
    ...
    IF AVAILABLE CcbADocu THEN DO:
        ASSIGN TTT-RUTAD.Libre_c03 = CcbADocu.Libre_C[9].
        FIND FIRST gn-prov WHERE gn-prov.CodCia  = PV-CODCIA 
            AND gn-prov.CodPro  = CcbADocu.Libre_C[9]
            NO-LOCK NO-ERROR.
        IF AVAILABLE gn-prov THEN TTT-RUTAD.Libre_c03 = TTT-RUTAD.Libre_c03 + " " + gn-prov.NomPro.
    END.
END.
...
FOR EACH TTT-RUTAD, FIRST Faccpedi NO-LOCK WHERE Faccpedi.codcia = TTT-RutaD.codcia
    AND Faccpedi.coddoc = TTT-RutaD.codref
    AND Faccpedi.nroped = TTT-RutaD.nroref:
