...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    FOR EACH B-RUTAC WHERE B-RUTAC.codcia = Di-RutaC.CodCia AND
        ...
        EXCLUSIVE-LOCK ON ERROR UNDO, THROW:
        /* ****************************************************************** */
        /* 16/02/2022 Hay dos casos: Picking por Rutas y Picking por Sectores */
        /* ****************************************************************** */
        FIND FIRST gn-divi WHERE gn-divi.codcia = DI-RUTAC.codcia
            AND gn-divi.coddiv = DI-RUTAC.coddiv
            NO-LOCK NO-ERROR.
        IF AVAILABLE gn-divi AND gn-divi.Campo-Char[6] = "PR" THEN B-RUTAC.FlgEst = "PF".
        ELSE B-RUTAC.FlgEst = "P".
        /* ****************************************************************** */
        /* Información Adicional */
        /* ****************************************************************** */
        ASSIGN
...
&IF DEFINED(EXCLUDE-GRE_Cierre_por_No_Entregado) = 0 &THEN

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE GRE_Cierre_por_No_Entregado Procedure 
PROCEDURE GRE_Cierre_por_No_Entregado :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEF INPUT PARAMETER pCodDiv LIKE gre_header.m_divorigen.
DEF INPUT PARAMETER pSerieGuia LIKE gre_header.SerieGuia.
DEF INPUT PARAMETER pNumeroGuia LIKE gre_header.NumeroGuia.

DEF OUTPUT PARAMETER pMensaje AS CHAR NO-UNDO.

/* Buscamos la GRE relacionada */
FIND gre_header WHERE gre_header.m_divorigen = pCodDiv AND
    gre_header.SerieGuia = pSerieGuia AND
    gre_header.NumeroGuia = pNumeroGuia
    NO-LOCK NO-ERROR.
IF ERROR-STATUS:ERROR = YES THEN DO:
    /*
    
    Por estabilizacion, luego se debe activar la validacion, coordinar con MAX RAMOS
    
    pMensaje = "NO se pudo ubicar la GRE " + STRING(pSerieGuia,'999') + STRING(pNumeroGuia,'99999999').
    RETURN 'ADM-ERROR'.
    */
    RETURN "OK".
END.
...
x-Rowid = ROWID(gre_header).
{lib/lock-genericov3.i ~
    &Tabla="gre_header" ~
    &Condicion="ROWID(gre_header) = x-Rowid" ~
    &Bloqueo="EXCLUSIVE-LOCK NO-ERROR" ~
    &Accion="RETRY" ~
    &Mensaje="NO" ~
    &TipoError="UNDO, RETURN 'ADM-ERROR'" }
...
ASSIGN
    gre_header_log.m_motivo_log = "BAJA EN SUNAT"
    gre_header_log.USER_log = USERID("dictdb")
    gre_header_log.cmotivo = "99"                       /* cMotivo */
    gre_header_log.dmotivo_detalle = "CIERRE DE H/R".   /*cMotivoDetalle*/

ASSIGN
    gre_header.m_rspta_sunat = "BAJA EN SUNAT".
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    */
    /* ********************************************************************************************** */
    /* POR ORDENES DE DESPACHO */
    /* 19/10/2023: BAJA EN SUNAT */
    /* ********************************************************************************************** */
    FOR EACH Di-RutaD OF Di-RutaC NO-LOCK WHERE Di-RutaD.codref = "G/R" AND
            (Di-RutaD.flgest = "N" OR Di-RutaD.flgest = "T"),   /* NO Entregado y Dejado en Tienda */
        FIRST Ccbcdocu NO-LOCK WHERE Ccbcdocu.codcia = Di-RutaD.codcia
            AND Ccbcdocu.coddoc = Di-RutaD.codref   /* G/R */
            AND Ccbcdocu.nrodoc = Di-RutaD.nroref
        ...
        END.
        CASE TRUE:
            WHEN Di-RutaD.FlgEst = "N" THEN DO:     /* NO ENTREGADO */
                /* ******************************************************************************************** */
                /* BAJA EN SUNAT */
                /* ******************************************************************************************** */
                DEF VAR pMensaje AS CHAR NO-UNDO.

                RUN GRE_Cierre_por_No_Entregado (INPUT Di-RutaD.CodDiv,
                                                 INPUT INTEGER(SUBSTRING(Di-RutaD.NroRef,1,3)),
                                                 INPUT INTEGER(SUBSTRING(Di-RutaD.NroRef,4)),
                                                 OUTPUT pMensaje).
                IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
                    MESSAGE 'NO se pudo dar de BAJA EN SUNAT la GRE relacionada a' Di-RutaD.codref Di-RutaD.nroref SKIP(1)
                        "ERROR:" SKIP pMensaje
                        VIEW-AS ALERT-BOX ERROR.
                    UNDO RLOOP, RETURN 'ADM-ERROR'.
                END.
        ...
        IF LAST-OF(Ccbcdocu.Libre_c02) THEN DO:
            ...
            IF x-Guias <> x-Guias-2 THEN NEXT.

            /* ******************************************************************************************** */
            /* CONTROL DE REPROGRAMACIONES */
            /* Solo NO ENTREGADOS Y REPROGRAMADOS y DEJADO EN TIENDA Y REPROGRAMADOS */
            /* ******************************************************************************************** */
            IF ( (Di-RutaD.flgest = "N" AND Di-RutaD.Libre_c02 = "R") OR
                 (Di-RutaD.flgest = "T" AND Di-RutaD.FlgEstDet <> "@DT") ) THEN DO:
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    */
    ...
    FOR EACH Di-RutaG OF Di-RutaC NO-LOCK WHERE 
        ...
        END.
        /* ******************************************************************************************** */
        /* 07/06/2023: Por cada G/R se anulan la GRE relacionada */
        /* ******************************************************************************************** */
        IF Di-RutaG.FlgEst = "N" THEN DO:
            DEF VAR pMensaje AS CHAR NO-UNDO.

            RUN GRE_Cierre_por_No_Entregado (INPUT Di-RutaG.CodDiv,
                                             INPUT Di-RutaG.serref,
                                             INPUT Di-RutaG.nroref,
                                             OUTPUT pMensaje).
            IF RETURN-VALUE = 'ADM-ERROR' THEN DO:
                MESSAGE 'NO se pudo anular la GRE relacionada a' Di-RutaD.codref Di-RutaD.nroref SKIP(1)
                    "ERROR:" SKIP pMensaje
                    VIEW-AS ALERT-BOX ERROR.
                UNDO RLOOP, RETURN 'ADM-ERROR'.
            END.
    ...
    FOR EACH Di-RutaD OF Di-RutaC NO-LOCK WHERE Di-RutaD.codref = "G/R" AND
        ...
        IF LAST-OF(Ccbcdocu.Libre_c02) THEN DO:
            ...
            IF x-Guias = x-Guias-2 THEN DO:
                ...
                /* ********************************************************************************************** */
                /* RHC 08/03/2019 Actualizamos CONTROL de PHR */
                /* 16/02/2022 Fernan Oblitas: Aceptar más de una PHR por cada H/R */
                /* ********************************************************************************************** */
                IF ENTRY(1, DI-RutaC.Libre_c03) = "PHR" THEN DO:
                    DEF VAR x-ItemPHR AS INTE NO-UNDO.
                    DEF VAR x-NroPHR  AS CHAR NO-UNDO.
                    /* Debemos rastrear en cual PHR está  la OD */
                    DO x-ItemPHR = 1 TO NUM-ENTRIES(DI-RutaC.libre_c03, '|'):
                        x-NroPHR = ENTRY(x-ItemPHR, DI-RutaC.libre_c03, '|').
                        FIND FIRST B-PHRD WHERE B-PHRD.codcia = DI-RutaC.codcia
                            AND B-PHRD.coddiv = DI-RutaC.coddiv
                            AND B-PHRD.coddoc = ENTRY(1, x-NroPHR)
                            AND B-PHRD.nrodoc = ENTRY(2, x-NroPHR)
                            AND B-PHRD.CodRef = Ccbcdocu.Libre_c01  /* O/D */
                            AND B-PHRD.NroRef = Ccbcdocu.Libre_c02 NO-LOCK NO-ERROR NO-WAIT.
                        IF NOT AVAILABLE B-PHRD THEN NEXT.
                        FIND CURRENT B-PHRD EXCLUSIVE-LOCK NO-ERROR NO-WAIT.
                        IF ERROR-STATUS:ERROR = YES THEN DO:
                            IF LOCKED(B-PHRD) THEN DO:
                                MESSAGE 'Registro de O/D en uso por otro usuario' VIEW-AS ALERT-BOX ERROR.
                                UNDO RLOOP, RETURN 'ADM-ERROR'.
                            END.
                        END.
...
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       SOLO aceptados por SUNAT
------------------------------------------------------------------------------*/
...
/* *************************************** */
/* 25/11/23: Si es TRUE se verifica la GRE */
/* *************************************** */
DEF VAR lGRE_ONLINE AS LOG INIT NO NO-UNDO.
...
FOR EACH Almcmov NO-LOCK WHERE Almcmov.CodCia = s-CodCia
    ...
    /* *********************************************************************** */
    /* 29-8-2023: Validamos la GRE */
    /* *********************************************************************** */
    IF lGRE_ONLINE THEN DO:
        FIND FIRST GRE WHERE GRE.serieGuia = Almcmov.nroser AND
            GRE.numeroGuia = Almcmov.nrodoc AND
            TRIM(GRE.m_rspta_sunat) = "ACEPTADO POR SUNAT"
            NO-LOCK NO-ERROR.
        IF NOT AVAILABLE GRE THEN NEXT.
    END.
    /* *********************************************************************** */
    /* ********************************************************************** */
...
DEFINE BUFFER GRE FOR gre_header.
...
FOR EACH x-Ccbcdocu NO-LOCK WHERE x-Ccbcdocu.codcia = s-codcia
    ...
    IF TRUE <> (x-DeliveryGroup > "") OR TRUE <> (x-InvoiCustomerGroup > "") THEN DO:
    ...
    /* *********************************************************************** */
    IF lGRE_ONLINE THEN DO:
        FIND FIRST GRE WHERE GRE.serieGuia = INTEGER(SUBSTRING(Ccbcdocu.nrodoc,1,3)) AND
            GRE.numeroGuia = INTEGER(SUBSTRING(Ccbcdocu.nrodoc,4)) AND
            TRIM(GRE.m_rspta_sunat) = "ACEPTADO POR SUNAT"
            NO-LOCK NO-ERROR.
        IF NOT AVAILABLE GRE THEN NEXT.
    END.
...
/* Tiene Grupo de Reparto */
DEFINE VAR hLogis AS HANDLE NO-UNDO.             /* Handle Libreria */
DEFINE VAR x-DeliveryGroup AS CHAR.
DEFINE VAR x-InvoiCustomerGroup AS CHAR.

RUN logis\logis-librerias.p PERSISTENT SET hLogis.
...
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    ...
    /* Bloqueamos PHR */
    /* ************************************************ */
    DEF BUFFER GUIAS_RE FOR Ccbcdocu.
                    ...
                    FOR EACH vtatabla NO-LOCK WHERE vtatabla.codcia = s-codcia AND
                        ...
                        IF FIRST-OF(vtatabla.llave_c2) THEN DO:
                            ...
                            IF NOT AVAILABLE T-OD-FAI-GR  THEN DO:
                                CREATE T-OD-FAI-GR.
                                ASSIGN 
                                    T-OD-FAI-GR.T-COD_OD = di-rutad.codref
                        ...
                        END.
                    END.
                    /* 13/7/203: CASO GRE F.Oblitas */
                    FOR EACH GUIAS_RE WHERE GUIAS_RE.codcia = s-codcia AND
                        GUIAS_RE.coddoc = x-Ccbcdocu.coddoc AND     /* FAI */
                        GUIAS_RE.nrodoc = x-Ccbcdocu.nrodoc AND
                        GUIAS_RE.codref = "G/R" AND
                        GUIAS_RE.flgest <> "A":
                        FIND FIRST T-OD-FAI-GR WHERE T-OD-FAI-GR.T-COD_OD = di-rutad.codref AND /* O/D */
                            T-OD-FAI-GR.T-NRO_OD = di-rutad.nroref AND
                            T-OD-FAI-GR.T-GR = GUIAS_RE.codref + "-" + GUIAS_RE.NroRef   /* G/R */ 
                        ...
                        IF NOT AVAILABLE T-OD-FAI-GR  THEN DO:
                            ...
                            ASSIGN 
                                T-OD-FAI-GR.T-COD_OD = di-rutad.codref
                                T-OD-FAI-GR.T-NRO_OD = di-rutad.nroref
                                T-OD-FAI-GR.T-GR = GUIAS_RE.codref + "-" + GUIAS_RE.NroRef 
                                T-OD-FAI-GR.T-SWT = 'NO'.
                        END.
                    END.
                END.
    ...
    IF Di-RutaC.FlgEst <> "P" THEN DO:
        pMensaje = "No están completos los comprobantes" + CHR(10) +
            TRIM(pMensaje) + CHR(10) + 
            "Proceso abortado".
        DELETE PROCEDURE hLogis.
        UNDO RLOOP, RETURN 'ADM-ERROR'.
    END.
...
DEF VAR x-Localizado AS LOG INIT NO NO-UNDO.
    ...
    CASE B-RutaD.CodRef:
        WHEN "O/D" OR WHEN "O/M" THEN DO:
            ...
            IF NOT AVAILABLE ORDENES THEN DO:
                ...
                ASSIGN 
                    T-RutaD.Glosa = "DOCUMENTO: " + B-RUTAD.CODREF + " " + B-RUTAD.NROREF + " NO ENCONTRADO".
            ...
            IF NOT AVAILABLE PEDIDO THEN DO:
                ...
                BUFFER-COPY B-RutaD TO T-RutaD.
                ASSIGN 
                    T-RutaD.Glosa = "DOCUMENTO: " + ORDENES.CODREF + " " + ORDENES.NROREF + " NO ENCONTRADO".
                pBloqueados = pBloqueados + 1.
                NEXT RLOOP.
            END.
            ...
            /* Procedimientos */
            RUN Grupo-reparto IN hProc (INPUT PEDIDO.codREF, 
                                        INPUT PEDIDO.nroREF, 
            ...
            IF TRUE <> (x-DeliveryGroup > "") OR TRUE <> (x-InvoiCustomerGroup > "") THEN DO:
                ...
                FOR EACH Ccbcdocu NO-LOCK WHERE Ccbcdocu.codcia = s-codcia
                    ...
                    END.
                END.
                IF x-Localizado = NO THEN DO:
                    CREATE T-RutaD.
                    BUFFER-COPY B-RutaD TO T-RutaD.
                    ASSIGN
                        T-RutaD.Glosa = "DOCUMENTO S/GRPO REPARTO: " + PEDIDO.CODDOC + " " + PEDIDO.NROPED + " SIN G/R (FLGEST='F')".
                    pBloqueados = pBloqueados + 1.
                    NEXT RLOOP.
                END.
            END.
            ELSE DO:    /* con Grupo de Reparto */
                ...
                FOR EACH Ccbcdocu NO-LOCK WHERE ccbcdocu.codcia = s-codcia AND
                    ccbcdocu.codped = ORDENES.codref AND
                    ccbcdocu.nroped = ORDENES.nroref:
                    IF ccbcdocu.codcli = ORDENES.codcli AND
                ...
                IF x-Localizado = NO THEN DO:
                    ...
                    BUFFER-COPY B-RutaD TO T-RutaD.
                    ASSIGN 
                        T-RutaD.Glosa = "DOCUMENTO C/GRPO REPARTO: " + ORDENES.CODDOC + " " + ORDENES.NROPED + " SIN FAI".
                    pBloqueados = pBloqueados + 1.
                    NEXT RLOOP.
                END.
                ...
                IF ( ccbcdocu.codref <> 'G/R' ) OR ( TRUE <> (ccbcdocu.codref > "") ) THEN DO:
                    ...
                    BUFFER-COPY B-RutaD TO T-RutaD.
                    ASSIGN 
                        T-RutaD.Glosa = "DOCUMENTO S/GRPO REPARTO: " + CCBCDOCU.CODDOC + " " + CCBCDOCU.NRODOC + " SIN G/R (CODREF<>'G/R')".
                    pBloqueados = pBloqueados + 1.
                    NEXT RLOOP.
                END.
            END.
            /* *********************************************************************** */
            /* 18-10-2023: Validamos la GRE */
            /* *********************************************************************** */
            IF lGRE_ONLINE THEN DO:
                FOR EACH Ccbcdocu NO-LOCK WHERE Ccbcdocu.codcia = s-codcia AND
                    ccbcdocu.codped = ORDENES.codref AND
                    ...
                    IF ccbcdocu.codcli = ORDENES.codcli AND
                        ...
                        Ccbcdocu.libre_c01 = ORDENES.coddoc AND
                        Ccbcdocu.libre_c02 = ORDENES.nroped AND
                        Ccbcdocu.coddiv = s-coddiv AND
                        Ccbcdocu.flgest <> "A" THEN DO:
                        /* Si encontramos una GRE no aceptada => se bloquea */
                        FIND FIRST GRE WHERE GRE.m_coddoc = Ccbcdocu.CodDoc AND 
                            GRE.m_nroser = INTEGER(SUBSTRING(Ccbcdocu.NroDoc,1,3)) AND
                            GRE.m_nrodoc = INTEGER(SUBSTRING(Ccbcdocu.NroDoc,4)) AND
                            TRIM(GRE.m_rspta_sunat) = "ACEPTADO POR SUNAT"
                            NO-LOCK NO-ERROR.
                        IF NOT AVAILABLE GRE THEN DO:
                            CREATE T-RutaD.
                            BUFFER-COPY B-RutaD TO T-RutaD.
                            ASSIGN 
                                T-RutaD.Glosa = "DOCUMENTO: " + CCBCDOCU.CODDOC + " " + CCBCDOCU.NRODOC + " GRE NO ACEPTADA POR SUNAT".
                            pBloqueados = pBloqueados + 1.
                            NEXT RLOOP.
                        END.
                        LEAVE.      /* Primera ocurrencia */
                    END.
                END.
            END.
            /* *********************************************************************** */
        ...
        WHEN "OTR" THEN DO:
            ...
            IF lGRE_ONLINE THEN DO:
                FIND FIRST Almcmov USE-INDEX Almc07 WHERE Almcmov.CodCia = s-CodCia
                    ...
                    AND Almcmov.FlgEst <> "A"
                    AND CAN-FIND(FIRST GRE WHERE GRE.SerieGuia = Almcmov.NroSer AND 
                                 GRE.NumeroGuia = Almcmov.NroDoc AND
                                 TRIM(GRE.m_rspta_sunat) = 'ACEPTADO POR SUNAT'
                                 NO-LOCK)
                    NO-LOCK NO-ERROR.
            ...
            IF NOT AVAILABLE Almcmov THEN DO:
                ...
                BUFFER-COPY B-RutaD TO T-RutaD.
                ASSIGN 
                    T-RutaD.Glosa = "DOCUMENTO: " + ORDENES.CODDOC + " " + ORDENES.NROPED + " NO ENCONTRADO MOV DE ALMACEN (S-03)".
                pBloqueados = pBloqueados + 1.
                NEXT RLOOP.
            END.
            /* *********************************************************************** */
            /* 8-9-2023: Validamos la GRE */
            /* *********************************************************************** */
/*             IF lGRE_ONLINE THEN DO:                                                                                           */
/*                 FIND FIRST GRE WHERE GRE.m_coddoc = ORDENES.CodDoc AND                                                        */
/*                     GRE.m_nroser = INTEGER(SUBSTRING(ORDENES.NroPed,1,3)) AND                                                 */
/*                     GRE.m_nrodoc = INTEGER(SUBSTRING(ORDENES.nroPed,4)) AND                                                   */
/*                     GRE.m_rspta_sunat BEGINS "ACEPTADO POR SUNAT"                                                             */
/*                     NO-LOCK NO-ERROR.                                                                                         */
/*                 IF NOT AVAILABLE GRE THEN DO:                                                                                 */
/*                     CREATE T-RutaD.                                                                                           */
/*                     BUFFER-COPY B-RutaD TO T-RutaD.                                                                           */
/*                     ASSIGN                                                                                                    */
/*                         T-RutaD.Glosa = "DOCUMENTO: " + ORDENES.CODDOC + " " + ORDENES.NROPED + " GRE NO ACEPTADA POR SUNAT". */
/*                     pBloqueados = pBloqueados + 1.                                                                            */
/*                     NEXT RLOOP.                                                                                               */
/*                 END.                                                                                                          */
/*             END.                                                                                                              */
...
FIRST Faccpedi NO-LOCK WHERE FacCPedi.CodCia = s-CodCia AND ~
    ...
    FIRST Almcrepo NO-LOCK USE-INDEX Llave01 WHERE almcrepo.CodCia = s-CodCia AND ~
...
/* Dato: Almcdocu Control de NO ENTREGADOS */
DEF INPUT PARAMETER pRowid AS ROWID.
...
PRINCIPAL:
DO TRANSACTION ON ERROR UNDO, RETURN 'ADM-ERROR' ON STOP UNDO, RETURN 'ADM-ERROR':
    /* 16/02/2022 Adaptarlo a multiPHR */
    DEF VAR x-ItemPHR AS INTE NO-UNDO.
    DEF VAR x-NroPHR  AS CHAR NO-UNDO.
    DO x-ItemPHR = 1 TO NUM-ENTRIES(DI-RutaC.libre_c03, '|'):
        x-NroPHR = ENTRY(x-ItemPHR, DI-RutaC.libre_c03, '|').
        FOR EACH B-PHRD EXCLUSIVE-LOCK WHERE B-PHRD.codcia = DI-RutaC.codcia
            AND B-PHRD.coddoc = ENTRY(1, x-NroPHR)    /* PHR */
            AND B-PHRD.nrodoc = ENTRY(2, x-NroPHR)
