...
DEFINE TEMP-TABLE tt-hojaruta
    ...
    FIELD tt-volumen AS DECI FORMAT '>>>,>>9.99'
...
------------------------------------------------------------------------------*/
    ...
    chWorkSheet:Range("AC2"):Value = "Moneda".
    chWorkSheet:Range("AD2"):Value = "Importe Original".
    chWorkSheet:Range("AE2"):Value = "Tipo Cambio".
    chWorkSheet:Range("AF2"):Value = "Importe Soles".  
    chWorkSheet:Range("AG2"):Value = "Serie Dcto".  
    chWorkSheet:Range("AH2"):Value = "Nro. Dcto".  
    chWorkSheet:Range("AI2"):Value = "Div. Origen".  
    chWorkSheet:Range("AJ2"):Value = "Descripcion Div. Origen".     
    chWorkSheet:Range("AK2"):Value = "Cant.Items".
    chWorkSheet:Range("AL2"):Value = "Costo Kardex".
    chWorkSheet:Range("AM2"):Value = "Cod.Tienda".
    chWorkSheet:Range("AN2"):Value = "Descripcion Tienda".
    chWorkSheet:Range("AO2"):Value = "Motivo".
    chWorkSheet:Range("AP2"):Value = "Area Responsable".
    chWorkSheet:Range("AQ2"):Value = "Ref.".
    chWorkSheet:Range("AR2"):Value = "Nro. Ref.".

    chWorkSheet:Range("AS2"):Value = "Termino de Pago".
    chWorkSheet:Range("AT2"):Value = "Responsable".

    chWorkSheet:Range("AU2"):Value = "Volumen (m3)".
    ...
    chWorkSheet:COLUMNS("AF"):NumberFormat = "@".
...
/*     chWorkSheet:Range(cRange):Value = tt-costo + tt-imp-horas-extras. */
    ...
    cRange = "AU" + cColumn.
    chWorkSheet:Range(cRange):Value = tt-volumen.
...
FOR EACH DI-RutaC NO-LOCK WHERE DI-RutaC.codcia = s-codcia AND 
    ...
    IF CAN-FIND(FIRST gn-prov WHERE gn-prov.codcia = pv-codcia AND
    ...
    FOR EACH DI-RutaD WHERE DI-RutaD.codcia = DI-RutaC.codcia AND 
        ...
        */
        ...
        ASSIGN 
            ...
            tt-horsal   = STRING(DI-RutaC.horsal, 'XX:XX')
            tt-horret   = STRING(DI-RutaC.horret, 'XX:XX')
        ...
        IF AVAILABLE Ccbcdocu THEN DO:
            FOR EACH Ccbddocu OF Ccbcdocu NO-LOCK, FIRST Almmmatg OF Ccbddocu NO-LOCK:
                tt-Volumen = tt-Volumen + (Ccbddocu.CanDes * Ccbddocu.Factor * Almmmatg.Libre_d02 / 1000000).
            END.
        END.
    ...
    FOR EACH DI-RutaG WHERE DI-RutaG.codcia = DI-rutaC.codcia AND 
        ...
        IF AVAILABLE Almcmov THEN DO:
            FOR EACH Almdmov OF Almcmov NO-LOCK, FIRST Almmmatg OF Almdmov NO-LOCK:
                tt-Volumen = tt-Volumen + (Almdmov.CanDes * Almdmov.Factor * Almmmatg.Libre_d02 / 1000000).
            END.
        END.
    END.
END.
