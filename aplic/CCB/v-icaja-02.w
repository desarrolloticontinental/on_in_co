...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  /*RUN CCB\r-recibo3-v2 (ROWID(CcbcCaja)).*/
...
DO WITH FRAME {&FRAME-NAME} :
    ...
    IF NOT AVAILABLE t-ccbdcaja THEN DO:
        MESSAGE "No ha ingresado información" VIEW-AS ALERT-BOX WARNING.
        RETURN "ADM-ERROR".
    END.
   ...
   FOR EACH T-CcbDCaja:
       IF x-mon <> T-CcbDCaja.CodMon THEN DO:
          MESSAGE "Documentos a cancelar con diferente moneda" VIEW-AS ALERT-BOX WARNING.
          RETURN "ADM-ERROR".         
       END.
   END.
   ...
   IF j > 0 AND j <> k THEN DO:
       MESSAGE "NO se pueden cancelar FAI's con otros comprobantes" VIEW-AS ALERT-BOX WARNING.
       RETURN 'ADM-ERROR'.
   END.
   ...
   */
   ...
   IF s-amortiza-letras = NO AND x-hay-letras = YES AND x-hay-nc = YES THEN DO:
       MESSAGE "En una liquidación donde se esta cancelando LETRAS, " SKIP 
               "no esta permitido la aplicacion de NOTAS DE CREDITO"
           VIEW-AS ALERT-BOX WARNING.
       RETURN "ADM-ERROR".
   END.
   /* 24/7/2024: Mejora contínua: Verificar que el comprobante esté aprobado por SUNAT */
   FOR EACH T-Ccbdcaja NO-LOCK WHERE T-Ccbdcaja.codref = "FAC",
       FIRST CcbCDocu NO-LOCK WHERE CcbCDocu.CodCia = T-CcbDCaja.CodCia
       AND CcbCDocu.CodDoc = T-CcbDCaja.CodRef
       ...
       x-estado-doc = "".
       x-cod-estado-doc = "".
       RUN gn/p-estado-documento-electronico (INPUT CcbCDocu.CodDoc,
       ...
       IF x-cod-estado-doc <> "AC_03" THEN DO:
           MESSAGE 'El comprobante' CcbCDocu.CodDoc CcbCDocu.NroDoc 'NO está ACEPTADO POR SUNAT'
               VIEW-AS ALERT-BOX WARNING.
           RETURN "ADM-ERROR".
       END.
       /* *************************************************************************** */
