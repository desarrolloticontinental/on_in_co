...
ASSIGN
    S-FLGSIT = SUBSTRING(pCodDoc,2,1)
    S-FLGUBI = SUBSTRING(pCodDoc,1,1)
...
&Scoped-Define ENABLED-OBJECTS RECT-25 COMBO-NroSer btnExcel 
...
DEFINE VARIABLE h_bletbcocob AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navico AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv12 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-updv14 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_qcanjexletra AS HANDLE NO-UNDO.
DEFINE VARIABLE h_tletbcocob AS HANDLE NO-UNDO.
DEFINE VARIABLE h_venvletbco AS HANDLE NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     btnExcel AT ROW 15.46 COL 41 WIDGET-ID 6
     RECT-25 AT ROW 2.54 COL 2 WIDGET-ID 2
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 118.86 BY 15.88
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "ENVIO DE LETRAS AL BANCO"
         HEIGHT             = 15.88
         WIDTH              = 118.86
         MAX-HEIGHT         = 22.65
         MAX-WIDTH          = 132.72
         VIRTUAL-HEIGHT     = 22.65
         VIRTUAL-WIDTH      = 132.72
         RESIZE             = no
         SCROLL-BARS        = no
         STATUS-AREA        = no
         BGCOLOR            = ?
...
ON END-ERROR OF W-Win /* ENVIO DE LETRAS AL BANCO */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* ENVIO DE LETRAS AL BANCO */
DO:
...
ON CHOOSE OF btnExcel IN FRAME F-Main /* Desde Excel */
DO:
    /* Valida que al menos haya ingresado la cta del banco */  
    RUN ue-desde-excel IN h_venvletbco.

    IF RETURN-VALUE = "ADM-ERROR" THEN DO:
        RETURN NO-APPLY.
    END.

    RUN ue-desde-excel IN h_tletbcocob.
END.
...
CASE s-FlgSit:
    WHEN "C" THEN W-Win:TITLE = W-Win:TITLE + "- COBRANZA".
    WHEN "D" THEN W-Win:TITLE = W-Win:TITLE + "- DESCUENTO".
END CASE.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ccb/venvletbco.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_venvletbco ).
       RUN set-position IN h_venvletbco ( 2.73 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 4.42 , 102.00 ) */
       ...
       /* Links to SmartViewer h_venvletbco. */
       RUN add-link IN adm-broker-hdl ( h_p-updv14 , 'TableIO':U , h_venvletbco ).
       RUN add-link IN adm-broker-hdl ( h_qcanjexletra , 'Record':U , h_venvletbco ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navico ,
             h_p-updv14 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_venvletbco ,
             COMBO-NroSer:HANDLE IN FRAME F-Main , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_qcanjexletra ,
             btnExcel:HANDLE IN FRAME F-Main , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ccb/bletbcocob.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_bletbcocob ).
       RUN set-position IN h_bletbcocob ( 7.35 , 2.00 ) NO-ERROR.
       RUN set-size IN h_bletbcocob ( 7.73 , 115.00 ) NO-ERROR.

       /* Links to SmartBrowser h_bletbcocob. */
       RUN add-link IN adm-broker-hdl ( h_qcanjexletra , 'Record':U , h_bletbcocob ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_bletbcocob ,
             h_venvletbco , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'ccb/tletbcocob.w':U ,
             INPUT  FRAME F-Main:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_tletbcocob ).
       RUN set-position IN h_tletbcocob ( 7.35 , 2.00 ) NO-ERROR.
       RUN set-size IN h_tletbcocob ( 7.73 , 114.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-updv12 ).
       RUN set-position IN h_p-updv12 ( 15.38 , 2.00 ) NO-ERROR.
       RUN set-size IN h_p-updv12 ( 1.35 , 34.00 ) NO-ERROR.

       /* Links to SmartBrowser h_tletbcocob. */
       RUN add-link IN adm-broker-hdl ( h_p-updv12 , 'TableIO':U , h_tletbcocob ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_tletbcocob ,
             h_venvletbco , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-updv12 ,
             h_tletbcocob , 'AFTER':U ).
    END. /* Page 2 */
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-25 COMBO-NroSer btnExcel 
      WITH FRAME F-Main IN WINDOW W-Win.
...
CASE L-Handle:
    WHEN "Pagina1"  THEN DO WITH FRAME {&FRAME-NAME}:    
        RUN select-page(1).
        IF VALID-HANDLE(h_bletbcocob) THEN RUN dispatch IN h_bletbcocob ('open-query':U).
