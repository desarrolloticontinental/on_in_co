...
FOR EACH x-ccbcdocu WHERE x-ccbcdocu.codcia = s-codcia AND
                            ...
                            x-ccbcdocu.nroref = pNroMov NO-LOCK:

    IF x-ccbcdocu.coddoc = 'LET' THEN NEXT.
    IF x-ccbcdocu.flges = 'A' THEN NEXT.
...
RUN letra-origen-del-movimiento(INPUT pCodDoc, 
                                INPUT pNroDoc, 
                                OUTPUT x-codmov, 
                                OUTPUT x-nromov).
...
FOR EACH x-Ccbdmvto NO-LOCK WHERE x-CcbDMvto.CodCia = s-codcia
    ...
    IF x-CcbDMvto.codref = "LET" THEN DO:
        RUN trazabilidad-mov-del-lpa(INPUT x-CcbDMvto.CodDoc, 
                                     INPUT x-CcbDMvto.NroDoc, 
                                     INPUT x-CcbDMvto.codref, 
                                     INPUT x-CcbDMvto.nroref,   /* LET */
                                     INPUT x-CcbDMvto.imptot, 
                                     OUTPUT pRetVal).
    END.
END.
