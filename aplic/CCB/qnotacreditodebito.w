...
AND Ccbcdocu.nrodoc BEGINS STRING(s-NroSer, ENTRY(1,x-Formato,'-'))
...
&Scoped-Define ENABLED-OBJECTS f-Numero RECT-1 
...
/* Query rebuild information for QUERY Query-Main
     ...
     _Where[1]         = "{&Condicion}"
     _Design-Parent    is WINDOW q-tables @ ( 1.27 , 12.72 )
*/  /* QUERY Query-Main */
&ANALYZE-RESUME
...
FIND CcbCDocu WHERE {&Condicion} 
    /*AND CcbCDocu.NroDoc = STRING(S-NroSer,"999") + STRING(f-Numero,"999999")*/
    AND CcbCDocu.NroDoc = STRING(S-NroSer, ENTRY(1,x-Formato,'-')) +
                            STRING(f-Numero, ENTRY(2,x-Formato,'-'))
    NO-LOCK NO-ERROR.
