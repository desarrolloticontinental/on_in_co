...
DEFINE FRAME F-Main
     ...
     "DEPOSITO DE TARJETAS DE CREDITO" VIEW-AS TEXT
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW W-Win ASSIGN
         HIDDEN             = YES
         TITLE              = "ASIENTO CONTABLE POR DEPOSITO DE TARJETAS DE CREDITO"
         HEIGHT             = 23.58
         WIDTH              = 133.86
         MAX-HEIGHT         = 23.58
         MAX-WIDTH          = 136.57
...
ON END-ERROR OF W-Win /* ASIENTO CONTABLE POR DEPOSITO DE TARJETAS DE CREDITO */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL W-Win W-Win
ON WINDOW-CLOSE OF W-Win /* ASIENTO CONTABLE POR DEPOSITO DE TARJETAS DE CREDITO */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-size IN h_b-chqdifcbd ( 18.38 , 132.00 ) NO-ERROR.
...
END.
/* CARGAMOS ASIENTO */
DEF VAR x-PorCom AS DEC NO-UNDO.
DEF VAR x-TpoCmb AS DEC NO-UNDO.
DEF VAR x-ImpComNac AS DEC NO-UNDO.
DEF VAR x-PorComNac AS DEC NO-UNDO.
DEF VAR x-ImpNeto AS DEC NO-UNDO.

FOR EACH T-DMOV WHERE T-DMOV.CodCta <> '' BREAK BY T-DMOV.CodDiv:
    FIND LAST gn-tcmb WHERE gn-tcmb.fecha <= T-DMOV.FchEmi NO-LOCK.
    ...
    ASSIGN
        ...
        x-PorComNac = 0
        x-ImpNeto = 0.
    FIND CcbCCaja WHERE CcbCCaja.CodCia = T-DMOV.CodCia AND
        CcbCCaja.CodDoc = T-DMOV.CodDoc AND
        CcbCCaja.NroDoc = T-DMOV.NroDoc
        NO-LOCK NO-ERROR.
    IF AVAILABLE CcbCCaja THEN DO:
        ASSIGN
            x-ImpComNac = T-DMOV.DepNac[1] + (T-DMOV.DepNac[2] * x-TpoCmb).
        /* Buscamos el Banco */
        ...
        x-NroDoc = TRIM(CcbCCaja.Voucher[4]).
        IF LENGTH(x-NroDoc) > 8 THEN x-NroDoc = SUBSTRING(x-NroDoc, LENGTH(x-NroDoc) - 7).
        IF AVAILABLE FacTabla THEN DO:
            FOR EACH VtaTabla WHERE VtaTabla.CodCia = FacTabla.CodCia
                AND VtaTabla.Tabla = FacTabla.Tabla
                AND VtaTabla.Llave_c1 = FacTabla.Codigo 
                AND (x-ImpComNac >= VtaTabla.Rango_valor[1] AND x-ImpComNac <= VtaTabla.Rango_valor[2]):
                x-PorComNac = VtaTabla.Valor[1].
            END.
        END.
    END.
    RUN Graba-Documentos ("13", x-CodCta, YES, x-Importe, x-NroDoc, x-CodMon, x-CodAux).
    /* ************* CUENTA 104XXXXX ***************** */
/*     IF  LOOKUP(f-Division, '00001,00002,00003') > 0                        */
/*         AND x-CodAux = '43240552'   /* El Proveedor de la tarjeta */       */
/*         THEN x-Importe = ( T-DMOV.DepNac[1] + T-DMOV.DepNac[2] ) * 0.9385. */
/*         ELSE x-Importe = ( T-DMOV.DepNac[1] + T-DMOV.DepNac[2] ) * 0.9485. */
    /* De acuerdo a tablas */
    x-ImpNeto = ROUND( ( T-DMOV.DepNac[1] + T-DMOV.DepNac[2] ) * (1 - x-PorComNac / 100), 2).
    ASSIGN
        x-CodCta = T-DMOV.codcta.
    RUN Graba-Documentos ("13", x-CodCta, NO, x-ImpNeto, x-NroDoc, x-CodMon, x-CodAux).
    /* Cuenta de comision */
    x-CodCta = '42131100'.
    x-Importe = ROUND(x-Importe - x-ImpNeto, 2).
...
ASSIGN
     ...
     X-GLOSA  = 'Depósito de tarjetas de crédito'.
...
IF x-CodCta = '42131100' THEN DO:
    FIND FIRST T-PREV WHERE t-prev.periodo = p-periodo AND
        t-prev.nromes  = p-mes AND
        t-prev.codope  = x-codope AND
        t-prev.coddiv  = T-DMOV.CodDiv AND
        t-prev.codcta  = x-codcta AND
        t-prev.codaux  = X-CodAux AND
        t-prev.tpomov  = x-tpomov AND
        t-prev.codmon  = x-codmon EXCLUSIVE-LOCK NO-ERROR.
...
IF x-CodMon = 1 THEN ASSIGN t-prev.impmn1  = t-prev.impmn1 + x-importe.
ELSE ASSIGN
