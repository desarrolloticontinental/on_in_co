...
DEFINE VARIABLE FILL-IN-NroDoc AS CHARACTER FORMAT "xxx-xxxxxxxxxxxx" 
     LABEL "Nro Doc" 
     VIEW-AS FILL-IN 
     SIZE 18 BY .81
...
DO:
    ...
    END.
    /* ******************************************************************* */
    /* RHC 28/04/17 Se amplia 12 meses el vencimiento para personal de CyC */
    /* ******************************************************************* */
    DEF VAR dFchVto LIKE Ccbcdocu.fchvto NO-UNDO.
    dFchVto = Ccbcdocu.fchvto.
    /* RHC 29/08/19 Ampliado de 1 año a 20 años solicitado por Julissa Calderon el 28 y
        autorizado por Llican el 29 */
    IF s-FlgSit = "NO" THEN dFchVto = ADD-INTERVAL(dFChVto, 20, 'year').
    IF Ccbcdocu.coddoc = "N/C" AND dFchVto < TODAY THEN DO:
...
/*     END.                                                                  */
    ...
    IF s-FlgSit = "YES" THEN DO:    /* Control Activo */
        ...
        END.
        /* ************************************************ */
        /* RHC 11/03/2016 Restricciones para tiendas utilex */
...
------------------------------------------------------------------------------*/
    ...
    DEFINE VARIABLE clave-cyc AS CHARACTER FORMAT "x(20)" LABEL "Clave" NO-UNDO.

    IF ccbccaja.flgest = "A" THEN DO:
        MESSAGE "Registro ya fue Anulado" VIEW-AS ALERT-BOX ERROR.
        RETURN "ADM-ERROR".
    END.
    
    IF s-FlgSit = "YES" THEN DO:
        IF ccbccaja.flgcie = "C" THEN DO:
            MESSAGE "Cierre de caja efectuado" VIEW-AS ALERT-BOX ERROR.
            RETURN "ADM-ERROR".
        END.
    ...
    ELSE DO:
        IF ccbccaja.flgcie = "C" THEN DO:
            MESSAGE "Cierre de caja efectuado" SKIP
                "Continuamos con la anulación?" VIEW-AS ALERT-BOX QUESTION
                BUTTONS YES-NO UPDATE rpta AS LOG.
            IF rpta = NO THEN RETURN "ADM-ERROR".

            /* 09Ago2021 - Pedido de Susana Leon, se forzo el tipo de documento para CYC */
            {adm/i-DocPssw-cyc.i s-CodCia 'CYC' ""DEL""}
        END.
    END.
