...
FIND FIRST integral.PF-G003 WHERE integral.PF-G003.Aplic-Id = "CJC"
    AND CAN-FIND(FIRST integral.PF-G004 OF integral.PF-G003 WHERE integral.PF-G004.User-Id = s-User-id
                 NO-LOCK)
    NO-LOCK NO-ERROR.
IF s-user-id = 'ADMIN' OR AVAILABLE integral.PF-G003 THEN RUN ccb/d-caja01.r (OUTPUT pOk).
...
/* Definimos Pantalla */
DEF VAR FILL-IN-Empresa     AS CHAR     LABEL 'Empresa'     FORMAT 'x(80)'  NO-UNDO.
DEF VAR FILL-IN-Division    AS CHAR     LABEL 'División'    FORMAT 'x(80)'  NO-UNDO.
DEF VAR FILL-IN-Almacen     AS CHAR     LABEL 'Almacén'     FORMAT 'x(80)'  NO-UNDO.
DEF VAR FILL-IN-Terminal    AS CHAR     LABEL 'Terminal'    FORMAT 'x(80)' NO-UNDO.
ASSIGN
...
DEFINE FRAME F1 
    Logotipo 
    FILL-IN-Empresa  AT COLUMN 60 ROW 2 BGCOLOR 15 FONT 4
    FILL-IN-Division AT COLUMN 60 ROW 3 BGCOLOR 15 FONT 4
    FILL-IN-Almacen  AT COLUMN 60 ROW 4 BGCOLOR 15 FONT 4
    FILL-IN-Terminal AT COLUMN 60 ROW 5 BGCOLOR 15 FONT 4
    WITH SIZE 132 BY 18 THREE-D SIDE-LABELS /*NO-LABELS*/.
...
------------------------------------------------------------------------------*/
   ...
   FOR EACH integral.PF-G003 NO-LOCK WHERE NOT integral.PF-G003.Aplic-Id BEGINS "@":
       s-seguridad = "".
       IF s-user-id <> "ADMIN" THEN DO:
           FIND FIRST integral.PF-G004 OF integral.PF-G003 WHERE integral.PF-G004.User-Id = s-User-id
               NO-LOCK NO-ERROR.
           IF NOT AVAILABLE integral.PF-G004 THEN NEXT.
           s-seguridad = PF-G004.Seguridad.
       END.
       /*FIRST integral.PF-G004 OF integral.PF-G003 NO-LOCK WHERE integral.PF-G004.User-Id = s-User-id:*/
       /* 1ro el Módulo, por ej. CONTABILIDAD */
       CREATE SUB-MENU hMenuItem 
           ASSIGN PARENT = hSubMenu
                    NAME = INTEGRAL.PF-G003.Aplic-Id
                   LABEL = INTEGRAL.PF-G003.Detalle .
       /* 2do sus submenus, por ej. Maestros,Movimientos, etc */
       
       /* Barremos por niveles */
       FOR EACH integral.PF-G002a NO-LOCK WHERE integral.PF-G002a.Aplic-Id = integral.PF-G003.Aplic-Id
...
/*
   FOR EACH integral.PF-G003 NO-LOCK WHERE NOT integral.PF-G003.Aplic-Id BEGINS "@",
       EACH integral.PF-G004 OF integral.PF-G003 NO-LOCK WHERE integral.PF-G004.User-Id = s-User-id:
       CREATE MENU-ITEM hMenuItem 
           ASSIGN PARENT = hSubMenu
                    NAME = INTEGRAL.PF-G003.Aplic-Id
                   LABEL = INTEGRAL.PF-G003.Detalle 
           TRIGGERS:
              ON CHOOSE PERSISTENT RUN bin/_medio.p( 001, INTEGRAL.PF-G003.Aplic-Id )..
           END TRIGGERS. 
   END.
*/
