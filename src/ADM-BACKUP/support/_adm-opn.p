...
ELSE DO:
  ...
  DO i = 1 TO cnt:
    ...
    IF ENTRY(2, key, '|':U) eq "y":U THEN DO:
      ...
      /* @VM 20/02/97 JORGE MIESES: FILTROS EN LOOKUPS 
      "       &Scope KEY-PHRASE ":U + db-tbl + ".":U + fld + " eq ":U + key-test. */
      "       &Scope KEY-PHRASE ( ~{&Filtro":U + STRING(I) + "} )":U.
      RUN put-sortby-cases ("       ":U).
      p_code = p_code + {&EOL} +
      "    END. /* " + key-name + " */".
...
&IF DEFINED(EXCLUDE-build-filter-variables) = 0 &THEN
...
&IF DEFINED(EXCLUDE-build-sortby-phrase) = 0 &THEN
...
&IF DEFINED(EXCLUDE-put-sortby-cases) = 0 &THEN
