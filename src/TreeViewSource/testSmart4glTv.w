...
DEFINE MENU POPUP-MENU-BtnPopupMenu 
       ...
       RULE
       MENU-ITEM m_RUN_OtherDropTargetWinw LABEL "RUN OtherDropTargetWin.w"
       RULE
...
DEFINE BUTTON bPopulateLargeTree 
     ...
     SIZE 25 BY 1.15 TOOLTIP "Populate a TV with 1000 nodes.  To test drag and drop on TV itself".

DEFINE BUTTON btnClearEditor 
     LABEL "Clear editor" 
     SIZE 12 BY 1.15 TOOLTIP "To clear the monitoring editor widget at the bottom".

DEFINE BUTTON btnDisplay 
     LABEL "Display TV  (time in ms =>)" 
     SIZE 28 BY 1.15 TOOLTIP "Already called at end of populate.  Button to test time of dummy refresh".
...
DEFINE BUTTON btnEmptyTv 
     LABEL "Empty tree" 
     SIZE 14 BY 1.15 TOOLTIP "To empty the treeview so one can populate another demo TV"
     FONT 6.

DEFINE BUTTON btnFocusInfo  NO-FOCUS
     LABEL "Info Focus" 
     SIZE 12 BY 1.15 TOOLTIP "Flat button to message info about the FOCUS system handle".

DEFINE BUTTON BtnPopulateMore 
     LABEL "Medium (vert scrolling)" 
     SIZE 25 BY 1.15 TOOLTIP "Populate a bigger TV, to test vertical scrolling".

DEFINE BUTTON btnPopulateSmall 
     LABEL "Small TV" 
     SIZE 25 BY 1.15 TOOLTIP "Populate a small demo TV.  Has some addOnExpand Nodes".

DEFINE BUTTON btnPopulateSrCustOrder 
     LABEL "Salesrep + custome + order + orderline" 
     SIZE 50 BY 1.15 TOOLTIP "A sample TV with data nodes from sports2000.  4 levels of nodes".

DEFINE BUTTON btnPopulateWide 
     LABEL "Wide tree" 
     SIZE 25 BY 1.15 TOOLTIP "Populate a wide TV to test both vertical and horizontal  scrolling".

DEFINE BUTTON BtnPopupMenu 
     LABEL "More test" 
     SIZE 12 BY 1.65 TOOLTIP "Right click to get popup menu with few more options to test".
...
DEFINE VARIABLE updKey AS CHARACTER FORMAT "X(256)":U INITIAL "n21" 
     LABEL "Key" 
     VIEW-AS FILL-IN 
     SIZE 13 BY .96 TOOLTIP "Node key of node to update"
     BGCOLOR 11  NO-UNDO.
...
DEFINE VARIABLE updLabIco AS CHARACTER FORMAT "X(256)":U INITIAL "long long very long label that takes a lot of space#tvpics/$.bmp" 
     LABEL "Lab#Ico" 
     VIEW-AS FILL-IN 
     SIZE 20 BY .96 TOOLTIP "new label and icone file name (separated by # here, please see the code)"
     BGCOLOR 11  NO-UNDO.

DEFINE VARIABLE updOptn AS CHARACTER FORMAT "X(256)":U INITIAL "private@hello world#refresh" 
     LABEL "Optn" 
     VIEW-AS FILL-IN 
     SIZE 43 BY .96 TOOLTIP "# and @ are replaced by CHR (1) and CHR(2) before sending to updateNode"
     BGCOLOR 11  NO-UNDO.
...
DEFINE FRAME fMain
     btnPopulateSmall AT ROW 1.23 COL 3
     BtnPopulateMore AT ROW 1.23 COL 28
     cSelectNode AT ROW 1.23 COL 66 COLON-ALIGNED
     sortChildren AT ROW 1.23 COL 104 COLON-ALIGNED
     btnFocusInfo AT ROW 7.92 COL 55
     btnPopulateWide AT ROW 2.42 COL 3
     bPopulateLargeTree AT ROW 2.42 COL 28
     giRowsToBatch AT ROW 2.42 COL 66 COLON-ALIGNED
     cSwapNode AT ROW 2.42 COL 85 COLON-ALIGNED
     cSwapWith AT ROW 2.42 COL 104 COLON-ALIGNED
     ...
     btnEmptyTv AT ROW 6.23 COL 3
     btnDisplay AT ROW 6.23 COL 17
     BtnPopupMenu AT ROW 6.23 COL 55
     etimeToDisplay AT ROW 6.35 COL 43 COLON-ALIGNED NO-LABEL
     updKey AT ROW 6.69 COL 73 COLON-ALIGNED
     updLabIco AT ROW 6.69 COL 96 COLON-ALIGNED
     lTraceEvents AT ROW 7.92 COL 3
     btnClearEditor AT ROW 7.92 COL 19
     btnDumpNodeTable AT ROW 7.92 COL 31
     updOptn AT ROW 7.92 COL 73 COLON-ALIGNED
     edMsg AT ROW 9.31 COL 1 NO-LABEL
     " Update node" VIEW-AS TEXT
          SIZE 14 BY .62 AT ROW 6 COL 86
     rectUpd AT ROW 6.23 COL 68
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 45.6 ROW 1
         SIZE 120 BY 15.24.
...
DO ON ERROR UNDO, LEAVE:
  DO n = 0 TO 999:
      ...
      RUN addNode IN h_pure4gltv ("k" + STRING(n)
                                   ,p
                                   ,"k " + STRING(n)
                                   ,IF n < 10 THEN "tvpics/books05.bmp" ELSE IF n < 100 THEN "tvpics/book02.bmp" ELSE "tvpics/present1.bmp"
                                   ,"expanded") NO-ERROR.
  END.
END.
...
DO ON ERROR UNDO, LEAVE:
  ...
  RUN addNode IN h_pure4gltv  ("n22"  ,"n2"  ,"node 22"  ,"tvpics/book02.bmp"  ,"expanded" + cFgColor) NO-ERROR.
  RUN addNode IN h_pure4gltv  ("n221" ,"n22" ,"221 dummy addOnExpand" ,"tvpics/book02.bmp"  ,"addOnExpand" + cFgColor)             NO-ERROR.
  RUN addNode IN h_pure4gltv  ("n222" ,"n22" ,"222 real addOnExpand","tvpics/book02.bmp"  ,"addOnExpand" + cFgColor)             NO-ERROR.
  RUN addNode IN h_pure4gltv  ("n3"   ,""    ,"node 3  dragSource option"   ,"tvpics/present1.bmp" ,"expanded" + CHR(1) + "dragSource") NO-ERROR.
  RUN addNode IN h_pure4gltv  ("n31"  ,"n3"  ,"node 31 nodragSource option"  ,"tvpics/$.bmp"        ,"noDragSource")             NO-ERROR.
  RUN addNode IN h_pure4gltv  ("n4"   ,""    ,"node 4  returns cancelDrag"   ,"tvpics/smile56.bmp"  ,"bgcolor=12" + CHR(1) + "font=6" + CHR(1) + "tooltip=This node uses the new options 'bgcolor=', 'font=', and 'tooltip='"         )   NO-ERROR.
  RUN addNode IN h_pure4gltv  ("n5"   ,""    ,"node 5  with unvalid picture file"   ,"tvpics/ThereIsNoSuchPicFile.bmp"  ,"")             NO-ERROR.
END.
...
*/
    REPEAT isr = 1 TO 100:
        ...
        RUN addNode IN h_pure4gltv ("sr=" + STRING(isr)
                                   ,""
                                   ,"salesrep.repname:" + STRING(isr)
                                   ,"tvpics/user.bmp"
                                   ,"addOnExpand").
  ...
  CASE currentPage: 
    ...
    WHEN 0 THEN DO:
       RUN constructObject (
             ...
             OUTPUT h_pure4gltv ).
       RUN repositionObject IN h_pure4gltv ( 1.23 , 1.00 ) NO-ERROR.
